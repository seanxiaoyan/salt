...
def ping(host, timeout=False, return_boolean=False):
    ...
    if timeout:
        if __grains__['kernel'] == 'SunOS':
            cmd = 'ping -c 4 {} {}'.format(__utils__['network.sanitize_host'](host), timeout)
        else:
            cmd = 'ping -W {} -c 4 {}'.format(timeout, __utils__['network.sanitize_host'](host))
    else:
...
def _ss_linux():
    ...
    for line in out.splitlines():
        ...
        if line.startswith('tcp'):
            ...
            ret.append({'proto': comps[0], 'recv-q': comps[2], 'send-q': comps[3], 'local-address': comps[4], 'remote-address': comps[5], 'state': ss_state, 'user': ss_user, 'inode': ss_inode, 'program': ss_program})
        if line.startswith('udp'):
            ret.append({'proto': comps[0], 'recv-q': comps[2], 'send-q': comps[3], 'local-address': comps[4], 'remote-address': comps[5], 'user': ss_user, 'inode': ss_inode, 'program': ss_program})
    return ret
...
def _netinfo_openbsd():
    ...
    """
    ret = {}
    _fstat_re = re.compile('internet(6)? (?:stream tcp 0x\\S+ (\\S+)|dgram udp (\\S+))(?: [<>=-]+ (\\S+))?$')
...
def _netinfo_freebsd_netbsd():
    ...
    """
    ret = {}
    out = __salt__['cmd.run']('sockstat -46 {} | tail -n+2'.format('-n' if __grains__['kernel'] == 'NetBSD' else ''), python_shell=True)
    for line in out.splitlines():
...
def _netstat_aix():
    ...
    for addr_family in ('inet',):
        ...
        for line in out.splitlines():
            ...
            if tcp_flag:
                if len(comps) >= 6:
                    ret.append({'proto': proto_seen, 'recv-q': comps[1], 'send-q': comps[2], 'local-address': comps[3], 'remote-address': comps[4], 'state': comps[5]})
            elif len(comps) >= 5:
                ret.append({'proto': proto_seen, 'local-address': comps[3], 'remote-address': comps[4]})
    return ret
...
def _netstat_route_linux():
    ...
    for line in out.splitlines():
        comps = line.split()
        ret.append({'addr_family': 'inet', 'destination': comps[0], 'gateway': comps[1], 'netmask': comps[2], 'flags': comps[3], 'interface': comps[7]})
    cmd = 'netstat -A inet6 -rn | tail -n+3'
    out = __salt__['cmd.run'](cmd, python_shell=True)
    for line in out.splitlines():
        comps = line.split()
        if len(comps) == 6:
            ret.append({'addr_family': 'inet6', 'destination': comps[0], 'gateway': comps[1], 'netmask': '', 'flags': comps[2], 'interface': comps[5]})
        elif len(comps) == 7:
            ret.append({'addr_family': 'inet6', 'destination': comps[0], 'gateway': comps[1], 'netmask': '', 'flags': comps[2], 'interface': comps[6]})
        else:
            continue
    return ret
...
def _ip_route_linux():
    ...
    for line in out.splitlines():
        ...
        if comps[0] == 'default':
            ...
            ret.append({'addr_family': 'inet', 'destination': '0.0.0.0', 'gateway': comps[2], 'netmask': '0.0.0.0', 'flags': 'UG', 'interface': ip_interface})
        else:
            address_mask = convert_cidr(comps[0])
            ip_interface = ''
            if comps[1] == 'dev':
                ip_interface = comps[2]
            ret.append({'addr_family': 'inet', 'destination': address_mask['network'], 'gateway': '0.0.0.0', 'netmask': address_mask['netmask'], 'flags': 'U', 'interface': ip_interface})
    cmd = 'ip -6 route show table all'
    out = __salt__['cmd.run'](cmd, python_shell=True)
    for line in out.splitlines():
        comps = line.split()
        if comps[0] in ('unicast', 'broadcast', 'throw', 'unreachable', 'prohibit', 'blackhole', 'nat', 'anycast', 'multicast'):
            continue
        if comps[0] == 'default':
            ip_interface = ''
            if comps[3] == 'dev':
                ip_interface = comps[4]
            ret.append({'addr_family': 'inet6', 'destination': '::/0', 'gateway': comps[2], 'netmask': '', 'flags': 'UG', 'interface': ip_interface})
        elif comps[0] == 'local':
            ip_interface = ''
            if comps[2] == 'dev':
                ip_interface = comps[3]
            local_address = comps[1] + '/128'
            ret.append({'addr_family': 'inet6', 'destination': local_address, 'gateway': '::', 'netmask': '', 'flags': 'U', 'interface': ip_interface})
        else:
            address_mask = convert_cidr(comps[0])
            ip_interface = ''
            if comps[1] == 'dev':
                ip_interface = comps[2]
            ret.append({'addr_family': 'inet6', 'destination': comps[0], 'gateway': '::', 'netmask': '', 'flags': 'U', 'interface': ip_interface})
    return ret
...
def _netstat_route_freebsd():
    ...
    for line in out.splitlines():
        ...
        if __grains__['os'] == 'FreeBSD' and int(__grains__.get('osmajorrelease', 0)) < 10:
            ret.append({'addr_family': 'inet', 'destination': comps[0], 'gateway': comps[1], 'netmask': comps[2], 'flags': comps[3], 'interface': comps[5]})
        else:
            ret.append({'addr_family': 'inet', 'destination': comps[0], 'gateway': comps[1], 'netmask': '', 'flags': comps[2], 'interface': comps[3]})
    cmd = 'netstat -f inet6 -rn | tail -n+5'
    out = __salt__['cmd.run'](cmd, python_shell=True)
    for line in out.splitlines():
        comps = line.split()
        ret.append({'addr_family': 'inet6', 'destination': comps[0], 'gateway': comps[1], 'netmask': '', 'flags': comps[2], 'interface': comps[3]})
    return ret
...
def _netstat_route_netbsd():
    ...
    for line in out.splitlines():
        comps = line.split()
        ret.append({'addr_family': 'inet', 'destination': comps[0], 'gateway': comps[1], 'netmask': '', 'flags': comps[3], 'interface': comps[6]})
    cmd = 'netstat -f inet6 -rn | tail -n+5'
    out = __salt__['cmd.run'](cmd, python_shell=True)
    for line in out.splitlines():
        comps = line.split()
        ret.append({'addr_family': 'inet6', 'destination': comps[0], 'gateway': comps[1], 'netmask': '', 'flags': comps[3], 'interface': comps[6]})
    return ret
...
def _netstat_route_openbsd():
    ...
    for line in out.splitlines():
        comps = line.split()
        ret.append({'addr_family': 'inet', 'destination': comps[0], 'gateway': comps[1], 'netmask': '', 'flags': comps[2], 'interface': comps[7]})
    cmd = 'netstat -f inet6 -rn | tail -n+5'
    out = __salt__['cmd.run'](cmd, python_shell=True)
    for line in out.splitlines():
        comps = line.split()
        ret.append({'addr_family': 'inet6', 'destination': comps[0], 'gateway': comps[1], 'netmask': '', 'flags': comps[2], 'interface': comps[7]})
    return ret
...
def traceroute(host):
    ...
    if __utils__['platform.is_sunos']() or __utils__['platform.is_aix']():
        ...
        try:
            version_raw = re.findall('.*[Vv]ersion (\\d+)\\.([\\w\\+]+)\\.*(\\w*)', version_out)[0]
    ...
    for line in out.splitlines():
        ...
        if skip_line:
            ...
            continue
        if 'Darwin' in str(traceroute_version[1]) or 'FreeBSD' in str(traceroute_version[1]) or __grains__['kernel'] in ('SunOS', 'AIX'):
            try:
                traceline = re.findall('\\s*(\\d*)\\s+(.*)\\s+\\((.*)\\)\\s+(.*)$', line)[0]
            except IndexError:
                traceline = re.findall('\\s*(\\d*)\\s+(\\*\\s+\\*\\s+\\*)', line)[0]
            log.debug('traceline: %s', traceline)
            delays = re.findall('(\\d+\\.\\d+)\\s*ms', str(traceline))
            try:
                if traceline[1] == '* * *':
                    result = {'count': traceline[0], 'hostname': '*'}
                else:
                    result = {'count': traceline[0], 'hostname': traceline[1], 'ip': traceline[2]}
                    for (idx, delay) in enumerate(delays):
                        result['ms{}'.format(idx + 1)] = delay
            except IndexError:
                result = {}
        elif traceroute_version[0] >= 2 and traceroute_version[2] >= 14 or (traceroute_version[0] >= 2 and traceroute_version[1] > 0):
            comps = line.split('  ')
            if len(comps) >= 2 and comps[1] == '* * *':
                result = {'count': int(comps[0]), 'hostname': '*'}
            elif len(comps) >= 5:
                result = {'count': int(comps[0]), 'hostname': comps[1].split()[0], 'ip': comps[1].split()[1].strip('()'), 'ms1': float(comps[2].split()[0]), 'ms2': float(comps[3].split()[0]), 'ms3': float(comps[4].split()[0])}
            else:
                result = {}
        else:
            comps = line.split()
            if len(comps) >= 8:
                result = {'count': comps[0], 'hostname': comps[1], 'ip': comps[2], 'ms1': comps[4], 'ms2': comps[6], 'ms3': comps[8], 'ping1': comps[3], 'ping2': comps[5], 'ping3': comps[7]}
            else:
                result = {}
        ret.append(result)
        if not result:
...
def mod_hostname(hostname):
    ...
    if hostname_cmd.endswith('hostnamectl'):
        result = __salt__['cmd.run_all']('{} set-hostname {}'.format(hostname_cmd, hostname))
        if result['retcode'] != 0:
            log.debug('%s was unable to set hostname. Error: %s', hostname_cmd, result['stderr'])
            return False
    elif not __utils__['platform.is_sunos']():
    ...
    if __grains__['os_family'] == 'RedHat':
        ...
        if __grains__['lsb_distrib_id'] == 'nilrt':
            ...
            nirtcfg_cmd += " --set section=SystemSettings,token='Host_Name',value='{}'".format(str_hostname)
            if __salt__['cmd.run_all'](nirtcfg_cmd)['retcode'] != 0:
                raise CommandExecutionError("Couldn't set hostname to: {}\n".format(str_hostname))
...
def connect(host, port=None, **kwargs):
    ...
    try:
        ...
        ret['result'] = False
        ret['comment'] = 'Unable to resolve host {} on {} port {}'.format(host, proto, port)
        return ret
    try:
        ...
        ret['result'] = False
        ret['comment'] = 'Unable to connect to {} ({}) on {} port {}'.format(host, _address[0], proto, port)
        return ret
    ret['result'] = True
    ret['comment'] = 'Successfully connected to {} ({}) on {} port {}'.format(host, _address[0], proto, port)
    return ret
...
def _mod_bufsize_linux(iface, *args, **kwargs):
    ...
    """
    ret = {'result': False, 'comment': 'Requires rx=<val> tx==<val> rx-mini=<val> and/or rx-jumbo=<val>'}
...
def default_route(family=None):
    ...
    elif __grains__['os'] in ['FreeBSD', 'NetBSD', 'OpenBSD', 'MacOS', 'Darwin'] or __grains__['kernel'] in ('SunOS', 'AIX'):
...
def get_route(ip):
    ...
    if __grains__['kernel'] == 'Linux':
        ...
        out = __salt__['cmd.run'](cmd, python_shell=True)
        regexp = re.compile('(via\\s+(?P<gateway>[\\w\\.:]+))?\\s+dev\\s+(?P<interface>[\\w\\.\\:\\-]+)\\s+.*src\\s+(?P<source>[\\w\\.:]+)')
        m = regexp.search(out.splitlines()[0])
        ret = {'destination': ip, 'gateway': m.group('gateway'), 'interface': m.group('interface'), 'source': m.group('source')}
        return ret
    if __grains__['kernel'] == 'SunOS':
...
def fqdns():
    ...
    try:
        with concurrent.futures.ThreadPoolExecutor(8) as pool:
            future_lookups = {pool.submit(_lookup_fqdn, address): address for address in addresses}
        ...
        log.error('Exception while creating a ThreadPoolExecutor for resolving FQDNs: %s', exc)
