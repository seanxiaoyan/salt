...
class Inspector(EnvLoader):
    ...
    def _syscall(self, command, input=None, env=None, *params):
        ...
        """
        return subprocess.Popen([command] + list(params), stdout=subprocess.PIPE, stdin=subprocess.PIPE, stderr=subprocess.STDOUT, env=env or os.environ).communicate(input=input)
    ...
    def request_snapshot(self, mode, priority=19, **kwargs):
        ...
        subprocess.run(['nice', '-{}'.format(priority), 'python', __file__, os.path.dirname(self.pidfile), os.path.dirname(self.dbfile), mode], check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
