...
class Query(EnvLoader):
    ...
    def __init__(self, scope, cachedir=None):
        ...
        if scope and scope not in self.SCOPES:
            raise InspectorQueryException('Unknown scope: {}. Must be one of: {}'.format(repr(scope), ', '.join(self.SCOPES)))
        elif not scope:
            raise InspectorQueryException('Scope cannot be empty. Must be one of: {}'.format(', '.join(self.SCOPES)))
        ...
        * **total**: Return a total amount of found payload files
        ...
        def _size_format(size, fmt):
            ...
            if fmt == 'b':
                ...
                return '{} Kb'.format(round(float(size) / 1024, 2))
            elif fmt == 'mb':
                return '{} Mb'.format(round(float(size) / 1024 / 1024, 2))
            elif fmt == 'gb':
                return '{} Gb'.format(round(float(size) / 1024 / 1024 / 1024, 2))
        filter = kwargs.get('filter')
        offset = kwargs.get('offset', 0)
        timeformat = kwargs.get('time', 'tz')
        if timeformat not in ['ticks', 'tz']:
            raise InspectorQueryException('Unknown "{}" value for parameter "time"'.format(timeformat))
        ...
        if size_fmt is not None and size_fmt.lower() not in ['b', 'kb', 'mb', 'gb']:
            raise InspectorQueryException('Unknown "{}" value for parameter "size". Should be either B, Kb, Mb or Gb'.format(timeformat))
        owners = kwargs.get('owners', 'id')
        if owners not in ['name', 'id']:
            raise InspectorQueryException('Unknown "{}" value for parameter "owners". Should be either name or id (default)'.format(owners))
        ...
        for i_type in incl_type:
            if i_type not in ['directory', 'dir', 'd', 'file', 'f', 'link', 'l']:
                raise InspectorQueryException('Unknown "{}" values for parameter "type". Should be comma separated one or more of dir, file and/or link.'.format(', '.join(incl_type)))
        self.db.open()
        if 'total' in args:
