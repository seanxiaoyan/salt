...
import salt.wheel
from salt.exceptions import CommandExecutionError, SaltInvocationError, SaltRenderError, SaltReqTimeoutError
...
def _sync(form, saltenv=None, extmod_whitelist=None, extmod_blacklist=None):
    ...
    (ret, touched) = salt.utils.extmods.sync(__opts__, form, saltenv=saltenv, extmod_whitelist=extmod_whitelist, extmod_blacklist=extmod_blacklist)
    if touched:
...
def sync_pillar(saltenv=None, refresh=True, extmod_whitelist=None, extmod_blacklist=None):
    ...
    if __opts__['file_client'] != 'local':
        raise CommandExecutionError('Pillar modules can only be synced to masterless minions')
...
def refresh_pillar(wait=False, timeout=30, clean_cache=True):
    ...
    try:
        if wait:
            with salt.utils.event.get_event('minion', opts=__opts__, listen=True) as eventer:
                ret = __salt__['event.fire'](data, 'pillar_refresh')
                log.trace('refresh_pillar waiting for pillar refresh to complete')
                event_ret = eventer.get_event(tag=salt.defaults.events.MINION_PILLAR_REFRESH_COMPLETE, wait=timeout)
    ...
    try:
        if asynchronous:
            ...
            with salt.utils.event.get_event('minion', opts=__opts__, listen=True) as eventer:
                ...
                eventer.get_event(tag=salt.defaults.events.MINION_MOD_REFRESH_COMPLETE, wait=30)
    except KeyError:
        log.error('Event module not available. Module refresh failed.')
        ret = False
    return ret
    ...
    for (root, dirs, files) in salt.utils.files.safe_walk(__opts__['cachedir'], followlinks=False):
        for name in files:
            try:
                ...
                log.error('Attempt to clear cache with saltutil.clear_cache FAILED with: %s', exc)
                return False
    return True
...
def find_cached_job(jid):
    ...
    if not os.path.isdir(job_dir):
        if not __opts__.get('cache_jobs'):
            return 'Local jobs cache directory not found; you may need to enable cache_jobs on this minion'
        else:
...
def signal_job(jid, sig):
    ...
    if HAS_PSUTIL is False:
        log.warning('saltutil.signal job called, but psutil is not installed. Install psutil to ensure more reliable and accurate PID management.')
    for data in running():
        if data['jid'] == jid:
            try:
                log.info('Trace')
                ...
                if HAS_PSUTIL is False and 'child_pids' in data:
                    for pid in data['child_pids']:
                        os.kill(int(pid), sig)
                return 'Signal {} sent to job {} at pid {}'.format(int(sig), jid, data['pid'])
            except OSError:
                log.info('Trace')
                path = os.path.join(__opts__['cachedir'], 'proc', str(jid))
                if os.path.isfile(path):
                    os.remove(path)
                return 'Job {} was not running and job data has been cleaned up'.format(jid)
    return ''
...
def revoke_auth(preserve_minion_cache=False):
    ...
    for master in masters:
        with salt.channel.client.ReqChannel.factory(__opts__, master_uri=master) as channel:
...
def _exec(client, tgt, fun, arg, timeout, tgt_type, ret, kwarg, batch=False, subset=False, **kwargs):
    fcn_ret = {}
    seen = 0
    cmd_kwargs = {'tgt': tgt, 'fun': fun, 'arg': arg, 'timeout': timeout, 'tgt_type': tgt_type, 'ret': ret, 'kwarg': kwarg}
    if batch:
    ...
    if batch:
        ...
        for (key, value) in old_ret.items():
            fcn_ret[key] = {'out': value.get('out', 'highstate') if isinstance(value, dict) else 'highstate', 'ret': value}
    return fcn_ret

def cmd(tgt, fun, arg=(), timeout=None, tgt_type='glob', ret='', kwarg=None, ssh=False, **kwargs):
    """
    ...
    master_cfgfile = '{}master'.format(cfgfile[:-6])
    if not fcn_ret and cfgfile.endswith('{}{}'.format(os.path.sep, 'minion')) and os.path.exists(master_cfgfile):
...
def runner(name, arg=None, kwarg=None, full_return=False, saltenv='base', jid=None, **kwargs):
    ...
    if jid:
        salt.utils.event.fire_args(__opts__, jid, {'type': 'runner', 'name': name, 'args': arg, 'kwargs': kwarg}, prefix='run')
...
def wheel(name, *args, **kwargs):
    ...
    try:
        ...
        if jid:
            salt.utils.event.fire_args(__opts__, jid, {'type': 'wheel', 'name': name, 'args': valid_kwargs}, prefix='run')
        ret = wheel_client.cmd(name, arg=args, pub_data=pub_data, kwarg=valid_kwargs, print_event=False, full_return=True)
    except SaltInvocationError:
        log.info('Trace')
        raise CommandExecutionError('This command can only be executed on a minion that is located on the master.')
    return ret
