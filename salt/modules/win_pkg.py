...
def _get_reg_software(include_components=True, include_updates=True):
    ...
    def skip_component(hive, key, sub_key, use_32bit_registry):
        ...
        if include_components:
            return False
        if __utils__['reg.value_exists'](hive=hive, key='{}\\{}'.format(key, sub_key), vname='SystemComponent', use_32bit_registry=use_32bit_registry):
            if __utils__['reg.read_value'](hive=hive, key='{}\\{}'.format(key, sub_key), vname='SystemComponent', use_32bit_registry=use_32bit_registry)['vdata'] > 0:
                return True
        return False
    ...
    def skip_win_installer(hive, key, sub_key, use_32bit_registry):
        ...
        if __utils__['reg.value_exists'](hive=hive, key='{}\\{}'.format(key, sub_key), vname='WindowsInstaller', use_32bit_registry=use_32bit_registry):
            if __utils__['reg.read_value'](hive=hive, key='{}\\{}'.format(key, sub_key), vname='WindowsInstaller', use_32bit_registry=use_32bit_registry)['vdata'] > 0:
    ...
    def skip_uninstall_string(hive, key, sub_key, use_32bit_registry):
        ...
        Returns:
            bool: True if the package needs to be skipped, otherwise False
        """
        if __utils__['reg.value_exists'](hive=hive, key='{}\\{}'.format(key, sub_key), vname='NoRemove', use_32bit_registry=use_32bit_registry):
            if __utils__['reg.read_value'](hive=hive, key='{}\\{}'.format(key, sub_key), vname='NoRemove', use_32bit_registry=use_32bit_registry)['vdata'] > 0:
                return False
        if not __utils__['reg.value_exists'](hive=hive, key='{}\\{}'.format(key, sub_key), vname='UninstallString', use_32bit_registry=use_32bit_registry):
            return True
        return False
    ...
    def skip_release_type(hive, key, sub_key, use_32bit_registry):
        ...
        if __utils__['reg.value_exists'](hive=hive, key='{}\\{}'.format(key, sub_key), vname='ReleaseType', use_32bit_registry=use_32bit_registry):
            if __utils__['reg.read_value'](hive=hive, key='{}\\{}'.format(key, sub_key), vname='ReleaseType', use_32bit_registry=use_32bit_registry)['vdata'] in skip_types:
                return True
        return False
    ...
    def skip_parent_key(hive, key, sub_key, use_32bit_registry):
        ...
        Returns:
            bool: True if the package needs to be skipped, otherwise False
        """
        if __utils__['reg.value_exists'](hive=hive, key='{}\\{}'.format(key, sub_key), vname='ParentKeyName', use_32bit_registry=use_32bit_registry):
            return True
        return False
    ...
    def add_software(hive, key, sub_key, use_32bit_registry):
        ...
        Windows update.
        """
        d_name_regdata = __utils__['reg.read_value'](hive=hive, key='{}\\{}'.format(key, sub_key), vname='DisplayName', use_32bit_registry=use_32bit_registry)
        ...
        if not include_updates:
            if re.match('^KB[0-9]{6}', d_name):
                return
        d_vers_regdata = __utils__['reg.read_value'](hive=hive, key='{}\\{}'.format(key, sub_key), vname='DisplayVersion', use_32bit_registry=use_32bit_registry)
        d_vers = 'Not Found'
    ...
    for sub_key in __utils__['reg.list_keys'](**kwargs):
        if not __utils__['reg.key_exists'](hive=kwargs['hive'], key='{}\\{}'.format(userdata_key, sub_key)):
            continue
        kwargs['sub_key'] = sub_key
        if skip_component(**kwargs):
            continue
...
def _get_repo_details(saltenv):
    ...
    try:
        ...
        if exc.errno != errno.EEXIST:
            raise CommandExecutionError('Failed to create {}: {}'.format(local_dest, exc))
    winrepo_age = -1
    try:
        ...
        if exc.errno != errno.ENOENT:
            raise CommandExecutionError('Failed to get age of {}: {}'.format(winrepo_file, exc))
    except AttributeError:
...
def install(name=None, refresh=False, pkgs=None, **kwargs):
    ...
    for (pkg_name, options) in pkg_params.items():
            ...
            if cache_file and cache_file.startswith('salt:'):
                ...
                if not cached_file:
                    try:
                        ...
                        log.exception(msg, exc_info=exc)
                        return '{}\n{}'.format(msg, exc)
                if __salt__['cp.hash_file'](cache_file, saltenv) != __salt__['cp.hash_file'](cached_file):
                    try:
                        cached_file = __salt__['cp.cache_file'](cache_file, saltenv)
                    except MinionError as exc:
                        msg = 'Failed to cache {}'.format(cache_file)
        ...
        if pkginfo[version_num].get('use_scheduler', False):
            __salt__['task.create_task'](name='update-salt-software', user_name='System', force=True, action_type='Execute', cmd=cmd_shell, arguments='/s /c "{}"'.format(arguments), start_in=cache_path, trigger_type='Once', start_date='1975-01-01', start_time='01:00', ac_only=False, stop_if_on_batteries=False)
            ...
            result = __salt__['cmd.run_all']('"{}" /s /c "{}"'.format(cmd_shell, arguments), cache_path, output_loglevel='trace', python_shell=False, redirect_stderr=True)
...
def remove(name=None, pkgs=None, **kwargs):
    ...
    for (pkgname, version_num) in pkg_params.items():
        ...
        if not removal_targets:
            ...
            ret[pkgname] = {'current': '{} no definition, cannot removed'.format(version_num)}
            continue
        for target in removal_targets:
            ...
            if uninstaller.startswith(('salt:', 'http:', 'https:', 'ftp:')):
                ...
                if uninstaller.startswith('salt:'):
                    if __salt__['cp.hash_file'](uninstaller, saltenv) != __salt__['cp.hash_file'](cached_pkg):
                        try:
                            cached_pkg = __salt__['cp.cache_file'](uninstaller, saltenv)
                        except MinionError as exc:
                            msg = 'Failed to cache {}'.format(uninstaller)
            ...
            if pkginfo[target].get('use_scheduler', False):
                ...
                result = __salt__['cmd.run_all']('"{}" /s /c "{}"'.format(cmd_shell, arguments), output_loglevel='trace', python_shell=False, redirect_stderr=True)
                if not result['retcode']:
