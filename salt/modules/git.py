...
def _check_worktree_support(failhard=True):
    ...
    if _LooseVersion(git_version) < _LooseVersion('2.5.0'):
        if failhard:
            raise CommandExecutionError('Worktrees are only supported in git 2.5.0 and newer (detected git version: {})'.format(git_version))
        return False
    return True

def _config_getter(get_opt, key, value_regex=None, cwd=None, user=None, password=None, ignore_retcode=False, output_encoding=None, **kwargs):
    """
...
def _format_git_opts(opts):
    ...
    if opts:
        ...
        if _LooseVersion(version_) < _LooseVersion('1.7.2'):
            raise SaltInvocationError('git_opts is only supported for git versions >= 1.7.2 (detected: {})'.format(version_))
...
def _git_run(command, cwd=None, user=None, password=None, identity=None, ignore_retcode=False, failhard=True, redirect_stderr=False, saltenv='base', output_encoding=None, **kwargs):
    ...
    if identity:
        ...
        for id_file in identity:
            ...
            elif not __salt__['file.file_exists'](id_file):
            ...
            if 'salt-call' not in _salt_cli and __utils__['ssh.key_is_encrypted'](id_file):
                errors.append('Identity file {} is passphrase-protected and cannot be used in a non-interactive command. Using salt-call from the minion will allow a passphrase-protected key to be used.'.format(id_file))
                continue
        ...
        if failhard:
            ...
            if missing_keys:
                if errors:
                    msg += '\n\n'
                msg += 'The following identity file(s) were not found: {}'.format(', '.join(missing_keys))
        ...
        if result['retcode'] == 0:
            ...
            if failhard:
                ...
                msg = "Command '{}' failed".format(salt.utils.url.redact_http_basic_auth(gitcommand))
                err = result['stdout' if redirect_stderr else 'stderr']
                if err:
...
def _which_git_config(global_, cwd, user, password, output_encoding=None):
    ...
    if version_ >= _LooseVersion('1.7.10.2'):
        ...
        return ['--file', _git_config(cwd, user, password, output_encoding=output_encoding)]

def add(cwd, filename, opts='', git_opts='', user=None, password=None, ignore_retcode=False, output_encoding=None):
    """
...
def archive(cwd, output, rev='HEAD', prefix=None, git_opts='', user=None, password=None, ignore_retcode=False, output_encoding=None, **kwargs):
    ...
    _git_run(command, cwd=cwd, user=user, password=password, ignore_retcode=ignore_retcode, output_encoding=output_encoding)
    return True
...
def clone(cwd, url=None, name=None, opts='', git_opts='', user=None, password=None, identity=None, https_user=None, https_pass=None, ignore_retcode=False, saltenv='base', output_encoding=None):
    """
    ...
    try:
        url = salt.utils.url.add_http_basic_auth(url, https_user, https_pass, https_only=True)
    ...
    _git_run(command, cwd=clone_cwd, user=user, password=password, identity=identity, ignore_retcode=ignore_retcode, saltenv=saltenv, output_encoding=output_encoding)
    return True

def commit(cwd, message, opts='', git_opts='', user=None, password=None, filename=None, ignore_retcode=False, output_encoding=None):
    """
...
def config_get(key, cwd=None, user=None, password=None, ignore_retcode=False, output_encoding=None, **kwargs):
    """
    ...
    result = _config_getter('--get-all', key, cwd=cwd, user=user, password=password, ignore_retcode=ignore_retcode, output_encoding=output_encoding, **kwargs)
    if result['retcode'] == 1:
        return None
...
def config_get_regexp(key, value_regex=None, cwd=None, user=None, password=None, ignore_retcode=False, output_encoding=None, **kwargs):
    ...
    """
    result = _config_getter('--get-regexp', key, value_regex=value_regex, cwd=cwd, user=user, password=password, ignore_retcode=ignore_retcode, output_encoding=output_encoding, **kwargs)
    ret = {}
    if result['retcode'] == 1:
        return ret
...
def config_set(key, value=None, multivar=None, cwd=None, user=None, password=None, ignore_retcode=False, output_encoding=None, **kwargs):
    ...
    if value is not None:
        ...
        for (idx, target) in enumerate(multivar):
            ...
            _git_run(command, cwd=cwd, user=user, password=password, ignore_retcode=ignore_retcode, output_encoding=output_encoding)
    return config_get(key, user=user, password=password, cwd=cwd, ignore_retcode=ignore_retcode, output_encoding=output_encoding, **{'all': True, 'global': global_})
...
def config_unset(key, value_regex=None, cwd=None, user=None, password=None, ignore_retcode=False, output_encoding=None, **kwargs):
    ...
    if value_regex is not None:
        command.append(value_regex)
    ret = _git_run(command, cwd=cwd if cwd != 'global' else None, user=user, password=password, ignore_retcode=ignore_retcode, failhard=False, output_encoding=output_encoding)
    retcode = ret['retcode']
    if retcode == 0:
        return True
    elif retcode == 1:
        ...
        if config_get(key, cwd=cwd, user=user, password=password, ignore_retcode=ignore_retcode, output_encoding=output_encoding) is None:
        ...
        msg = "Failed to unset key '{}', git config returned exit code {}".format(key, retcode)
        if ret['stderr']:
            msg += '; ' + ret['stderr']
        raise CommandExecutionError(msg)

def current_branch(cwd, user=None, password=None, ignore_retcode=False, output_encoding=None):
    """
...
def describe(cwd, rev='HEAD', user=None, password=None, ignore_retcode=False, output_encoding=None):
    """
...
def diff(cwd, item1=None, item2=None, opts='', git_opts='', user=None, password=None, no_index=False, cached=False, paths=None, output_encoding=None):
    """
    ...
    if no_index and cached:
        raise CommandExecutionError("The 'no_index' and 'cached' options cannot be used together")
    command = ['git'] + _format_git_opts(git_opts)
    command.append('diff')
    command.extend(_format_opts(opts))
    ...
    if no_index:
        if _LooseVersion(version(versioninfo=False)) < _LooseVersion('1.5.1'):
            raise CommandExecutionError("The 'no_index' option is only supported in Git 1.5.1 and newer")
        ignore_retcode = True
        failhard = False
        command.append('--no-index')
        for value in [x for x in (item1, item2) if x]:
            log.warning("Revision '%s' ignored in git diff, as revisions cannot be used when no_index=True", value)
    elif cached:
        command.append('--cached')
        if item1:
            command.append(item1)
        if item2:
            log.warning("Second revision '%s' ignored in git diff, at most one revision is considered when cached=True", item2)
    else:
        for value in [x for x in (item1, item2) if x]:
    ...
    return _git_run(command, cwd=cwd, user=user, password=password, ignore_retcode=ignore_retcode, failhard=failhard, redirect_stderr=True, output_encoding=output_encoding)['stdout']
...
def fetch(cwd, remote=None, force=False, refspecs=None, opts='', git_opts='', user=None, password=None, identity=None, ignore_retcode=False, saltenv='base', output_encoding=None):
    ...
    output = _git_run(command, cwd=cwd, user=user, password=password, identity=identity, ignore_retcode=ignore_retcode, redirect_stderr=True, saltenv=saltenv, output_encoding=output_encoding)['stdout']
    update_re = re.compile('[\\s*]*(?:([0-9a-f]+)\\.\\.([0-9a-f]+)|\\[(?:new (tag|branch)|tag update)\\])\\s+(.+)->')
    ret = {}
...
def list_branches(cwd, remote=False, user=None, password=None, ignore_retcode=False, output_encoding=None):
    """
    .. versionadded:: 2015.8.0
    ...
    """
    cwd = _expand_path(cwd, user)
    command = ['git', 'for-each-ref', '--format', '%(refname:short)', 'refs/{}/'.format('heads' if not remote else 'remotes')]
    return _git_run(command, cwd=cwd, user=user, password=password, ignore_retcode=ignore_retcode, output_encoding=output_encoding)['stdout'].splitlines()

def list_tags(cwd, user=None, password=None, ignore_retcode=False, output_encoding=None):
    """
    .. versionadded:: 2015.8.0
...
def list_worktrees(cwd, stale=False, user=None, password=None, output_encoding=None, **kwargs):
    ...
    if has_native_list_subcommand:
        ...
        try:
            log.info('Trace')
            worktree_root = rev_parse(cwd, opts=['--git-path', 'worktrees'], user=user, password=password, output_encoding=output_encoding)
        ...
        if not os.path.isdir(worktree_root):
            raise CommandExecutionError('Worktree admin directory {} not present'.format(worktree_root))
...
def ls_remote(cwd=None, remote='origin', ref=None, opts='', git_opts='', user=None, password=None, identity=None, https_user=None, https_pass=None, ignore_retcode=False, output_encoding=None, saltenv='base'):
    log.info('Trace')
    ...
    try:
        log.info('Trace')
        remote = salt.utils.url.add_http_basic_auth(remote, https_user, https_pass, https_only=True)
    except ValueError as exc:
        log.info('Trace')
        raise SaltInvocationError(exc.__str__())
    command = ['git'] + _format_git_opts(git_opts)
    ...
    if ref:
        command.append(ref)
    output = _git_run(command, cwd=cwd, user=user, password=password, identity=identity, ignore_retcode=ignore_retcode, saltenv=saltenv, output_encoding=output_encoding)['stdout']
    ret = {}
...
def merge(cwd, rev=None, opts='', git_opts='', user=None, password=None, identity=None, ignore_retcode=False, output_encoding=None, **kwargs):
    ...
    if rev:
        command.append(rev)
    return _git_run(command, cwd=cwd, user=user, password=password, identity=identity, ignore_retcode=ignore_retcode, output_encoding=output_encoding)['stdout']

def merge_base(cwd, refs=None, octopus=False, is_ancestor=False, independent=False, fork_point=None, opts='', git_opts='', user=None, password=None, ignore_retcode=False, output_encoding=None, **kwargs):
    """
    .. versionadded:: 2015.8.0
    ...
    if all_ and (independent or is_ancestor or fork_point):
        raise SaltInvocationError("The 'all' argument is not compatible with 'independent', 'is_ancestor', or 'fork_point'")
    if refs is None:
        refs = []
    elif not isinstance(refs, (list, tuple)):
        refs = [x.strip() for x in str(refs).split(',')]
    mutually_exclusive_count = len([x for x in (octopus, independent, is_ancestor, fork_point) if x])
    if mutually_exclusive_count > 1:
        raise SaltInvocationError("Only one of 'octopus', 'independent', 'is_ancestor', and 'fork_point' is permitted")
    elif is_ancestor:
        if len(refs) != 2:
            raise SaltInvocationError("Two refs/commits are required if 'is_ancestor' is True")
    elif fork_point:
        if len(refs) > 1:
            raise SaltInvocationError("At most one ref/commit can be passed if 'fork_point' is specified")
        elif not refs:
            refs = ['HEAD']
    if is_ancestor:
        if _LooseVersion(version(versioninfo=False)) < _LooseVersion('1.8.0'):
            first_commit = rev_parse(cwd, rev=refs[0], opts=['--verify'], user=user, password=password, ignore_retcode=ignore_retcode, output_encoding=output_encoding)
            return merge_base(cwd, refs=refs, is_ancestor=False, user=user, password=password, ignore_retcode=ignore_retcode, output_encoding=output_encoding) == first_commit
    command = ['git'] + _format_git_opts(git_opts)
    ...
    command.extend(refs)
    result = _git_run(command, cwd=cwd, user=user, password=password, ignore_retcode=ignore_retcode, failhard=False if is_ancestor else True, output_encoding=output_encoding)
    if is_ancestor:
...
def merge_tree(cwd, ref1, ref2, base=None, user=None, password=None, ignore_retcode=False, output_encoding=None):
    """
    .. versionadded:: 2015.8.0
    ...
    if base is None:
        try:
            ...
            raise CommandExecutionError('Unable to determine merge base for {} and {}'.format(ref1, ref2))
    command.extend([base, ref1, ref2])
    return _git_run(command, cwd=cwd, user=user, password=password, ignore_retcode=ignore_retcode, output_encoding=output_encoding)['stdout']

def pull(cwd, opts='', git_opts='', user=None, password=None, identity=None, ignore_retcode=False, saltenv='base', output_encoding=None):
    """
    ...
    command.append('pull')
    command.extend(_format_opts(opts))
    return _git_run(command, cwd=cwd, user=user, password=password, identity=identity, ignore_retcode=ignore_retcode, saltenv=saltenv, output_encoding=output_encoding)['stdout']
...
def remote_get(cwd, remote='origin', user=None, password=None, redact_auth=True, ignore_retcode=False, output_encoding=None):
    """
    ...
    """
    cwd = _expand_path(cwd, user)
    all_remotes = remotes(cwd, user=user, password=password, redact_auth=redact_auth, ignore_retcode=ignore_retcode, output_encoding=output_encoding)
    if remote not in all_remotes:
        raise CommandExecutionError("Remote '{}' not present in git checkout located at {}".format(remote, cwd))
    return all_remotes[remote]

def remote_refs(url, heads=False, tags=False, user=None, password=None, identity=None, https_user=None, https_pass=None, ignore_retcode=False, output_encoding=None, saltenv='base', **kwargs):
    log.info('Trace')
    ...
    try:
        log.info('Trace')
        command.append(salt.utils.url.add_http_basic_auth(url, https_user, https_pass, https_only=True))
    except ValueError as exc:
        log.info('Trace')
        raise SaltInvocationError(exc.__str__())
    if filter_:
        command.append(filter_)
    output = _git_run(command, user=user, password=password, identity=identity, ignore_retcode=ignore_retcode, saltenv=saltenv, output_encoding=output_encoding)['stdout']
    ret = {}
    for line in salt.utils.itertools.split(output, '\n'):
        try:
            log.info('Trace')
...
def remote_set(cwd, url, remote='origin', user=None, password=None, https_user=None, https_pass=None, push_url=None, push_https_user=None, push_https_pass=None, ignore_retcode=False, output_encoding=None):
    """
    cwd
        The path to the git checkout
    ...
    """
    if remote in remotes(cwd, user=user, password=password, output_encoding=output_encoding):
        log.debug("Remote '%s' already exists in git checkout located at %s, removing so it can be re-added", remote, cwd)
    ...
    if push_url:
        ...
        try:
            push_url = salt.utils.url.add_http_basic_auth(push_url, push_https_user, push_https_pass, https_only=True)
        except ValueError as exc:
            raise SaltInvocationError(str(exc))
        command = ['git', 'remote', 'set-url', '--push', remote, push_url]
        _git_run(command, cwd=cwd, user=user, password=password, ignore_retcode=ignore_retcode, output_encoding=output_encoding)
    return remote_get(cwd=cwd, remote=remote, user=user, password=password, ignore_retcode=ignore_retcode, output_encoding=output_encoding)
...
def revision(cwd, rev='HEAD', short=False, user=None, password=None, ignore_retcode=False, output_encoding=None):
    """
...
def rm_(cwd, filename, opts='', git_opts='', user=None, password=None, ignore_retcode=False, output_encoding=None):
    """
...
def submodule(cwd, command, opts='', git_opts='', user=None, password=None, identity=None, ignore_retcode=False, saltenv='base', output_encoding=None, **kwargs):
    """
    .. versionchanged:: 2015.8.0
    ...
    cwd = _expand_path(cwd, user)
    if init_:
        raise SaltInvocationError("The 'init' argument is no longer supported. Either set 'command' to 'init', or include '--init' in the 'opts' argument and set 'command' to 'update'.")
    cmd = ['git'] + _format_git_opts(git_opts)
    cmd.extend(['submodule', command])
    cmd.extend(_format_opts(opts))
    return _git_run(cmd, cwd=cwd, user=user, password=password, identity=identity, ignore_retcode=ignore_retcode, saltenv=saltenv, output_encoding=output_encoding)['stdout']

def symbolic_ref(cwd, ref, value=None, opts='', git_opts='', user=None, password=None, ignore_retcode=False, output_encoding=None):
    """
    .. versionadded:: 2015.8.0
    ...
    if value is not None and any((x in opts for x in ('-d', '--delete'))):
        raise SaltInvocationError('Value cannot be set for symbolic ref if -d/--delete is included in opts')
    command.extend(opts)
    command.append(ref)
    if value:
...
def tag(cwd, name, ref='HEAD', message=None, opts='', git_opts='', user=None, password=None, ignore_retcode=False, output_encoding=None):
    ...
    if any((x == '-m' or '--message' in x for x in formatted_opts)):
        raise SaltInvocationError('Tag messages must be passed in the "message" argument')
...
def worktree_add(cwd, worktree_path, ref=None, reset_branch=None, force=None, detach=False, opts='', git_opts='', user=None, password=None, ignore_retcode=False, output_encoding=None, **kwargs):
    """
    .. versionadded:: 2015.8.0
    ...
    if detach:
        if force:
            log.warning("'force' argument to git.worktree_add is ignored when detach=True")
...
def worktree_prune(cwd, dry_run=False, verbose=True, expire=None, opts='', git_opts='', user=None, password=None, ignore_retcode=False, output_encoding=None):
    """
    .. versionadded:: 2015.8.0
    ...
    command.extend(_format_opts(opts))
    result = _git_run(command, cwd=cwd, user=user, password=password, ignore_retcode=ignore_retcode, output_encoding=output_encoding)
    git_version = version(versioninfo=False)
