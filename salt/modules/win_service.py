...
def start(name, timeout=90):
    ...
    try:
        ...
        if exc.winerror != 1056:
            raise CommandExecutionError('Failed To Start {}: {}'.format(name, exc.strerror))
        log.debug('Service "%s" is running', name)
    srv_status = _status_wait(service_name=name, end_time=time.time() + int(timeout), service_states=['Start Pending', 'Stopped'])
...
def stop(name, timeout=90):
    ...
    try:
        ...
        if exc.winerror != 1062:
            raise CommandExecutionError('Failed To Stop {}: {}'.format(name, exc.strerror))
        log.debug('Service "%s" is not running', name)
    srv_status = _status_wait(service_name=name, end_time=time.time() + int(timeout), service_states=['Running', 'Stop Pending'])
...
def modify(name, bin_path=None, exe_args=None, display_name=None, description=None, service_type=None, start_type=None, start_delayed=None, error_control=None, load_order_group=None, dependencies=None, account_name=None, account_password=None, run_interactive=None):
    log.info('Trace')
    ...
    try:
        log.info('Trace')
        handle_svc = win32service.OpenService(handle_scm, name, win32service.SERVICE_CHANGE_CONFIG | win32service.SERVICE_QUERY_CONFIG)
    except pywintypes.error as exc:
        log.info('Trace')
    ...
    win32service.ChangeServiceConfig(handle_svc, service_type, start_type, error_control, bin_path, load_order_group, 0, dependencies, account_name, account_password, display_name)
    if description is not None:
        win32service.ChangeServiceConfig2(handle_svc, win32service.SERVICE_CONFIG_DESCRIPTION, description)
        changes['Description'] = description
    if start_delayed is not None:
        if start_type == -1 and config_info[1] == 2 or start_type == 2:
            win32service.ChangeServiceConfig2(handle_svc, win32service.SERVICE_CONFIG_DELAYED_AUTO_START_INFO, start_delayed)
...
def create(name, bin_path, exe_args=None, display_name=None, description=None, service_type='own', start_type='manual', start_delayed=False, error_control='normal', load_order_group=None, dependencies=None, account_name='.\\LocalSystem', account_password=None, run_interactive=False, **kwargs):
    """
    ...
    if start_delayed:
        if start_type != 2:
            raise CommandExecutionError('Invalid Parameter: start_delayed requires start_type "auto"')
    if account_name in ['LocalSystem', '.\\LocalSystem', 'LocalService', '.\\LocalService', 'NetworkService', '.\\NetworkService']:
        account_password = ''
...
def delete(name, timeout=90):
    ...
    try:
        ...
        log.info('Trace')
        raise CommandExecutionError('Failed to delete {}. {}'.format(name, exc.strerror))
    finally:
