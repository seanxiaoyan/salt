...
SCHEDULE_CONF = ['name', 'maxrunning', 'function', 'splay', 'range', 'when', 'once', 'once_fmt', 'returner', 'jid_include', 'args', 'kwargs', '_seconds', 'seconds', 'minutes', 'hours', 'days', 'enabled', 'return_job', 'metadata', 'cron', 'until', 'after', 'return_config', 'return_kwargs', 'run_on_start', 'skip_during_range', 'run_after_skip_range']
...
def _get_schedule_config_file():
    ...
    minion_d_dir = os.path.join(config_dir, os.path.dirname(__opts__.get('default_include', salt.config.DEFAULT_MINION_OPTS['default_include'])))
    if not os.path.isdir(minion_d_dir):
        os.makedirs(minion_d_dir)
    return os.path.join(minion_d_dir, '_schedule.conf')

def list_(show_all=False, show_disabled=True, where=None, return_yaml=True, offline=False):
    log.info('Trace')
    ...
    if offline:
        ...
        try:
            ...
            with salt.utils.event.get_event('minion', opts=__opts__) as event_bus:
                res = __salt__['event.fire']({'func': 'list', 'where': where}, 'manage_schedule')
                if res:
...
def purge(**kwargs):
    ...
    current_schedule = list_(show_all=True, return_yaml=False, offline=kwargs.get('offline'))
    for name in pycopy.deepcopy(current_schedule):
        ...
        if 'test' in kwargs and kwargs['test']:
            ret['result'] = True
            ret['comment'].append('Job: {} would be deleted from schedule.'.format(name))
        elif kwargs.get('offline'):
            ...
            try:
                log.info('Trace')
                with salt.utils.event.get_event('minion', opts=__opts__) as event_bus:
                    res = __salt__['event.fire']({'name': name, 'func': 'delete', 'persist': persist}, 'manage_schedule')
                    if res:
                        ...
                        if event_ret and event_ret['complete']:
                            ...
                            if name not in _schedule_ret:
                                ...
                                ret['comment'].append('Failed to delete job {} from schedule.'.format(name))
                                ret['result'] = True
            except KeyError:
                log.info('Trace')
    ...
    if kwargs.get('offline'):
        ...
        try:
            ...
            with salt.utils.files.fopen(schedule_conf, 'wb+') as fp_:
                fp_.write(salt.utils.stringutils.to_bytes(salt.utils.yaml.safe_dump({'schedule': current_schedule})))
        except OSError:
...
def delete(name, **kwargs):
    ...
    ret = {'comment': 'Failed to delete job {} from schedule.'.format(name), 'result': False, 'changes': {}}
    if not name:
    ...
    if 'test' in kwargs and kwargs['test']:
        ...
        log.info('Trace')
        current_schedule = list_(show_all=True, where='opts', return_yaml=False, offline=kwargs.get('offline'))
        del current_schedule[name]
        schedule_conf = _get_schedule_config_file()
        try:
            with salt.utils.files.fopen(schedule_conf, 'wb+') as fp_:
        ...
        persist = kwargs.get('persist', True)
        if name in list_(show_all=True, where='opts', return_yaml=False, offline=kwargs.get('offline')):
            event_data = {'name': name, 'func': 'delete', 'persist': persist}
        elif name in list_(show_all=True, where='pillar', return_yaml=False, offline=kwargs.get('offline')):
                ...
                if res:
                    ...
                    if event_ret and event_ret['complete']:
                        ...
                        if name not in schedule:
                            ...
                            ret['comment'] = 'Failed to delete job {} from schedule.'.format(name)
                        return ret
        except KeyError:
            log.info('Trace')
            ret['comment'] = 'Event module not available. Schedule add failed.'
        ...
        if not _WHEN_SUPPORTED:
            ...
            for _when in validate_when:
                try:
                    ...
                    log.info('Trace')
                    ret['result'] = False
                    ret['comment'] = 'Schedule item {} for "when" in invalid.'.format(_when)
                    return ret
    for item in ['range', 'when', 'once', 'once_fmt', 'cron', 'returner', 'after', 'return_config', 'return_kwargs', 'until', 'run_on_start', 'skip_during_range']:
        if item in kwargs:
            schedule[name][item] = kwargs[item]
...
def add(name, **kwargs):
    ...
    """
    ret = {'comment': 'Failed to add job {} to schedule.'.format(name), 'result': False, 'changes': {}}
    current_schedule = list_(show_all=True, return_yaml=False, offline=kwargs.get('offline'))
    ...
    if 'test' in kwargs and kwargs['test']:
        ...
        try:
            with salt.utils.event.get_event('minion', opts=__opts__) as event_bus:
                ...
                if res:
                    event_ret = event_bus.get_event(tag='/salt/minion/minion_schedule_add_complete', wait=30)
                    if event_ret and event_ret['complete']:
                        schedule = event_ret['schedule']
                        if name in schedule:
                            ret['result'] = True
...
def modify(name, **kwargs):
    ...
    if _new == _current:
        ...
        return ret
    ret['changes'][name] = {'old': salt.utils.odict.OrderedDict(_current), 'new': salt.utils.odict.OrderedDict(_new)}
    if 'test' in kwargs and kwargs['test']:
    ...
    if 'test' in __opts__ and __opts__['test']:
        ...
        try:
            ...
            with salt.utils.event.get_event('minion', opts=__opts__) as event_bus:
                res = __salt__['event.fire'](event_data, 'manage_schedule')
                if res:
                    event_ret = event_bus.get_event(tag='/salt/minion/minion_schedule_enabled_job_complete', wait=30)
                    if event_ret and event_ret['complete']:
                        schedule = event_ret['schedule']
                        if name in schedule and schedule[name]['enabled']:
                            ...
                            ret['result'] = False
                            ret['comment'] = 'Failed to enable job {} in schedule.'.format(name)
                        return ret
        except KeyError:
            log.info('Trace')
    ...
    if 'test' in kwargs and kwargs['test']:
        ...
        try:
            with salt.utils.event.get_event('minion', opts=__opts__) as event_bus:
                res = __salt__['event.fire'](event_data, 'manage_schedule')
                if res:
                    event_ret = event_bus.get_event(tag='/salt/minion/minion_schedule_disabled_job_complete', wait=30)
                    if event_ret and event_ret['complete']:
                        schedule = event_ret['schedule']
                        if name in schedule and (not schedule[name]['enabled']):
                            ...
                            ret['result'] = False
                            ret['comment'] = 'Failed to disable job {} in schedule.'.format(name)
                        return ret
        except KeyError:
            ret['comment'] = 'Event module not available. Schedule enable job failed.'
    return ret
    ...
    if 'test' in kwargs and kwargs['test']:
        ...
        try:
            with salt.utils.event.get_event('minion', opts=__opts__) as event_bus:
                ...
                if res:
                    event_ret = event_bus.get_event(tag='/salt/minion/minion_schedule_saved', wait=30)
                    if event_ret and event_ret['complete']:
                        ret['result'] = True
    ...
    if 'test' in kwargs and kwargs['test']:
        ...
        try:
            with salt.utils.event.get_event('minion', opts=__opts__) as event_bus:
                ...
                if res:
                    event_ret = event_bus.get_event(tag='/salt/minion/minion_schedule_enabled_complete', wait=30)
                    if event_ret and event_ret['complete']:
                        schedule = event_ret['schedule']
    ...
    if 'test' in kwargs and kwargs['test']:
        ...
        try:
            with salt.utils.event.get_event('minion', opts=__opts__) as event_bus:
                ...
                if res:
                    event_ret = event_bus.get_event(tag='/salt/minion/minion_schedule_disabled_complete', wait=30)
                    if event_ret and event_ret['complete']:
                        schedule = event_ret['schedule']
...
def reload_():
    ...
    if 'schedule' in __pillar__:
        ...
        if out:
            ...
            ret['result'] = False
    sfn = '{}/{}/schedule.conf'.format(__opts__['config_dir'], os.path.dirname(__opts__['default_include']))
    if os.path.isfile(sfn):
        with salt.utils.files.fopen(sfn, 'rb') as fp_:
            try:
                schedule = salt.utils.yaml.safe_load(fp_)
            except salt.utils.yaml.YAMLError as exc:
                ret['comment'].append('Unable to read existing schedule file: {}'.format(exc))
        if schedule:
...
def postpone_job(name, current_time, new_time, **kwargs):
    ...
    if 'test' in __opts__ and __opts__['test']:
        ...
        if name in list_(show_all=True, where='opts', return_yaml=False):
            event_data = {'name': name, 'time': current_time, 'new_time': new_time, 'time_fmt': time_fmt, 'func': 'postpone_job'}
        elif name in list_(show_all=True, where='pillar', return_yaml=False):
            event_data = {'name': name, 'time': current_time, 'new_time': new_time, 'time_fmt': time_fmt, 'where': 'pillar', 'func': 'postpone_job'}
        else:
            ret['comment'] = 'Job {} does not exist.'.format(name)
            ret['result'] = False
            return ret
        try:
            with salt.utils.event.get_event('minion', opts=__opts__) as event_bus:
                res = __salt__['event.fire'](event_data, 'manage_schedule')
                if res:
                    event_ret = event_bus.get_event(tag='/salt/minion/minion_schedule_postpone_job_complete', wait=30)
                    if event_ret and event_ret['complete']:
                        schedule = event_ret['schedule']
                        if name in schedule and schedule[name]['enabled']:
                            ret['result'] = True
                            ret['comment'] = 'Postponed Job {} in schedule.'.format(name)
                        else:
                            ret['result'] = False
                            ret['comment'] = 'Failed to postpone job {} in schedule.'.format(name)
                        return ret
        except KeyError:
...
def skip_job(name, current_time, **kwargs):
    ...
    if 'test' in __opts__ and __opts__['test']:
        ...
        if name in list_(show_all=True, where='opts', return_yaml=False):
            event_data = {'name': name, 'time': current_time, 'time_fmt': time_fmt, 'func': 'skip_job'}
        elif name in list_(show_all=True, where='pillar', return_yaml=False):
            event_data = {'name': name, 'time': current_time, 'time_fmt': time_fmt, 'where': 'pillar', 'func': 'skip_job'}
        else:
            ret['comment'] = 'Job {} does not exist.'.format(name)
            ret['result'] = False
            return ret
        try:
            ...
            with salt.utils.event.get_event('minion', opts=__opts__) as event_bus:
                res = __salt__['event.fire'](event_data, 'manage_schedule')
                if res:
                    event_ret = event_bus.get_event(tag='/salt/minion/minion_schedule_skip_job_complete', wait=30)
                    if event_ret and event_ret['complete']:
                        schedule = event_ret['schedule']
                        if name in schedule and schedule[name]['enabled']:
                            ret['result'] = True
                            ret['comment'] = 'Added Skip Job {} in schedule.'.format(name)
                        else:
                            ret['result'] = False
                            ret['comment'] = 'Failed to skip job {} in schedule.'.format(name)
                        return ret
        except KeyError:
            log.info('Trace')
...
def show_next_fire_time(name, **kwargs):
    ...
    try:
        ...
        with salt.utils.event.get_event('minion', opts=__opts__) as event_bus:
            res = __salt__['event.fire'](event_data, 'manage_schedule')
            if res:
                event_ret = event_bus.get_event(tag='/salt/minion/minion_schedule_next_fire_time_complete', wait=30)
    except KeyError:
        ret = {}
...
def job_status(name, time_fmt='%Y-%m-%dT%H:%M:%S'):
    ...
    def convert_datetime_objects_in_dict_to_string(data_dict, time_fmt):
        return {key: value.strftime(time_fmt) if isinstance(value, datetime.datetime) else value for (key, value) in data_dict.items()}
    schedule = {}
    try:
        log.info('Trace')
        with salt.utils.event.get_event('minion', opts=__opts__) as event_bus:
            res = __salt__['event.fire']({'func': 'job_status', 'name': name, 'fire_event': True}, 'manage_schedule')
            if res:
                event_ret = event_bus.get_event(tag='/salt/minion/minion_schedule_job_status_complete', wait=30)
