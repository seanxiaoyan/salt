...
def __virtual__():
    ...
    if HAS_SSL and OpenSSL_version >= _LooseVersion('0.10'):
        if OpenSSL_version < _LooseVersion('0.14'):
            ...
            log.debug('You should upgrade pyOpenSSL to at least 0.14.1 to enable the use of X509 extensions in the tls module')
        elif OpenSSL_version <= _LooseVersion('0.15'):
            log.debug('You should upgrade pyOpenSSL to at least 0.15.1 to enable the full use of X509 extensions in the tls module')
        return True
    else:
        X509_EXT_ENABLED = False
        return (False, 'PyOpenSSL version 0.10 or later must be installed before this module can be used.')
...
def cert_base_path(cacert_path=None):
    ...
    """
    if not cacert_path:
        cacert_path = __context__.get('ca.contextual_cert_base_path', __salt__['config.option']('ca.contextual_cert_base_path'))
    if not cacert_path:
        cacert_path = __context__.get('ca.cert_base_path', __salt__['config.option']('ca.cert_base_path'))
    return cacert_path
...
def _new_serial(ca_name):
    ...
    """
    hashnum = int(binascii.hexlify(b'_'.join((salt.utils.stringutils.to_bytes(_microtime()), os.urandom(5)))), 16)
...
def _get_basic_info(ca_name, cert, ca_dir=None):
    ...
    subject = '/'
    subject += '/'.join(['{}={}'.format(x, y) for (x, y) in cert.get_subject().get_components()])
    subject += '\n'
...
def _write_cert_to_database(ca_name, cert, cacert_path=None, status='V'):
    ...
    index_data = '{}\t{}\t\t{}\tunknown\t{}'.format(status, expire_date, serial_number, subject)
    ...
    with salt.utils.files.fopen(certp) as fic:
        ...
        if cert.get_version() == 3:
            ...
            with salt.utils.files.fopen(ca_keyp) as fic2:
                try:
                    log.info('Trace')
                    key = OpenSSL.crypto.load_privatekey(OpenSSL.crypto.FILETYPE_PEM, fic2.read())
                    bits = key.bits()
                except Exception:
                    log.info('Trace')
                    bits = 2048
                try:
                    log.info('Trace')
                    days = (datetime.strptime(cert.get_notAfter(), '%Y%m%d%H%M%SZ') - datetime.utcnow()).days
                except (ValueError, TypeError):
                    log.info('Trace')
                    days = 365
                subj = cert.get_subject()
                create_ca(ca_name, bits=bits, days=days, CN=subj.CN, C=subj.C, ST=subj.ST, L=subj.L, O=subj.O, OU=subj.OU, emailAddress=subj.emailAddress, fixmode=True)
    ...
    if isinstance(cert, str):
        try:
            ...
            with salt.utils.files.fopen(cert) as rfh:
                return OpenSSL.crypto.load_certificate(OpenSSL.crypto.FILETYPE_PEM, rfh.read())
        except Exception:
            log.exception('Failed to read cert from path %s', cert)
            return None
    elif not hasattr(cert, 'get_notAfter'):
...
def validate(cert, ca_name, crl_file):
    ...
    if cert_obj is None:
        raise CommandExecutionError('Failed to read cert from {}, see log for details'.format(cert))
    ca_dir = '{}/{}'.format(cert_base_path(), ca_name)
...
def _get_expiration_date(cert):
    ...
    if cert_obj is None:
        raise CommandExecutionError('Failed to read cert from {}, see log for details'.format(cert))
    return datetime.strptime(salt.utils.stringutils.to_str(cert_obj.get_notAfter()), four_digit_year_fmt)
...
def create_ca(ca_name, bits=2048, days=365, CN='localhost', C='US', ST='Utah', L='Salt Lake City', O='SaltStack', OU=None, emailAddress=None, fixmode=False, cacert_path=None, ca_filename=None, digest='sha256', onlyif=None, unless=None, replace=False):
    log.info('Trace')
    ...
    if os.path.exists(ca_keyp):
        with salt.utils.files.fopen(ca_keyp) as fic2:
            try:
                ...
                log.warning('Error loading existing private key %s, generating a new key: %s', ca_keyp, err)
                bck = '{}.unloadable.{}'.format(ca_keyp, datetime.utcnow().strftime('%Y%m%d%H%M%S'))
    ...
    if X509_EXT_ENABLED:
        log.info('Trace')
        ca.add_extensions([OpenSSL.crypto.X509Extension(b'basicConstraints', True, b'CA:TRUE, pathlen:0'), OpenSSL.crypto.X509Extension(b'keyUsage', True, b'keyCertSign, cRLSign'), OpenSSL.crypto.X509Extension(b'subjectKeyIdentifier', False, b'hash', subject=ca)])
        ca.add_extensions([OpenSSL.crypto.X509Extension(b'authorityKeyIdentifier', False, b'issuer:always,keyid:always', issuer=ca)])
    ...
    if os.path.exists(ca_keyp):
        ...
        with salt.utils.files.fopen(ca_keyp) as fic:
            ...
            if old_key.strip() == keycontent.strip():
                ...
                fp = os.open(bck, os.O_CREAT | os.O_RDWR, 384)
                with salt.utils.files.fopen(fp, 'w') as bckf:
                    bckf.write(old_key)
    if write_key:
        log.info('Trace')
        fp = os.open(ca_keyp, os.O_CREAT | os.O_RDWR, 384)
    ...
    with salt.utils.files.fopen(certp, 'wb') as ca_crt:
        ca_crt.write(salt.utils.stringutils.to_bytes(OpenSSL.crypto.dump_certificate(OpenSSL.crypto.FILETYPE_PEM, ca)))
    _write_cert_to_database(ca_name, ca)
    ret = 'Created Private Key: "{}/{}/{}.key." '.format(cert_base_path(), ca_name, ca_filename)
    ret += 'Created CA "{0}": "{1}/{0}/{2}.crt."'.format(ca_name, cert_base_path(), ca_filename)
    return ret
...
def get_extensions(cert_type):
    ...
    assert X509_EXT_ENABLED, 'X509 extensions are not supported in pyOpenSSL prior to version 0.15.1. Your version: {}'.format(OpenSSL_version)
    ext = {}
    if cert_type == '':
    ...
    if not ext['common'] or ext['common'] == '':
        log.info('Trace')
        ext['common'] = {'csr': {'basicConstraints': 'CA:FALSE'}, 'cert': {'authorityKeyIdentifier': 'keyid,issuer:always', 'subjectKeyIdentifier': 'hash'}}
    try:
        log.info('Trace')
    ...
    if not ext['server'] or ext['server'] == '':
        log.info('Trace')
        ext['server'] = {'csr': {'extendedKeyUsage': 'serverAuth', 'keyUsage': 'digitalSignature, keyEncipherment'}, 'cert': {}}
    try:
        log.info('Trace')
    ...
    if not ext['client'] or ext['client'] == '':
        log.info('Trace')
        ext['client'] = {'csr': {'extendedKeyUsage': 'clientAuth', 'keyUsage': 'nonRepudiation, digitalSignature, keyEncipherment'}, 'cert': {}}
    if cert_type not in ext:
        try:
            log.info('Trace')
            ext[cert_type] = __salt__['pillar.get']('tls.extensions:{}'.format(cert_type))
        except NameError as e:
            log.debug('pillar, tls:extensions:%s not available or not operating in a salt context\n%s', cert_type, e)
    retval = ext['common']
    for Use in retval:
        retval[Use].update(ext[cert_type][Use])
    return retval

def create_csr(ca_name, bits=2048, CN='localhost', C='US', ST='Utah', L='Salt Lake City', O='SaltStack', OU=None, emailAddress=None, subjectAltName=None, cacert_path=None, ca_filename=None, csr_path=None, csr_filename=None, digest='sha256', type_ext=False, cert_type='server', replace=False):
    """
    ...
    if not ca_exists(ca_name, ca_filename=ca_filename):
        return 'Certificate for CA named "{}" does not exist, please create it first.'.format(ca_name)
    if not csr_path:
    ...
    try:
        ...
        for (ext, value) in extensions.items():
            ...
            extension_adds.append(OpenSSL.crypto.X509Extension(salt.utils.stringutils.to_bytes(ext), False, value))
    ...
    if subjectAltName:
        if X509_EXT_ENABLED:
            ...
            extension_adds.append(OpenSSL.crypto.X509Extension(b'subjectAltName', False, b', '.join(salt.utils.data.encode(subjectAltName))))
        else:
            raise ValueError('subjectAltName cannot be set as X509 extensions are not supported in pyOpenSSL prior to version 0.15.1. Your version: {}.'.format(OpenSSL_version))
    if X509_EXT_ENABLED:
    ...
    fp = os.open(priv_keyp, os.O_CREAT | os.O_RDWR, 384)
    with salt.utils.files.fopen(fp, 'wb+') as priv_key:
        priv_key.write(salt.utils.stringutils.to_bytes(OpenSSL.crypto.dump_privatekey(OpenSSL.crypto.FILETYPE_PEM, key)))
    with salt.utils.files.fopen(csr_f, 'wb+') as csr:
        csr.write(salt.utils.stringutils.to_bytes(OpenSSL.crypto.dump_certificate_request(OpenSSL.crypto.FILETYPE_PEM, req)))
    ret = 'Created Private Key: "{}{}.key." '.format(csr_path, csr_filename)
    ret += 'Created CSR for "{}": "{}{}.csr."'.format(CN, csr_path, csr_filename)
    return ret

def create_self_signed_cert(tls_dir='tls', bits=2048, days=365, CN='localhost', C='US', ST='Utah', L='Salt Lake City', O='SaltStack', OU=None, emailAddress=None, cacert_path=None, cert_filename=None, digest='sha256', replace=False):
    """
    ...
    if not cert_filename:
        cert_filename = CN
    if not replace and os.path.exists('{}/{}/certs/{}.crt'.format(cert_base_path(), tls_dir, cert_filename)):
    ...
    priv_key_path = '{}/{}/certs/{}.key'.format(cert_base_path(), tls_dir, cert_filename)
    fp = os.open(priv_key_path, os.O_CREAT | os.O_RDWR, 384)
    with salt.utils.files.fopen(fp, 'wb+') as priv_key:
        priv_key.write(salt.utils.stringutils.to_bytes(OpenSSL.crypto.dump_privatekey(OpenSSL.crypto.FILETYPE_PEM, key)))
    ...
    ret = 'Created Private Key: "{}/{}/certs/{}.key." '.format(cert_base_path(), tls_dir, cert_filename)
    ret += 'Created Certificate: "{}/{}/certs/{}.crt."'.format(cert_base_path(), tls_dir, cert_filename)
    return ret
...
def create_ca_signed_cert(ca_name, CN, days=365, cacert_path=None, ca_filename=None, cert_path=None, cert_filename=None, digest='sha256', cert_type=None, type_ext=False, replace=False):
    ...
    if not replace and os.path.exists(os.path.join(os.path.sep.join('{}/{}/certs/{}.crt'.format(cert_base_path(), ca_name, cert_filename).split('/')))):
        return 'Certificate "{}" already exists'.format(cert_filename)
    try:
        maybe_fix_ssl_version(ca_name, cacert_path=cacert_path, ca_filename=ca_filename)
        with salt.utils.files.fopen('{}/{}/{}.crt'.format(cert_base_path(), ca_name, ca_filename)) as fhr:
            ca_cert = OpenSSL.crypto.load_certificate(OpenSSL.crypto.FILETYPE_PEM, fhr.read())
        with salt.utils.files.fopen('{}/{}/{}.key'.format(cert_base_path(), ca_name, ca_filename)) as fhr:
    ...
    try:
        ...
        ret['retcode'] = 1
        ret['comment'] = 'There is no CSR that matches the CN "{}"'.format(cert_filename)
        return ret
    exts = []
    try:
    ...
    return 'Created Certificate for "{}": "{}/{}.crt"'.format(CN, cert_path, cert_filename)
...
def create_pkcs12(ca_name, CN, passphrase='', cacert_path=None, replace=False):
    ...
    set_ca_path(cacert_path)
    if not replace and os.path.exists('{}/{}/certs/{}.p12'.format(cert_base_path(), ca_name, CN)):
        return 'Certificate "{}" already exists'.format(CN)
    try:
        log.info('Trace')
        with salt.utils.files.fopen('{0}/{1}/{1}_ca_cert.crt'.format(cert_base_path(), ca_name)) as fhr:
            ca_cert = OpenSSL.crypto.load_certificate(OpenSSL.crypto.FILETYPE_PEM, fhr.read())
    except OSError:
        log.info('Trace')
    ...
    try:
        log.info('Trace')
        with salt.utils.files.fopen('{}/{}/certs/{}.crt'.format(cert_base_path(), ca_name, CN)) as fhr:
            cert = OpenSSL.crypto.load_certificate(OpenSSL.crypto.FILETYPE_PEM, fhr.read())
        with salt.utils.files.fopen('{}/{}/certs/{}.key'.format(cert_base_path(), ca_name, CN)) as fhr:
            key = OpenSSL.crypto.load_privatekey(OpenSSL.crypto.FILETYPE_PEM, fhr.read())
    except OSError:
        log.info('Trace')
    ...
    with salt.utils.files.fopen('{}/{}/certs/{}.p12'.format(cert_base_path(), ca_name, CN), 'wb') as ofile:
        ofile.write(pkcs12.export(passphrase=salt.utils.stringutils.to_bytes(passphrase)))
    return 'Created PKCS#12 Certificate for "{0}": "{1}/{2}/certs/{0}.p12"'.format(CN, cert_base_path(), ca_name)
...
def cert_info(cert, digest='sha256'):
    ...
    for (key, value) in cert.get_subject().get_components():
        ...
        if isinstance(value, bytes):
            value = salt.utils.stringutils.to_unicode(value)
        subject[key] = value
    ret = {'fingerprint': salt.utils.stringutils.to_unicode(cert.digest(salt.utils.stringutils.to_str(digest))), 'subject': subject, 'issuer': issuer, 'serial_number': cert.get_serial_number(), 'not_before': calendar.timegm(time.strptime(str(cert.get_notBefore().decode(__salt_system_encoding__)), date_fmt)), 'not_after': calendar.timegm(time.strptime(cert.get_notAfter().decode(__salt_system_encoding__), date_fmt))}
    ...
    if 'subjectAltName' in ret.get('extensions', {}):
        ...
        for name in str(ret['extensions']['subjectAltName']).split(', '):
            ...
            if entry not in valid_entries:
                log.error('Cert %s has an entry (%s) which does not start with %s', ret['subject'], name, '/'.join(valid_entries))
            else:
...
def create_empty_crl(ca_name, cacert_path=None, ca_filename=None, crl_file=None, digest='sha256'):
    log.info('Trace')
    ...
    try:
        log.info('Trace')
        with salt.utils.files.fopen('{}/{}/{}.crt'.format(cert_base_path(), ca_name, ca_filename)) as fp_:
            ca_cert = OpenSSL.crypto.load_certificate(OpenSSL.crypto.FILETYPE_PEM, fp_.read())
        with salt.utils.files.fopen('{}/{}/{}.key'.format(cert_base_path(), ca_name, ca_filename)) as fp_:
            ca_key = OpenSSL.crypto.load_privatekey(OpenSSL.crypto.FILETYPE_PEM, fp_.read())
    except OSError:
        log.info('Trace')
        return 'There is no CA named "{}"'.format(ca_name)
    crl = OpenSSL.crypto.CRL()
    crl_text = crl.export(ca_cert, ca_key, digest=salt.utils.stringutils.to_bytes(digest))
...
def revoke_cert(ca_name, CN, cacert_path=None, ca_filename=None, cert_path=None, cert_filename=None, crl_file=None, digest='sha256'):
    ...
    (index_file, expire_date, serial_number, subject) = _get_basic_info(ca_name, client_cert, ca_dir)
    index_serial_subject = '{}\tunknown\t{}'.format(serial_number, subject)
    index_v_data = 'V\t{}\t\t{}'.format(expire_date, index_serial_subject)
    index_r_data_pattern = re.compile('R\\t' + expire_date + '\\t\\d{12}Z\\t' + re.escape(index_serial_subject))
    index_r_data = 'R\t{}\t{}\t{}'.format(expire_date, _four_digit_year_to_two_digit(datetime.utcnow()), index_serial_subject)
    ret = {}
        ...
        for line in fp_:
            ...
            if index_r_data_pattern.match(line):
                ...
                try:
                    ...
                    return '"{}/{}.crt" was already revoked, serial number: {}'.format(cert_path, cert_filename, serial_number)
                except ValueError:
                    ret['retcode'] = 1
                    ret['comment'] = "Revocation date '{}' does not matchformat '{}'".format(revoke_date, two_digit_year_fmt)
                    return ret
            elif index_serial_subject in line:
                __salt__['file.replace'](index_file, index_v_data, index_r_data, backup=False)
                break
    crl = OpenSSL.crypto.CRL()
    with salt.utils.files.fopen(index_file) as fp_:
        for line in fp_:
            ...
            if line.startswith('R'):
                ...
                revoked.set_rev_date(salt.utils.stringutils.to_bytes(revoke_date_2_digit.strftime(four_digit_year_fmt)))
    ...
    return 'Revoked Certificate: "{}/{}.crt", serial number: {}'.format(cert_path, cert_filename, serial_number)
if __name__ == '__main__':
    create_csr('koji', CN='test_system', C='US', ST='Utah', L='Centerville', O='SaltStack', OU=None, emailAddress='test_system@saltstack.org')
