...
def _conf(family='ipv4'):
    ...
    if __grains__['os_family'] == 'RedHat':
        ...
        raise SaltException('Saving iptables to file is not' + ' supported on {}.'.format(__grains__['os']) + ' Please file an issue with SaltStack')
...
def _regex_iptables_save(cmd_output, filters=None):
    ...
    if __context__['iptables.save_filters']:
        _filtered_cmd_output = [line for line in cmd_output.splitlines(True) if not any((reg.search(line) for reg in __context__['iptables.save_filters']))]
...
def build_rule(table='filter', chain=None, command=None, position='', full=None, family='ipv4', **kwargs):
    """
    ...
    if 'protocol' in kwargs:
        if not protocol:
            rule.append('{}-p {}'.format(maybe_add_negation('protocol'), kwargs['protocol']))
            protocol = True
        del kwargs['protocol']
    if 'match' in kwargs:
        ...
        for match in match_value:
            ...
            if 'name_' in kwargs and match.strip() in ('pknock', 'quota2', 'recent'):
                ...
                del kwargs['name_']
        if 'proto' in kwargs and kwargs.get('match') == 'policy':
            rule.append('{}--proto {}'.format(maybe_add_negation('proto'), kwargs['proto']))
            del kwargs['proto']
        del kwargs['match']
    ...
    if 'connstate' in kwargs:
        if '-m state' not in rule:
            rule.append('-m state')
        rule.append('{}--state {}'.format(maybe_add_negation('connstate'), kwargs['connstate']))
    ...
    for multiport_arg in ('dports', 'sports'):
        if multiport_arg in kwargs:
            ...
            elif str(mp_value).startswith('!') or str(mp_value).startswith('not'):
    ...
    if full:
        ...
        return '{} {} -t {} {}{} {} {} {}'.format(_iptables_cmd(family), wait, table, flag, command, chain, position, ' '.join(rule))
    return ' '.join(rule)
...
def set_policy(table='filter', chain=None, policy=None, family='ipv4'):
    ...
    wait = '--wait' if _has_option('--wait', family) else ''
    cmd = '{} {} -t {} -P {} {}'.format(_iptables_cmd(family), wait, table, chain, policy)
...
def check(table='filter', chain=None, rule=None, family='ipv4'):
    ...
    if _has_option('--check', family):
        ...
        __salt__['cmd.run']('{} -t {} -A {} {}'.format(ipt_cmd, table, _chain_name, rule))
...
def insert(table='filter', chain=None, position=None, rule=None, family='ipv4'):
    ...
    if isinstance(returnCheck, bool) and returnCheck:
        return False
    cmd = '{} {} -t {} -I {} {} {}'.format(_iptables_cmd(family), wait, table, chain, position, rule)
    out = __salt__['cmd.run_stderr'](cmd)
    return out
            ...
            while index + 1 < len(args):
                ...
                if args[index].startswith('-'):
                    ...
                    if args[index].startswith('-') or (args[index] == '!' and (not swap)):
                        ...
                        while index + 1 < len(args) and args[index + 1] != '!' and (not args[index + 1].startswith('-')):
...
def _parser():
    ...
    add_arg('--dst-cc', '--destination-country', dest='destination-country', action='append')
