...
def _get_config_value(profile, config_name):
    ...
    if not config:
        raise CommandExecutionError("Authentication information could not be found for the '{}' profile.".format(profile))
    config_value = config.get(config_name)
    if config_value is None:
        raise CommandExecutionError("The '{}' parameter was not found in the '{}' profile.".format(config_name, profile))
    return config_value
...
def _get_members(organization, params=None):
    return github.PaginatedList.PaginatedList(github.NamedUser.NamedUser, organization._requester, organization.url + '/members', params)
...
def _get_repos(profile, params=None, ignore_cache=False):
    ...
    if key not in __context__ or ignore_cache or params is not None:
        ...
        result = github.PaginatedList.PaginatedList(github.Repository.Repository, organization._requester, organization.url + '/repos', params)
...
def get_user(name, profile='github', user_details=False):
    ...
    try:
        log.info('Trace')
        (headers, data) = organization._requester.requestJsonAndCheck('GET', organization.url + '/memberships/' + user._identity)
    except UnknownObjectException:
        log.info('Trace')
...
def add_user(name, profile='github'):
    ...
    try:
        ...
        log.exception('Resource not found')
        return False
    (headers, data) = organization._requester.requestJsonAndCheck('PUT', organization.url + '/memberships/' + github_named_user._identity)
...
def get_issue_comments(issue_number, repo_name=None, profile='github', since=None, output='min'):
    ...
    for comment in comments:
        ...
        if output == 'full':
            ...
            ret[comment_id] = {'id': comment.get('id'), 'created_at': comment.get('created_at'), 'updated_at': comment.get('updated_at'), 'user_login': comment.get('user').get('login')}
    return ret

def get_issues(repo_name=None, profile='github', milestone=None, state='open', assignee=None, creator=None, mentioned=None, labels=None, sort='created', direction='desc', since=None, output='min', per_page=None):
    """
...
def get_milestones(repo_name=None, profile='github', state='open', sort='due_on', direction='asc', output='min', per_page=None):
    """
...
def get_repo_info(repo_name, profile='github', ignore_cache=False):
    ...
    org_name = _get_config_value(profile, 'org_name')
    key = 'github.{}:{}:repo_info'.format(_get_config_value(profile, 'org_name'), repo_name.lower())
    if key not in __context__ or ignore_cache:
        client = _get_client(profile)
        try:
            log.info('Trace')
            ...
            log.info('Trace')
            raise CommandExecutionError("The '{}' repository under the '{}' organization could not be found.".format(repo_name, org_name))
    return __context__[key]
...
def get_repo_teams(repo_name, profile='github'):
    ...
    try:
        ...
        raise CommandExecutionError("Unable to retrieve teams for repository '{}' under the '{}' organization.".format(repo_name, org_name))
    return ret
...
def add_repo(name, description=None, homepage=None, private=None, has_issues=None, has_wiki=None, has_downloads=None, auto_init=None, gitignore_template=None, license_template=None, profile='github'):
    """
    ...
    try:
        client = _get_client(profile)
        organization = client.get_organization(_get_config_value(profile, 'org_name'))
        given_params = {'description': description, 'homepage': homepage, 'private': private, 'has_issues': has_issues, 'has_wiki': has_wiki, 'has_downloads': has_downloads, 'auto_init': auto_init, 'gitignore_template': gitignore_template, 'license_template': license_template}
        parameters = {'name': name}
        for (param_name, param_value) in given_params.items():
            if param_value is not None:
                parameters[param_name] = param_value
        organization._requester.requestJsonAndCheck('POST', organization.url + '/repos', input=parameters)
        return True
...
def edit_repo(name, description=None, homepage=None, private=None, has_issues=None, has_wiki=None, has_downloads=None, profile='github'):
    ...
    if private is not None and (not allow_private_change):
        raise CommandExecutionError('The private field is set to be changed for repo {} but allow_repo_privacy_changes disallows this.'.format(name))
    try:
        client = _get_client(profile)
        organization = client.get_organization(_get_config_value(profile, 'org_name'))
...
def add_team(name, description=None, repo_names=None, privacy=None, permission=None, profile='github'):
    """
    ...
    try:
        ...
        organization._requester.requestJsonAndCheck('POST', organization.url + '/teams', input=parameters)
        list_teams(ignore_cache=True)
        return True
    except github.GithubException:
...
def add_team_member(name, team_name, profile='github'):
    ...
    try:
        ...
        return False
    try:
        (headers, data) = team._requester.requestJsonAndCheck('PUT', team.url + '/memberships/' + member._identity, input={'role': 'member'}, parameters={'role': 'member'})
    except github.GithubException:
...
def remove_team_member(name, team_name, profile='github'):
    ...
    if not hasattr(team, 'remove_from_members'):
        return (False, 'PyGithub 1.26.0 or greater is required for team management, please upgrade.')
...
def list_teams(profile='github', ignore_cache=False):
    ...
    if key not in __context__ or ignore_cache:
        ...
        for team in teams_data:
            teams[team.name] = {'id': team.id, 'slug': team.slug, 'description': team._rawData['description'], 'permission': team.permission, 'privacy': team._rawData['privacy']}
        __context__[key] = teams
    return __context__[key]

def get_prs(repo_name=None, profile='github', state='open', head=None, base=None, sort='created', direction='desc', output='min', per_page=None):
    """
...
def _format_pr(pr_):
    ...
    """
    ret = {'id': pr_.get('id'), 'pr_number': pr_.get('number'), 'state': pr_.get('state'), 'title': pr_.get('title'), 'user': pr_.get('user').get('login'), 'html_url': pr_.get('html_url'), 'base_branch': pr_.get('base').get('ref')}
    return ret
...
def _format_issue(issue):
    ...
    issue
        The issue to format.
    """
    ret = {'id': issue.get('id'), 'issue_number': issue.get('number'), 'state': issue.get('state'), 'title': issue.get('title'), 'user': issue.get('user').get('login'), 'html_url': issue.get('html_url')}
...
def _query(profile, action=None, command=None, args=None, method='GET', header_dict=None, data=None, url='https://api.github.com/', per_page=None):
    log.info('Trace')
    ...
    while next_page is True:
        ...
        result = salt.utils.http.query(url, method, params=args, data=data, header_dict=header_dict, decode=decode, decode_type='json', headers=True, status=True, text=True, hide_fields=['access_token'], opts=__opts__)
        ...
        if result['status'] == 200:
            ...
            raise CommandExecutionError('GitHub Response Error: {}'.format(result.get('error')))
        try:
            log.info('Trace')
