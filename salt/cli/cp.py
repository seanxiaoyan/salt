...
class SaltCP:
    ...
    def _load_files(self):
        ...
        for fn_ in self.opts['src']:
            if os.path.isfile(fn_):
                ...
                salt.utils.stringutils.print_cli('{} is a directory, only files are supported in non-chunked mode. Use "--chunked" command line argument.'.format(fn_))
                sys.exit(1)
        return files
    ...
    def run_chunked(self):
        ...
        reader = salt.utils.gzip_util.compress_file if gzip else salt.utils.itertools.read_file
        _res = salt.utils.minions.CkMinions(self.opts).check_minions(tgt, tgt_type=selected_target_option or 'glob')
        minions = _res['minions']

        def _get_remote_path(fn_):
            if fn_ in self.opts['src']:
                return os.path.join(dest, os.path.basename(fn_)) if dest_is_dir else dest
            else:
        ...
        with salt.client.get_local_client(self.opts['conf_file']) as local:
            for (fn_, mode) in files.items():
                ...
                for chunk in reader(fn_, chunk_size=self.opts['salt_cp_chunk_size']):
                    ...
                    append = index > 1
                    log.debug("Copying %s to %starget '%s' as %s%s", fn_, '{} '.format(selected_target_option) if selected_target_option else '', tgt, remote_path, ' (chunk #{})'.format(index) if append else '')
                    args = [tgt, 'cp.recv_chunked', [remote_path, chunk, append, gzip, mode], timeout]
                    if selected_target_option is not None:
                        log.info('Trace')
                        args.append(selected_target_option)
                    ...
                    if not result:
                        log.info('Trace')
                        msg = 'Publish failed.{} It may be necessary to decrease salt_cp_chunk_size (current value: {})'.format(' File partially transferred.' if index > 1 else '', self.opts['salt_cp_chunk_size'])
            ...
            for dirname in empty_dirs:
                ...
                log.debug("Creating empty dir %s on %starget '%s'", dirname, '{} '.format(selected_target_option) if selected_target_option else '', tgt)
