...
from salt.exceptions import CommandExecutionError, CommandNotFoundError, SaltClientError, SaltInvocationError
...
class BaseCaller:
    ...
    def run(self):
        ...
        try:
            ...
            try:
                ...
                salt.utils.profile.output_profile(pr, stats_path=self.opts.get('profiling_path', '/tmp/stats'), stop=True)
            ...
            salt.output.display_output({'local': print_ret}, out=out, opts=self.opts, _retcode=ret.get('retcode', 0))
    ...
    def call(self):
        ...
        try:
            log.info('Trace')
            sdata = {'fun': fun, 'pid': os.getpid(), 'jid': ret['jid'], 'tgt': 'salt-call'}
            if metadata is not None:
                sdata['metadata'] = metadata
            (args, kwargs) = salt.minion.load_args_and_kwargs(self.minion.functions[fun], salt.utils.args.parse_input(self.opts['arg'], no_parse=self.opts.get('no_parse', [])), data=sdata)
            try:
                ...
                sys.stderr.write('Cannot write to process directory. Do you have permissions to write to {} ?\n'.format(proc_fn))
            func = self.minion.functions[fun]
            data = {'arg': args, 'fun': fun}
            data.update(kwargs)
            executors = getattr(self.minion, 'module_executors', []) or salt.utils.args.yamlify_arg(self.opts.get('module_executors', '[direct_call]'))
            if self.opts.get('executor_opts', None):
                data['executor_opts'] = salt.utils.args.yamlify_arg(self.opts['executor_opts'])
            ...
            if retcode == 0:
                try:
                    func_result = all((ret['return'].get(x, True) for x in ('result', 'success')))
                except Exception:
                    func_result = True
                if not func_result:
                    retcode = salt.defaults.exitcodes.EX_GENERIC
        ...
        if hasattr(self.minion.functions[fun], '__outputter__'):
            ...
            if isinstance(oput, str):
                ret['out'] = oput
        is_local = self.opts['local'] or self.opts.get('file_client', False) == 'local' or self.opts.get('master_type') == 'disable'
...
class ZeroMQCaller(BaseCaller):
    ...
    def return_pub(self, ret):
        ...
        with salt.channel.client.ReqChannel.factory(self.opts, usage='salt_call') as channel:
