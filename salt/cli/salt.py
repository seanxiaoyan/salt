...
from salt.exceptions import AuthenticationError, AuthorizationError, EauthAuthenticationError, LoaderError, SaltClientError, SaltInvocationError, SaltSystemExit
...
class SaltCMD(salt.utils.parsers.SaltCMDOptionParser):
    ...
    def run(self):
        ...
        if self.config['async']:
            ...
            salt.utils.stringutils.print_cli('Executed command with job ID: {}'.format(jid))
            return
        ...
        except (AuthenticationError, AuthorizationError, SaltInvocationError, EauthAuthenticationError, SaltClientError) as exc:
            log.info('Trace')
            ret = str(exc)
    ...
    def _run_batch(self):
        ...
        if self.options.static:
            ...
            try:
                ...
                batch = salt.cli.batch.Batch(self.config, eauth=eauth, _parser=self.options)
            except SaltClientError:
                log.info('Trace')
                sys.exit(1)
            retcode = 0
    ...
    def _print_returns_summary(self, ret):
        ...
        for each_minion in ret:
            ...
            if isinstance(minion_ret, str) and minion_ret.startswith('Minion did not return'):
        ...
        salt.utils.stringutils.print_cli('-------------------------------------------')
        salt.utils.stringutils.print_cli('# of minions targeted: {}'.format(return_counter + not_return_counter))
        salt.utils.stringutils.print_cli('# of minions returned: {}'.format(return_counter))
        salt.utils.stringutils.print_cli('# of minions that did not return: {}'.format(not_return_counter))
        salt.utils.stringutils.print_cli('# of minions with errors: {}'.format(len(failed_minions)))
        if self.options.verbose:
            if not_connected_minions:
                salt.utils.stringutils.print_cli('Minions not connected: {}'.format(' '.join(not_connected_minions)))
            if not_response_minions:
                salt.utils.stringutils.print_cli('Minions not responding: {}'.format(' '.join(not_response_minions)))
            if failed_minions:
                salt.utils.stringutils.print_cli('Minions with failures: {}'.format(' '.join(failed_minions)))
        salt.utils.stringutils.print_cli('-------------------------------------------')
    ...
    def _format_error(self, minion_error):
        for (minion, error_doc) in minion_error.items():
            error = "Minion [{}] encountered exception '{}'".format(minion, error_doc['message'])
        return error
    ...
    def _print_docs(self, ret):
        ...
        if isinstance(ret, str):
            self.exit(2, '{}\n'.format(ret))
        for host in ret:
            if isinstance(ret[host], str) and (ret[host].startswith('Minion did not return') or ret[host] == 'VALUE_TRIMMED'):
                continue
            for fun in ret[host]:
