...
STATE_REQUISITE_KEYWORDS = frozenset(['onchanges', 'onchanges_any', 'onfail', 'onfail_any', 'onfail_all', 'onfail_stop', 'prereq', 'prerequired', 'watch', 'watch_any', 'require', 'require_any', 'listen'])
STATE_REQUISITE_IN_KEYWORDS = frozenset(['onchanges_in', 'onfail_in', 'prereq_in', 'watch_in', 'require_in', 'listen_in'])
STATE_RUNTIME_KEYWORDS = frozenset(['fun', 'state', 'check_cmd', 'failhard', 'onlyif', 'unless', 'creates', 'retry', 'order', 'parallel', 'prereq', 'prereq_in', 'prerequired', 'reload_modules', 'reload_grains', 'reload_pillar', 'runas', 'runas_password', 'fire_event', 'saltenv', 'use', 'use_in', '__env__', '__sls__', '__id__', '__orchestration_jid__', '__pub_user', '__pub_arg', '__pub_jid', '__pub_fun', '__pub_tgt', '__pub_ret', '__pub_pid', '__pub_tgt_type', '__prereq__', '__prerequired__'])
...
def _l_tag(name, id_):
    low = {'name': 'listen_{}'.format(name), '__id__': 'listen_{}'.format(id_), 'state': 'Listen_Error', 'fun': 'Listen_Error'}
    return _gen_tag(low)
...
def find_sls_ids(sls, high):
    ...
    for (nid, item) in high.items():
        try:
            ...
            if nid != '__exclude__':
                log.error("Invalid non-dict item '%s' in high data. Value: %r", nid, item)
            continue
        else:
            if sls_tgt == sls:
                for st_ in item:
...
def mock_ret(cdata):
    ...
    return {'name': name, 'comment': 'Not called, mocked', 'changes': {}, 'result': True}
...
class Compiler:
    ...
    def render_template(self, template, **kwargs):
        ...
        """
        high = compile_template(template, self.rend, self.opts['renderer'], self.opts['renderer_blacklist'], self.opts['renderer_whitelist'], **kwargs)
        if not high:
            return high
    ...
    def verify_high(self, high):
        ...
        for (name, body) in high.items():
            ...
            if not isinstance(name, str):
                errors.append("ID '{}' in SLS '{}' is not formed as a string, but is a {}".format(name, body['__sls__'], type(name).__name__))
            if not isinstance(body, dict):
                err = 'The type {} in {} is not formatted as a dictionary'.format(name, body)
                errors.append(err)
                continue
            for state in body:
                ...
                if not isinstance(body[state], list):
                    errors.append("State '{}' in SLS '{}' is not formed as a list".format(name, body['__sls__']))
                else:
                    fun = 0
                    if '.' in state:
                        fun += 1
                    for arg in body[state]:
                        if isinstance(arg, str):
                            fun += 1
                            if ' ' in arg.strip():
                                errors.append('The function "{}" in state "{}" in SLS "{}" has whitespace, a function with whitespace is not supported, perhaps this is an argument that is missing a ":"'.format(arg, name, body['__sls__']))
                        elif isinstance(arg, dict):
                            argfirst = next(iter(arg))
                            if argfirst in ('require', 'watch', 'prereq', 'onchanges'):
                                if not isinstance(arg[argfirst], list):
                                    errors.append("The {} statement in state '{}' in SLS '{}' needs to be formed as a list".format(argfirst, name, body['__sls__']))
                                else:
                                    ...
                                    for req in arg[argfirst]:
                                        ...
                                        if not isinstance(req, dict):
                                            errors.append('Requisite declaration {} in SLS {} is not formed as a single key dictionary'.format(req, body['__sls__']))
                                            continue
                                        req_key = next(iter(req))
                                        req_val = req[req_key]
                                        if '.' in req_key:
                                            errors.append("Invalid requisite type '{}' in state '{}', in SLS '{}'. Requisite types must not contain dots, did you mean '{}'?".format(req_key, name, body['__sls__'], req_key[:req_key.find('.')]))
                                        if not ishashable(req_val):
                                            errors.append('Illegal requisite "{}", is SLS {}\n'.format(str(req_val), body['__sls__']))
                                            continue
                                            ...
                                            if name in reqs[req_val]:
                                                if reqs[req_val][name] == state:
                                                    if reqs[req_val]['state'] == reqs[name][req_val]:
                                                        errors.append('A recursive requisite was found, SLS "{}" ID "{}" ID "{}"'.format(body['__sls__'], name, req_val))
                                if len(list(arg)) != 1:
                                    errors.append("Multiple dictionaries defined in argument of state '{}' in SLS '{}'".format(name, body['__sls__']))
                    if not fun:
                        if state == 'require' or state == 'watch':
                            continue
                        errors.append("No function declared in state '{}' in SLS '{}'".format(state, body['__sls__']))
                    elif fun > 1:
                        errors.append("Too many functions declared in state '{}' in SLS '{}'".format(state, body['__sls__']))
        return errors
...
class State:
    ...
    def __init__(self, opts, pillar_override=None, jid=None, pillar_enc=None, proxy=None, context=None, mocked=False, loader='states', initial_pillar=None):
        log.info('Trace')
        self._init_kwargs = {'opts': opts, 'pillar_override': pillar_override, 'jid': jid, 'pillar_enc': pillar_enc, 'proxy': proxy, 'context': context, 'mocked': mocked, 'loader': loader, 'initial_pillar': initial_pillar}
        ...
        if initial_pillar and (not self._pillar_override):
            ...
            if self._pillar_override:
                self.opts['pillar'] = salt.utils.dictupdate.merge(self.opts['pillar'], self._pillar_override, self.opts.get('pillar_source_merging_strategy', 'smart'), self.opts.get('renderer', 'yaml'), self.opts.get('pillar_merge_lists', False))
    ...
    def _gather_pillar(self):
        ...
        if self._pillar_override:
            if self._pillar_enc:
                try:
                    log.info('Trace')
                    self._pillar_override = salt.utils.crypt.decrypt(self._pillar_override, self._pillar_enc, translate_newlines=True, renderers=getattr(self, 'rend', None), opts=self.opts, valid_rend=self.opts['decrypt_pillar_renderers'])
                except Exception as exc:
                    log.error('Failed to decrypt pillar override: %s', exc)
            if isinstance(self._pillar_override, str):
                try:
                    log.info('Trace')
                    self._pillar_override = yamlloader.load(self._pillar_override, Loader=yamlloader.SaltYamlSafeLoader)
                except Exception as exc:
        ...
        pillar = salt.pillar.get_pillar(self.opts, self.opts['grains'], self.opts['id'], self.opts['saltenv'], pillar_override=self._pillar_override, pillarenv=self.opts.get('pillarenv'))
    ...
    def _mod_init(self, low):
        ...
        try:
            log.info('Trace')
            self.states['{}.{}'.format(low['state'], low['fun'])]
        except KeyError:
            log.info('Trace')
            return
    ...
    def _aggregate_requisites(self, low, chunks):
        ...
        for chunk in chunks:
            if chunk['state'] == low['state'] and chunk.get('__agg__'):
                for req in frozenset.union(*[STATE_REQUISITE_KEYWORDS, STATE_REQUISITE_IN_KEYWORDS]):
                    if req in chunk:
    ...
    def _run_check(self, low_data):
        ...
        cmd_opts = {}
        POSSIBLE_CMD_ARGS = ('cwd', 'root', 'runas', 'env', 'prepend_path', 'umask', 'timeout', 'success_retcodes')
    ...
    def _run_check_onlyif(self, low_data, cmd_opts):
        ...
        def _check_cmd(cmd):
            if cmd != 0 and ret['result'] is False:
                ret.update({'comment': 'onlyif condition is false', 'skip_watch': True, 'result': True})
                return False
            elif cmd == 0:
    ...
    def _run_check_unless(self, low_data, cmd_opts):
        ...
        def _check_cmd(cmd):
            if cmd == 0:
                ret.update({'comment': 'unless condition is true', 'skip_watch': True, 'result': True})
                return False
            else:
    ...
    def _run_check_creates(self, low_data):
        ...
        if isinstance(low_data['creates'], str) and os.path.exists(low_data['creates']):
            ...
            ret['result'] = True
            ret['skip_watch'] = True
        elif isinstance(low_data['creates'], list) and all([os.path.exists(path) for path in low_data['creates']]):
    ...
    def _load_states(self):
        ...
        if self.states_loader == 'thorium':
            self.states = salt.loader.thorium(self.opts, self.functions, {})
        else:
            self.states = salt.loader.states(self.opts, self.functions, self.utils, self.serializers, context=self.state_con, proxy=self.proxy)
    ...
    def load_modules(self, data=None, proxy=None):
        ...
        if isinstance(data, dict):
            ...
            if data.get('provider', False):
                ...
                for provider in providers:
                    for mod in provider:
                        funcs = salt.loader.raw_mod(self.opts, provider[mod], self.functions)
                        if funcs:
                            for func in funcs:
        ...
        self._load_states()
        self.rend = salt.loader.render(self.opts, self.functions, states=self.states, proxy=self.proxy, context=self.state_con)
    ...
    def verify_data(self, data):
        ...
        if data['name'] and (not isinstance(data['name'], str)):
            errors.append("ID '{}' {}is not formed as a string, but is a {}".format(data['name'], "in SLS '{}' ".format(data['__sls__']) if '__sls__' in data else '', type(data['name']).__name__))
        if errors:
            return errors
        full = data['state'] + '.' + data['fun']
        if full not in self.states:
            if '__sls__' in data:
                errors.append("State '{}' was not found in SLS '{}'".format(full, data['__sls__']))
            ...
            for ind in range(arglen - deflen):
                if aspec.args[ind] not in data:
                    errors.append('Missing parameter {} for state {}'.format(aspec.args[ind], full))
        reqdec = ''
        if 'require' in data:
            reqdec = 'require'
        if 'watch' in data:
            ...
            for req in data[reqdec]:
                ...
                if data['state'] == reqfirst:
                    if fnmatch.fnmatch(data['name'], req[reqfirst]) or fnmatch.fnmatch(data['__id__'], req[reqfirst]):
                        errors.append('Recursive require detected in SLS {} for require {} in ID {}'.format(data['__sls__'], req, data['__id__']))
        return errors
    ...
    def verify_high(self, high):
        ...
        for (name, body) in high.items():
            try:
                ...
                pass
            if not isinstance(name, str):
                errors.append("ID '{}' in SLS '{}' is not formed as a string, but is a {}. It may need to be quoted.".format(name, body['__sls__'], type(name).__name__))
            if not isinstance(body, dict):
                err = 'The type {} in {} is not formatted as a dictionary'.format(name, body)
                errors.append(err)
                continue
            for state in body:
                ...
                if body[state] is None:
                    errors.append("ID '{}' in SLS '{}' contains a short declaration ({}) with a trailing colon. When not passing any arguments to a state, the colon must be omitted.".format(name, body['__sls__'], state))
                    continue
                if not isinstance(body[state], list):
                    errors.append("State '{}' in SLS '{}' is not formed as a list".format(name, body['__sls__']))
                else:
                    ...
                    for arg in body[state]:
                        if isinstance(arg, str):
                            ...
                            if argfirst == 'names':
                                if not isinstance(arg[argfirst], list):
                                    errors.append("The 'names' argument in state '{}' in SLS '{}' needs to be formed as a list".format(name, body['__sls__']))
                            if argfirst in ('require', 'watch', 'prereq', 'onchanges'):
                                if not isinstance(arg[argfirst], list):
                                    errors.append("The {} statement in state '{}' in SLS '{}' needs to be formed as a list".format(argfirst, name, body['__sls__']))
                                else:
                                    ...
                                    for req in arg[argfirst]:
                                        ...
                                        if '.' in req_key:
                                            errors.append("Invalid requisite type '{}' in state '{}', in SLS '{}'. Requisite types must not contain dots, did you mean '{}'?".format(req_key, name, body['__sls__'], req_key[:req_key.find('.')]))
                                        if not ishashable(req_val):
                                            errors.append('Illegal requisite "{}", please check your syntax.\n'.format(req_val))
                                            continue
                                        reqs[name][req_val] = req_key
                                        if req_val in reqs:
                                            if name in reqs[req_val]:
                        ...
                        for (hind, val) in enumerate(high[name][state]):
                            ...
                            if isinstance(arg, dict) and isinstance(val, dict):
                                ...
                                if argfirst == next(iter(high[name][state][hind])):
                                    ...
                                    update = True
                                if argfirst == 'name' and next(iter(high[name][state][hind])) == 'names':
                                    high[name][state][hind] = arg
                        if not update:
                    ...
                    if isinstance(arg, dict):
                        ...
                        if isinstance(items, dict):
                            for (_state, name) in items.items():
                                ...
                                if '.' in _state:
                                    errors.append("Invalid requisite in {}: {} for {}, in SLS '{}'. Requisites must not contain dots, did you mean '{}'?".format(rkey, _state, name, body['__sls__'], _state[:_state.find('.')]))
                        ...
                        if isinstance(items, list):
                            ...
                            for ind in items:
                                if not isinstance(ind, dict):
                                    if ind in high:
                                        ...
                                        for _id in iter(high):
                                            for st8 in [_st8 for _st8 in iter(high[_id]) if not _st8.startswith('__')]:
                                ...
                                if '.' in pstate:
                                    errors.append("Invalid requisite in {}: {} for {}, in SLS '{}'. Requisites must not contain dots, did you mean '{}'?".format(rkey, pstate, pname, body['__sls__'], pstate[:pstate.find('.')]))
                                ...
                                for tup in hinges:
                                    ...
                                    if key == 'prereq':
                                        ext_ids = find_name(name, _state, high, strict=True)
                                        for (ext_id, _req_state) in ext_ids:
                                            ...
                                            extend[ext_id][_req_state].append({'prerequired': [{state: id_}]})
                                        continue
                                    if key == 'use_in':
                                        ext_ids = find_name(name, _state, high, strict=True)
                                        for (ext_id, _req_state) in ext_ids:
    ...
    def _call_parallel_target(cls, instance, init_kwargs, name, cdata, low):
        ...
        try:
            ...
            ret = {'result': False, 'name': name, 'changes': {}, 'comment': 'An exception occurred in this state: {}'.format(trb)}
        utc_finish_time = datetime.datetime.utcnow()
        ...
        if 'retry' in low:
            ...
            if not sys.modules[instance.states[cdata['full']].__module__].__opts__['test']:
                while low['retry']['attempts'] >= retries:
                    ...
                    log.info('State result does not match retry until value, state will be re-run in %s seconds', interval)
                    time.sleep(interval)
                    retry_ret = instance.states[cdata['full']](*cdata['args'], **cdata['kwargs'])
                    utc_start_time = datetime.datetime.utcnow()
                    utc_finish_time = datetime.datetime.utcnow()
                    delta = utc_finish_time - utc_start_time
                    duration = (delta.seconds * 1000000 + delta.microseconds) / 1000.0
                    ...
                    if not ret['comment']:
                        ...
                        _comment = 'Attempt {}: Returned a result of "{}", with the following comment: "{}"'.format(retries, ret['result'], ret['comment'])
                ...
                ret['comment'] = '  '.join(['' if not ret['comment'] else str(ret['comment']), 'The state would be retried every {interval} seconds (with a splay of up to {splay} seconds) a maximum of {attempts} times or until a result of {until} is returned'.format(**low['retry'])])
    ...
    def call_parallel(self, cdata, low):
        ...
        if salt.utils.platform.spawning_platform():
            ...
            instance = self
        proc = salt.utils.process.Process(target=self._call_parallel_target, args=(instance, self._init_kwargs, name, cdata, low), name='ParallelState({})'.format(name))
        proc.start()
        ret = {'name': name, 'result': None, 'changes': {}, 'comment': 'Started in a separate process', 'proc': proc}
        return ret
    ...
    def call(self, low, chunks=None, running=None, retries=1):
        ...
        utc_start_time = datetime.datetime.utcnow()
        local_start_time = utc_start_time - (datetime.datetime.utcnow() - datetime.datetime.now())
        log.info('Running state [%s] at time %s', low['name'].strip() if isinstance(low['name'], str) else low['name'], local_start_time.time().isoformat())
        errors = self.verify_data(low)
        if errors:
            ret = {'result': False, 'name': low['name'], 'changes': {}, 'comment': ''}
            for err in errors:
                ret['comment'] += '{}\n'.format(err)
        ...
        if not low.get('__prereq__'):
            log.info('Executing state %s.%s for [%s]', low['state'], low['fun'], low['name'].strip() if isinstance(low['name'], str) else low['name'])
        if 'provider' in low:
        ...
        inject_globals = {'__low__': immutabletypes.freeze(low), '__running__': immutabletypes.freeze(running) if running else {}, '__instance_id__': self.instance_id, '__lowstate__': immutabletypes.freeze(chunks) if chunks else {}}
        if '__env__' in low:
        ...
        log.info('Completed state [%s] at time %s (duration_in_ms=%s)', low['name'].strip() if isinstance(low['name'], str) else low['name'], local_finish_time.time().isoformat(), duration)
            ...
            if not sys.modules[self.states[cdata['full']].__module__].__opts__['test']:
                if low['retry']['until'] != ret['result']:
                    if low['retry']['attempts'] > retries:
                        ...
                        orig_ret = ret
                        ret = retry_ret
                        ret['comment'] = '\n'.join(['Attempt {}: Returned a result of "{}", with the following comment: "{}"'.format(retries, orig_ret['result'], orig_ret['comment']), '' if not ret['comment'] else ret['comment']])
                        ret['duration'] = ret['duration'] + orig_ret['duration'] + interval * 1000
                        if retries == 1:
                            ret['start_time'] = orig_ret['start_time']
            else:
    ...
    def __eval_slot(self, slot):
        ...
        if fmt[1] != 'salt':
            log.warning('Malformed slot: %s', slot)
            log.warning('Only execution modules are currently supported in slots. This means slot should start with "__slot__:salt:"')
            return slot
        (fun, args, kwargs) = salt.utils.args.parse_function(fmt[2])
        if not fun or fun not in self.functions:
            log.warning('Malformed slot: %s', slot)
            log.warning("Execution module should be specified in a function call format: test.arg('arg', kw='kwarg')")
            return slot
        ...
        if return_get:
            ...
            slot_return = salt.utils.data.traverse_dict_and_list(slot_return, return_get, default=None, delimiter='.')
        if append_data:
    ...
    def verify_retry_data(self, retry_data):
        ...
        """
        retry_defaults = {'until': True, 'attempts': 2, 'splay': 0, 'interval': 30}
        expected_data = {'until': bool, 'attempts': int, 'interval': int, 'splay': int}
            ...
            for (expected_key, value_type) in expected_data.items():
                if expected_key in retry_data:
                    if isinstance(retry_data[expected_key], value_type):
                        ...
                        log.warning("An invalid value was passed for the retry %s, using default value '%s'", expected_key, retry_defaults[expected_key])
                        validated_retry_data[expected_key] = retry_defaults[expected_key]
                else:
                    validated_retry_data[expected_key] = retry_defaults[expected_key]
        else:
            log.warning('State is set to retry, but a valid dict for retry configuration was not found.  Using retry defaults')
            ...
            for low in chunks[:]:
                ...
                for pat in self.opts['grains']['state_runs_disabled']:
                    if fnmatch.fnmatch(state_, pat):
                        comment = 'The state function "{0}" is currently disabled by "{1}", to re-enable, run state.enable {1}.'.format(state_, pat)
                        _tag = _gen_tag(low)
                        disabled[_tag] = {'changes': {}, 'result': False, 'comment': comment, '__run_num__': self.__run_num, '__sls__': low['__sls__']}
                        self.__run_num += 1
                        chunks.remove(low)
                        break
        running = {}
        ...
        for tag in running:
            ...
            if proc:
                if not proc.is_alive():
                    log.info('Trace')
                    ret_cache = os.path.join(self.opts['cachedir'], self.jid, salt.utils.hashutils.sha1_digest(tag))
                    if not os.path.isfile(ret_cache):
                        ret = {'result': False, 'comment': 'Parallel process failed to return', 'name': running[tag]['name'], 'changes': {}}
                    try:
                        with salt.utils.files.fopen(ret_cache, 'rb') as fp_:
                            ret = msgpack_deserialize(fp_.read())
                    except OSError:
                        ret = {'result': False, 'comment': 'Parallel cache failure', 'name': running[tag]['name'], 'changes': {}}
    ...
    def check_requisite(self, low, running, chunks, pre=False):
        ...
        reqs = {'require': [], 'require_any': [], 'watch': [], 'watch_any': [], 'prereq': [], 'onfail': [], 'onfail_any': [], 'onfail_all': [], 'onchanges': [], 'onchanges_any': []}
        if pre:
            reqs['prerequired'] = []
        for r_state in reqs:
                    ...
                    for chunk in chunks:
                        ...
                        try:
                            ...
                            if isinstance(req_val, str):
                                if fnmatch.fnmatch(chunk['name'], req_val) or fnmatch.fnmatch(chunk['__id__'], req_val):
                            ...
                            log.info('Trace')
                            raise SaltRenderError('Could not locate requisite of [{}] present in state with name [{}]'.format(req_key, chunk['name']))
                        except TypeError:
                            log.info('Trace')
                            raise SaltRenderError('Could not locate requisite of [{}] present in state with name [{}]'.format(req_key, chunk['name']))
                    if not found:
            ...
            for chunk in chunks:
                ...
                if r_state.startswith('onfail'):
                    if run_dict[tag]['result'] is True:
                        req_stats.add('onfail')
                        continue
                elif run_dict[tag]['result'] is False:
                    req_stats.add('fail')
                    continue
                ...
                if r_state.startswith('prereq') and (not run_dict[tag]['result'] is None):
                    if not r_state.startswith('prerequired'):
                        req_stats.add('pre')
                elif run_dict[tag].get('__state_ran__', True):
                    req_stats.add('met')
    ...
    def event(self, chunk_ret, length, fire_event=False):
        ...
        if not self.opts.get('local') and (self.opts.get('state_events', True) or fire_event):
            if not self.opts.get('master_uri'):
                ev_func = lambda ret, tag, preload=None: salt.utils.event.get_master_event(self.opts, self.opts['sock_dir'], listen=False).fire_event(ret, tag)
            else:
                ev_func = self.functions['event.fire_master']
            ret = {'ret': chunk_ret}
            if fire_event is True:
                tag = salt.utils.event.tagify([self.jid, self.opts['id'], str(chunk_ret['name'])], 'state_result')
            elif isinstance(fire_event, str):
                tag = salt.utils.event.tagify([self.jid, self.opts['id'], str(fire_event)], 'state_result')
            else:
                tag = salt.utils.event.tagify([self.jid, 'prog', self.opts['id'], str(chunk_ret['__run_num__'])], 'job')
                ret['len'] = length
    ...
    def call_chunk(self, low, running, chunks, depth=0):
        ...
        if not low.get('prerequired'):
            self.active.add(tag)
        requisites = ['require', 'require_any', 'watch', 'watch_any', 'prereq', 'onfail', 'onfail_any', 'onchanges', 'onchanges_any']
        if not low.get('__prereq__'):
        ...
        if status == 'unmet':
            ...
            if lost['require'] or lost['watch'] or lost['prereq'] or lost['onfail'] or lost['onchanges'] or lost['require_any'] or lost['watch_any'] or lost['onfail_any'] or lost['onchanges_any'] or lost.get('prerequired'):
                ...
                run_dict[tag] = {'changes': {}, 'result': False, 'duration': duration, 'start_time': start_time, 'comment': comment, '__run_num__': self.__run_num, '__sls__': low['__sls__']}
                self.__run_num += 1
                ...
                if ctag not in running:
                    if ctag in self.active:
                        if chunk.get('__prerequired__'):
                            ...
                            running[tag] = {'changes': {}, 'result': False, 'comment': 'Recursive requisite found', '__run_num__': self.__run_num, '__sls__': low['__sls__']}
                        self.__run_num += 1
                ...
                for req_lows in reqs.values():
                    for req_low in req_lows:
                        ...
                        if req_ret['result'] is False:
                            key = '{sls}.{_id}'.format(sls=req_low['__sls__'], _id=req_low['__id__'])
                            failed_requisites.add(key)
                _cmt = 'One or more requisite failed: {}'.format(', '.join((str(i) for i in failed_requisites)))
                (start_time, duration) = _calculate_fake_duration()
                running[tag] = {'changes': {}, 'result': False, 'duration': duration, 'start_time': start_time, 'comment': _cmt, '__run_num__': self.__run_num, '__sls__': low['__sls__']}
            ...
            (start_time, duration) = _calculate_fake_duration()
            pre_ret = {'changes': {}, 'result': True, 'duration': duration, 'start_time': start_time, 'comment': 'No changes detected', '__run_num__': self.__run_num, '__sls__': low['__sls__']}
            running[tag] = pre_ret
            self.pre[tag] = pre_ret
            self.__run_num += 1
        elif status == 'onfail':
            (start_time, duration) = _calculate_fake_duration()
            running[tag] = {'changes': {}, 'result': True, 'duration': duration, 'start_time': start_time, 'comment': 'State was not run because onfail req did not change', '__state_ran__': False, '__run_num__': self.__run_num, '__sls__': low['__sls__']}
            self.__run_num += 1
        elif status == 'onchanges':
            (start_time, duration) = _calculate_fake_duration()
            running[tag] = {'changes': {}, 'result': True, 'duration': duration, 'start_time': start_time, 'comment': 'State was not run because none of the onchanges reqs changed', '__state_ran__': False, '__run_num__': self.__run_num, '__sls__': low['__sls__']}
            self.__run_num += 1
        elif low.get('__prereq__'):
            self.pre[tag] = self.call(low, chunks, running)
        else:
            running[tag] = self.call(low, chunks, running)
        if tag in running:
            ...
            for sub_state_data in running[tag].pop('sub_state_run', ()):
                ...
                running[sub_tag] = {'name': sub_state_data['low']['name'], 'changes': sub_state_data['changes'], 'result': sub_state_data['result'], 'duration': sub_state_data.get('duration', duration), 'start_time': sub_state_data.get('start_time', start_time), 'comment': sub_state_data.get('comment', ''), '__state_ran__': True, '__run_num__': self.__run_num, '__sls__': low['__sls__']}
        return running
            ...
            for (key, val) in l_dict.items():
                for listen_to in val:
                    if not isinstance(listen_to, dict):
                        found = False
                        for chunk in chunks:
                            if chunk['__id__'] == listen_to or chunk['name'] == listen_to:
                    ...
                    for (lkey, lval) in listen_to.items():
                        if not any((lkey == cref[0] and lval in cref for cref in crefs)):
                            rerror = {_l_tag(lkey, lval): {'comment': 'Referenced state {}: {} does not exist'.format(lkey, lval), 'name': 'listen_{}:{}'.format(lkey, lval), 'result': False, 'changes': {}}}
                            errors.update(rerror)
                            continue
                        to_tags = [_gen_tag(data) for (cref, data) in crefs.items() if lkey == cref[0] and lval in cref]
                        for to_tag in to_tags:
                            if to_tag not in running:
                                continue
                            if running[to_tag]['changes']:
                                if not any((key[0] == cref[0] and key[1] in cref for cref in crefs)):
                                    rerror = {_l_tag(key[0], key[1]): {'comment': 'Referenced state {}: {} does not exist'.format(key[0], key[1]), 'name': 'listen_{}:{}'.format(key[0], key[1]), 'result': False, 'changes': {}}}
                                    errors.update(rerror)
                                    continue
                                new_chunks = [data for (cref, data) in crefs.items() if key[0] == cref[0] and key[1] in cref]
    ...
    def call_high(self, high, orchestration_jid=None):
        ...
        def _cleanup_accumulator_data():
            accum_data_path = os.path.join(get_accumulator_dir(self.opts['cachedir']), self.instance_id)
            try:
    ...
    def render_template(self, high, template):
        ...
        if not high:
            return (high, errors)
        if not isinstance(high, dict):
            errors.append('Template {} does not render to a dictionary'.format(template))
            return (high, errors)
        invalid_items = ('include', 'exclude', 'extends')
        for item in invalid_items:
            if item in high:
                errors.append("The '{}' declaration found on '{}' is invalid when rendering single templates".format(item, template))
                return (high, errors)
        for name in high:
            if not isinstance(high[name], dict):
                if isinstance(high[name], str):
                    if '.' in high[name]:
                        ...
                        high[name] = {comps[0]: [comps[1]]}
                        continue
                    errors.append('ID {} in template {} is not a dictionary'.format(name, template))
                    continue
            skeys = set()
            for key in sorted(high[name]):
                if key.startswith('_'):
                ...
                if high[name][key] is None:
                    errors.append("ID '{}' in template {} contains a short declaration ({}) with a trailing colon. When not passing any arguments to a state, the colon must be omitted.".format(name, template, key))
                    continue
                if not isinstance(high[name][key], list):
                    continue
                if '.' in key:
                    comps = key.split('.')
                    if comps[0] in skeys:
                        errors.append("ID '{}' in template '{}' contains multiple state declarations of the same type".format(name, template))
                        continue
                    high[name][comps[0]] = high[name].pop(key)
                    high[name][comps[0]].append(comps[1])
                    skeys.add(comps[0])
    ...
    def call_template(self, template):
        ...
        Enforce the states in a template
        """
        high = compile_template(template, self.rend, self.opts['renderer'], self.opts['renderer_blacklist'], self.opts['renderer_whitelist'])
        if not high:
            return high
    ...
    def call_template_str(self, template):
        ...
        """
        high = compile_template_str(template, self.rend, self.opts['renderer'], self.opts['renderer_blacklist'], self.opts['renderer_whitelist'])
        if not high:
            return high
...
class BaseHighState:
    ...
    def __gen_opts(self, opts):
        ...
        if not isinstance(mopts, dict):
            ...
            opts['state_auto_order'] = mopts.get('state_auto_order', opts['state_auto_order'])
            opts['file_roots'] = mopts['file_roots']
            opts['top_file_merging_strategy'] = mopts.get('top_file_merging_strategy', opts.get('top_file_merging_strategy'))
            ...
            opts['state_aggregate'] = opts.get('state_aggregate') or mopts.get('state_aggregate') or False
    ...
    def get_tops(self):
        ...
        if merging_strategy == 'same' and (not self.opts['saltenv']):
            if not self.opts['default_top']:
                raise SaltRenderError("top_file_merging_strategy set to 'same', but no default_top configuration option was set")
        if self.opts['saltenv']:
            contents = self.client.cache_file(self.opts['state_top'], self.opts['saltenv'])
            if contents:
                found = 1
                tops[self.opts['saltenv']] = [compile_template(contents, self.state.rend, self.state.opts['renderer'], self.state.opts['renderer_blacklist'], self.state.opts['renderer_whitelist'], saltenv=self.opts['saltenv'])]
            else:
            ...
            for saltenv in [state_top_saltenv] if state_top_saltenv else self._get_envs():
                contents = self.client.cache_file(self.opts['state_top'], saltenv)
                if contents:
                    found = found + 1
                    tops[saltenv].append(compile_template(contents, self.state.rend, self.state.opts['renderer'], self.state.opts['renderer_blacklist'], self.state.opts['renderer_whitelist'], saltenv=saltenv))
                else:
                    tops[saltenv].append({})
            ...
            if found > 1 and merging_strategy == 'merge' and (not self.opts.get('env_order', None)):
                log.warning("top_file_merging_strategy is set to '%s' and multiple top files were found. Merging order is not deterministic, it may be desirable to either set top_file_merging_strategy to 'same' or use the 'env_order' configuration parameter to specify the merging order.", merging_strategy)
        if found == 0:
            log.debug("No contents found in top file. If this is not expected, verify that the 'file_roots' specified in 'etc/master' are accessible. The 'file_roots' configuration is: %s", repr(self.state.opts['file_roots']))
                ...
                for sls_match in states:
                    for sls in fnmatch.filter(self.avail[saltenv], sls_match):
                        if sls in done[saltenv]:
                            continue
                        tops[saltenv].append(compile_template(self.client.get_state(sls, saltenv).get('dest', False), self.state.rend, self.state.opts['renderer'], self.state.opts['renderer_blacklist'], self.state.opts['renderer_whitelist'], saltenv))
    ...
    def merge_tops(self, tops):
        ...
        try:
            ...
            log.warning("Invalid top_file_merging_strategy '%s', falling back to 'merge'", merging_strategy)
            ...
            for ctop in ctops:
                for (saltenv, targets) in ctop.items():
                    if saltenv == 'include':
                        ...
                        log.debug("Section for saltenv '%s' in the '%s' saltenv's top file will be ignored, as the top_file_merging_strategy is set to 'merge' and the saltenvs do not match", saltenv, cenv)
                        continue
                    elif saltenv in top:
                        log.debug("Section for saltenv '%s' in the '%s' saltenv's top file will be ignored, as this saltenv was already defined in the 'base' top file", saltenv, cenv)
                        continue
                    try:
                        for tgt in targets:
                            top[saltenv][tgt] = ctop[saltenv][tgt]
        ...
        for (cenv, ctops) in tops.items():
            if all([x == {} for x in ctops]):
                ...
                if all([x == {} for x in fallback_tops]):
                    log.error("The '%s' saltenv has no top file, and the fallback saltenv specified by default_top (%s) also has no top file", cenv, default_top)
                    continue
                for ctop in fallback_tops:
                    for (saltenv, targets) in ctop.items():
                        if saltenv != cenv:
                            continue
                        log.debug("The '%s' saltenv has no top file, using the default_top saltenv (%s)", cenv, default_top)
                        for tgt in targets:
                            top[saltenv][tgt] = ctop[saltenv][tgt]
                        break
                    else:
                        log.error("The '%s' saltenv has no top file, and no matches were found in the top file for the default_top saltenv (%s)", cenv, default_top)
                continue
            else:
                for ctop in ctops:
                    for (saltenv, targets) in ctop.items():
                        if saltenv == 'include':
                            ...
                            log.debug("Section for saltenv '%s' in the '%s' saltenv's top file will be ignored, as the top_file_merging_strategy is set to 'same' and the saltenvs do not match", saltenv, cenv)
                            continue
                        try:
                            log.info('Trace')
                            for tgt in targets:
    ...
    def verify_tops(self, tops):
        ...
        for (saltenv, matches) in tops.items():
            ...
            if not isinstance(saltenv, str):
                errors.append('Environment {} in top file is not formed as a string'.format(saltenv))
            if saltenv == '':
                errors.append('Empty saltenv statement in top file')
            if not isinstance(matches, dict):
                errors.append('The top file matches for saltenv {} are not formatted as a dict'.format(saltenv))
            for slsmods in matches.values():
                if not isinstance(slsmods, list):
                    errors.append('Malformed topfile (state declarations not formed as a list)')
                    continue
                for slsmod in slsmods:
                    if isinstance(slsmod, dict):
                        for val in slsmod.values():
                            if not val:
                                errors.append('Improperly formatted top file matcher in saltenv {}: {} file'.format(slsmod, val))
                    elif isinstance(slsmod, str):
                        if not slsmod:
                            errors.append('Environment {} contains an empty sls index'.format(saltenv))
        return errors
    ...
    def render_state(self, sls, saltenv, mods, matches, local=False, context=None):
        ...
        if not local:
            ...
            if not os.path.isfile(fn_):
                errors.append('Specified SLS {} on local filesystem cannot be found.'.format(sls))
        state = None
        if not fn_:
            errors.append('Specified SLS {} in saltenv {} is not available on the salt master or through a configured fileserver'.format(sls, saltenv))
        else:
            try:
                log.info('Trace')
                state = compile_template(fn_, self.state.rend, self.state.opts['renderer'], self.state.opts['renderer_blacklist'], self.state.opts['renderer_whitelist'], saltenv, sls, rendered_sls=mods, context=context)
            ...
            if not isinstance(state, dict):
                ...
                if 'include' in state:
                    if not isinstance(state['include'], list):
                        err = 'Include Declaration in SLS {} is not formed as a list'.format(sls)
                        errors.append(err)
                    else:
                ...
                for inc_sls in include:
                    ...
                    if env_key not in self.avail and '__env__' not in self.avail:
                        msg = "Nonexistent saltenv '{}' found in include of '{}' within SLS '{}:{}'".format(env_key, inc_sls, saltenv, sls)
                        log.error(msg)
                        errors.append(msg)
                        continue
                    if inc_sls.startswith('.'):
                        log.info('Trace')
                        match = re.match('^(\\.+)(.*)$', inc_sls)
                        if match:
                            (levels, include) = match.groups()
                        else:
                            msg = "Badly formatted include {} found in include in SLS '{}:{}'".format(inc_sls, saltenv, sls)
                            log.error(msg)
                            errors.append(msg)
                            continue
                        ...
                        if level_count > len(p_comps):
                            msg = "Attempted relative include of '{}' within SLS '{}:{}' goes beyond top level package ".format(inc_sls, saltenv, sls)
                            log.error(msg)
                            errors.append(msg)
                            continue
                    ...
                    if len(resolved_envs) == 1 or saltenv in resolved_envs:
                        sls_targets = fnmatch.filter(self.avail[saltenv], inc_sls) or [inc_sls]
                        for sls_target in sls_targets:
                            ...
                            if mod_tgt not in mods:
                                (nstate, err) = self.render_state(sls_target, r_env, mods, matches)
                                if nstate:
                        ...
                        if not resolved_envs:
                            msg = 'Unknown include: Specified SLS {}: {} is not available on the salt master in saltenv(s): {} '.format(env_key, inc_sls, ', '.join(matches) if env_key == xenv_key else env_key)
                        elif len(resolved_envs) > 1:
                            msg = 'Ambiguous include: Specified SLS {}: {} is available on the salt master in multiple available saltenvs: {}'.format(env_key, inc_sls, ', '.join(resolved_envs))
                        log.critical(msg)
                        errors.append(msg)
                try:
                    log.info('Trace')
        ...
        for name in state:
            if not isinstance(state[name], dict):
                ...
                if isinstance(state[name], str):
                    if '.' in state[name]:
                        ...
                        state[name] = {'__sls__': sls, '__env__': saltenv, comps[0]: [comps[1]]}
                        continue
            ...
            for key in list(state[name]):
                ...
                if '.' in key:
                    comps = key.split('.')
                    if comps[0] in skeys:
                        errors.append("ID '{}' in SLS '{}' contains multiple state declarations of the same type".format(name, sls))
                        continue
    ...
    def _handle_extend(self, state, sls, saltenv, errors):
        ...
        if 'extend' in state:
            ...
            if not isinstance(ext, dict):
                errors.append("Extension value in SLS '{}' is not a dictionary".format(sls))
                return
            for name in ext:
                if not isinstance(ext[name], dict):
                    errors.append("Extension name '{}' in SLS '{}' is not a dictionary".format(name, sls))
                    continue
    ...
    def _handle_exclude(self, state, sls, saltenv, errors):
        ...
        if 'exclude' in state:
            ...
            if not isinstance(exc, list):
                err = 'Exclude Declaration in SLS {} is not formed as a list'.format(sls)
                errors.append(err)
        ...
        for (saltenv, states) in matches.items():
            for sls_match in states:
                if saltenv in self.avail:
                    ...
                    all_errors.append("No matching salt environment for environment '{}' found".format(saltenv))
                if not statefiles:
                ...
                for sls in statefiles:
                    ...
                    if r_env in mods:
                        continue
                    (state, errors) = self.render_state(sls, saltenv, mods, matches, context=context)
                    if state:
        ...
        for id_ in state:
            if id_ in highstate:
                if highstate[id_] != state[id_]:
                    errors.append("Detected conflicting IDs, SLS IDs need to be globally unique.\n    The conflicting ID is '{}' and is found in SLS '{}:{}' and SLS '{}:{}'".format(id_, highstate[id_]['__env__'], highstate[id_]['__sls__'], state[id_]['__env__'], state[id_]['__sls__']))
        try:
            log.info('Trace')
    ...
    def call_highstate(self, exclude=None, cache=None, cache_name='highstate', force=False, whitelist=None, orchestration_jid=None):
        log.info('Trace')
        '\n        Run the sequence to execute the salt highstate for this minion\n        '
        tag_name = 'no_|-states_|-states_|-None'
        ret = {tag_name: {'result': False, 'comment': 'No states found for this minion', 'name': 'No States', 'changes': {}, '__run_num__': 0}}
        ...
        if not matches:
            msg = 'No Top file or master_tops data matches found. Please see master log for details.'
        ...
        with salt.utils.files.set_umask(63):
            try:
                ...
                if salt.utils.platform.is_windows():
                    self.state.functions['cmd.run'](['attrib', '-R', cfn], python_shell=False, output_loglevel='quiet')
    ...
    def compile_state_usage(self):
        ...
        for (saltenv, states) in self.avail.items():
            env_usage = {'used': [], 'unused': [], 'count_all': 0, 'count_used': 0, 'count_unused': 0}
...
class HighState(BaseHighState):
    ...
    def __init__(self, opts, pillar_override=None, jid=None, pillar_enc=None, proxy=None, context=None, mocked=False, loader='states', initial_pillar=None):
        ...
        self.state = State(self.opts, pillar_override, jid, pillar_enc, proxy=proxy, context=context, mocked=mocked, loader=loader, initial_pillar=initial_pillar)
