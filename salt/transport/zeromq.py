    ...
    if source_ip or source_port:
        if LIBZMQ_VERSION_INFO >= (4, 1, 6) and ZMQ_VERSION_INFO >= (16, 0, 1):
            if source_ip and source_port:
                master_uri = 'tcp://{source_ip}:{source_port};{master_ip}:{master_port}'.format(source_ip=ip_bracket(source_ip), source_port=source_port, master_ip=ip_bracket(master_ip), master_port=master_port)
            elif source_ip and (not source_port):
                master_uri = 'tcp://{source_ip}:0;{master_ip}:{master_port}'.format(source_ip=ip_bracket(source_ip), master_ip=ip_bracket(master_ip), master_port=master_port)
            elif source_port and (not source_ip):
                ip_any = '0.0.0.0' if ipaddress.ip_address(master_ip).version == 4 else ip_bracket('::')
                master_uri = 'tcp://{ip_any}:{source_port};{master_ip}:{master_port}'.format(ip_any=ip_any, source_port=source_port, master_ip=ip_bracket(master_ip), master_port=master_port)
        else:
...
class PublishClient(salt.transport.base.PublishClient):
    ...
    def __init__(self, opts, io_loop, **kwargs):
        ...
        if self.opts['recon_randomize']:
            recon_delay = randint(self.opts['recon_default'], self.opts['recon_default'] + self.opts['recon_max'])
            log.debug("Generated random reconnect delay between '%sms' and '%sms' (%s)", self.opts['recon_default'], self.opts['recon_default'] + self.opts['recon_max'], recon_delay)
        log.debug("Setting zmq_reconnect_ivl to '%sms'", recon_delay)
        self._socket.setsockopt(zmq.RECONNECT_IVL, recon_delay)
        if hasattr(zmq, 'RECONNECT_IVL_MAX'):
            log.debug("Setting zmq_reconnect_ivl_max to '%sms'", self.opts['recon_default'] + self.opts['recon_max'])
    ...
    def master_pub(self):
        ...
        """
        return _get_master_uri(self.opts['master_ip'], self.publish_port, source_ip=self.opts.get('source_ip'), source_port=self.opts.get('source_publish_port'))
    ...
    def _decode_messages(self, messages):
        ...
        if messages_len == 1:
            ...
            if self.opts.get('__role') != 'syndic' and message_target not in ('broadcast', self.hexid) or (self.opts.get('__role') == 'syndic' and message_target not in ('broadcast', 'syndic')):
                log.debug('Publish received for not this minion: %s', message_target)
                raise salt.ext.tornado.gen.Return(None)
            payload = salt.payload.loads(messages[1])
        else:
            raise Exception('Invalid number of messages ({}) in zeromq pubmessage from master'.format(len(messages_len)))
...
class RequestServer(salt.transport.base.DaemonizedRequestServer):
    ...
    def zmq_device(self):
        ...
        if self.opts.get('ipc_mode', '') == 'tcp':
            self.w_uri = 'tcp://127.0.0.1:{}'.format(self.opts.get('tcp_master_workers', 4515))
        else:
            self.w_uri = 'ipc://{}'.format(os.path.join(self.opts['sock_dir'], 'workers.ipc'))
...
class PublishServer(salt.transport.base.DaemonizedPublishServer):
    ...
    def pull_uri(self):
        if self.opts.get('ipc_mode', '') == 'tcp':
            pull_uri = 'tcp://127.0.0.1:{}'.format(self.opts.get('tcp_master_publish_pull', 4514))
        else:
            pull_uri = 'ipc://{}'.format(os.path.join(self.opts['sock_dir'], 'publish_pull.ipc'))
        return pull_uri
    ...
    def pre_fork(self, process_manager):
        ...
        """
        process_manager.add_process(self.publish_daemon, args=(self.publish_payload,))
...
class RequestClient(salt.transport.base.RequestClient):
    ...
    def __init__(self, opts, io_loop):
        ...
        self.message_client = AsyncReqMessageClient(self.opts, master_uri, io_loop=io_loop)
    ...
    def get_master_uri(opts):
        ...
        if 'master_ip' in opts:
            return _get_master_uri(opts['master_ip'], opts['master_port'], source_ip=opts.get('source_ip'), source_port=opts.get('source_ret_port'))
