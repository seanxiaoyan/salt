...
class FutureWithTimeout(salt.ext.tornado.concurrent.Future):
    ...
    def __init__(self, io_loop, future, timeout):
        ...
        if timeout is not None:
            if timeout < 0.1:
                timeout = 0.1
            self._timeout_handle = self.io_loop.add_timeout(self.io_loop.time() + timeout, self._timeout_callback)
        else:
...
class IPCServer:
    ...
    def start(self):
        ...
        with salt.utils.asynchronous.current_ioloop(self.io_loop):
            salt.ext.tornado.netutil.add_accept_handler(self.sock, self.handle_connection)
        self._started = True
...
class IPCMessageSubscriber(IPCClient):
    ...
    def _read(self, timeout, callback=None):
        try:
            try:
                yield self._read_in_progress.acquire(timeout=1e-08)
            ...
            try:
                while True:
                    ...
                    if timeout is None:
                        ...
                        wire_bytes = (yield FutureWithTimeout(self.io_loop, self._read_stream_future, timeout))
                    self._read_stream_future = None
                    timeout = None
