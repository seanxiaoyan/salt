...
class Resolver:
    ...
    def _config_resolver(cls, num_threads=10):
        salt.ext.tornado.netutil.Resolver.configure('salt.ext.tornado.netutil.ThreadedResolver', num_threads=num_threads)
...
class TCPPubClient(salt.transport.base.PublishClient):
    ...
    def connect(self, publish_port, connect_callback=None, disconnect_callback=None):
        ...
        self.message_client = MessageClient(self.opts, self.opts['master_ip'], int(self.publish_port), io_loop=self.io_loop, connect_callback=connect_callback, disconnect_callback=disconnect_callback, source_ip=self.opts.get('source_ip'), source_port=self.opts.get('source_publish_port'))
...
class SaltMessageServer(salt.ext.tornado.tcpserver.TCPServer):
    ...
    @salt.ext.tornado.gen.coroutine
    def handle_stream(self, stream, address, _StreamClosedError=salt.ext.tornado.iostream.StreamClosedError):
        """
...
class TCPClientKeepAlive(salt.ext.tornado.tcpclient.TCPClient):
    ...
    def _create_stream(self, max_buffer_size, af, addr, **kwargs):
        ...
        stream = salt.ext.tornado.iostream.IOStream(sock, max_buffer_size=max_buffer_size)
...
class MessageClient:
    ...
    def getstream(self, **kwargs):
        ...
        while stream is None and (not self._closed and (not self._closing)):
            try:
                log.info('Trace')
                stream = (yield self._tcp_client.connect(self.host, self.port, ssl_options=self.opts.get('ssl'), **kwargs))
...
class TCPPublishServer(salt.transport.base.DaemonizedPublishServer):
    ...
    def publish_daemon(self, publish_payload, presence_callback=None, remove_presence_callback=None):
        ...
        pull_sock = salt.transport.ipc.IPCMessageServer(pull_uri, io_loop=io_loop, payload_handler=publish_payload)
    ...
    def publish(self, payload, **kwargs):
        ...
        if not self.pub_sock:
            self.pub_sock = salt.utils.asynchronous.SyncWrapper(salt.transport.ipc.IPCMessageClient, (pull_uri,), loop_kwarg='io_loop')
...
class TCPReqClient(salt.transport.base.RequestClient):
    ...
    def __init__(self, opts, io_loop, **kwargs):
        ...
        self.message_client = salt.transport.tcp.MessageClient(opts, master_host, int(master_port), io_loop=io_loop, resolver=resolver, source_ip=opts.get('source_ip'), source_port=opts.get('source_ret_port'))
