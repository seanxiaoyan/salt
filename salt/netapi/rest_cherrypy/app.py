...
try:
    ...
    cpstats = None
    logger.warning('Import of cherrypy.cpstats failed. Possible upstream bug: https://github.com/cherrypy/cherrypy/issues/1444')
except ImportError:
    cpstats = None
...
def cors_tool():
    ...
    if cherrypy.request.method == 'OPTIONS':
        ...
        allowed_headers = ['Content-Type', 'X-Auth-Token', 'X-Requested-With']
        ...
        return True
ct_out_map = (('application/json', salt.utils.json.dumps), ('application/x-yaml', functools.partial(salt.utils.yaml.safe_dump, default_flow_style=False)))
...
def hypermedia_handler(*args, **kwargs):
    ...
    try:
    ...
    except (salt.exceptions.AuthenticationError, salt.exceptions.AuthorizationError, salt.exceptions.EauthAuthenticationError, salt.exceptions.TokenAuthenticationError):
        log.info('Trace')
        raise cherrypy.HTTPError(401)
    except salt.exceptions.SaltInvocationError:
        log.info('Trace')
        raise cherrypy.HTTPError(400)
    except (salt.exceptions.SaltDaemonNotRunning, salt.exceptions.SaltReqTimeoutError) as exc:
        log.info('Trace')
        ...
        if hasattr(cherrypy, 'TimeoutError') and isinstance(exc, cherrypy.TimeoutError):
            raise cherrypy.HTTPError(504)
        import traceback
        logger.debug('Error while processing request for: %s', cherrypy.request.path_info, exc_info=True)
        cherrypy.response.status = 500
        ret = {'status': cherrypy.response.status, 'return': '{}'.format(traceback.format_exc()) if cherrypy.config['debug'] else 'An unexpected error occurred'}
...
def urlencoded_processor(entity):
    ...
    if 'kwarg' in unserialized_data:
        unserialized_data['kwarg'] = salt.utils.args.yamlify_arg(unserialized_data['kwarg'])
...
tools_config = {'on_start_resource': [('html_override', html_override_tool), ('salt_token', salt_token_tool)], 'before_request_body': [('cors_tool', cors_tool), ('salt_auth', salt_auth_tool), ('hypermedia_in', hypermedia_in)], 'before_handler': [('lowdata_fmt', lowdata_fmt), ('hypermedia_out', hypermedia_out), ('salt_ip_verify', salt_ip_verify_tool)]}
for (hook, tool_list) in tools_config.items():
    for (idx, tool_config) in enumerate(tool_list):
        (tool_name, tool_fn) = tool_config
        setattr(cherrypy.tools, tool_name, cherrypy.Tool(hook, tool_fn, priority=50 + idx))
...
class LowDataAdapter:
    ...
    """
    exposed = True
    _cp_config = {'tools.salt_token.on': True, 'tools.sessions.on': True, 'tools.sessions.timeout': 60 * 10, 'tools.hypermedia_out.on': True, 'tools.hypermedia_in.on': True, 'tools.lowdata_fmt.on': True, 'tools.salt_ip_verify.on': True}
...
class Minions(LowDataAdapter):
    ...
    def POST(self, **kwargs):
        ...
        **Example response:**
        ...
        return {'return': job_data, '_links': {'jobs': [{'href': '/jobs/{}'.format(i['jid'])} for i in job_data if i]}}
...
class Keys(LowDataAdapter):
    ...
    def POST(self, **kwargs):
        ...
        **Example response:**
        ...
        headers['Content-Disposition'] = 'attachment; filename="saltkeys-{}.tar"'.format(lowstate[0]['id_'])
...
class Login(LowDataAdapter):
    ...
    def POST(self, **kwargs):
        ...
        try:
            ...
            log.info('Trace')
            logger.debug('Configuration for external_auth malformed for eauth %r, and user %r.', token.get('eauth'), token.get('name'), exc_info=True)
            perms = None
...
class Logout(LowDataAdapter):
    ...
    """
    _cp_config = dict(LowDataAdapter._cp_config, **{'tools.salt_auth.on': True, 'tools.lowdata_fmt.on': False})
...
class Events:
    ...
    """
    exposed = True
    _cp_config = dict(LowDataAdapter._cp_config, **{'response.stream': True, 'tools.encode.encoding': 'utf-8', 'tools.salt_auth.on': False, 'tools.hypermedia_in.on': False, 'tools.hypermedia_out.on': False})
    ...
    def GET(self, token=None, salt_token=None):
        ...
        * Be cautious in writing Salt's event stream directly to the DOM. It is
        ...
        def listen():
            ...
            """
            with salt.utils.event.get_event('master', sock_dir=self.opts['sock_dir'], opts=self.opts, listen=True) as event:
...
class WebsocketEndpoint:
    ...
    """
    exposed = True
    _cp_config = dict(LowDataAdapter._cp_config, **{'response.stream': True, 'tools.encode.encoding': 'utf-8', 'tools.salt_auth.on': False, 'tools.hypermedia_in.on': False, 'tools.hypermedia_out.on': False, 'tools.websocket.on': True, 'tools.websocket.handler_cls': websockets.SynchronizingWebsocket})
                    ...
                    if data:
                        try:
                            ...
                            if 'format_events' in kwargs:
                                ...
                                handler.send('data: {}\n\n'.format(salt.utils.json.dumps(data)), False)
...
class Webhook:
    ...
    _cp_config = dict(LowDataAdapter._cp_config, **{'tools.lowdata_fmt.on': True, 'tools.salt_auth.on': True})

    def __init__(self):
        self.opts = cherrypy.config['saltopts']
        self.event = salt.utils.event.get_event('master', sock_dir=self.opts['sock_dir'], opts=self.opts, listen=False)
...
class API:
    ...
    """
    url_map = {'index': LowDataAdapter, 'login': Login, 'logout': Logout, 'token': Token, 'minions': Minions, 'run': Run, 'jobs': Jobs, 'keys': Keys, 'events': Events, 'stats': Stats}
    ...
    def _setattr_url_map(self):
        ...
        if self.apiopts.get('enable_sessions', True) is False:
            ...
            url_blacklist = []
        urls = ((url, cls) for (url, cls) in self.url_map.items() if url not in url_blacklist)
        for (url, cls) in urls:
