...
def get_application(opts):
    ...
    if mod_opts.get('websockets', False):
        ...
        token_pattern = '([0-9A-Fa-f]{{{0}}})'.format(len(getattr(hashlib, opts.get('hash_type', 'md5'))().hexdigest()))
        all_events_pattern = '/all_events/{}'.format(token_pattern)
        formatted_events_pattern = '/formatted_events/{}'.format(token_pattern)
...
def start():
    ...
    if not mod_opts.get('disable_ssl', False):
        if 'ssl_crt' not in mod_opts:
            log.error("Not starting '%s'. Options 'ssl_crt' and 'ssl_key' are required if SSL is not disabled.", __name__)
            return None
