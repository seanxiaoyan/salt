...
ERROR_MAP = {"found character '\\t' that cannot start any token": 'Illegal tab character'}
...
class Loader(BaseLoader):
    ...
    def construct_yaml_omap(self, node):
        ...
        for (key_node, value_node) in node.value:
            ...
            try:
                ...
                log.info('Trace')
                raise ConstructorError('While constructing a mapping {} found unacceptable key {}'.format(node.start_mark, key_node.start_mark))
    ...
    def resolve_sls_tag(self, node):
        ...
        return (tag, deep)
Loader.add_constructor('!aggregate', Loader.construct_sls_aggregate)
Loader.add_constructor('!reset', Loader.construct_sls_reset)
Loader.add_constructor('tag:yaml.org,2002:omap', Loader.construct_yaml_omap)
Loader.add_constructor('tag:yaml.org,2002:str', Loader.construct_sls_str)
Loader.add_constructor('tag:yaml.org,2002:int', Loader.construct_sls_int)
...
Dumper.add_multi_representer(dict, Dumper.represent_odict)
