...
def uefi():
    ...
    efivars_dir = next(filter(os.path.exists, ['/sys/firmware/efi/efivars', '/sys/firmware/efi/vars']), None)
    grains = {'efi': bool(efivars_dir), 'efi-secure-boot': __secure_boot(efivars_dir) if efivars_dir else False}
    return grains
