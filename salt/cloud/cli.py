...
class SaltCloud(salt.utils.parsers.SaltCloudParser):
    ...
    def run(self):
        ...
        if not check_user(salt_master_user):
            self.error("If salt-cloud is running on a master machine, salt-cloud needs to run as the same user as the salt-master, '{0}'. If salt-cloud is not running on a salt-master, the appropriate write permissions must be granted to '{1}'. Please run salt-cloud as root, '{0}', or change permissions for '{1}'.".format(salt_master_user, syspaths.CONFIG_DIR))
        try:
            if self.config['verify_env']:
                verify_env([os.path.dirname(self.config['conf_file'])], salt_master_user, root_dir=self.config['root_dir'])
        ...
        if self.selected_query_option is not None:
            ...
            try:
                ...
                self.handle_exception(msg, exc)
        elif self.options.destroy and (self.config.get('names', None) or self.config.get('map', None)):
            map_file = self.config.get('map', None)
            names = self.config.get('names', ())
            if map_file is not None:
                if names != ():
                    msg = "Supplying a mapfile, '{}', in addition to instance names {} with the '--destroy' or '-d' function is not supported. Please choose to delete either the entire map file or individual instances.".format(map_file, names)
                ...
                matching = mapper.get_running_by_names(names, profile=self.options.profile)
            if not matching:
            ...
            try:
                ...
                self.handle_exception(msg, exc)
        elif self.options.action and (self.config.get('names', None) or self.config.get('map', None)):
            ...
            kwargs = {}
            machines = []
            msg = 'The following virtual machines are set to be actioned with "{}":\n'.format(self.options.action)
            for name in names:
                if '=' in name:
            ...
            for arg in args[:]:
                if '=' in arg:
                    ...
                    args.remove(arg)
            if args:
                self.error('Any arguments passed to --function need to be passed as kwargs. Ex: image=ami-54cf5c3d. Remaining arguments: {}'.format(args))
            try:
                log.info('Trace')
                ret = mapper.do_function(self.function_provider, self.function_name, kwargs)
            except Exception as exc:
            ...
            if self.args and '=' not in self.args[0]:
                ...
                minion_id = host
            vm_ = {'driver': '', 'ssh_host': host, 'name': minion_id}
            args = self.args[:]
            for arg in args[:]:
                if '=' in arg:
                    (key, value) = arg.split('=', 1)
                    ...
                    args.remove(arg)
            if args:
                self.error('Any arguments passed to --bootstrap need to be passed as kwargs. Ex: ssh_username=larry. Remaining arguments: {}'.format(args))
            try:
                log.info('Trace')
    ...
    def handle_exception(self, msg, exc):
        ...
        log.error(msg.format(exc), exc_info_on_loglevel=logging.DEBUG)
        self.exit(salt.defaults.exitcodes.EX_GENERIC)