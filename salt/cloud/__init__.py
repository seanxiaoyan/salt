...
from salt.exceptions import SaltCloudConfigError, SaltCloudException, SaltCloudNotFound, SaltCloudSystemExit
...
def enter_mainloop(target, mapped_args=None, args=None, kwargs=None, pool=None, pool_size=None, callback=None, queue=None):
    ...
    if mapped_args is not None and (not mapped_args):
        msg = 'We are called to asynchronously execute {} but we do no have anything to execute, weird, we bail out'.format(target)
        log.error(msg)
...
class CloudClient:
    ...
    def _opts_defaults(self, **kwargs):
        ...
        if profile:
            ...
            providers = [a.get('provider', '').split(':')[0] for a in tmp_profiles.values() if a.get('provider', '')]
            if providers:
    ...
    def profile(self, profile, names, vm_overrides=None, **kwargs):
        ...
        if isinstance(names, str):
            names = names.split(',')
        return salt.utils.data.simple_types_filter(mapper.run_profile(profile, names, vm_overrides=vm_overrides))
...
class Cloud:
    ...
    def lookup_providers(self, lookup):
        ...
        if ':' in lookup:
            ...
            if alias not in self.opts['providers'] or driver not in self.opts['providers'][alias]:
                raise SaltCloudSystemExit("No cloud providers matched '{}'. Available: {}".format(lookup, ', '.join(self.get_configured_providers())))
        providers = set()
        for (alias, drivers) in self.opts['providers'].items():
            for driver in drivers:
                if lookup in (alias, driver):
                    providers.add((alias, driver))
        if not providers:
            raise SaltCloudSystemExit("No cloud providers matched '{}'. Available selections: {}".format(lookup, ', '.join(self.get_configured_providers())))
        return providers
        ...
        for (alias, drivers) in self.opts['providers'].items():
            for (driver, details) in drivers.items():
                ...
                try:
                    ...
                    log.debug("Failed to execute '%s()' while querying for running nodes: %s", fun, err, exc_info_on_loglevel=logging.DEBUG)
                    pmap[alias][driver] = []
    ...
    def map_providers_parallel(self, query='list_nodes', cached=False):
        ...
        for (alias, drivers) in opts['providers'].items():
            this_query = query
            for (driver, details) in drivers.items():
                if opts.get('selected_query_option') is None and '{}.list_nodes_min'.format(driver) in self.clouds:
            ...
            for (driver, vms) in drivers.items():
                ...
                for (vm_name, details) in vms.items():
                    if vm_name not in names:
                        continue
                    elif driver == 'ec2' and 'aws' in handled_drivers and ('aws' in matches[handled_drivers['aws']]) and (vm_name in matches[handled_drivers['aws']]['aws']):
                        continue
                    elif driver == 'aws' and 'ec2' in handled_drivers and ('ec2' in matches[handled_drivers['ec2']]) and (vm_name in matches[handled_drivers['ec2']]['ec2']):
                        continue
    ...
    def location_list(self, lookup='all'):
        ...
        for (alias, driver) in lookups:
            ...
            if fun not in self.clouds:
                log.debug("The '%s' cloud driver defined under '%s' provider alias is unable to get the locations information", driver, alias)
                continue
            ...
            try:
                ...
                log.error("Failed to get the output of '%s()': %s", fun, err, exc_info_on_loglevel=logging.DEBUG)
        return data
    ...
    def image_list(self, lookup='all'):
        ...
        for (alias, driver) in lookups:
            ...
            if fun not in self.clouds:
                log.debug("The '%s' cloud driver defined under '%s' provider alias is unable to get the images information", driver, alias)
                continue
            if alias not in data:
                data[alias] = {}
            try:
    ...
    def size_list(self, lookup='all'):
        ...
        for (alias, driver) in lookups:
            ...
            if fun not in self.clouds:
                log.debug("The '%s' cloud driver defined under '%s' provider alias is unable to get the sizes information", driver, alias)
                continue
            if alias not in data:
                data[alias] = {}
            try:
    ...
    def destroy(self, names, cached=False):
        ...
        for (alias, driver, name) in vms_to_destroy:
            ...
            if not ret:
                continue
            vm_ = {'name': name, 'profile': None, 'provider': ':'.join([alias, driver]), 'driver': driver}
            minion_dict = salt.config.get_cloud_config_value('minion', vm_, self.opts, default={})
            key_file = os.path.join(self.opts['pki_dir'], 'minions', minion_dict.get('id', name))
            ...
            if not os.path.isfile(key_file) and globbed_key_file and (len(globbed_key_file) == 1):
                salt.utils.cloud.remove_key(self.opts['pki_dir'], os.path.basename(globbed_key_file[0]))
                continue
            print("There are several minion keys who's name starts with '{}'. We need to ask you which one should be deleted:".format(name))
            while True:
        ...
        if names and (not processed):
            raise SaltCloudSystemExit("The following VM's were not found: {}".format(', '.join(names)))
    ...
    def create(self, vm_, local_master=True, sync_sleep=3):
        ...
        if fun not in self.clouds:
            log.error("Creating '%s' using '%s' as the provider cannot complete since '%s' is not available", vm_['name'], vm_['provider'], driver)
            return
        ...
        if deploy:
            if not make_master and 'master' not in minion_dict:
                log.warning("There's no master defined on the '%s' VM settings.", vm_['name'])
        ...
        try:
            ...
            if output is not False and 'sync_after_install' in self.opts:
                if self.opts['sync_after_install'] and self.opts['sync_after_install'] not in ('all', 'beacons', 'clouds', 'engines', 'executors', 'grains', 'log', 'matchers', 'modules', 'output', 'pillar', 'proxymodules', 'renderers', 'returners', 'sdb', 'serializers', 'states', 'thorium', 'utils'):
                    log.warning("Bad option for sync_after_install. Defaulting to 'all'")
                ...
                while time.time() < expiration_time:
                    ...
                    mopts_.update(salt.config.master_config(os.path.join(conf_path, 'master')))
                    with salt.client.get_local_client(mopts=mopts_) as client:
                        ret = client.cmd(vm_['name'], 'saltutil.sync_{}'.format(self.opts['sync_after_install']), timeout=self.opts['timeout'])
                        if ret:
                            log.info('Synchronized the following dynamic modules: %s', ret)
                            break
        except KeyError as exc:
            log.exception('Failed to create VM %s. Configuration value %s needs to be set', vm_['name'], exc)
        try:
    ...
    def extras(self, extra_):
        ...
        if fun not in self.clouds:
            log.error("Creating '%s' using '%s' as the provider cannot complete since '%s' is not available", extra_['name'], extra_['provider'], driver)
            return
        try:
            ...
            log.exception('Failed to perform %s.%s on %s. Configuration value %s needs to be set', extra_['provider'], extra_['action'], extra_['name'], exc)
        return output
    ...
    def run_profile(self, profile, names, vm_overrides=None):
        ...
        for name in names:
            if name in vms:
                ...
                ret[name] = {'Error': msg}
                continue
            vm_ = self.vm_config(name, main_cloud_config, provider_details, profile_details, vm_overrides)
            if self.opts['parallel']:
                process = multiprocessing.Process(target=self.create, args=(vm_,))
                process.start()
                ret[name] = {'Provisioning': 'VM being provisioned in parallel. PID: {}'.format(process.pid)}
                continue
            try:
                ...
                for (vm_name, vm_details) in vms.items():
                    ...
                    if vm_name not in names:
                        ...
                        if 'id' in vm_details and vm_details['id'] in names:
                            ...
                            log.debug("vm:%s in provider:%s is not in name list:'%s'", vm_name, driver, names)
                            continue
                    ...
                    with salt.utils.context.func_globals_inject(self.clouds[fun], __active_provider_name__=':'.join([alias, driver])):
                        ...
                        if kwargs:
                            ret[alias][driver][vm_name] = self.clouds[fun](vm_name, kwargs, call='action')
                        else:
                            ret[alias][driver][vm_name] = self.clouds[fun](vm_name, call='action')
    ...
    def do_function(self, prov, func, kwargs):
        ...
        if len(matches) > 1:
            raise SaltCloudSystemExit("More than one results matched '{}'. Please specify one of: {}".format(prov, ', '.join(['{}:{}'.format(alias, driver) for (alias, driver) in matches])))
        (alias, driver) = matches.pop()
        fun = '{}.{}'.format(driver, func)
        if fun not in self.clouds:
            raise SaltCloudSystemExit("The '{}' cloud provider alias, for the '{}' driver, does not define the function '{}'".format(alias, driver, func))
...
class Map(Cloud):
    ...
    def interpolated_map(self, query='list_nodes', cached=False):
        ...
        for (profile, mapped_vms) in rendered_map.items():
            ...
            if profile not in self.opts['profiles']:
                ...
                msg = "No provider for the mapped '{}' profile was found. Skipped VMS: {}".format(profile, ', '.join(names))
                log.info(msg)
    ...
    def get_vmnames_by_action(self, action):
        ...
        matching_states = {'start': ['stopped'], 'stop': ['running', 'active'], 'reboot': ['running', 'active']}
        vm_names = []
            ...
            for (driver, vms) in drivers.items():
                for (vm_name, vm_details) in vms.items():
                    try:
                        ...
                        log.error("The use of '%s' as an action is not supported in this context. Only 'start', 'stop', and 'reboot' are supported options.", action)
        ...
        if self.opts.get('map', None) is None:
            if self.opts.get('map_data', None) is None:
                if self.opts.get('map_pillar', None) is None:
                    ...
                    log.error("The specified map not found in pillar at 'cloud:maps:%s'", self.opts['map_pillar'])
            ...
            if not os.path.isfile(self.opts['map']):
                if not self.opts['map'].startswith('salt://'):
                    log.error("The specified map file does not exist: '%s'", self.opts['map'])
                    raise SaltCloudNotFound()
            ...
            try:
                ...
                map_ = compile_template(cached_map, rend, renderer, blacklist, whitelist)
            except Exception as exc:
                log.error('Rendering map %s failed, render error:\n%s', self.opts['map'], exc, exc_info_on_loglevel=logging.DEBUG)
                return {}
            if 'include' in map_:
                ...
                for mapping in mapped:
                    ...
                    for (name, overrides) in mapping.items():
                        ...
                        try:
                            ...
                            log.error("Cannot use 'name' as a minion id in a cloud map as it is a reserved word. Please change 'name' to a different minion id reference.")
                            return {}
    ...
    def map_data(self, cached=False):
        ...
        for (profile_name, nodes) in rendered_map.items():
            if profile_name not in self.opts['profiles']:
                msg = "The required profile, '{}', defined in the map does not exist. The defined nodes, {}, will not be created.".format(profile_name, ', '.join(("'{}'".format(node) for node in nodes)))
                log.error(msg)
            ...
            for (nodename, overrides) in nodes.items():
                ...
                for setting in ('grains', 'master', 'minion', 'volumes', 'requires'):
                    ...
                    if deprecated in overrides:
                        log.warning("The use of '%s' on the '%s' mapping has been deprecated. The preferred way now is to just define '%s'. For now, salt-cloud will do the proper thing and convert the deprecated mapping into the preferred one.", deprecated, nodename, setting)
            ...
            for (driver, vms) in drivers.items():
                for (name, details) in vms.items():
                    ...
                    for item in matching:
                        if name not in ret['create']:
                            break
                        log.warning('%r already exists, removing from the create map.', name)
    ...
    def run_map(self, dmap):
        ...
        try:
            ...
            if not isinstance(out, dict):
                ...
                raise SaltCloudSystemExit('An error occurred while creating the master, not continuing: {}'.format(out['Errors']))
            ...
            if master_host is None:
                raise SaltCloudSystemExit('Host for new master {} was not found, aborting map'.format(master_name))
        ...
        for (name, profile) in create_list:
            if name in (master_name, master_minion_name):
                continue
            if 'minion' in profile and profile['minion'].get('local_master', False) and (profile['minion'].get('master', None) is not None):
                local_master = True
...
def create_multiprocessing(parallel_data, queue=None):
    ...
    try:
        output = cloud.create(parallel_data['profile'], local_master=parallel_data['local_master'])
    except SaltCloudException as exc:
        log.error("Failed to deploy '%s'. Error: %s", parallel_data['name'], exc, exc_info_on_loglevel=logging.DEBUG)
...
def destroy_multiprocessing(parallel_data, queue=None):
    ...
    try:
        ...
        with salt.utils.context.func_globals_inject(fun, __active_provider_name__=':'.join([parallel_data['alias'], parallel_data['driver']])):
            output = fun(parallel_data['name'])
    except SaltCloudException as exc:
        log.error('Failed to destroy %s. Error: %s', parallel_data['name'], exc, exc_info_on_loglevel=logging.DEBUG)
        return {parallel_data['name']: {'Error': str(exc)}}
    return {parallel_data['name']: salt.utils.data.simple_types_filter(output)}
...
def run_parallel_map_providers_query(data, queue=None):
    ...
    try:
        with salt.utils.context.func_globals_inject(cloud.clouds[data['fun']], __active_provider_name__=':'.join([data['alias'], data['driver']])):
            return (data['alias'], data['driver'], salt.utils.data.simple_types_filter(cloud.clouds[data['fun']]()))
    except Exception as err:
