...
try:
    ...
    from azure.common import AzureConflictHttpError, AzureMissingResourceHttpError, AzureException
...
def get_configured_provider():
    ...
    """
    return config.is_provider_configured(__opts__, _get_active_provider_name() or __virtualname__, ('subscription_id', 'certificate_path'))
...
def get_conn():
    ...
    """
    certificate_path = config.get_cloud_config_value('certificate_path', get_configured_provider(), __opts__, search_global=False)
    subscription_id = salt.utils.stringutils.to_str(config.get_cloud_config_value('subscription_id', get_configured_provider(), __opts__, search_global=False))
    management_host = config.get_cloud_config_value('management_host', get_configured_provider(), __opts__, search_global=False, default='management.core.windows.net')
    return azure.servicemanagement.ServiceManagementService(subscription_id, certificate_path, management_host)
...
def list_hosted_services(conn=None, call=None):
    ...
    """
    if call == 'action':
        raise SaltCloudSystemExit('The list_hosted_services function must be called with -f or --function')
    if not conn:
        conn = get_conn()
    ret = {}
    ...
    for service in services:
        ...
        for deployment in deployments.deployments:
            ...
            for role in deployment.role_list:
                ret[service.service_name]['deployments'][deployment.name]['role_list'][role.role_name] = {'role_name': role.role_name, 'os_version': role.os_version}
                role_info = conn.get_role(service_name=service.service_name, deployment_name=deployment.name, role_name=role.role_name)
                ret[service.service_name]['deployments'][deployment.name]['role_list'][role.role_name]['role_info'] = {'availability_set_name': role_info.availability_set_name, 'configuration_sets': role_info.configuration_sets, 'data_virtual_hard_disks': role_info.data_virtual_hard_disks, 'os_version': role_info.os_version, 'role_name': role_info.role_name, 'role_size': role_info.role_size, 'role_type': role_info.role_type}
...
def create(vm_):
    ...
    try:
        log.info('Trace')
        if vm_['profile'] and config.is_profile_configured(__opts__, _get_active_provider_name() or 'azure', vm_['profile'], vm_=vm_) is False:
            return False
    except AttributeError:
        log.info('Trace')
        pass
    ...
    if 'location' in vm_:
        ...
        loc_error = True
    if loc_error:
        raise SaltCloudSystemExit('Either a location or affinity group must be specified, but not both')
    ...
    __utils__['cloud.fire_event']('event', 'requesting instance', 'salt/cloud/{}/requesting'.format(vm_['name']), args=__utils__['cloud.filter_event']('requesting', event_kwargs, list(event_kwargs)), sock_dir=__opts__['sock_dir'], transport=__opts__['transport'])
    log.debug('vm_kwargs: %s', vm_kwargs)
    try:
        log.info('Trace')
        ...
        if error in str(exc):
            log.error('Error creating %s on Azure.\n\nThe hosted service name is invalid. The name can contain only letters, numbers, and hyphens. The name must start with a letter and must end with a letter or a number.', vm_['name'], exc_info_on_loglevel=logging.DEBUG)
        else:
    ...
    try:
        ...
        error = 'The hosted service name is invalid.'
        if error in str(exc):
            log.error('Error creating %s on Azure.\n\nThe VM name is invalid. The name can contain only letters, numbers, and hyphens. The name must start with a letter and must end with a letter or a number.', vm_['name'], exc_info_on_loglevel=logging.DEBUG)
        else:
    ...
    if volumes:
        __utils__['cloud.fire_event']('event', 'attaching volumes', 'salt/cloud/{}/attaching_volumes'.format(vm_['name']), args=__utils__['cloud.filter_event']('attaching_volumes', vm_, ['volumes']), sock_dir=__opts__['sock_dir'], transport=__opts__['transport'])
...
def create_attach_volumes(name, kwargs, call=None, wait_to_finish=True):
    ...
    for volume in volumes:
        ...
        volume.setdefault('media_link', kwargs['media_link'][:-4] + '-disk-{}.vhd'.format(volume['lun']))
        volume.setdefault('disk_label', kwargs['role_name'] + '-disk-{}'.format(volume['lun']))
        volume_dict = {'volume_name': volume['lun'], 'disk_label': volume['disk_label']}
        kwargs_add_data_disk = ['lun', 'host_caching', 'media_link', 'disk_label', 'disk_name', 'logical_disk_size_in_gb', 'source_media_link']
        for key in set(volume.keys()) - set(kwargs_add_data_disk):
            del volume[key]
        attach = conn.add_data_disk(kwargs['service_name'], kwargs['deployment_name'], kwargs['role_name'], **volume)
        log.debug(attach)
        if attach:
            msg = '{} attached to {} (aka {})'.format(volume_dict['volume_name'], kwargs['role_name'], name)
            log.info(msg)
            ret.append(msg)
        else:
...
def create_attach_volumes(name, kwargs, call=None, wait_to_finish=True):
    ...
    for volume in volumes:
        ...
        for key in set(volume.keys()) - set(kwargs_add_data_disk):
            del volume[key]
        result = conn.add_data_disk(kwargs['service_name'], kwargs['deployment_name'], kwargs['role_name'], **volume)
        _wait_for_async(conn, result.request_id)
        msg = '{} attached to {} (aka {})'.format(volume_dict['volume_name'], kwargs['role_name'], name)
        log.info(msg)
        ret.append(msg)
...
def _wait_for_async(conn, request_id):
    ...
    if result.status != 'Succeeded':
        log.info('Trace')
        raise AzureException('Operation failed. {message} ({code})'.format(message=result.error.message, code=result.error.code))
...
def destroy(name, conn=None, call=None, kwargs=None):
    ...
    _wait_for_async(conn, result.request_id)
    ret[name] = {delete_type: {'request_id': result.request_id}}
    if __opts__.get('update_cachedir', False) is True:
        __utils__['cloud.delete_minion_cachedir'](name, _get_active_provider_name().split(':')[0], __opts__)
    cleanup_disks = config.get_cloud_config_value('cleanup_disks', get_configured_provider(), __opts__, search_global=False, default=False)
    if cleanup_disks:
        cleanup_vhds = kwargs.get('delete_vhd', config.get_cloud_config_value('cleanup_vhds', get_configured_provider(), __opts__, search_global=False, default=False))
        ...
        def wait_for_destroy():
            ...
            time.sleep(5)
            return False
        data = salt.utils.cloud.wait_for_fun(wait_for_destroy, timeout=config.get_cloud_config_value('wait_for_fun_timeout', {}, __opts__, default=15 * 60))
        ret[name]['delete_disk'] = {'name': disk_name, 'delete_vhd': cleanup_vhds, 'data': data}
        cleanup_services = config.get_cloud_config_value('cleanup_services', get_configured_provider(), __opts__, search_global=False, default=False)
        if cleanup_services:
            log.info('Trace')
            ...
            def wait_for_disk_delete():
                ...
                time.sleep(5)
                return False
            data = salt.utils.cloud.wait_for_fun(wait_for_disk_delete, timeout=config.get_cloud_config_value('wait_for_fun_timeout', {}, __opts__, default=15 * 60))
...
def get_operation_status(kwargs=None, conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The show_instance function must be called with -f or --function.')
    if kwargs is None:
        kwargs = {}
    if 'id' not in kwargs:
    ...
    ret = {'http_status_code': data.http_status_code, 'id': kwargs['id'], 'status': data.status}
    if hasattr(data.error, 'code'):
        ret['error'] = {'code': data.error.code, 'message': data.error.message}
    return ret
...
def list_storage(kwargs=None, conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The list_storage function must be called with -f or --function.')
    if not conn:
        conn = get_conn()
...
def show_storage(kwargs=None, conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The show_storage function must be called with -f or --function.')
    if not conn:
        conn = get_conn()
    if kwargs is None:
        kwargs = {}
...
def show_storage_keys(kwargs=None, conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The show_storage_keys function must be called with -f or --function.')
    if not conn:
        conn = get_conn()
    if kwargs is None:
        kwargs = {}
...
def update_storage(kwargs=None, conn=None, call=None):
    ...
    if 'name' not in kwargs:
        raise SaltCloudSystemExit('A name must be specified as "name"')
    data = conn.update_storage_account(service_name=kwargs['name'], label=kwargs.get('label', None), description=kwargs.get('description', None), extended_properties=kwargs.get('extended_properties', None), geo_replication_enabled=kwargs.get('geo_replication_enabled', None), account_type=kwargs.get('account_type', 'Standard_GRS'))
...
def regenerate_storage_keys(kwargs=None, conn=None, call=None):
    ...
    try:
        data = conn.regenerate_storage_account_keys(service_name=kwargs['name'], key_type=kwargs['key_type'])
...
def delete_storage(kwargs=None, conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The delete_storage function must be called with -f or --function.')
    if kwargs is None:
        kwargs = {}
    if 'name' not in kwargs:
        raise SaltCloudSystemExit('A name must be specified as "name"')
...
def list_services(kwargs=None, conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The list_services function must be called with -f or --function.')
    if not conn:
        conn = get_conn()
...
def create_service(kwargs=None, conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The create_service function must be called with -f or --function.')
    if not conn:
        conn = get_conn()
    if kwargs is None:
        kwargs = {}
    ...
    if 'location' not in kwargs and 'affinity_group' not in kwargs:
        raise SaltCloudSystemExit('Either a location or an affinity_group must be specified (but not both)')
    try:
        data = conn.create_hosted_service(kwargs['name'], kwargs['label'], kwargs.get('description', None), kwargs.get('location', None), kwargs.get('affinity_group', None), kwargs.get('extended_properties', None))
...
def delete_service(kwargs=None, conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The delete_service function must be called with -f or --function.')
    if kwargs is None:
        kwargs = {}
    if 'name' not in kwargs:
        raise SaltCloudSystemExit('A name must be specified as "name"')
...
def list_disks(kwargs=None, conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The list_disks function must be called with -f or --function.')
    if not conn:
        conn = get_conn()
...
def update_disk(kwargs=None, conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The show_disk function must be called with -f or --function.')
    if not conn:
        conn = get_conn()
    if kwargs is None:
        kwargs = {}
    ...
    data = conn.update_disk(disk_name=kwargs['name'], has_operating_system=kwargs.get('has_operating_system', old_data['has_operating_system']), label=kwargs.get('label', old_data['label']), media_link=kwargs.get('media_link', old_data['media_link']), name=kwargs.get('new_name', old_data['name']), os=kwargs.get('os', old_data['os']))
...
def list_service_certificates(kwargs=None, conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The list_service_certificates function must be called with -f or --function.')
    if kwargs is None:
        kwargs = {}
    if 'name' not in kwargs:
...
def show_service_certificate(kwargs=None, conn=None, call=None):
    ...
    data = conn.get_service_certificate(kwargs['name'], kwargs['thumbalgorithm'], kwargs['thumbprint'])
    return object_to_dict(data)
...
def add_service_certificate(kwargs=None, conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The add_service_certificate function must be called with -f or --function.')
    if not conn:
        conn = get_conn()
    if kwargs is None:
        kwargs = {}
    ...
    try:
        data = conn.add_service_certificate(kwargs['name'], kwargs['data'], kwargs['certificate_format'], kwargs['password'])
        return {'Success': 'The service certificate was successfully added'}
    except AzureConflictHttpError:
        raise SaltCloudSystemExit('There was a conflict. This usually means that the service certificate already exists.')
...
def delete_service_certificate(kwargs=None, conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The delete_service_certificate function must be called with -f or --function.')
    if kwargs is None:
        kwargs = {}
    if 'name' not in kwargs:
        raise SaltCloudSystemExit('A name must be specified as "name"')
    ...
    try:
        log.info('Trace')
        data = conn.delete_service_certificate(kwargs['name'], kwargs['thumbalgorithm'], kwargs['thumbprint'])
...
def list_management_certificates(kwargs=None, conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The list_management_certificates function must be called with -f or --function.')
    if not conn:
        conn = get_conn()
...
def show_management_certificate(kwargs=None, conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The get_management_certificate function must be called with -f or --function.')
    if not conn:
        conn = get_conn()
    if kwargs is None:
        kwargs = {}
...
def add_management_certificate(kwargs=None, conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The add_management_certificate function must be called with -f or --function.')
    if not conn:
        conn = get_conn()
    if kwargs is None:
        kwargs = {}
    ...
    try:
        log.info('Trace')
        conn.add_management_certificate(kwargs['name'], kwargs['thumbprint'], kwargs['data'])
        return {'Success': 'The management certificate was successfully added'}
    except AzureConflictHttpError:
        log.info('Trace')
        raise SaltCloudSystemExit('There was a conflict. This usually means that the management certificate already exists.')
...
def delete_management_certificate(kwargs=None, conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The delete_management_certificate function must be called with -f or --function.')
    if kwargs is None:
        kwargs = {}
    if 'thumbprint' not in kwargs:
        raise SaltCloudSystemExit('A thumbprint must be specified as "thumbprint"')
...
def list_virtual_networks(kwargs=None, conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The list_virtual_networks function must be called with -f or --function.')
...
def list_input_endpoints(kwargs=None, conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The list_input_endpoints function must be called with -f or --function.')
    if kwargs is None:
        kwargs = {}
    ...
    path = 'services/hostedservices/{}/deployments/{}'.format(kwargs['service'], kwargs['deployment'])
    data = query(path)
    if data is None:
        raise SaltCloudSystemExit('There was an error listing endpoints with the {} service on the {} deployment.'.format(kwargs['service'], kwargs['deployment']))
    ret = {}
    for item in data:
        if 'Role' in item:
            role = item['Role']
...
def show_input_endpoint(kwargs=None, conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The show_input_endpoint function must be called with -f or --function.')
    if kwargs is None:
        kwargs = {}
    if 'name' not in kwargs:
...
def update_input_endpoint(kwargs=None, conn=None, call=None, activity='update'):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The update_input_endpoint function must be called with -f or --function.')
    if kwargs is None:
        kwargs = {}
    if 'service' not in kwargs:
        raise SaltCloudSystemExit('A service name must be specified as "service"')
    ...
    for endpoint in old_endpoints:
        if old_endpoints[endpoint]['Name'] == kwargs['name']:
            ...
            this_endpoint_xml = endpoint_xml.format(local_port=old_endpoints[endpoint]['LocalPort'], name=old_endpoints[endpoint]['Name'], port=old_endpoints[endpoint]['Port'], protocol=old_endpoints[endpoint]['Protocol'], enable_direct_server_return=old_endpoints[endpoint]['EnableDirectServerReturn'], timeout_for_tcp_idle_connection=old_endpoints[endpoint].get('IdleTimeoutInMinutes', 4))
            endpoints_xml += this_endpoint_xml
    ...
    path = 'services/hostedservices/{}/deployments/{}/roles/{}'.format(kwargs['service'], kwargs['deployment'], kwargs['role'])
    query(path=path, method='PUT', header_dict={'Content-Type': 'application/xml'}, data=request_xml, decode=False)
    return True
...
def show_deployment(kwargs=None, conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The get_deployment function must be called with -f or --function.')
    if not conn:
        conn = get_conn()
    if kwargs is None:
        kwargs = {}
    ...
    data = conn.get_deployment_by_name(service_name=kwargs['service_name'], deployment_name=kwargs['deployment_name'])
    return object_to_dict(data)
...
def show_affinity_group(kwargs=None, conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The show_affinity_group function must be called with -f or --function.')
    if not conn:
        conn = get_conn()
    if kwargs is None:
        kwargs = {}
...
def create_affinity_group(kwargs=None, conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The create_affinity_group function must be called with -f or --function.')
    if not conn:
        conn = get_conn()
    if kwargs is None:
        kwargs = {}
    ...
    try:
        conn.create_affinity_group(kwargs['name'], kwargs['label'], kwargs['location'], kwargs.get('description', None))
...
def update_affinity_group(kwargs=None, conn=None, call=None):
    ...
    if 'label' not in kwargs:
        raise SaltCloudSystemExit('A label must be specified as "label"')
    conn.update_affinity_group(affinity_group_name=kwargs['name'], label=kwargs['label'], description=kwargs.get('description', None))
...
def make_blob_url(kwargs=None, storage_conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The make_blob_url function must be called with -f or --function.')
    if kwargs is None:
        kwargs = {}
    ...
    data = storage_conn.make_blob_url(kwargs['container'], kwargs['blob'], kwargs.get('account', None), kwargs.get('protocol', None), kwargs.get('host_base', None))
    ret = {}
...
def list_storage_containers(kwargs=None, storage_conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The list_storage_containers function must be called with -f or --function.')
    if not storage_conn:
        storage_conn = get_storage_conn(conn_kwargs=kwargs)
...
def create_storage_container(kwargs=None, storage_conn=None, call=None):
    ...
    if not storage_conn:
        storage_conn = get_storage_conn(conn_kwargs=kwargs)
    try:
        storage_conn.create_container(container_name=kwargs['name'], x_ms_meta_name_values=kwargs.get('meta_name_values', None), x_ms_blob_public_access=kwargs.get('blob_public_access', None), fail_on_exist=kwargs.get('fail_on_exist', False))
...
def show_storage_container(kwargs=None, storage_conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The show_storage_container function must be called with -f or --function.')
    if kwargs is None:
        kwargs = {}
    if 'name' not in kwargs:
    ...
    if not storage_conn:
        storage_conn = get_storage_conn(conn_kwargs=kwargs)
    data = storage_conn.get_container_properties(container_name=kwargs['name'], x_ms_lease_id=kwargs.get('lease_id', None))
    return data
...
def show_storage_container_metadata(kwargs=None, storage_conn=None, call=None):
    ...
    if not storage_conn:
        storage_conn = get_storage_conn(conn_kwargs=kwargs)
    data = storage_conn.get_container_metadata(container_name=kwargs['name'], x_ms_lease_id=kwargs.get('lease_id', None))
    return data
...
def set_storage_container_metadata(kwargs=None, storage_conn=None, call=None):
    ...
    if 'name' not in kwargs:
        raise SaltCloudSystemExit('An storage container name must be specified as "name"')
    x_ms_meta_name_values = salt.utils.yaml.safe_load(kwargs.get('meta_name_values', ''))
    if not storage_conn:
        storage_conn = get_storage_conn(conn_kwargs=kwargs)
    try:
        storage_conn.set_container_metadata(container_name=kwargs['name'], x_ms_meta_name_values=x_ms_meta_name_values, x_ms_lease_id=kwargs.get('lease_id', None))
...
def show_storage_container_acl(kwargs=None, storage_conn=None, call=None):
    ...
    if not storage_conn:
        storage_conn = get_storage_conn(conn_kwargs=kwargs)
    data = storage_conn.get_container_acl(container_name=kwargs['name'], x_ms_lease_id=kwargs.get('lease_id', None))
    return data
...
def set_storage_container_acl(kwargs=None, storage_conn=None, call=None):
    ...
    if not storage_conn:
        storage_conn = get_storage_conn(conn_kwargs=kwargs)
    try:
        data = storage_conn.set_container_acl(container_name=kwargs['name'], signed_identifiers=kwargs.get('signed_identifiers', None), x_ms_blob_public_access=kwargs.get('blob_public_access', None), x_ms_lease_id=kwargs.get('lease_id', None))
        return {'Success': 'The storage container was successfully updated'}
    except AzureConflictHttpError:
        raise SaltCloudSystemExit('There was a conflict.')
...
def delete_storage_container(kwargs=None, storage_conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The delete_storage_container function must be called with -f or --function.')
    if kwargs is None:
        kwargs = {}
    if 'name' not in kwargs:
        raise SaltCloudSystemExit('An storage container name must be specified as "name"')
    if not storage_conn:
        storage_conn = get_storage_conn(conn_kwargs=kwargs)
    data = storage_conn.delete_container(container_name=kwargs['name'], fail_not_exist=kwargs.get('fail_not_exist', None), x_ms_lease_id=kwargs.get('lease_id', None))
    return data
...
def lease_storage_container(kwargs=None, storage_conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The lease_storage_container function must be called with -f or --function.')
    if kwargs is None:
        kwargs = {}
    if 'name' not in kwargs:
        raise SaltCloudSystemExit('An storage container name must be specified as "name"')
    ...
    if kwargs.get('lease_action', None) not in lease_actions:
        raise SaltCloudSystemExit('A lease_action must be one of: {}'.format(', '.join(lease_actions)))
    if kwargs['lease_action'] != 'acquire' and 'lease_id' not in kwargs:
        raise SaltCloudSystemExit('A lease ID must be specified for the "{}" lease action as "lease_id"'.format(kwargs['lease_action']))
    if not storage_conn:
        storage_conn = get_storage_conn(conn_kwargs=kwargs)
    data = storage_conn.lease_container(container_name=kwargs['name'], x_ms_lease_action=kwargs['lease_action'], x_ms_lease_id=kwargs.get('lease_id', None), x_ms_lease_duration=kwargs.get('lease_duration', 60), x_ms_lease_break_period=kwargs.get('lease_break_period', None), x_ms_proposed_lease_id=kwargs.get('proposed_lease_id', None))
    return data
...
def list_blobs(kwargs=None, storage_conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The list_blobs function must be called with -f or --function.')
    if kwargs is None:
        kwargs = {}
    if 'container' not in kwargs:
...
def show_blob_service_properties(kwargs=None, storage_conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The show_blob_service_properties function must be called with -f or --function.')
    if not storage_conn:
        storage_conn = get_storage_conn(conn_kwargs=kwargs)
...
def set_blob_service_properties(kwargs=None, storage_conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The set_blob_service_properties function must be called with -f or --function.')
    if kwargs is None:
        kwargs = {}
    ...
    if not storage_conn:
        storage_conn = get_storage_conn(conn_kwargs=kwargs)
    data = storage_conn.get_blob_service_properties(storage_service_properties=kwargs['properties'], timeout=kwargs.get('timeout', None))
    return data
...
def show_blob_properties(kwargs=None, storage_conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The show_blob_properties function must be called with -f or --function.')
    if kwargs is None:
        kwargs = {}
    if 'container' not in kwargs:
    ...
    if not storage_conn:
        storage_conn = get_storage_conn(conn_kwargs=kwargs)
    try:
        data = storage_conn.get_blob_properties(container_name=kwargs['container'], blob_name=kwargs['blob'], x_ms_lease_id=kwargs.get('lease_id', None))
    except AzureMissingResourceHttpError:
...
def set_blob_properties(kwargs=None, storage_conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The set_blob_properties function must be called with -f or --function.')
    if kwargs is None:
        kwargs = {}
    if 'container' not in kwargs:
    ...
    if not storage_conn:
        storage_conn = get_storage_conn(conn_kwargs=kwargs)
    data = storage_conn.get_blob_properties(container_name=kwargs['container'], blob_name=kwargs['blob'], x_ms_blob_cache_control=kwargs.get('blob_cache_control', None), x_ms_blob_content_type=kwargs.get('blob_content_type', None), x_ms_blob_content_md5=kwargs.get('blob_content_md5', None), x_ms_blob_content_encoding=kwargs.get('blob_content_encoding', None), x_ms_blob_content_language=kwargs.get('blob_content_language', None), x_ms_lease_id=kwargs.get('lease_id', None), x_ms_blob_content_disposition=kwargs.get('blob_content_disposition', None))
    return data
...
def put_blob(kwargs=None, storage_conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The put_blob function must be called with -f or --function.')
    if kwargs is None:
        kwargs = {}
    if 'container' not in kwargs:
        raise SaltCloudSystemExit('The blob container name must be specified as "container"')
    ...
    if 'blob_path' not in kwargs and 'blob_content' not in kwargs:
        raise SaltCloudSystemExit('Either a path to a file needs to be passed in as "blob_path" or the contents of a blob as "blob_content."')
    if not storage_conn:
        storage_conn = get_storage_conn(conn_kwargs=kwargs)
...
def get_blob(kwargs=None, storage_conn=None, call=None):
    ...
    """
    if call != 'function':
        raise SaltCloudSystemExit('The get_blob function must be called with -f or --function.')
    if kwargs is None:
        kwargs = {}
    if 'container' not in kwargs:
        raise SaltCloudSystemExit('The blob container name must be specified as "container"')
    ...
    if 'local_path' not in kwargs and 'return_content' not in kwargs:
        raise SaltCloudSystemExit('Either a local path needs to be passed in as "local_path" or "return_content" to return the blob contents directly')
    if not storage_conn:
        storage_conn = get_storage_conn(conn_kwargs=kwargs)
...
def query(path, method='GET', data=None, params=None, header_dict=None, decode=True):
    ...
    subscription_id = salt.utils.stringutils.to_str(config.get_cloud_config_value('subscription_id', get_configured_provider(), __opts__, search_global=False))
    management_host = config.get_cloud_config_value('management_host', get_configured_provider(), __opts__, search_global=False, default='management.core.windows.net')
    backend = config.get_cloud_config_value('backend', get_configured_provider(), __opts__, search_global=False)
    url = 'https://{management_host}/{subscription_id}/{path}'.format(management_host=management_host, subscription_id=subscription_id, path=path)
    if header_dict is None:
        header_dict = {}
    header_dict['x-ms-version'] = '2014-06-01'
    result = salt.utils.http.query(url, method=method, params=params, data=data, header_dict=header_dict, port=443, text=True, cert=certificate_path, backend=backend, decode=decode, decode_type='xml')
    if 'dict' in result:
        return result['dict']
    return