...
LINODE_STATUS = {'boot_failed': {'code': -2, 'descr': 'Boot Failed (not in use)'}, 'beeing_created': {'code': -1, 'descr': 'Being Created'}, 'brand_new': {'code': 0, 'descr': 'Brand New'}, 'running': {'code': 1, 'descr': 'Running'}, 'poweroff': {'code': 2, 'descr': 'Powered Off'}, 'shutdown': {'code': 3, 'descr': 'Shutting Down (not in use)'}, 'save_to_disk': {'code': 4, 'descr': 'Saved to Disk (not in use)'}}
...
def _get_api_version():
    ...
    """
    return config.get_cloud_config_value('api_version', get_configured_provider(), __opts__, search_global=False, default='v3')
...
def _get_root_disk_size(vm_):
    ...
    """
    return config.get_cloud_config_value('disk_size', vm_, __opts__, search_global=False)
...
def _warn_for_api_v3():
    ...
    if not HAS_WARNED_FOR_API_V3:
        log.warning('Linode APIv3 has been deprecated and support will be removed in future releases. Please plan to upgrade to APIv4. For more information, see https://docs.saltproject.io/en/latest/topics/cloud/linode.html#migrating-to-apiv4.')
...
class LinodeAPIv4(LinodeAPI):
    ...
    def boot(self, name=None, kwargs=None):
        ...
        response = self._query('/linode/instances/{}/boot'.format(linode_id), method='POST', data={'config_id': config_id})
    ...
    def clone(self, kwargs=None):
        ...
        if 'datacenter_id' in kwargs:
            log.warning("The 'datacenter_id' argument has been deprecated and will be removed in future releases. Please use 'location' instead.")
        if 'plan_id' in kwargs:
            log.warning("The 'plan_id' argument has been deprecated and will be removed in future releases. Please use 'size' instead.")
        ...
        return self._query('/linode/instances/{}/clone'.format(linode_id), method='POST', data={'region': location, 'type': size})
    ...
    def create_config(self, kwargs=None):
        ...
        return self._query('/linode/instances/{}/configs'.format(linode_id), method='POST', data={'label': name, 'devices': devices})
    ...
    def create(self, vm_):
        ...
        __utils__['cloud.fire_event']('event', 'waiting for ssh', 'salt/cloud/{}/waiting_for_ssh'.format(name), sock_dir=__opts__['sock_dir'], args={'ip_address': vm_['ssh_host']}, transport=__opts__['transport'])
    ...
    def _get_linode_by_name(self, name):
        ...
        for instance in instances:
            if instance['label'] == name:
                return instance
        raise SaltCloudNotFound('The specified name, {}, could not be found.'.format(name))
    ...
    def _poll(self, description, getter, condition, timeout=None, poll_interval=None):
        ...
        while True:
            ...
            if condition(result):
                ...
                raise SaltCloudException('timed out: polling for {}'.format(description))

    def _wait_for_entity_status(self, getter, status, entity_name='item', identifier='some', timeout=None):
        return self._poll("{} (id={}) status to be '{}'".format(entity_name, identifier, status), getter, lambda item: item.get('status') == status, timeout=timeout)
    ...
    def _wait_for_event(self, action, entity, entity_id, status, timeout=None):
        ...
        while True:
            ...
            for event in events:
                ...
                if condition(event):
                    return True
                return self._poll("event {} to be '{}'".format(event_id, status), lambda : self._query('/account/events/{}'.format(event_id)), condition, timeout=timeout)
        return False
...
class LinodeAPIv3(LinodeAPI):
    ...
    def _query(self, action=None, command=None, args=None, method='GET', header_dict=None, data=None, url='https://api.linode.com/'):
        """
        ...
        if LASTCALL >= now:
            time.sleep(ratelimit_sleep)
        result = __utils__['http.query'](url, method, params=args, data=data, header_dict=header_dict, decode=decode, decode_type='json', text=True, status=True, hide_fields=['api_key', 'rootPass'], opts=__opts__)
        if 'ERRORARRAY' in result['dict']:
            if result['dict']['ERRORARRAY']:
                ...
                raise SaltCloudException('Linode API reported error(s): {}'.format(', '.join(error_list)))
        LASTCALL = int(time.mktime(datetime.datetime.now().timetuple()))
    ...
    def boot(self, name=None, kwargs=None):
        ...
        if check_running:
            ...
            if status == '1':
                raise SaltCloudSystemExit('Cannot boot Linode {0}. ' + 'Linode {} is already running.'.format(linode_item))
        response = self._query('linode', 'boot', args={'LinodeID': linode_id, 'ConfigID': config_id})['DATA']
    ...
    def clone(self, kwargs=None):
        ...
        for item in required_params:
            if item is None:
                raise SaltCloudSystemExit("The clone function requires a 'linode_id', 'datacenter_id', and 'plan_id' to be provided.")
        clone_args = {'LinodeID': linode_id, 'DatacenterID': datacenter_id, 'PlanID': plan_id}
    ...
    def create(self, vm_):
        ...
        if cloning:
            ...
            kwargs = {'clonefrom': clonefrom_name, 'image': 'Clone of {}'.format(clonefrom_name)}
            if size is None:
            ...
            try:
                result = clone(kwargs={'linode_id': linode_id, 'datacenter_id': datacenter_id, 'plan_id': plan_id})
            except Exception as err:
                log.error("Error cloning '%s' on Linode.\n\nThe following exception was thrown by Linode when trying to clone the specified machine:\n%s", clonefrom_name, err, exc_info_on_loglevel=logging.DEBUG)
                return False
        else:
            kwargs['image'] = vm_['image']
            try:
                result = self._query('linode', 'create', args={'PLANID': plan_id, 'DATACENTERID': datacenter_id})
            except Exception as err:
                log.error('Error creating %s on Linode\n\nThe following exception was thrown by Linode when trying to run the initial deployment:\n%s', name, err, exc_info_on_loglevel=logging.DEBUG)
                return False
        if 'ERRORARRAY' in result:
            for error_data in result['ERRORARRAY']:
                ...
                return False
        __utils__['cloud.fire_event']('event', 'requesting instance', 'salt/cloud/{}/requesting'.format(name), args=__utils__['cloud.filter_event']('requesting', vm_, ['name', 'profile', 'provider', 'driver']), sock_dir=__opts__['sock_dir'], transport=__opts__['transport'])
    ...
    def create_config(self, kwargs=None):
        ...
        config_args = {'LinodeID': int(linode_id), 'KernelID': int(kernel_id), 'Label': name, 'DiskList': disklist}
    ...
    def _create_disk_from_distro(self, vm_, linode_id):
        ...
        kwargs.update({'LinodeID': linode_id, 'DistributionID': self._get_distribution_id(vm_), 'Label': vm_['name'], 'Size': self._get_disk_size(vm_, swap_size, linode_id)})
    ...
    def _create_swap_disk(self, vm_, linode_id, swap_size=None):
        ...
        if not swap_size:
            swap_size = _get_swap_size(vm_)
        kwargs.update({'LinodeID': linode_id, 'Label': vm_['name'], 'Type': 'swap', 'Size': swap_size})
        result = self._query('linode', 'disk.create', args=kwargs)
        return self._clean_data(result)

    def _create_data_disk(self, vm_=None, linode_id=None, data_size=None):
        kwargs = {}
        kwargs.update({'LinodeID': linode_id, 'Label': vm_['name'] + '_data', 'Type': 'ext4', 'Size': data_size})
        result = self._query('linode', 'disk.create', args=kwargs)
        return self._clean_data(result)
    ...
    def destroy(self, name):
        ...
        linode_id = self._get_linode_id_from_name(name)
        response = self._query('linode', 'delete', args={'LinodeID': linode_id, 'skipChecks': True})
        __utils__['cloud.fire_event']('event', 'destroyed instance', 'salt/cloud/{}/destroyed'.format(name), args={'name': name}, sock_dir=__opts__['sock_dir'], transport=__opts__['transport'])
        if __opts__.get('update_cachedir', False) is True:
            __utils__['cloud.delete_minion_cachedir'](name, _get_active_provider_name().split(':')[0], __opts__)
        return response
    ...
    def _decode_linode_plan_label(self, label):
        ...
        if label not in sizes:
            if 'GB' in label:
                raise SaltCloudException('Invalid Linode plan ({}) specified - call avail_sizes() for all available options'.format(label))
            else:
                plan = label.split()
                if len(plan) != 2:
                    raise SaltCloudException('Invalid Linode plan ({}) specified - call avail_sizes() for all available options'.format(label))
                ...
                if new_label not in sizes:
                    raise SaltCloudException('Invalid Linode plan ({}) specified - call avail_sizes() for all available options'.format(new_label))
                log.warning('An outdated Linode plan label was detected in your Cloud Profile (%s). Please update the profile to use the new label format (%s) for the requested Linode plan size.', label, new_label)
                label = new_label
    ...
    def _get_disk_size(self, vm_, swap, linode_id):
        ...
        return config.get_cloud_config_value('disk_size', vm_, __opts__, default=disk_size - swap)
    ...
    def _get_distribution_id(self, vm_):
        ...
        for distro in distributions:
            if vm_image_name == distro['LABEL']:
                ...
                return distro_id
        if not distro_id:
            raise SaltCloudNotFound("The DistributionID for the '{}' profile could not be found.\nThe '{}' instance could not be provisioned. The following distributions are available:\n{}".format(vm_image_name, vm_['name'], pprint.pprint(sorted((distro['LABEL'].encode(__salt_system_encoding__) for distro in distributions)))))
    ...
    def _wait_for_job(self, linode_id, job_id, timeout=300, quiet=True):
        ...
        for i in range(0, iterations):
            ...
            if jobs_result[0]['JOBID'] == job_id and jobs_result[0]['HOST_SUCCESS'] == 1:
                return True
            time.sleep(interval)
            log.log(logging.INFO if not quiet else logging.DEBUG, 'Still waiting on Job %s for Linode %s.', job_id, linode_id)
        return False
    ...
    def list_nodes_min(self):
        ...
        for node in nodes:
            name = node['LABEL']
            ret[name] = {'id': str(node['LINODEID']), 'state': self._get_status_descr_by_id(int(node['STATUS']))}
        return ret
    ...
    def show_instance(self, name):
        ...
        ips = self._get_ips(node_id)
        state = int(node_data['STATUS'])
        return {'id': node_data['LINODEID'], 'image': node_data['DISTRIBUTIONVENDOR'], 'name': node_data['LABEL'], 'size': node_data['TOTALRAM'], 'state': self._get_status_descr_by_id(state), 'private_ips': ips['private_ips'], 'public_ips': ips['public_ips']}
