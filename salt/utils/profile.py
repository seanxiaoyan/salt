    ...
    if pr is not None and HAS_CPROFILE:
        try:
            ...
            if pyprof:
                ...
                failed = False
                try:
                    pro = subprocess.Popen(cmd, shell=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                except OSError:
                    failed = True
                if pro.returncode:
                    failed = True
                ...
                log.trace(salt.utils.stringutils.to_str(pro.stdout.read()).strip() + salt.utils.stringutils.to_str(pro.stderr.read()).strip())
            else:
