...
def __change_case(data, attr, preserve_dict_class=False):
    ...
    if isinstance(data, Mapping):
        return (data_type if preserve_dict_class else dict)(((__change_case(key, attr, preserve_dict_class), __change_case(val, attr, preserve_dict_class)) for (key, val) in data.items()))
    if isinstance(data, Sequence):
        return data_type((__change_case(item, attr, preserve_dict_class) for item in data))
    return data
...
def _remove_circular_refs(ob, _seen=None):
    ...
    if _seen is None:
        _seen = set()
    if id(ob) in _seen:
        log.exception('Caught a circular reference in data structure below.Cleaning and continuing execution.\n%r\n', ob)
        return None
    _seen.add(id(ob))
    res = ob
    if isinstance(ob, dict):
        res = {_remove_circular_refs(k, _seen): _remove_circular_refs(v, _seen) for (k, v) in ob.items()}
    elif isinstance(ob, (list, tuple, set, frozenset)):
        res = type(ob)((_remove_circular_refs(v, _seen) for v in ob))
    _seen.remove(id(ob))
    return res

def decode(data, encoding=None, errors='strict', keep=False, normalize=False, preserve_dict_class=False, preserve_tuples=False, to_str=False):
    log.info('Trace')
...
def decode_dict(data, encoding=None, errors='strict', keep=False, normalize=False, preserve_dict_class=False, preserve_tuples=False, to_str=False):
    ...
    for (key, value) in data.items():
        if isinstance(key, tuple):
            ...
            try:
                log.info('Trace')
                key = decode(key, encoding, errors, keep, normalize, preserve_dict_class, preserve_tuples, to_str)
            except TypeError:
                log.info('Trace')
                pass
            except UnicodeDecodeError:
        ...
        if isinstance(value, list):
            ...
            try:
                log.info('Trace')
                value = decode(value, encoding, errors, keep, normalize, preserve_dict_class, preserve_tuples, to_str)
...
def decode_list(data, encoding=None, errors='strict', keep=False, normalize=False, preserve_dict_class=False, preserve_tuples=False, to_str=False):
    log.info('Trace')
    '\n    Decode all string values to Unicode. Optionally use to_str=True to ensure\n    strings are str types and not unicode on Python 2.\n    '
    data = _remove_circular_refs(data)
    ret = []
    for item in data:
        if isinstance(item, list):
            ...
            try:
                log.info('Trace')
                item = decode(item, encoding, errors, keep, normalize, preserve_dict_class, preserve_tuples, to_str)
            except TypeError:
                log.info('Trace')
                pass
            except UnicodeDecodeError:
...
def decode_tuple(data, encoding=None, errors='strict', keep=False, normalize=False, preserve_dict_class=False, to_str=False):
    """
    Decode all string values to Unicode. Optionally use to_str=True to ensure
    strings are str types and not unicode on Python 2.
    """
...
def encode_dict(data, encoding=None, errors='strict', keep=False, preserve_dict_class=False, preserve_tuples=False):
    """
    ...
    for (key, value) in data.items():
        if isinstance(key, tuple):
            key = encode_tuple(key, encoding, errors, keep, preserve_dict_class) if preserve_tuples else encode_list(key, encoding, errors, keep, preserve_dict_class, preserve_tuples)
        else:
            try:
                ...
                if not keep:
                    raise
        if isinstance(value, list):
            value = encode_list(value, encoding, errors, keep, preserve_dict_class, preserve_tuples)
        elif isinstance(value, tuple):
            value = encode_tuple(value, encoding, errors, keep, preserve_dict_class) if preserve_tuples else encode_list(value, encoding, errors, keep, preserve_dict_class, preserve_tuples)
        elif isinstance(value, Mapping):
            value = encode_dict(value, encoding, errors, keep, preserve_dict_class, preserve_tuples)
        else:
            try:
...
def encode_list(data, encoding=None, errors='strict', keep=False, preserve_dict_class=False, preserve_tuples=False):
    ...
    for item in data:
        if isinstance(item, list):
            ...
            item = encode_tuple(item, encoding, errors, keep, preserve_dict_class) if preserve_tuples else encode_list(item, encoding, errors, keep, preserve_dict_class, preserve_tuples)
        elif isinstance(item, Mapping):
...
def encode_tuple(data, encoding=None, errors='strict', keep=False, preserve_dict_class=False):
    """
...
def subdict_match(data, expr, delimiter=DEFAULT_TARGET_DELIM, regex_match=False, exact_match=False):
    """
    ...
    def _dict_match(target, pattern, regex_match=False, exact_match=False):
        ...
        if not ret and pattern in target:
            ret = True
        if not ret and subdict_match(target, pattern, regex_match=regex_match, exact_match=exact_match):
            ret = True
        if not ret and wildcard:
            for key in target:
                if isinstance(target[key], dict):
                    if _dict_match(target[key], pattern, regex_match=regex_match, exact_match=exact_match):
                        return True
                elif isinstance(target[key], list):
                    for item in target[key]:
                        if _match(item, pattern, regex_match=regex_match, exact_match=exact_match):
                            return True
                elif _match(target[key], pattern, regex_match=regex_match, exact_match=exact_match):
                    return True
        return ret
    ...
    for idx in range(num_splits - 1, 0, -1):
        ...
        log.debug("Attempting to match '%s' in '%s' using delimiter '%s'", matchstr, key, delimiter)
        if match == {}:
            continue
        if isinstance(match, dict):
            if _dict_match(match, matchstr, regex_match=regex_match, exact_match=exact_match):
                return True
            continue
        if isinstance(match, (list, tuple)):
            for member in match:
                if isinstance(member, dict):
                    if _dict_match(member, matchstr, regex_match=regex_match, exact_match=exact_match):
                        return True
                if _match(member, matchstr, regex_match=regex_match, exact_match=exact_match):
                    return True
            continue
...
def repack_dictlist(data, strict=False, recurse=False, key_cb=None, val_cb=None):
    ...
    if isinstance(data, list):
        for element in data:
            ...
            if isinstance(element, dict):
                if len(element) != 1:
                    log.error('Invalid input for repack_dictlist: key/value pairs must contain only one element (data passed: %s).', element)
                    return {}
            else:
                log.error('Invalid input for repack_dictlist: element %s is not a string/dict/numeric value', element)
                return {}
    else:
        log.error('Invalid input for repack_dictlist, data passed is not a list (%s)', data)
        return {}
    ret = {}
    for element in data:
        if isinstance(element, valid_non_dict):
            ...
            if is_dictlist(val):
                if recurse:
                    ...
                    log.error('Invalid input for repack_dictlist: nested dictlist found, but recurse is set to False')
                    return {}
                else:
...
def filter_falsey(data, recurse_depth=None, ignore_types=()):
    ...
    .. versionadded:: 3000
    """
    filter_element = functools.partial(filter_falsey, recurse_depth=recurse_depth - 1, ignore_types=ignore_types) if recurse_depth else lambda x: x
    if isinstance(data, dict):
        processed_elements = [(key, filter_element(value)) for (key, value) in data.items()]
        return type(data)([(key, value) for (key, value) in processed_elements if _is_not_considered_falsey(value, ignore_types=ignore_types)])
    if is_iter(data):
        processed_elements = (filter_element(value) for value in data)
        return type(data)([value for value in processed_elements if _is_not_considered_falsey(value, ignore_types=ignore_types)])
    return data

def recursive_diff(old, new, ignore_keys=None, ignore_order=False, ignore_missing_keys=False):
    """
    ...
    if isinstance(old, OrderedDict) and isinstance(new, OrderedDict) and (not ignore_order):
        ...
        for (key_old, key_new) in zip(old, new):
            if key_old == key_new:
                if key_old in ignore_keys:
                    ...
                    res = recursive_diff(old[key_old], new[key_new], ignore_keys=ignore_keys, ignore_order=ignore_order, ignore_missing_keys=ignore_missing_keys)
                    if not res:
                        del ret_old[key_old]
                        del ret_new[key_new]
                    else:
        ...
        for key in set(list(old) + list(new)):
            if key in ignore_keys:
                ...
                res = recursive_diff(old[key], new[key], ignore_keys=ignore_keys, ignore_order=ignore_order, ignore_missing_keys=ignore_missing_keys)
                if not res:
                    del ret_old[key]
                    del ret_new[key]
                else:
        ...
        if ignore_order:
            for item_old in old:
                for item_new in new:
                    res = recursive_diff(item_old, item_new, ignore_keys=ignore_keys, ignore_order=ignore_order, ignore_missing_keys=ignore_missing_keys)
                    if not res:
            ...
            for (index, (iter_old, iter_new)) in enumerate(zip(old, new)):
                res = recursive_diff(iter_old, iter_new, ignore_keys=ignore_keys, ignore_order=ignore_order, ignore_missing_keys=ignore_missing_keys)
                if not res:
    ...
    if path:
        ...
        if key.startswith('{') and key.endswith('}'):
            ...
            def _append_placeholder(value_dict, key):
                ...
                return value_dict
            values = [[_append_placeholder(item, key) for item in get_value(val, next_path, default)] for (key, val) in items]
    ...
    for element in data:
        if not preserve_nulls and element in (None, 'None', 'null'):
            ...
            if levels is None:
                ...
                ret.extend(flatten(element, levels=int(levels) - 1, preserve_nulls=preserve_nulls, _ids=_ids))
            else:
                ret.append(element)
        else:
            ret.append(element)
