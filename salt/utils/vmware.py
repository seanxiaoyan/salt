...
def esxcli(host, user, pwd, cmd, protocol=None, port=None, esxi_host=None, credstore=None):
    ...
    if not esxi_host:
        esx_cmd += " -s {} -u {} -p '{}' --protocol={} --portnumber={} {}".format(host, user, pwd, protocol, port, cmd)
    else:
        esx_cmd += " -s {} -h {} -u {} -p '{}' --protocol={} --portnumber={} {}".format(host, esxi_host, user, pwd, protocol, port, cmd)
...
def _get_service_instance(host, username, password, protocol, port, mechanism, principal, domain, verify_ssl=True):
    log.info('Trace')
    ...
    if mechanism == 'userpass':
        if username is None:
            raise salt.exceptions.CommandExecutionError("Login mechanism userpass was specified but the mandatory parameter 'username' is missing")
        if password is None:
            raise salt.exceptions.CommandExecutionError("Login mechanism userpass was specified but the mandatory parameter 'password' is missing")
    elif mechanism == 'sspi':
        if principal is not None and domain is not None:
            ...
            err_msg = "Login mechanism '{}' was specified but the mandatory parameters are missing".format(mechanism)
    ...
    log.trace("Connecting using the '%s' mechanism, with username '%s'", mechanism, username)
    default_msg = "Could not connect to host '{}'. Please check the debug log for more information.".format(host)
    try:
        log.info('Trace')
        if verify_ssl:
            service_instance = SmartConnect(host=host, user=username, pwd=password, protocol=protocol, port=port, b64token=token, mechanism=mechanism)
        ...
        if isinstance(exc, vim.fault.HostConnectFault) and '[SSL: CERTIFICATE_VERIFY_FAILED]' in exc.msg or '[SSL: CERTIFICATE_VERIFY_FAILED]' in str(exc):
            err_msg = 'Could not verify the SSL certificate. You can use verify_ssl: False if you do not want to verify the SSL certificate. This is not recommended as it is considered insecure.'
        else:
            log.exception(exc)
    ...
    if not verify_ssl:
        ...
        try:
            log.info('Trace')
            service_instance = SmartConnect(host=host, user=username, pwd=password, protocol=protocol, port=port, sslContext=ssl._create_unverified_context(), b64token=token, mechanism=mechanism)
        except Exception as exc:
            log.info('Trace')
            if 'certificate verify failed' in str(exc):
                ...
                try:
                    log.info('Trace')
                    service_instance = SmartConnect(host=host, user=username, pwd=password, protocol=protocol, port=port, sslContext=context, b64token=token, mechanism=mechanism)
                except Exception as exc:
                    log.exception(exc)
                    err_msg = exc.msg if hasattr(exc, 'msg') else str(exc)
                    raise salt.exceptions.VMwareConnectionError("Could not connect to host '{}': {}".format(host, err_msg))
            else:
                err_msg = exc.msg if hasattr(exc, 'msg') else default_msg
                log.trace(exc)
                raise salt.exceptions.VMwareConnectionError(err_msg)
...
def get_service_instance(host, username=None, password=None, protocol=None, port=None, mechanism='userpass', principal=None, domain=None, verify_ssl=True):
    """
    ...
    if not service_instance:
        service_instance = _get_service_instance(host, username, password, protocol, port, mechanism, principal, domain, verify_ssl=verify_ssl)
...
def get_new_service_instance_stub(service_instance, path, ns=None, version=None):
    ...
    new_stub = SoapStubAdapter(host=hostname, ns=ns, path=path, version=version, poolSize=0, sslContext=context)
...
def is_connection_to_a_vcenter(service_instance):
    ...
    if api_type == 'VirtualCenter':
        ...
        raise salt.exceptions.VMwareApiError("Unexpected api type '{}' . Supported types: 'VirtualCenter/HostAgent'".format(api_type))
...
def get_hardware_grains(service_instance):
    ...
    if get_inventory(service_instance).about.apiType == 'HostAgent':
        ...
        if view and view.view:
            ...
            hw_grain_data['fqdn'] = '{}{}{}'.format(view.view[0].configManager.networkSystem.dnsConfig.hostName, '.' if view.view[0].configManager.networkSystem.dnsConfig.domainName else '', view.view[0].configManager.networkSystem.dnsConfig.domainName)
...
def get_content(service_instance, obj_type, property_list=None, container_ref=None, traversal_spec=None, local_properties=False):
    log.info('Trace')
    ...
    if not traversal_spec and (not local_properties):
        ...
        try:
            obj_ref = service_instance.content.viewManager.CreateContainerView(container_ref, [obj_type], True)
        except vim.fault.NoPermission as exc:
            log.exception(exc)
            raise salt.exceptions.VMwareApiError('Not enough permissions. Required privilege: {}'.format(exc.privilegeId))
        except vim.fault.VimFault as exc:
    ...
    obj_spec = vmodl.query.PropertyCollector.ObjectSpec(obj=obj_ref, skip=True if not local_properties else False, selectSet=[traversal_spec] if not local_properties else None)
...
def get_mors_with_properties(service_instance, object_type, property_list=None, container_ref=None, traversal_spec=None, local_properties=False):
    """
...
def get_properties_of_managed_object(mo_ref, properties):
    ...
    try:
        items = get_mors_with_properties(service_instance, type(mo_ref), container_ref=mo_ref, property_list=['name'], local_properties=True)
        mo_name = items[0]['name']
    except vmodl.query.InvalidProperty:
        log.info('Trace')
        mo_name = '<unnamed>'
    log.trace("Retrieving properties '%s' of %s '%s'", properties, type(mo_ref).__name__, mo_name)
    items = get_mors_with_properties(service_instance, type(mo_ref), container_ref=mo_ref, property_list=properties, local_properties=True)
    if not items:
        raise salt.exceptions.VMwareApiError("Properties of managed object '{}' weren't retrieved".format(mo_name))
    return items[0]
...
def get_dvss(dc_ref, dvs_names=None, get_all_dvss=False):
    ...
    log.trace("Retrieving DVSs in datacenter '%s', dvs_names='%s', get_all_dvss=%s", dc_name, ','.join(dvs_names) if dvs_names else None, get_all_dvss)
    properties = ['name']
    ...
    items = [i['object'] for i in get_mors_with_properties(service_instance, vim.DistributedVirtualSwitch, container_ref=dc_ref, property_list=properties, traversal_spec=traversal_spec) if get_all_dvss or (dvs_names and i['name'] in dvs_names)]
    return items
...
def get_network_folder(dc_ref):
    ...
    service_instance = get_service_instance_from_managed_object(dc_ref)
    traversal_spec = vmodl.query.PropertyCollector.TraversalSpec(path='networkFolder', skip=False, type=vim.Datacenter)
    entries = get_mors_with_properties(service_instance, vim.Folder, container_ref=dc_ref, property_list=['name'], traversal_spec=traversal_spec)
    if not entries:
        raise salt.exceptions.VMwareObjectRetrievalError("Network folder in datacenter '{}' wasn't retrieved".format(dc_name))
...
def set_dvs_network_resource_management_enabled(dvs_ref, enabled):
    ...
    """
    dvs_name = get_managed_object_name(dvs_ref)
    log.trace("Setting network resource management enable to %s on dvs '%s'", enabled, dvs_name)
    try:
...
def get_dvportgroups(parent_ref, portgroup_names=None, get_all_portgroups=False):
    ...
    if not isinstance(parent_ref, (vim.Datacenter, vim.DistributedVirtualSwitch)):
        raise salt.exceptions.ArgumentValueError('Parent has to be either a datacenter, or a distributed virtual switch')
    parent_name = get_managed_object_name(parent_ref)
    log.trace("Retrieving portgroup in %s '%s', portgroups_names='%s', get_all_portgroups=%s", type(parent_ref).__name__, parent_name, ','.join(portgroup_names) if portgroup_names else None, get_all_portgroups)
    properties = ['name']
    ...
    items = [i['object'] for i in get_mors_with_properties(service_instance, vim.DistributedVirtualPortgroup, container_ref=parent_ref, property_list=properties, traversal_spec=traversal_spec) if get_all_portgroups or (portgroup_names and i['name'] in portgroup_names)]
    return items
...
def get_uplink_dvportgroup(dvs_ref):
    ...
    items = [entry['object'] for entry in get_mors_with_properties(service_instance, vim.DistributedVirtualPortgroup, container_ref=dvs_ref, property_list=['tag'], traversal_spec=traversal_spec) if entry['tag'] and [t for t in entry['tag'] if t.key == 'SYSTEM/DVS.UPLINKPG']]
    if not items:
        raise salt.exceptions.VMwareObjectRetrievalError("Uplink portgroup of DVS '{}' wasn't found".format(dvs_name))
    return items[0]
...
def get_license_assignment_manager(service_instance):
    ...
    if not lic_assignment_manager:
        raise salt.exceptions.VMwareObjectRetrievalError('License assignment manager was not retrieved')
...
def get_assigned_licenses(service_instance, entity_ref=None, entity_name=None, license_assignment_manager=None):
    """
    ...
    if entity_type == 'uuid' and len(assignments) > 1:
        ...
        raise salt.exceptions.VMwareObjectRetrievalError('Unexpected return. Expect only a single assignment')
    if check_name:
        if entity_name != assignments[0].entityDisplayName:
            log.trace('Getting license info for wrong vcenter: %s != %s', entity_name, assignments[0].entityDisplayName)
            raise salt.exceptions.VMwareObjectRetrievalError('Got license assignment info for a different vcenter')
    return [a.assignedLicense for a in assignments]

def assign_license(service_instance, license_key, license_name, entity_ref=None, entity_name=None, license_assignment_manager=None):
    """
    ...
    try:
        vmware_license = license_assignment_manager.UpdateAssignedLicense(entity_id, license_key, license_name)
    except vim.fault.NoPermission as exc:
        log.exception(exc)
        raise salt.exceptions.VMwareApiError('Not enough permissions. Required privilege: {}'.format(exc.privilegeId))
    except vim.fault.VimFault as exc:
...
def get_datacenter(service_instance, datacenter_name):
    ...
    if not items:
        raise salt.exceptions.VMwareObjectRetrievalError("Datacenter '{}' was not found".format(datacenter_name))
    return items[0]
...
def get_cluster(dc_ref, cluster):
    ...
    traversal_spec = vmodl.query.PropertyCollector.TraversalSpec(path='hostFolder', skip=True, type=vim.Datacenter, selectSet=[vmodl.query.PropertyCollector.TraversalSpec(path='childEntity', skip=False, type=vim.Folder)])
    items = [i['object'] for i in get_mors_with_properties(si, vim.ClusterComputeResource, container_ref=dc_ref, property_list=['name'], traversal_spec=traversal_spec) if i['name'] == cluster]
    if not items:
        raise salt.exceptions.VMwareObjectRetrievalError("Cluster '{}' was not found in datacenter '{}'".format(cluster, dc_name))
    return items[0]
...
def get_datastore_files(service_instance, directory, datastores, container_object, browser_spec):
    ...
    for datobj in datastore_objects:
        try:
            log.info('Trace')
            task = datobj.browser.SearchDatastore_Task(datastorePath='[{}] {}'.format(datobj.name, directory), searchSpec=browser_spec)
        except vim.fault.NoPermission as exc:
            log.exception(exc)
            raise salt.exceptions.VMwareApiError('Not enough permissions. Required privilege: {}'.format(exc.privilegeId))
        except vim.fault.VimFault as exc:
...
def get_datastores(service_instance, reference, datastore_names=None, backing_disk_ids=None, get_all_datastores=False):
    log.info('Trace')
    ...
    if get_all_datastores:
        ...
        log.trace("Retrieving datastores visible to '%s': names = (%s); backing disk ids = (%s)", obj_name, datastore_names, backing_disk_ids)
        if backing_disk_ids and (not isinstance(reference, vim.HostSystem)):
            log.info('Trace')
            raise salt.exceptions.ArgumentValueError("Unsupported reference type '{}' when backing disk filter is set".format(reference.__class__.__name__))
    if not get_all_datastores and backing_disk_ids:
        ...
        for vol in [i.volume for i in mount_infos if isinstance(i.volume, vim.HostVmfsVolume)]:
            if not [e for e in vol.extent if e.diskName in backing_disk_ids]:
                continue
            log.trace("Found datastore '%s' for disk id(s) '%s'", vol.name, [e.diskName for e in vol.extent])
    ...
    if not get_all_datastores and (not datastore_names):
        log.trace("No datastore to be filtered after retrieving the datastores backed by the disk id(s) '%s'", backing_disk_ids)
        return []
    log.trace('datastore_names = %s', datastore_names)
    if isinstance(reference, vim.HostSystem):
        traversal_spec = vmodl.query.PropertyCollector.TraversalSpec(name='host_datastore_traversal', path='datastore', skip=False, type=vim.HostSystem)
    elif isinstance(reference, vim.ClusterComputeResource):
        traversal_spec = vmodl.query.PropertyCollector.TraversalSpec(name='cluster_datastore_traversal', path='datastore', skip=False, type=vim.ClusterComputeResource)
    elif isinstance(reference, vim.Datacenter):
        traversal_spec = vmodl.query.PropertyCollector.TraversalSpec(name='datacenter_datastore_traversal', path='datastore', skip=False, type=vim.Datacenter)
    elif isinstance(reference, vim.StoragePod):
        traversal_spec = vmodl.query.PropertyCollector.TraversalSpec(name='datastore_cluster_traversal', path='childEntity', skip=False, type=vim.StoragePod)
    elif isinstance(reference, vim.Folder) and get_managed_object_name(reference) == 'Datacenters':
        traversal_spec = vmodl.query.PropertyCollector.TraversalSpec(path='childEntity', selectSet=[vmodl.query.PropertyCollector.TraversalSpec(path='datastore', skip=False, type=vim.Datacenter)], skip=False, type=vim.Folder)
    else:
        raise salt.exceptions.ArgumentValueError("Unsupported reference type '{}'".format(reference.__class__.__name__))
    items = get_mors_with_properties(service_instance, object_type=vim.Datastore, property_list=['name'], container_ref=reference, traversal_spec=traversal_spec)
...
def get_storage_system(service_instance, host_ref, hostname=None):
    ...
    traversal_spec = vmodl.query.PropertyCollector.TraversalSpec(path='configManager.storageSystem', type=vim.HostSystem, skip=False)
    objs = get_mors_with_properties(service_instance, vim.HostStorageSystem, property_list=['systemFile'], container_ref=host_ref, traversal_spec=traversal_spec)
    if not objs:
        raise salt.exceptions.VMwareObjectRetrievalError("Host's '{}' storage system was not retrieved".format(hostname))
...
def _get_new_computed_partition_spec(storage_system, device_path, partition_info):
    ...
    """
    log.trace('Adding a partition at the end of the disk and getting the new computed partition spec')
    free_partitions = [p for p in partition_info.layout.partition if p.type == 'none']
    if not free_partitions:
        raise salt.exceptions.VMwareObjectNotFoundError("Free partition was not found on device '{}'".format(partition_info.deviceName))
    free_partition = free_partitions[0]
    layout = vim.HostDiskPartitionLayout(total=partition_info.layout.total, partition=partition_info.layout.partition)
    free_partition.type = 'vmfs'
    try:
        computed_partition_info = storage_system.ComputeDiskPartitionInfo(devicePath=device_path, partitionFormat=vim.HostDiskPartitionInfoPartitionFormat.gpt, layout=layout)
    except vim.fault.NoPermission as exc:
        log.exception(exc)
        raise salt.exceptions.VMwareApiError('Not enough permissions. Required privilege: {}'.format(exc.privilegeId))
    except vim.fault.VimFault as exc:
    ...
    partition_numbers = [p.partition for p in computed_partition_info.layout.partition if (p.start.block == free_partition.start.block or (free_partition.start.block == 0 and p.start.block == 1)) and p.end.block == free_partition.end.block and (p.type == 'vmfs')]
    if not partition_numbers:
        raise salt.exceptions.VMwareNotFoundError("New partition was not found in computed partitions of device '{}'".format(partition_info.deviceName))
    log.trace('new partition number = %s', partition_numbers[0])
    return (partition_numbers[0], computed_partition_info.spec)

def create_vmfs_datastore(host_ref, datastore_name, disk_ref, vmfs_major_version, storage_system=None):
    """
    ...
    log.debug("Creating datastore '%s' on host '%s', scsi disk '%s', vmfs v%s", datastore_name, hostname, disk_id, vmfs_major_version)
    ...
    (new_partition_number, partition_spec) = _get_new_computed_partition_spec(storage_system, target_disk.devicePath, partition_info)
    spec = vim.VmfsDatastoreCreateSpec(vmfs=vim.HostVmfsSpec(majorVersion=vmfs_major_version, volumeName=datastore_name, extent=vim.HostScsiDiskPartition(diskName=disk_id, partition=new_partition_number)), diskUuid=target_disk.uuid, partition=partition_spec)
    try:
...
def get_host_datastore_system(host_ref, hostname=None):
    ...
    traversal_spec = vmodl.query.PropertyCollector.TraversalSpec(path='configManager.datastoreSystem', type=vim.HostSystem, skip=False)
    objs = get_mors_with_properties(service_instance, vim.HostDatastoreSystem, property_list=['datastore'], container_ref=host_ref, traversal_spec=traversal_spec)
    if not objs:
        raise salt.exceptions.VMwareObjectRetrievalError("Host's '{}' datastore system was not retrieved".format(hostname))
...
def remove_datastore(service_instance, datastore_ref):
    ...
    if not ds_hosts:
        raise salt.exceptions.VMwareApiError("Datastore '{}' can't be removed. No attached hosts found".format(ds_name))
...
def get_hosts(service_instance, datacenter_name=None, host_names=None, cluster_name=None, get_all_hosts=False):
    ...
    hosts = get_mors_with_properties(service_instance, vim.HostSystem, container_ref=start_point, property_list=properties)
...
def _get_scsi_address_to_lun_key_map(service_instance, host_ref, storage_system=None, hostname=None):
    """
    ...
    try:
        ...
        raise salt.exceptions.VMwareRuntimeError(exc.msg)
    if not device_info:
        raise salt.exceptions.VMwareObjectRetrievalError("Host's '{}' storage device info was not retrieved".format(hostname))
    multipath_info = device_info.multipathInfo
    if not multipath_info:
        raise salt.exceptions.VMwareObjectRetrievalError("Host's '{}' multipath info was not retrieved".format(hostname))
    if multipath_info.lun is None:
        raise salt.exceptions.VMwareObjectRetrievalError("No luns were retrieved from host '{}'".format(hostname))
...
def get_all_luns(host_ref, storage_system=None, hostname=None):
    ...
    if scsi_luns:
        log.trace("Retrieved scsi luns in host '%s': %s", hostname, [l.canonicalName for l in scsi_luns])
        return scsi_luns
...
def get_scsi_address_to_lun_map(host_ref, storage_system=None, hostname=None):
    ...
    if not storage_system:
        storage_system = get_storage_system(si, host_ref, hostname)
    lun_ids_to_scsi_addr_map = _get_scsi_address_to_lun_key_map(si, host_ref, storage_system, hostname)
    luns_to_key_map = {d.key: d for d in get_all_luns(host_ref, storage_system, hostname)}
    return {scsi_addr: luns_to_key_map[lun_key] for (scsi_addr, lun_key) in lun_ids_to_scsi_addr_map.items()}
...
def get_disks(host_ref, disk_ids=None, scsi_addresses=None, get_all_disks=False):
    ...
    if get_all_disks:
        ...
        log.trace("Retrieving disks in host '%s': ids = (%s); scsi addresses = (%s)", hostname, disk_ids, scsi_addresses)
    ...
    disk_keys = []
    if scsi_addresses:
        lun_key_by_scsi_addr = _get_scsi_address_to_lun_key_map(si, host_ref, storage_system, hostname)
        disk_keys = [key for (scsi_addr, key) in lun_key_by_scsi_addr.items() if scsi_addr in scsi_addresses]
        log.trace('disk_keys based on scsi_addresses = %s', disk_keys)
    scsi_luns = get_all_luns(host_ref, storage_system)
    scsi_disks = [disk for disk in scsi_luns if isinstance(disk, vim.HostScsiDisk) and (get_all_disks or (disk_ids and disk.canonicalName in disk_ids) or disk.key in disk_keys)]
    log.trace("Retrieved disks in host '%s': %s", hostname, [d.canonicalName for d in scsi_disks])
    return scsi_disks
...
def get_disk_partition_info(host_ref, disk_id, storage_system=None):
    ...
    if not props.get('storageDeviceInfo.scsiLun'):
        raise salt.exceptions.VMwareObjectRetrievalError("No devices were retrieved in host '{}'".format(hostname))
    log.trace('[%s] Retrieved %s devices: %s', hostname, len(props['storageDeviceInfo.scsiLun']), ', '.join([l.canonicalName for l in props['storageDeviceInfo.scsiLun']]))
    disks = [l for l in props['storageDeviceInfo.scsiLun'] if isinstance(l, vim.HostScsiDisk) and l.canonicalName == disk_id]
    if not disks:
        raise salt.exceptions.VMwareObjectRetrievalError("Disk '{}' was not found in host '{}'".format(disk_id, hostname))
...
def erase_disk_partitions(service_instance, host_ref, disk_id, hostname=None, storage_system=None):
    ...
    if not storage_system:
        storage_system = get_storage_system(service_instance, host_ref, hostname)
    traversal_spec = vmodl.query.PropertyCollector.TraversalSpec(path='configManager.storageSystem', type=vim.HostSystem, skip=False)
    results = get_mors_with_properties(service_instance, vim.HostStorageSystem, ['storageDeviceInfo.scsiLun'], container_ref=host_ref, traversal_spec=traversal_spec)
    if not results:
        raise salt.exceptions.VMwareObjectRetrievalError("Host's '{}' devices were not retrieved".format(hostname))
    log.trace('[%s] Retrieved %s devices: %s', hostname, len(results[0].get('storageDeviceInfo.scsiLun', [])), ', '.join([l.canonicalName for l in results[0].get('storageDeviceInfo.scsiLun', [])]))
    disks = [l for l in results[0].get('storageDeviceInfo.scsiLun', []) if isinstance(l, vim.HostScsiDisk) and l.canonicalName == disk_id]
    if not disks:
        raise salt.exceptions.VMwareObjectRetrievalError("Disk '{}' was not found in host '{}'".format(disk_id, hostname))
    log.trace('[%s] device_path = %s', hostname, disks[0].devicePath)
    try:
...
def get_diskgroups(host_ref, cache_disk_ids=None, get_all_disk_groups=False):
    ...
    if get_all_disk_groups:
        ...
        log.trace("Retrieving disk groups from host '%s', with cache disk ids : (%s)", hostname, cache_disk_ids)
    ...
    if not vsan_host_config:
        raise salt.exceptions.VMwareObjectRetrievalError("No host config found on host '{}'".format(hostname))
    vsan_storage_info = vsan_host_config.storageInfo
    if not vsan_storage_info:
        raise salt.exceptions.VMwareObjectRetrievalError("No vsan storage info found on host '{}'".format(hostname))
    vsan_disk_mappings = vsan_storage_info.diskMapping
    if not vsan_disk_mappings:
        return []
    disk_groups = [dm for dm in vsan_disk_mappings if get_all_disk_groups or dm.ssd.canonicalName in cache_disk_ids]
    log.trace("Retrieved disk groups on host '%s', with cache disk ids : %s", hostname, [d.ssd.canonicalName for d in disk_groups])
    return disk_groups
...
def _check_disks_in_diskgroup(disk_group, cache_disk_id, capacity_disk_ids):
    ...
    if not disk_group.ssd.canonicalName == cache_disk_id:
        raise salt.exceptions.ArgumentValueError("Incorrect diskgroup cache disk; got id: '{}'; expected id: '{}'".format(disk_group.ssd.canonicalName, cache_disk_id))
    non_ssd_disks = [d.canonicalName for d in disk_group.nonSsd]
    if sorted(non_ssd_disks) != sorted(capacity_disk_ids):
        raise salt.exceptions.ArgumentValueError("Incorrect capacity disks; got ids: '{}'; expected ids: '{}'".format(sorted(non_ssd_disks), sorted(capacity_disk_ids)))
...
def get_host_cache(host_ref, host_cache_manager=None):
    ...
    if not host_cache_manager:
        traversal_spec = vmodl.query.PropertyCollector.TraversalSpec(path='configManager.cacheConfigurationManager', type=vim.HostSystem, skip=False)
        results = get_mors_with_properties(service_instance, vim.HostCacheConfigurationManager, ['cacheConfigurationInfo'], container_ref=host_ref, traversal_spec=traversal_spec)
...
def configure_host_cache(host_ref, datastore_ref, swap_size_MiB, host_cache_manager=None):
    ...
    if not host_cache_manager:
        ...
        if not props.get('configManager.cacheConfigurationManager'):
            raise salt.exceptions.VMwareObjectRetrievalError("Host '{}' has no host cache".format(hostname))
        host_cache_manager = props['configManager.cacheConfigurationManager']
    log.trace("Configuring the host cache on host '%s', datastore '%s', swap size=%s MiB", hostname, datastore_ref.name, swap_size_MiB)
...
def get_resource_pools(service_instance, resource_pool_names, datacenter_name=None, get_all_resource_pools=False):
    """
    ...
    if datacenter_name:
        ...
        container_ref = get_root_folder(service_instance)
    resource_pools = get_mors_with_properties(service_instance, vim.ResourcePool, container_ref=container_ref, property_list=properties)
    selected_pools = []
    for pool in resource_pools:
        if get_all_resource_pools or pool['name'] in resource_pool_names:
            selected_pools.append(pool['object'])
    if not selected_pools:
        raise salt.exceptions.VMwareObjectRetrievalError('The resource pools with properties names={} get_all={} could not be found'.format(selected_pools, get_all_resource_pools))
...
def wait_for_task(task, instance_name, task_type, sleep_seconds=1, log_level='debug'):
    ...
    while task_info.state == 'running' or task_info.state == 'queued':
        if time_counter % sleep_seconds == 0:
            msg = '[ {} ] Waiting for {} task to finish [{} s]'.format(instance_name, task_type, time_counter)
            if log_level == 'info':
                log.info(msg)
            else:
                log.debug(msg)
    ...
    if task_info.state == 'success':
        msg = '[ {} ] Successfully completed {} task in {} seconds'.format(instance_name, task_type, time_counter)
        if log_level == 'info':
            log.info(msg)
        else:
            log.debug(msg)
...
def get_vm_by_property(service_instance, name, datacenter=None, vm_properties=None, traversal_spec=None, parent_ref=None):
    """
    ...
    if not vm_properties:
        vm_properties = ['name', 'config.hardware.device', 'summary.storage.committed', 'summary.storage.uncommitted', 'summary.storage.unshared', 'layoutEx.file', 'config.guestFullName', 'config.guestId', 'guest.net', 'config.hardware.memoryMB', 'config.hardware.numCPU', 'config.files.vmPathName', 'summary.runtime.powerState', 'guest.toolsStatus']
    ...
    if not vm_formatted:
        ...
        raise salt.exceptions.VMwareMultipleObjectsError(' '.join(['Multiple virtual machines were found with thesame name, please specify a container.']))
...
def get_folder(service_instance, datacenter, placement, base_vm_name=None):
    ...
    if base_vm_name:
        ...
        if 'parent' in vm_props:
            ...
            raise salt.exceptions.VMwareObjectRetrievalError(' '.join(['The virtual machine parent', 'object is not defined']))
        ...
        if len(folder_objects) > 1:
            raise salt.exceptions.VMwareMultipleObjectsError(' '.join(['Multiple instances are available of the', 'specified folder {}'.format(placement['folder'])]))
...
def get_placement(service_instance, datacenter, placement=None):
    ...
    if 'host' in placement:
        ...
        if not host_objects:
            raise salt.exceptions.VMwareObjectRetrievalError(' '.join(['The specified host', '{} cannot be found.'.format(placement['host'])]))
        try:
            host_props = get_properties_of_managed_object(host_objects[0], properties=['resourcePool'])
            resourcepool_object = host_props['resourcePool']
        except vmodl.query.InvalidProperty:
            traversal_spec = vmodl.query.PropertyCollector.TraversalSpec(path='parent', skip=True, type=vim.HostSystem, selectSet=[vmodl.query.PropertyCollector.TraversalSpec(path='resourcePool', skip=False, type=vim.ClusterComputeResource)])
            resourcepools = get_mors_with_properties(service_instance, vim.ResourcePool, container_ref=host_objects[0], property_list=['name'], traversal_spec=traversal_spec)
            if resourcepools:
                resourcepool_object = resourcepools[0]['object']
            else:
                raise salt.exceptions.VMwareObjectRetrievalError('The resource pool of host {} cannot be found.'.format(placement['host']))
        placement_object = host_objects[0]
    elif 'resourcepool' in placement:
        resourcepool_objects = get_resource_pools(service_instance, [placement['resourcepool']], datacenter_name=datacenter)
        if len(resourcepool_objects) > 1:
            raise salt.exceptions.VMwareMultipleObjectsError(' '.join(['Multiple instances are available of the', 'specified host {}.'.format(placement['host'])]))
        ...
        if 'parent' in res_props:
            ...
            raise salt.exceptions.VMwareObjectRetrievalError(' '.join(["The resource pool's parent", 'object is not defined']))
        ...
        if 'resourcePool' in clus_props:
            ...
            raise salt.exceptions.VMwareObjectRetrievalError(' '.join(["The cluster's resource pool", 'object is not defined']))
...
def power_cycle_vm(virtual_machine, action='on'):
    ...
    try:
        ...
        log.info('Trace')
        raise salt.exceptions.VMwarePowerOnError(' '.join(['An error occurred during power', 'operation, a file was not found: {}'.format(exc)]))
...
def create_vm(vm_name, vm_config_spec, folder_object, resourcepool_object, host_object=None):
    ...
    try:
        if host_object and isinstance(host_object, vim.HostSystem):
            task = folder_object.CreateVM_Task(vm_config_spec, pool=resourcepool_object, host=host_object)
        else:
...
def register_vm(datacenter, name, vmx_path, resourcepool_object, host_object=None):
    ...
    try:
        if host_object:
            task = datacenter.vmFolder.RegisterVM_Task(path=vmx_path, name=name, asTemplate=False, host=host_object, pool=resourcepool_object)
        else:
            task = datacenter.vmFolder.RegisterVM_Task(path=vmx_path, name=name, asTemplate=False, pool=resourcepool_object)
    except vim.fault.NoPermission as exc:
        log.exception(exc)
        raise salt.exceptions.VMwareApiError('Not enough permissions. Required privilege: {}'.format(exc.privilegeId))
    except vim.fault.VimFault as exc:
    ...
    try:
        ...
        raise salt.exceptions.VMwareVmRegisterError('An error occurred during registration operation, the configuration file was not found: {}'.format(exc))
    return vm_ref
