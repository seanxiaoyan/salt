...
TAGPARTER = '/'
SALT = 'salt'
TAGS = {'auth': 'auth', 'job': 'job', 'key': 'key', 'minion': 'minion', 'syndic': 'syndic', 'run': 'run', 'wheel': 'wheel', 'cloud': 'cloud', 'fileserver': 'fileserver', 'queue': 'queue'}

def get_event(node, sock_dir=None, transport=None, opts=None, listen=True, io_loop=None, keep_loop=False, raise_errors=False):
    """
    ...
    """
    if transport:
        salt.utils.versions.warn_until('Chlorine', "The 'transport' kwarg has been deprecated and it will be removed in the Chlorine release, as such, its usage is no longer required.")
    sock_dir = sock_dir or opts['sock_dir']
    if node == 'master':
        return MasterEvent(sock_dir, opts, listen=listen, io_loop=io_loop, keep_loop=keep_loop, raise_errors=raise_errors)
    return SaltEvent(node, sock_dir, opts, listen=listen, io_loop=io_loop, keep_loop=keep_loop, raise_errors=raise_errors)
...
class SaltEvent:
    ...
    def __load_uri(self, sock_dir, node):
        ...
        if node == 'master':
            ...
            id_hash = hash_type(salt.utils.stringutils.to_bytes(minion_id)).hexdigest()[:10]
            puburi = os.path.join(sock_dir, 'minion_event_{}_pub.ipc'.format(id_hash))
            pulluri = os.path.join(sock_dir, 'minion_event_{}_pull.ipc'.format(id_hash))
        ...
        if self._run_io_loop_sync:
            with salt.utils.asynchronous.current_ioloop(self.io_loop):
                if self.subscriber is None:
                    self.subscriber = salt.utils.asynchronous.SyncWrapper(salt.transport.ipc.IPCMessageSubscriber, args=(self.puburi,), kwargs={'io_loop': self.io_loop}, loop_kwarg='io_loop')
                try:
                    log.info('Trace')
                    ...
                    log.info('An exception occurred connecting publisher: %s', exc, exc_info_on_loglevel=logging.DEBUG)
        else:
            if self.subscriber is None:
        ...
        if self._run_io_loop_sync:
            with salt.utils.asynchronous.current_ioloop(self.io_loop):
                if self.pusher is None:
                    self.pusher = salt.utils.asynchronous.SyncWrapper(salt.transport.ipc.IPCMessageClient, args=(self.pulluri,), kwargs={'io_loop': self.io_loop}, loop_kwarg='io_loop')
                try:
                    log.info('Trace')
                    self.pusher.connect(timeout=timeout)
                    self.cpush = True
                except Exception as exc:
                    log.error('Unable to connect pusher: %s', exc, exc_info_on_loglevel=logging.DEBUG)
        else:
            if self.pusher is None:
    ...
    def unpack(cls, raw):
        log.info('Trace')
        (mtag, sep, mdata) = raw.partition(salt.utils.stringutils.to_bytes(TAGEND))
    ...
    def fire_event_async(self, data, tag, cb=None, timeout=1000):
        ...
        serialized_data = salt.utils.dicttrim.trim_dict(dump_data, self.opts['max_event_size'], is_msgpacked=True, use_bin_type=True)
        log.debug('Sending event: tag = %s; data = %s', tag, data)
        event = b''.join([salt.utils.stringutils.to_bytes(tag), salt.utils.stringutils.to_bytes(tagend), serialized_data])
        ...
        if self._run_io_loop_sync:
            with salt.utils.asynchronous.current_ioloop(self.io_loop):
                try:
                    ...
                    log.debug('Publisher send failed with exception: %s', exc, exc_info_on_loglevel=logging.DEBUG)
                    raise
        else:
        ...
        try:
            for (tag, data) in ret.items():
                ...
                if data.get('result') is False:
                    self.fire_event(data, '{}.{}'.format(tags[0], tags[-1]))
                    data['jid'] = load['jid']
                    data['id'] = load['id']
                    data['success'] = False
            ...
            log.error('Event iteration failed with exception: %s', exc, exc_info_on_loglevel=logging.DEBUG)
...
class MasterEvent(SaltEvent):
    ...
    def __init__(self, sock_dir, opts=None, listen=True, io_loop=None, keep_loop=False, raise_errors=False):
        super().__init__('master', sock_dir, opts, listen=listen, io_loop=io_loop, keep_loop=keep_loop, raise_errors=raise_errors)
...
class MinionEvent(SaltEvent):
    ...
    def __init__(self, opts, listen=True, io_loop=None, raise_errors=False):
        super().__init__('minion', sock_dir=opts.get('sock_dir'), opts=opts, listen=listen, io_loop=io_loop, raise_errors=raise_errors)
...
class AsyncEventPublisher:
    ...
    def __init__(self, opts, io_loop=None):
        ...
        epub_sock_path = os.path.join(self.opts['sock_dir'], 'minion_event_{}_pub.ipc'.format(id_hash))
        if os.path.exists(epub_sock_path):
            os.unlink(epub_sock_path)
        epull_sock_path = os.path.join(self.opts['sock_dir'], 'minion_event_{}_pull.ipc'.format(id_hash))
        ...
        if not os.path.isdir(minion_sock_dir):
            try:
                log.info('Trace')
                os.makedirs(minion_sock_dir, 493)
            except OSError as exc:
                log.info('Trace')
                ...
                if not os.path.isdir(default_minion_sock_dir):
                    ...
                    try:
                        log.info('Trace')
                        os.makedirs(default_minion_sock_dir, 493)
                    except OSError as exc:
                        log.error('Could not create SOCK_DIR: %s', exc)
                        raise
...
class EventReturn(salt.utils.process.SignalHandlingProcess):
    ...
    def flush_events(self):
        if isinstance(self.opts['event_return'], list):
            ...
            log.debug('Calling event returner %s, only one configured.', self.opts['event_return'])
    ...
    def _flush_event_single(self, event_return):
        if event_return in self.minion.returners:
            try:
                ...
                log.error("Could not store events - returner '%s' raised exception: %s", event_return, exc)
                if log.level <= logging.DEBUG:
                    log.debug('Event data that caused an exception: %s', self.event_queue)
        else:
            log.error("Could not store return for event(s) - returner '%s' not found.", event_return)
...
class StateFire:
    ...
    def fire_master(self, data, tag, preload=None):
        ...
        if preload:
            load.update(preload)
        load.update({'id': self.opts['id'], 'tag': tag, 'data': data, 'cmd': '_minion_event', 'tok': self.auth.gen_token(b'salt')})
    ...
    def fire_running(self, running):
        ...
        for stag in sorted(running, key=lambda k: running[k].get('__run_num__', 0)):
            if running[stag]['result'] and (not running[stag]['changes']):
                continue
            tag = 'state_{}_{}'.format(str(running[stag]['result']), 'True' if running[stag]['changes'] else 'False')
