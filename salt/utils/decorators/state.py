...
class OutputUnifier:
    ...
    def _run_policies(self, data):
        for pls in self.policies:
            try:
                ...
                data = {'result': False, 'name': 'later', 'changes': {}, 'comment': 'An exception occurred in this state: {}'.format(exc)}
        return data
    ...
    def __call__(self, func):
        ...
        def _func(*args, **kwargs):
            ...
            if sub_state_run:
                result['sub_state_run'] = [self._run_policies(sub_state_data) for sub_state_data in sub_state_run]
            return result
        return _func
        ...
        if not isinstance(result, dict):
            ...
            for val in ['name', 'result', 'changes', 'comment']:
                if val not in result:
                    missing.append(val)
            if missing:
                err_msg = 'The following keys were not present in the state return: {}.'.format(', '.join(missing))
            else:
                err_msg = None
        if err_msg:
    ...
    def unify(self, result):
        ...
        if isinstance(result.get('comment'), list):
            result['comment'] = '\n'.join([salt.utils.stringutils.to_unicode(elm) for elm in result['comment']])
