...
def _get_base_properties(i_face):
    ...
    return {'alias': i_face.Name, 'description': i_face.Description, 'id': i_face.Id, 'receive_only': i_face.IsReceiveOnly, 'type': i_face_type, 'status': enum_operational_status[i_face.OperationalStatus], 'physical_address': ':'.join((raw_mac[i:i + 2] for i in range(0, 12, 2)))}
...
def _get_ip_unicast_info(i_face):
    ...
    if ip_properties.UnicastAddresses.Count > 0:
        ...
        for addrs in ip_properties.UnicastAddresses:
            ...
            ip_info.update({'prefix_length': addrs.PrefixLength, 'prefix_origin': enum_prefix_suffix[addrs.PrefixOrigin], 'suffix_origin': enum_prefix_suffix[addrs.SuffixOrigin]})
...
def _get_ip_gateway_info(i_face):
    ...
    if ip_properties.GatewayAddresses.Count > 0:
        ...
        for addrs in ip_properties.GatewayAddresses:
            int_dict.setdefault(names[addrs.Address.AddressFamily], []).append(addrs.Address.ToString().split('%')[0])
    return int_dict
...
def _get_ip_dns_info(i_face):
    ...
    if ip_properties.DnsAddresses.Count > 0:
        ...
        for addrs in ip_properties.DnsAddresses:
            int_dict.setdefault(names[addrs.AddressFamily], []).append(addrs.ToString().split('%')[0])
    return int_dict
...
def _get_ip_anycast_info(i_face):
    ...
    if ip_properties.AnycastAddresses.Count > 0:
        ...
        for addrs in ip_properties.AnycastAddresses:
            int_dict.setdefault(names[addrs.Address.AddressFamily], []).append(addrs.Address.ToString())
    return int_dict
