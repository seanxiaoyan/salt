...
def _win_user_token_is_admin(user_token):
    ...
    class SID_IDENTIFIER_AUTHORITY(ctypes.Structure):
        _fields_ = [('byte0', ctypes.c_byte), ('byte1', ctypes.c_byte), ('byte2', ctypes.c_byte), ('byte3', ctypes.c_byte), ('byte4', ctypes.c_byte), ('byte5', ctypes.c_byte)]
    ...
    try:
        ...
        if ctypes.windll.advapi32.CheckTokenMembership(user_token, administrators_group, ctypes.byref(is_admin)) == 0:
...
def chugid(runas, group=None):
    ...
    if group:
        try:
            ...
            raise CommandExecutionError('Failed to fetch the GID for {}. Error: {}'.format(group, err))
    else:
    ...
    if os.getgid() != target_pw_gid:
        try:
            ...
            raise CommandExecutionError('Failed to change from gid {} to {}. Error: {}'.format(os.getgid(), target_pw_gid, err))
    if sorted(os.getgroups()) != sorted(supgroups):
        try:
            os.setgroups(supgroups)
        except OSError as err:
            raise CommandExecutionError('Failed to set supplemental groups to {}. Error: {}'.format(supgroups, err))
    if os.getuid() != uinfo.pw_uid:
        try:
            os.setuid(uinfo.pw_uid)
        except OSError as err:
            raise CommandExecutionError('Failed to change from uid {} to {}. Error: {}'.format(os.getuid(), uinfo.pw_uid, err))
...
def chugid_and_umask(runas, umask, group=None):
    ...
    if group:
        ...
        if group != current_grp:
            set_grp = True
    elif runas and runas != current_user:
...
def get_default_group(user):
    ...
    """
    return grp.getgrgid(pwd.getpwnam(user).pw_gid).gr_name if HAS_GRP and HAS_PWD else None
