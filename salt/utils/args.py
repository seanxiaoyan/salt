...
KWARG_REGEX = re.compile('^([^\\d\\W][\\w.-]*)=(?!=)(.*)$', re.UNICODE)
...
def invalid_kwargs(invalid_kwargs, raise_exc=True):
    ...
    msg = 'The following keyword arguments are not valid: {}'.format(', '.join(invalid_kwargs))
    if raise_exc:
...
def parse_input(args, condition=True, no_parse=None):
    ...
    for arg in args:
        if isinstance(arg, str):
            ...
            if arg_name:
                _kwargs[arg_name] = yamlify_arg(arg_value) if arg_name not in no_parse else arg_value
            else:
...
def yamlify_arg(arg):
    ...
    if arg.strip() in ('', '---', '...'):
        ...
        return arg
    elif any((np_char in arg for np_char in ('\t', '\r', '\n'))):
        return arg
    try:
        log.info('Trace')
        import salt.utils.yaml
...
def shlex_split(s, **kwargs):
    ...
    """
    if isinstance(s, str):
        return salt.utils.data.decode(shlex.split(salt.utils.stringutils.to_str(s), **kwargs))
    else:
        return s
...
def format_call(fun, data, initial_ret=None, expected_extra_kws=(), is_class_method=None):
    """
    ...
    if missing_args:
        ...
        raise SaltInvocationError('{} takes at least {} argument{} ({} given)'.format(fun.__name__, args_count, args_count > 1 and 's' or '', used_args_count))
    ...
    if extra:
        if len(extra) == 1:
            msg = "'{0[0]}' is an invalid keyword argument for '{1}'".format(list(extra.keys()), ret.get('full', '{}.{}'.format(fun.__module__, fun.__name__)))
        else:
            msg = "{} and '{}' are invalid keyword arguments for '{}'".format(', '.join(["'{}'".format(e) for e in extra][:-1]), list(extra.keys())[-1], ret.get('full', '{}.{}'.format(fun.__module__, fun.__name__)))
        raise SaltInvocationError(msg)
    return ret
