...
def get_xml_node(node, xpath):
    ...
    if res is None:
        ...
        matcher = re.match('(?P<tag>[^[]+)(?:\\[@(?P<attr>\\w+)=["\'](?P<value>[^"\']+)["\']])?', segment)
        attrib = {matcher.group('attr'): matcher.group('value')} if matcher.group('attr') and matcher.group('value') else {}
...
def clean_node(parent_map, node, ignored=None):
    ...
    removed = False
    if len(node.attrib.keys() - (ignored or [])) == 0 and (not list(node)) and (not has_text) and parent:
        parent.remove(node)
        removed = True
...
def attribute(path, xpath, attr_name, ignored=None, convert=None):
    ...
    """
    entry = {'path': path, 'xpath': xpath, 'get': lambda n: n.get(attr_name), 'set': lambda n, v: n.set(attr_name, str(v)), 'del': salt.utils.xmlutil.del_attribute(attr_name, ignored)}
    if convert:
...
def int_attribute(path, xpath, attr_name, ignored=None):
    ...
    :param ignored: the list of attributes to ignore when cleaning up the node
    """
    return {'path': path, 'xpath': xpath, 'get': lambda n: int(n.get(attr_name)) if n.get(attr_name) else None, 'set': lambda n, v: n.set(attr_name, str(v)), 'del': salt.utils.xmlutil.del_attribute(attr_name, ignored)}
...
def change_xml(doc, data, mapping):
    ...
    for param in mapping:
        ...
        if not xpath.startswith('./'):
            xpath = './{}'.format(xpath)
        placeholders = [s[1:-1] for s in param['path'].split(':') if s.startswith('{') and s.endswith('}')]
        ctx = {placeholder: '$$$' for placeholder in placeholders}
        all_nodes_xpath = string.Template(xpath).substitute(ctx)
        all_nodes_xpath = re.sub('(?:=[\'"]\\$\\$\\$["\'])|(?:\\[\\$\\$\\$\\])', '', all_nodes_xpath)
        kept_nodes = set()
        for value_item in values:
            ...
            if new_value is not None:
                converters = {p: (lambda n: n + 1) if '[${}]'.format(p) in xpath else lambda n: n for p in placeholders}
                ctx = {placeholder: converters[placeholder](value_item.get(placeholder, '')) for placeholder in placeholders}
