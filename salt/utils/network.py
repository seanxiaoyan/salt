...
def host_to_ips(host):
    ...
    ips = []
    try:
        for (family, socktype, proto, canonname, sockaddr) in socket.getaddrinfo(host, 0, socket.AF_UNSPEC, socket.SOCK_STREAM):
...
def _generate_minion_id():
    ...
    class DistinctList(list):
        ...
        """
        localhost_matchers = ['localhost.*', 'ip6-.*', '127[.]\\d', '0\\.0\\.0\\.0', '::1.*', 'ipv6-.*', 'fe00::.*', 'fe02::.*', '1.0.0.*.ip6.arpa']
    ...
    hosts = DistinctList().append(salt.utils.stringutils.to_unicode(socket.getfqdn(salt.utils.stringutils.to_bytes(hostname)))).append(platform.node()).append(hostname)
    if not hosts:
        try:
            for a_nfo in socket.getaddrinfo(hosts.first() or 'localhost', None, socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_IP, socket.AI_CANONNAME):
                if len(a_nfo) > 3:
    ...
    for f_name in ('/etc/hostname', '/etc/nodename', '/etc/hosts', '{win}\\system32\\drivers\\etc\\hosts'.format(win=os.getenv('WINDIR'))):
        try:
...
def _ip_options(ip_obj, version, options=None):
    options_fun_map = {'global': _ip_options_global, 'link-local': _ip_options_link_local, 'linklocal': _ip_options_link_local, 'll': _ip_options_link_local, 'link_local': _ip_options_link_local, 'loopback': _ip_options_loopback, 'lo': _ip_options_loopback, 'multicast': _ip_options_multicast, 'private': _ip_options_private, 'public': _ip_options_global, 'reserved': _ip_options_reserved, 'site-local': _ip_options_site_local, 'sl': _ip_options_site_local, 'site_local': _ip_options_site_local, 'unspecified': _ip_options_unspecified}
    if not options:
        return str(ip_obj)
...
def _ipv_filter(value, version, options=None):
    ...
    if isinstance(value, (str, bytes)):
        return _is_ipv(value, version, options=options)
    elif isinstance(value, (list, tuple, types.GeneratorType)):
        return [_is_ipv(addr, version, options=options) for addr in value if _is_ipv(addr, version, options=options) is not None]
    return None
...
def _network_hosts(ip_addr_entry):
    return [str(host) for host in ipaddress.ip_network(ip_addr_entry, strict=False).hosts()]
...
def _interfaces_ifconfig(out):
    ...
    if salt.utils.platform.is_sunos():
        pip = re.compile('.*?(?:inet\\s+)([0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+)(.*)')
        pip6 = re.compile('.*?(?:inet6 )([0-9a-fA-F:]+)')
        pmask6 = re.compile('.*?(?:inet6 [0-9a-fA-F:]+/(\\d+)).*')
    else:
        pip = re.compile('.*?(?:inet addr:|inet [^\\d]*)(.*?)\\s')
        pip6 = re.compile('.*?(?:inet6 addr: (.*?)/|inet6 )([0-9a-fA-F:]+)')
        pmask6 = re.compile('.*?(?:inet6 addr: [0-9a-fA-F:]+/(\\d+)|prefixlen (\\d+))(?: Scope:([a-zA-Z]+)| scopeid (0x[0-9a-fA-F]))?')
            ...
            if mmac:
                ...
                if salt.utils.platform.is_sunos():
                    ...
                    for chunk in data['hwaddr'].split(':'):
                        expand_mac.append('0{}'.format(chunk) if len(chunk) < 2 else '{}'.format(chunk))
...
def linux_interfaces():
    ...
    if ip_path:
        cmd1 = subprocess.Popen([ip_path, 'link', 'show'], close_fds=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT).communicate()[0]
        cmd2 = subprocess.Popen([ip_path, 'addr', 'show'], close_fds=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT).communicate()[0]
        ifaces = _interfaces_ip('{}\n{}'.format(salt.utils.stringutils.to_str(cmd1), salt.utils.stringutils.to_str(cmd2)))
    elif ifconfig_path:
        cmd = subprocess.Popen([ifconfig_path, '-a'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT).communicate()[0]
...
def _netbsd_interfaces_ifconfig(out):
    ...
    pip = re.compile('.*?inet [^\\d]*(.*?)/([\\d]*)\\s')
    pip6 = re.compile('.*?inet6 ([0-9a-f:]+)%([a-zA-Z0-9]*)/([\\d]*)\\s')
    pupdown = re.compile('UP')
    pbcast = re.compile('.*?broadcast ([\\d\\.]+)')
    groups = re.compile('\r?\n(?=\\S)').split(out)
    for group in groups:
        data = dict()
        iface = ''
...
def _junos_interfaces_ifconfig(out):
    ...
    pip = re.compile('.*?inet\\s*(primary)*\\s+mtu (\\d+)\\s+local=[^\\d]*(.*?)\\s{0,40}dest=[^\\d]*(.*?)\\/([\\d]*)\\s{0,40}bcast=((?:[0-9]{1,3}\\.){3}[0-9]{1,3})')
    pip6 = re.compile('.*?inet6 mtu [^\\d]+\\s{0,40}local=([0-9a-f:]+)%([a-zA-Z0-9]*)/([\\d]*)\\s')
    pupdown = re.compile('UP')
    pbcast = re.compile('.*?broadcast ([\\d\\.]+)')
    groups = re.compile('\r?\n(?=\\S)').split(out)
    for group in groups:
...
def _interfaces_ipconfig(out):
    ...
    iface = None
    addr = None
    adapter_iface_regex = re.compile('adapter (\\S.+):$')
        ...
        if iface:
            ...
            if addr and key == 'Subnet Mask':
                ...
                if 'inet' not in iface:
                    iface['inet'] = list()
                addr = {'address': val.rstrip('(Preferred)'), 'netmask': None, 'broadcast': None}
...
def _get_iface_info(iface):
    ...
    if iface in iface_info.keys():
        ...
        error_msg = 'Interface "{}" not in available interfaces: "{}"'.format(iface, '", "'.join(iface_info.keys()))
        log.error(error_msg)
...
def _hw_addr_aix(iface):
    ...
    """
    cmd = subprocess.Popen(['grep', 'Hardware Address'], stdin=subprocess.Popen(['entstat', '-d', iface], stdout=subprocess.PIPE, stderr=subprocess.STDOUT).stdout, stdout=subprocess.PIPE, stderr=subprocess.STDOUT).communicate()[0]
    if cmd:
        comps = cmd.split(' ')
        if len(comps) == 3:
            mac_addr = comps[2].strip("'").strip()
            return mac_addr
    error_msg = 'Interface "{}" either not available or does not contain a hardware address'.format(iface)
    log.error(error_msg)
    return error_msg
...
def _get_ips(ifaces, proto='inet'):
    ...
    for ip_info in ifaces.values():
        ...
        ret.extend([addr for addr in ip_info.get('secondary', []) if addr.get('type') == proto])
    return ret
...
def _filter_interfaces(interface=None, interface_data=None):
    ...
    if interface is None:
        ...
        ret = {k: v for (k, v) in ifaces.items() if any((fnmatch.fnmatch(k, pat) for pat in interface))}
    return ret
...
def _ip_networks(interface=None, include_loopback=False, verbose=False, interface_data=None, proto='inet'):
    """
    ...
    if not verbose:
        return [str(addr) for addr in sorted(ret)]
    verbose_ret = {str(x): {'address': str(x.network_address), 'netmask': str(x.netmask), 'num_addresses': x.num_addresses, 'prefixlen': x.prefixlen} for x in ret}
    return verbose_ret

def ip_networks(interface=None, include_loopback=False, verbose=False, interface_data=None):
    """
    ...
    be returned.
    """
    return _ip_networks(interface=interface, include_loopback=include_loopback, verbose=verbose, interface_data=interface_data, proto='inet')

def ip_networks6(interface=None, include_loopback=False, verbose=False, interface_data=None):
    """
    ...
    be returned.
    """
    return _ip_networks(interface=interface, include_loopback=include_loopback, verbose=verbose, interface_data=interface_data, proto='inet6')
...
def hex2ip(hex_ip, invert=False):
    ...
    return '{}.{}.{}.{}'.format(hip >> 24 & 255, hip >> 16 & 255, hip >> 8 & 255, hip & 255)
...
def mac2eui64(mac, prefix=None):
    ...
    """
    eui64 = re.sub('[.:-]', '', mac).lower()
    eui64 = eui64[0:6] + 'fffe' + eui64[6:]
    eui64 = hex(int(eui64[0:2], 16) | 2)[2:].zfill(2) + eui64[2:]
    if prefix is None:
        return ':'.join(re.findall('.{4}', eui64))
    else:
        try:
    ...
    for statf in ['/proc/net/tcp', '/proc/net/tcp6']:
        ...
        with salt.utils.files.fopen(statf, 'r') as fp_:
            for line in fp_:
                ...
                iret = _parse_tcp_line(line)
                slot = next(iter(iret))
                if iret[slot][which_end] == port and iret[slot]['state'] == 1:
...
def _netlink_tool_remote_on(port, which_end):
    ...
    try:
        data = subprocess.check_output(['ss', '-ant', tcp_end, ':{}'.format(port)])
...
def _freebsd_remotes_on(port, which_end):
    ...
    for line in lines:
        ...
        if which_end == 'local' and int(lport) != port:
            continue
        if which_end == 'remote' and int(rport) != port:
            continue
        remotes.add(rhost)
    return remotes
...
def _linux_remotes_on(port, which_end):
    ...
    if lsof_binary is None:
        return remotes
    try:
        data = subprocess.check_output([lsof_binary, '-iTCP:{:d}'.format(port), '-n', '-P'])
    except subprocess.CalledProcessError as ex:
        if ex.returncode == 1:
...
def gen_mac(prefix='AC:DE:48'):
    ...
    """
    return '{}:{:02X}:{:02X}:{:02X}'.format(prefix, random.randint(0, 255), random.randint(0, 255), random.randint(0, 255))
...
def dns_check(addr, port, safe=False, ipv6=None):
    ...
    try:
        ...
        log.info('Trace')
        raise SaltSystemExit(code=42, msg="Attempt to resolve address '{}' failed. Invalid or unresolveable address".format(addr))
    except OSError:
        log.info('Trace')
        socket_error = True
    if socket_error and ipv6:
        try:
            log.info('Trace')
            refresh_dns()
            addrinfo = socket.getaddrinfo(addr, port, socket.AF_INET, socket.SOCK_STREAM)
            ip_addrs = _test_addrs(addrinfo, port)
        except TypeError:
            log.info('Trace')
            raise SaltSystemExit(code=42, msg="Attempt to resolve address '{}' failed. Invalid or unresolveable address".format(addr))
...
def parse_host_port(host_port):
    ...
    if _s_[0] == '[':
        ...
        try:
            ...
            errmsg = 'host_port "{}" port value "{}" is not an integer.'.format(host_port, port)
            log.error(errmsg)
...
def filter_by_networks(values, networks):
    ...
    """
    _filter = lambda ips, networks: [ip for ip in ips for net in networks if ipaddress.ip_address(ip) in net]
    if networks is not None:
        networks = [ipaddress.ip_network(network) for network in networks]
        if isinstance(values, Mapping):
            return {interface: _filter(values[interface], networks) for interface in values}
