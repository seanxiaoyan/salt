...
def query(function, api_key=None, args=None, method='GET', header_dict=None, data=None, opts=None):
    """
    ...
    slack_functions = {'rooms': {'request': 'channels.list', 'response': 'channels'}, 'users': {'request': 'users.list', 'response': 'members'}, 'message': {'request': 'chat.postMessage', 'response': 'channel'}}
    if not api_key:
    ...
    result = salt.utils.http.query(url, method, params=query_params, data=data, decode=True, status=True, header_dict=header_dict, opts=opts)
