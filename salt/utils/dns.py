...
class RFC:
    ...
    TLSA_USAGE = OrderedDict(((0, 'pkixta'), (1, 'pkixee'), (2, 'daneta'), (3, 'daneee')))
...
def _tree(domain, tld=False):
    ...
    if not tld:
        if HAS_TLDEXTRACT:
            ...
            tld = re.search('((?:(?:ac|biz|com?|info|edu|gov|mil|name|net|n[oi]m|org)\\.)?[^.]+)$', domain).group()
            log.info('Without tldextract, dns.util resolves the TLD of %s to %s', domain, tld)
    res = [domain]
    while True:
        idx = domain.find('.')
        if idx < 0:
...
def _data2rec(schema, rec_data):
    ...
    try:
        ...
        if len(schema) == 1:
            ...
            res = {field_name: _cast(rec_field, rec_cast) for ((field_name, rec_cast), rec_field) in zip(schema.items(), rec_fields)}
        return res
...
def _data2rec_group(schema, recs_data, group_key):
    ...
    try:
        ...
        log.info('Trace')
        raise ValueError('Unable to cast "{}" as a group of "{}": {}'.format(','.join(recs_data), ' '.join(schema.keys()), e))
...
def _lookup_dig(name, rdtype, timeout=None, servers=None, secure=None):
    ...
    if secure:
        cmd += '+dnssec +adflag '
    cmd = __salt__['cmd.run_all']('{} {}'.format(cmd, name), python_shell=False, output_loglevel='quiet')
...
def _lookup_dnspython(name, rdtype, timeout=None, servers=None, secure=None):
    ...
    try:
        log.info('Trace')
        res = [_data_clean(rr.to_text()) for rr in resolver.query(name, rdtype, raise_on_no_answer=False)]
        return res
    except dns.rdatatype.UnknownRdatatype:
        log.info('Trace')
        raise ValueError('Invalid DNS type {}'.format(rdtype))
    except (dns.resolver.NXDOMAIN, dns.resolver.YXDOMAIN, dns.resolver.NoNameservers, dns.exception.Timeout):
        log.info('Trace')
        return False
...
def _lookup_nslookup(name, rdtype, timeout=None, server=None):
    ...
    cmd = __salt__['cmd.run_all'](cmd, python_shell=False, output_loglevel='quiet')
    if cmd['retcode'] != 0:
        log.warning('nslookup returned (%s): %s', cmd['retcode'], cmd['stdout'].splitlines()[-1].strip(string.whitespace + ';'))
        return False
    lookup_res = iter(cmd['stdout'].splitlines())
    res = []
    try:
...
def lookup(name, rdtype, method=None, servers=None, timeout=None, walk=False, walk_tld=False, secure=None):
    """
    ...
    rdtype = rdtype.upper()
    query_methods = (('gai', _lookup_gai, not any((rdtype not in ('A', 'AAAA'), servers, secure))), ('dnspython', _lookup_dnspython, HAS_DNSPYTHON), ('dig', _lookup_dig, HAS_DIG), ('drill', _lookup_drill, HAS_DRILL), ('host', _lookup_host, HAS_HOST and (not secure)), ('nslookup', _lookup_nslookup, HAS_NSLOOKUP and (not secure)))
    try:
        log.info('Trace')
        if method == 'auto':
        ...
        log.error('Unable to lookup %s/%s: Resolver method %s invalid, unsupported or unable to perform query', method, rdtype, name)
        return False
    res_kwargs = {'rdtype': rdtype}
    if servers:
...
def query(name, rdtype, method=None, servers=None, timeout=None, walk=False, walk_tld=False, secure=None):
    """
    ...
    """
    rdtype = rdtype.upper()
    qargs = {'method': method, 'servers': servers, 'timeout': timeout, 'walk': walk, 'walk_tld': walk_tld, 'secure': secure}
    ...
    if rdtype == 'SPF':
        ...
        qres = lookup(name, rdtype, **qargs)
    rec_map = {'A': a_rec, 'AAAA': aaaa_rec, 'CAA': caa_rec, 'MX': mx_rec, 'SOA': soa_rec, 'SPF': spf_rec, 'SRV': srv_rec, 'SSHFP': sshfp_rec, 'TLSA': tlsa_rec}
...
def sshfp_rec(rdata):
    ...
    :return: dict w/fields
    """
    rschema = OrderedDict((('algorithm', RFC.SSHFP_ALGO), ('fp_hash', RFC.SSHFP_HASH), ('fingerprint', lambda val: val.lower())))
    return _data2rec(rschema, rdata)
...
def tlsa_rec(rdata):
    ...
    :return: dict w/fields
    """
    rschema = OrderedDict((('usage', RFC.TLSA_USAGE), ('selector', RFC.TLSA_SELECT), ('matching', RFC.TLSA_MATCHING), ('pub', str)))
    return _data2rec(rschema, rdata)
...
def parse_resolv(src='/etc/resolv.conf'):
    ...
    try:
                            ...
                            try:
                                ...
                                if '/' not in ip_raw:
                                    if ip_net.version == 4:
                                        ...
                                        ip_net = ipaddress.ip_network('{}{}'.format(ip_addr, mask), strict=False)
                                    if ip_net.version == 6:
                                        pass
        ...
        return {'nameservers': nameservers, 'ip4_nameservers': ip4_nameservers, 'ip6_nameservers': ip6_nameservers, 'sortlist': [ip.with_netmask for ip in sortlist], 'domain': domain, 'search': search, 'options': options}
    except OSError:
        return {}