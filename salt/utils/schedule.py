...
class Schedule:
    ...
    def __new__(cls, opts, functions, returners=None, intervals=None, cleanup=None, proxy=None, standalone=False, new_instance=False, utils=None, _subprocess_list=None):
        """
        ...
        if cls.instance is None or new_instance is True:
            ...
            instance.__singleton_init__(opts, functions, returners=returners, intervals=intervals, cleanup=cleanup, proxy=proxy, standalone=standalone, utils=utils, _subprocess_list=_subprocess_list)
    ...
    def __init__(self, opts, functions, returners=None, intervals=None, cleanup=None, proxy=None, standalone=False, new_instance=False, utils=None, _subprocess_list=None):
        pass

    def __singleton_init__(self, opts, functions, returners=None, intervals=None, cleanup=None, proxy=None, standalone=False, utils=None, _subprocess_list=None):
        log.info('Trace')
        self.opts = opts
        self.proxy = proxy
        ...
        try:
            ...
            log.warning('Unable to obtain correct timezone offset, defaulting to 0000', exc_info_on_loglevel=logging.DEBUG)
    ...
    def _get_schedule(self, include_opts=True, include_pillar=True, remove_hidden=False):
        """
    ...
    def persist(self):
        ...
        try:
            ...
            with salt.utils.files.fopen(schedule_conf, 'wb+') as fp_:
                fp_.write(salt.utils.stringutils.to_bytes(salt.utils.yaml.safe_dump({'schedule': schedule_data})))
        except OSError:
    ...
    def run_job(self, name):
        ...
        if 'run' not in data:
            data['run'] = True
        if not self.standalone:
            data = self._check_max_running(func, data, self.opts, datetime.datetime.now())
        if data.get('run'):
    ...
    def postpone_job(self, name, data):
        ...
        if name in self.opts['schedule']:
            ...
            self.opts['schedule'][name]['skip_explicit'].append({'time': time, 'time_fmt': time_fmt})
            if 'run_explicit' not in self.opts['schedule'][name]:
                self.opts['schedule'][name]['run_explicit'] = []
            self.opts['schedule'][name]['run_explicit'].append({'time': new_time, 'time_fmt': time_fmt})
        elif name in self._get_schedule(include_opts=False):
            log.warning("Cannot modify job %s, it's in the pillar!", name)
        with salt.utils.event.get_event('minion', opts=self.opts, listen=False) as evt:
            evt.fire_event({'complete': True, 'schedule': self._get_schedule()}, tag='/salt/minion/minion_schedule_postpone_job_complete')
    ...
    def skip_job(self, name, data):
        ...
        if name in self.opts['schedule']:
            ...
            log.warning("Cannot modify job %s, it's in the pillar!", name)
        with salt.utils.event.get_event('minion', opts=self.opts, listen=False) as evt:
            evt.fire_event({'complete': True, 'schedule': self._get_schedule()}, tag='/salt/minion/minion_schedule_skip_job_complete')
    ...
    def get_next_fire_time(self, name, fmt='%Y-%m-%dT%H:%M:%S'):
        ...
        with salt.utils.event.get_event('minion', opts=self.opts, listen=False) as evt:
            evt.fire_event({'complete': True, 'next_fire_time': _next_fire_time}, tag='/salt/minion/minion_schedule_next_fire_time_complete')
    ...
    def job_status(self, name, fire_event=False):
        ...
        if fire_event:
            ...
            with salt.utils.event.get_event('minion', opts=self.opts, listen=False) as evt:
                evt.fire_event({'complete': True, 'data': data}, tag='/salt/minion/minion_schedule_job_status_complete')
        else:
            schedule = self._get_schedule()
    ...
    def handle_func(self, multiprocessing_enabled, func, data, jid=None):
        ...
        """
        if salt.utils.platform.spawning_platform() or self.opts.get('transport') == 'zeromq':
        ...
        ret = {'id': self.opts.get('id', 'master'), 'fun': func, 'fun_args': [], 'schedule': data['name'], 'jid': jid}
        if 'metadata' in data:
        ...
        try:
            ...
            if self.opts['__role'] == 'master':
                ...
                namespaced_event = salt.utils.event.NamespacedEvent(salt.utils.event.get_event(self.opts['__role'], self.opts['sock_dir'], opts=self.opts, listen=False), tag, print_func=None)
                ...
                if data_returner or self.schedule_returner:
                    ...
                    for returner in OrderedDict.fromkeys(rets):
                        ...
                        if ret_str in self.returners:
                            ...
                            log.info('Job %s using invalid returner: %s. Ignoring.', func, returner)
        except Exception:
            log.info('Trace')
    ...
    def eval(self, now=None):
        ...
        def _handle_once(data, loop_interval):
            ...
            if data['_next_fire_time']:
                if data['_next_fire_time'] < now - loop_interval or (data['_next_fire_time'] > now and (not data['_splay'])):
                    data['_continue'] = True
            if not data['_next_fire_time'] and (not data['_splay']):
                ...
                if not isinstance(once, datetime.datetime):
                    ...
                    try:
                        ...
                        data['_error'] = 'Date string could not be parsed: {}, {}. Ignoring job {}.'.format(data['once'], once_fmt, data['name'])
        ...
        def _handle_when(data, loop_interval):
            ...
            if not _WHEN_SUPPORTED:
                data['_error'] = 'Missing python-dateutil. Ignoring job {}.'.format(data['name'])
                log.error(data['_error'])
                return
            ...
            for i in _when_data:
                if 'pillar' in self.opts and 'whens' in self.opts['pillar'] and (i in self.opts['pillar']['whens']):
                    if not isinstance(self.opts['pillar']['whens'], dict):
                        data['_error'] = 'Pillar item "whens" must be a dict. Ignoring job {}.'.format(data['name'])
                        log.error(data['_error'])
                        return
                    ...
                    if not isinstance(self.opts['grains']['whens'], dict):
                        data['_error'] = 'Grain "whens" must be a dict. Ignoring job {}.'.format(data['name'])
                        log.error(data['_error'])
                        return
                ...
                if not isinstance(when_, datetime.datetime):
                    try:
                        ...
                        data['_error'] = 'Invalid date string {}. Ignoring job {}.'.format(i, data['name'])
                        log.error(data['_error'])
                        return
                _when.append(when_)
            if data['_splay']:
            ...
            if _when:
                when = _when[0]
                if when < now - loop_interval and (not data.get('_run', False)) and (not run) and (not data['_splay']):
        ...
        def _handle_cron(data, loop_interval):
            ...
            if not _CRON_SUPPORTED:
                data['_error'] = 'Missing python-croniter. Ignoring job {}.'.format(data['name'])
                log.error(data['_error'])
                return
            if data['_next_fire_time'] is None:
                try:
                    ...
                    data['_error'] = 'Invalid cron string. Ignoring job {}.'.format(data['name'])
                    log.error(data['_error'])
                    return
        ...
        def _handle_run_explicit(data, loop_interval):
            ...
            for _run_time in data['run_explicit']:
                if isinstance(_run_time, datetime.datetime):
                    ...
                    _run_explicit.append(datetime.datetime.strptime(_run_time['time'], _run_time['time_fmt']))
            data['run'] = False
        ...
        def _handle_skip_explicit(data, loop_interval):
            ...
            for _skip_time in data['skip_explicit']:
                if isinstance(_skip_time, datetime.datetime):
                    ...
                    _skip_explicit.append(datetime.datetime.strptime(_skip_time['time'], _skip_time['time_fmt']))
        ...
        def _handle_skip_during_range(data, loop_interval):
            ...
            if not isinstance(data['skip_during_range'], dict):
                data['_error'] = 'schedule.handle_func: Invalid, range must be specified as a dictionary. Ignoring job {}.'.format(data['name'])
                log.error(data['_error'])
                return
            ...
            if not isinstance(start, datetime.datetime):
                try:
                    ...
                    data['_error'] = 'Invalid date string for start in skip_during_range. Ignoring job {}.'.format(data['name'])
                    log.error(data['_error'])
                    return
            if not isinstance(end, datetime.datetime):
                try:
                    ...
                    data['_error'] = 'Invalid date string for end in skip_during_range. Ignoring job {}.'.format(data['name'])
                    log.error(data['_error'])
                    return
            if 'run_after_skip_range' in data and data['run_after_skip_range']:
                ...
                if _run_immediate not in data['run_explicit']:
                    data['run_explicit'].append({'time': _run_immediate, 'time_fmt': '%Y-%m-%dT%H:%M:%S'})
            if end > start:
                if start <= now <= end:
                    if self.skip_function:
                        data['run'] = True
                ...
                data['_error'] = 'schedule.handle_func: Invalid range, end must be larger than start. Ignoring job {}.'.format(data['name'])
                log.error(data['_error'])
        ...
        def _handle_range(data):
            ...
            if not isinstance(start, datetime.datetime):
                try:
                    ...
                    data['_error'] = 'Invalid date string for start. Ignoring job {}.'.format(data['name'])
                    log.error(data['_error'])
                    return
            if not isinstance(end, datetime.datetime):
                try:
                    ...
                    data['_error'] = 'Invalid date string for end. Ignoring job {}.'.format(data['name'])
                    log.error(data['_error'])
                    return
            if end > start:
        ...
        for (job, data) in schedule.items():
            if job in _hidden:
                continue
            for item in ['_continue', '_error', '_enabled', '_skipped', '_skip_reason', '_skipped_time']:
                if item in data:
                    del data[item]
            run = False
            if 'name' in data:
            ...
            if not isinstance(data, dict):
                log.error('Scheduled job "%s" should have a dict value, not %s', job_name, type(data))
                continue
            if 'function' in data:
                func = data['function']
            elif 'func' in data:
            ...
            if any((i <= schedule_keys for i in invalid_sched_combos)):
                log.error('Unable to use "%s" options together. Ignoring.', '", "'.join(scheduling_elements))
                continue
            invalid_time_combos = []
            ...
            if any((set(x) <= schedule_keys for x in invalid_time_combos)):
                log.error('Unable to use "%s" with "%s" options. Ignoring', '", "'.join(time_elements), '", "'.join(scheduling_elements))
                continue
            ...
            if 'splay' in data and data['splay']:
                if not data['_splay']:
                    ...
                    if now < data['_next_fire_time'] + datetime.timedelta(seconds=splay):
                        log.debug('schedule.handle_func: Adding splay of %s seconds to next run.', splay)
            ...
            try:
                if run:
                    if 'jid_include' not in data or data['jid_include']:
                        ...
                        log.debug('schedule: Job %s was scheduled with jid_include, adding to cache (jid_include defaults to True)', job_name)
                        if 'maxrunning' in data:
                            log.debug('schedule: Job %s was scheduled with a max number of %s', job_name, data['maxrunning'])
                        else:
                            log.info('schedule: maxrunning parameter was not specified for job %s, defaulting to 1.', job_name)
                            data['maxrunning'] = 1
                    if not self.standalone:
                        data['run'] = run
                ...
                if run:
                    jid = salt.utils.jid.gen_jid(self.opts)
                    jids.append(jid)
                    log.info('Running scheduled job: %s%s with jid %s', job_name, miss_msg, jid)
        ...
        try:
            ...
            if multiprocessing_enabled:
                with salt.utils.process.default_signals(signal.SIGINT, signal.SIGTERM):
                    proc = thread_cls(target=self.handle_func, args=(multiprocessing_enabled, func, data, jid), name=name)
                    proc.start()
        ...
        with salt.utils.files.fopen(fn_, 'rb') as fp_:
            ...
            try:
                ...
                try:
                    ...
                    log.info('Trace')
                    continue
            log.debug('schedule.clean_proc_dir: checking job %s for process existence', job)
            if job is not None and 'pid' in job:
                if salt.utils.process.os_is_running(job['pid']):
                    log.debug('schedule.clean_proc_dir: Cleaning proc dir, pid %s still exists.', job['pid'])
                else:
                    if salt.utils.platform.is_windows():
                        fp_.close()
                    try:
