...
CLIENT_BDM2_KEYS = {'id': 'uuid', 'source': 'source_type', 'dest': 'destination_type', 'bus': 'disk_bus', 'device': 'device_name', 'size': 'volume_size', 'format': 'guest_format', 'bootindex': 'boot_index', 'type': 'device_type', 'shutdown': 'delete_on_termination'}
...
def check_nova():
    if HAS_NOVA:
        ...
        if min_ver <= novaclient_ver <= max_ver:
            ...
            log.debug('Older novaclient version required. Maximum: %s', NOVACLIENT_MAXVER)
            return False
...
if check_nova():
    try:
        ...
        log.debug('Using novaclient version 7.0.0 or newer. Authentication plugin auth_plugin.py is not available anymore.')
...
def _parse_block_device_mapping_v2(block_device=None, boot_volume=None, snapshot=None, ephemeral=None, swap=None):
    ...
    if boot_volume is not None:
        bdm_dict = {'uuid': boot_volume, 'source_type': 'volume', 'destination_type': 'volume', 'boot_index': 0, 'delete_on_termination': False}
        bdm.append(bdm_dict)
    if snapshot is not None:
        bdm_dict = {'uuid': snapshot, 'source_type': 'snapshot', 'destination_type': 'volume', 'boot_index': 0, 'delete_on_termination': False}
        bdm.append(bdm_dict)
    ...
    for ephemeral_spec in ephemeral:
        ...
        bdm.append(bdm_dict)
    if swap is not None:
        bdm_dict = {'source_type': 'blank', 'destination_type': 'local', 'boot_index': -1, 'delete_on_termination': True, 'guest_format': 'swap', 'volume_size': swap}
        bdm.append(bdm_dict)
    return bdm
    ...
    for service_entry in catalog:
        if service_entry['type'] == service_type:
            for endpoint_entry in service_entry['endpoints']:
                if endpoint_entry['region'] == region_name and endpoint_entry['interface'] == 'public':
                    return endpoint_entry['url']
    return None

def sanatize_novaclient(kwargs):
    variables = ('username', 'api_key', 'project_id', 'auth_url', 'insecure', 'timeout', 'proxy_tenant_id', 'proxy_token', 'region_name', 'endpoint_type', 'extensions', 'service_type', 'service_name', 'volume_service_name', 'timings', 'bypass_url', 'os_cache', 'no_cache', 'http_log_debug', 'auth_system', 'auth_plugin', 'auth_token', 'cacert', 'tenant_id')
    ret = {}
    for var in kwargs:
        if var in variables:
            ret[var] = kwargs[var]
    ...
    def volume_detach(self, name, timeout=300):
        ...
        while True:
            ...
            try:
                ...
                time.sleep(1)
                if time.time() - start > timeout:
                    log.error('Timed out after %d seconds while waiting for data', timeout)
                    return False
