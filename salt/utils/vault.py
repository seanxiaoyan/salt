...
def _get_token_and_url_from_master():
    ...
    if __opts__.get('__role', 'minion') == 'minion':
        ...
        result = __salt__['publish.runner']('vault.generate_token', arg=[minion_id, signature, False, ttl, uses])
    else:
        private_key = '{}/master.pem'.format(pki_dir)
        log.debug('Running on master, signing token request for %s with key %s', minion_id, private_key)
        signature = base64.b64encode(salt.crypt.sign_message(private_key, minion_id))
        result = __salt__['saltutil.runner']('vault.generate_token', minion_id=minion_id, signature=signature, impersonated_by_master=True, ttl=ttl, uses=uses)
    if not result:
    ...
    if not isinstance(result, dict):
        ...
        raise salt.exceptions.CommandExecutionError(result)
    if 'error' in result:
        log.error('Failed to get token from master! An error was returned: %s', result['error'])
        raise salt.exceptions.CommandExecutionError(result)
    ...
    return {'url': result['url'], 'token': result['token'], 'verify': result.get('verify', None), 'namespace': result.get('namespace'), 'uses': result.get('uses', 1), 'lease_duration': result['lease_duration'], 'issued': result['issued']}
...
def get_vault_connection():
            ...
            if __opts__['vault']['auth']['method'] == 'wrapped_token':
                ...
                if _wrapped_token_valid():
                    ...
                    if response.status_code != 200:
                        ...
                        raise salt.exceptions.CommandExecutionError(errmsg)
                    __opts__['vault']['auth']['token'] = response.json()['auth']['client_token']
            return {'url': __opts__['vault']['url'], 'namespace': namespace, 'token': __opts__['vault']['auth']['token'], 'verify': __opts__['vault'].get('verify', None), 'issued': int(round(time.time())), 'ttl': 3600}
        except KeyError as err:
            log.info('Trace')
            errmsg = 'Minion has "vault" config section, but could not find key "{}" within'.format(err)
            raise salt.exceptions.CommandExecutionError(errmsg)
    ...
    elif any((__opts__.get('local', None), __opts__.get('file_client', None) == 'local', __opts__.get('master_type', None) == 'disable')):
        return _use_local_config()
    else:
        log.debug('Contacting master for Vault connection details')
        return _get_token_and_url_from_master()
...
def write_cache(connection):
    ...
    """
    if connection.get('uses', None) == 1 and 'unlimited_use_token' not in connection and ('vault_secret_path_metadata' not in connection):
        log.debug('Not caching vault single use token')
        __context__['vault_token'] = connection
        return True
    elif 'vault_secret_path_metadata' in __context__ and 'vault_secret_path_metadata' not in connection:
    ...
    try:
        ...
        with salt.utils.files.fpopen(temp_file, 'w', mode=384) as fp_:
            fp_.write(salt.utils.json.dumps(connection))
        os.close(temp_fp)
        os.rename(temp_file, cache_file)
        return True
    except OSError:
        log.error('Failed to cache vault information', exc_info_on_loglevel=logging.DEBUG)
        return False
...
def make_request(method, resource, token=None, vault_url=None, namespace=None, get_token_url=False, retry=False, **args):
    """
    ...
    if not response.ok and response.json().get('errors', None) == ['permission denied']:
        ...
        if not retry:
            ...
            response = make_request(method, resource, token=None, vault_url=vault_url, get_token_url=get_token_url, retry=True, **args)
        else:
        ...
        return response
    if 'uses' in connection and (not connection.get('unlimited_use_token')) and (not resource.startswith('v1/sys')):
...
def _selftoken_expired():
    ...
    try:
        ...
        log.info('Trace')
        raise salt.exceptions.CommandExecutionError('Error while looking up self token : {}'.format(e))
...
def _wrapped_token_valid():
    ...
    try:
        ...
        log.info('Trace')
        raise salt.exceptions.CommandExecutionError('Error while looking up wrapped token : {}'.format(e))
...
def is_v2(path):
    ...
    if ret['type'] == 'kv' and path_metadata['options'] is not None and (path_metadata.get('options', {}).get('version', '1') in ['2']):
        ret['v2'] = True
