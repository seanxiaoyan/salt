...
class BaseSchemaItemMeta(type):
    ...
    def __call__(cls, *args, **kwargs):
        instance = object.__new__(cls)
        if args:
            raise RuntimeError('Please pass all arguments as named arguments. Un-named arguments are not supported')
    ...
    def __validate_attributes__(self):
        if self.enum is not None:
            if not isinstance(self.enum, (list, tuple, set)):
                raise RuntimeError("Only the 'list', 'tuple' and 'set' python types can be used to define 'enum'")
        ...
        if self.enumNames is not None:
            if not isinstance(self.enumNames, (list, tuple, set)):
                raise RuntimeError("Only the 'list', 'tuple' and 'set' python types can be used to define 'enumNames'")
            if len(self.enum) != len(self.enumNames):
                raise RuntimeError("The size of 'enumNames' must match the size of 'enum'")
...
class NumberItem(BaseSchemaItem):
    __type__ = 'number'
    __serialize_attr_aliases__ = {'multiple_of': 'multipleOf', 'exclusive_minimum': 'exclusiveMinimum', 'exclusive_maximum': 'exclusiveMaximum'}
    multiple_of = None
    minimum = None
...
class ArrayItem(BaseSchemaItem):
    __type__ = 'array'
    __serialize_attr_aliases__ = {'min_items': 'minItems', 'max_items': 'maxItems', 'unique_items': 'uniqueItems', 'additional_items': 'additionalItems'}
    items = None
    min_items = None
    max_items = None
    unique_items = None
            ...
            if isinstance(self.items, (list, tuple)):
                for item in self.items:
                    if not isinstance(item, (Schema, SchemaItem)):
                        raise RuntimeError('All items passed in the item argument tuple/list must be a subclass of Schema, SchemaItem or BaseSchemaItem, not {}'.format(type(item)))
            elif not isinstance(self.items, (Schema, SchemaItem)):
                raise RuntimeError('The items argument passed must be a subclass of Schema, SchemaItem or BaseSchemaItem, not {}'.format(type(self.items)))
...
class DictItem(BaseSchemaItem):
    __type__ = 'object'
    __serialize_attr_aliases__ = {'min_properties': 'minProperties', 'max_properties': 'maxProperties', 'pattern_properties': 'patternProperties', 'additional_properties': 'additionalProperties'}
    properties = None
    ...
    def __validate_attributes__(self):
        if not self.properties and (not self.pattern_properties) and (not self.additional_properties):
            raise RuntimeError('One of properties, pattern_properties or additional_properties must be passed')
        if self.properties is not None:
            if not isinstance(self.properties, (Schema, dict)):
                raise RuntimeError("The passed properties must be passed as a dict or  a Schema not '{}'".format(type(self.properties)))
            if not isinstance(self.properties, Schema):
                for (key, prop) in self.properties.items():
                    if not isinstance(prop, (Schema, SchemaItem)):
                        raise RuntimeError("The passed property who's key is '{}' must be of type Schema, SchemaItem or BaseSchemaItem, not '{}'".format(key, type(prop)))
        if self.pattern_properties is not None:
            if not isinstance(self.pattern_properties, dict):
                raise RuntimeError("The passed pattern_properties must be passed as a dict not '{}'".format(type(self.pattern_properties)))
            for (key, prop) in self.pattern_properties.items():
                if not isinstance(prop, (Schema, SchemaItem)):
                    raise RuntimeError("The passed pattern_property who's key is '{}' must be of type Schema, SchemaItem or BaseSchemaItem, not '{}'".format(key, type(prop)))
        if self.additional_properties is not None:
            if not isinstance(self.additional_properties, (bool, Schema, SchemaItem)):
                raise RuntimeError("The passed additional_properties must be of type bool, Schema, SchemaItem or BaseSchemaItem, not '{}'".format(type(self.pattern_properties)))
...
class RequirementsItem(SchemaItem):
    ...
    def __validate_attributes__(self):
        ...
        if not isinstance(self.requirements, (SchemaItem, list, tuple, set)):
            raise RuntimeError("The passed requirements must be passed as a list, tuple, set SchemaItem or BaseSchemaItem, not '{}'".format(self.requirements))
        if not isinstance(self.requirements, SchemaItem):
            ...
            for (idx, item) in enumerate(self.requirements):
                if not isinstance(item, ((str,), SchemaItem)):
                    raise RuntimeError("The passed requirement at the {} index must be of type str or SchemaItem, not '{}'".format(idx, type(item)))
...
class OneOfItem(SchemaItem):
    ...
    def __validate_attributes__(self):
        ...
        if not isinstance(self.items, (list, tuple)):
            raise RuntimeError("The passed items must be passed as a list/tuple not '{}'".format(type(self.items)))
        for (idx, item) in enumerate(self.items):
            if not isinstance(item, (Schema, SchemaItem)):
                raise RuntimeError("The passed item at the {} index must be of type Schema, SchemaItem or BaseSchemaItem, not '{}'".format(idx, type(item)))
...
class NotItem(SchemaItem):
    ...
    def __validate_attributes__(self):
        ...
        if not isinstance(self.item, (Schema, SchemaItem)):
            raise RuntimeError("The passed item be of type Schema, SchemaItem or BaseSchemaItem, not '{}'".format(type(self.item)))
...
class ComplexSchemaItem(BaseSchemaItem):
    ...
    def __init__(self, definition_name=None, required=None):
        ...
        self._definition_name = definition_name if definition_name else self.__class__.__name__
    ...
    def serialize(cls, id_=None):
        ...
        while aux_items:
            ...
            if isinstance(item, OneOfItem):
                ...
                if item.additional_properties and isinstance(item.additional_properties, SchemaItem):
