...
class MasterPillarUtil:
    ...
    ** However, the minion grains are still possibly entirely
    ...
    def __init__(self, tgt='', tgt_type='glob', saltenv=None, use_cached_grains=True, use_cached_pillar=True, grains_fallback=True, pillar_fallback=True, opts=None):
        ...
        if opts is None:
            ...
            raise SaltException('{}: Missing master opts init arg.'.format(self.__class__.__name__))
        else:
            self.opts = opts
        self.tgt = tgt
        ...
        log.debug("Init settings: tgt: '%s', tgt_type: '%s', saltenv: '%s', use_cached_grains: %s, use_cached_pillar: %s, grains_fallback: %s, pillar_fallback: %s", tgt, tgt_type, saltenv, use_cached_grains, use_cached_pillar, grains_fallback, pillar_fallback)

    def _get_cached_mine_data(self, *minion_ids):
        mine_data = {minion_id: {} for minion_id in minion_ids}
        if not self.opts.get('minion_data_cache', False) and (not self.opts.get('enforce_mine_cache', False)):
            log.debug('Skipping cached mine data minion_data_cacheand enfore_mine_cache are both disabled.')
            return mine_data
        if not minion_ids:
    ...
    def _get_cached_minion_data(self, *minion_ids):
        ...
        for minion_id in minion_ids:
            ...
            if not isinstance(mdata, dict):
                log.warning('cache.fetch should always return a dict. ReturnedType: %s, MinionId: %s', type(mdata).__name__, minion_id)
                continue
            if 'grains' in mdata:
    ...
    def _get_live_minion_grains(self, minion_ids):
        ...
        with salt.client.get_local_client(self.opts['conf_file']) as client:
            return client.cmd(','.join(minion_ids), 'grains.items', timeout=self.opts['timeout'], tgt_type='list')
    ...
    def _get_live_minion_pillar(self, minion_id=None, minion_grains=None):
        ...
        pillar = salt.pillar.Pillar(self.opts, minion_grains, minion_id, self.saltenv, self.opts['ext_pillar'])
    ...
    def _get_minion_grains(self, *minion_ids, **kwargs):
        ...
        if self.use_cached_grains:
            cret = {minion_id: mcache for (minion_id, mcache) in cached_grains.items() if mcache}
            ...
            ret = dict(list({minion_id: {} for minion_id in minion_ids}.items()) + list(lret.items()) + list(cret.items()))
        else:
    ...
    def _get_minion_pillar(self, *minion_ids, **kwargs):
        ...
        if self.use_cached_pillar:
            cret = {minion_id: mcache for (minion_id, mcache) in cached_pillar.items() if mcache}
            missed_minions = [minion_id for minion_id in minion_ids if minion_id not in cret]
            log.debug('Missed cached minion pillars for: %s', missed_minions)
            if self.pillar_fallback:
                lret = {minion_id: self._get_live_minion_pillar(minion_id, grains.get(minion_id, {})) for minion_id in missed_minions}
            ret = dict(list({minion_id: {} for minion_id in minion_ids}.items()) + list(lret.items()) + list(cret.items()))
        else:
            lret = {minion_id: self._get_live_minion_pillar(minion_id, grains.get(minion_id, {})) for minion_id in minion_ids}
            missed_minions = [minion_id for minion_id in minion_ids if minion_id not in lret]
    ...
    def get_minion_pillar(self):
        ...
        minion_grains = self._get_minion_grains(*minion_ids, cached_grains=cached_minion_grains)
        log.debug('Getting minion pillar data for: %s', minion_ids)
        minion_pillars = self._get_minion_pillar(*minion_ids, grains=minion_grains, cached_pillar=cached_minion_pillars)
    ...
    def clear_cached_minion_data(self, clear_pillar=False, clear_grains=False, clear_mine=False, clear_mine_func=None):
        ...
        try:
            ...
            for minion_id in minion_ids:
                ...
                if clear_pillar and clear_grains or (clear_pillar and (not minion_grains)) or (clear_grains and (not minion_pillar)):
                ...
                try:
                    ...
                    if isinstance(data, str):
                        if data not in self.minions:
                            log.debug('ConCache Adding minion %s to cache', new_c_data[0])
