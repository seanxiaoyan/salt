...
from salt.exceptions import SaltCloudConfigError, SaltCloudException, SaltCloudExecutionFailure, SaltCloudExecutionTimeout, SaltCloudPasswordError, SaltCloudSystemExit
...
NSTATES = {0: 'running', 1: 'rebooting', 2: 'terminated', 3: 'pending'}
SSH_PASSWORD_PROMP_RE = re.compile('(?:.*)[Pp]assword(?: for .*)?:\\ *$', re.M)
SSH_PASSWORD_PROMP_SUDO_RE = re.compile('(?:.*sudo)(?:.*)[Pp]assword(?: for .*)?:', re.M)
...
def os_script(os_, vm_=None, opts=None, minion=''):
    ...
    for search_path in opts['deploy_scripts_search_path']:
        ...
        if os.path.isfile(os.path.join(search_path, '{}.sh'.format(os_))):
            return __render_script(os.path.join(search_path, '{}.sh'.format(os_)), vm_, opts, minion)
    return ''
...
def minion_config(opts, vm_):
    ...
    minion.update(salt.config.get_cloud_config_value('minion', vm_, opts, default={}, search_global=True))
...
def bootstrap(vm_, opts=None):
    ...
    if opts is None:
        opts = __opts__
    deploy_config = salt.config.get_cloud_config_value('deploy', vm_, opts, default=False)
    inline_script_config = salt.config.get_cloud_config_value('inline_script', vm_, opts, default=None)
    ...
    key_filename = salt.config.get_cloud_config_value('key_filename', vm_, opts, search_global=False, default=salt.config.get_cloud_config_value('ssh_keyfile', vm_, opts, search_global=False, default=None))
    if key_filename is not None and (not os.path.isfile(key_filename)):
        raise SaltCloudConfigError("The defined ssh_keyfile '{}' does not exist".format(key_filename))
    has_ssh_agent = False
    if opts.get('ssh_agent', False) and 'SSH_AUTH_SOCK' in os.environ and stat.S_ISSOCK(os.stat(os.environ['SSH_AUTH_SOCK']).st_mode):
        has_ssh_agent = True
    if key_filename is None and salt.config.get_cloud_config_value('password', vm_, opts, default=None) is None and (salt.config.get_cloud_config_value('win_password', vm_, opts, default=None) is None) and (has_ssh_agent is False):
        raise SaltCloudSystemExit("Cannot deploy Salt in a VM if the 'key_filename' setting is not set and there is no password set for the VM. Check the provider docs for 'change_password' option if it is supported by your provider.")
    ret = {}
    minion_conf = minion_config(opts, vm_)
    deploy_script_code = os_script(salt.config.get_cloud_config_value('os', vm_, opts, default='bootstrap-salt'), vm_, opts, minion_conf)
    ssh_username = salt.config.get_cloud_config_value('ssh_username', vm_, opts, default='root')
    ...
    if 'pub_key' not in vm_ and 'priv_key' not in vm_:
        ...
        (vm_['priv_key'], vm_['pub_key']) = gen_keys(salt.config.get_cloud_config_value('keysize', vm_, opts))
        key_id = vm_.get('name')
    ...
    deploy_kwargs['make_minion'] = salt.config.get_cloud_config_value('make_minion', vm_, opts, default=True)
    if saltify_driver:
        deploy_kwargs['wait_for_passwd_maxtries'] = 0
    win_installer = salt.config.get_cloud_config_value('win_installer', vm_, opts)
    if win_installer:
        deploy_kwargs['port'] = salt.config.get_cloud_config_value('smb_port', vm_, opts, default=445)
        deploy_kwargs['win_installer'] = win_installer
        minion = minion_config(opts, vm_)
        deploy_kwargs['master'] = minion['master']
        deploy_kwargs['username'] = salt.config.get_cloud_config_value('win_username', vm_, opts, default='Administrator')
        win_pass = salt.config.get_cloud_config_value('win_password', vm_, opts, default='')
        if win_pass:
            deploy_kwargs['password'] = win_pass
        deploy_kwargs['use_winrm'] = salt.config.get_cloud_config_value('use_winrm', vm_, opts, default=False)
        deploy_kwargs['winrm_port'] = salt.config.get_cloud_config_value('winrm_port', vm_, opts, default=5986)
        deploy_kwargs['winrm_use_ssl'] = salt.config.get_cloud_config_value('winrm_use_ssl', vm_, opts, default=True)
        deploy_kwargs['winrm_verify_ssl'] = salt.config.get_cloud_config_value('winrm_verify_ssl', vm_, opts, default=True)
        if saltify_driver:
            deploy_kwargs['port_timeout'] = 1
    ...
    fire_event('event', 'executing deploy script', 'salt/cloud/{}/deploying'.format(vm_['name']), args={'kwargs': salt.utils.data.simple_types_filter(event_kwargs)}, sock_dir=opts.get('sock_dir', os.path.join(__opts__['sock_dir'], 'master')), transport=opts.get('transport', 'zeromq'))
    ...
    return {'Error': {'Not Deployed': 'Failed to start Salt on host {}'.format(vm_['name'])}}
...
def wait_for_port(host, port=22, timeout=900, gateway=None, server_alive_interval=SERVER_ALIVE_INTERVAL, server_alive_count_max=SERVER_ALIVE_COUNT_MAX):
    """
    ...
    ssh_args = []
    ssh_args.extend(['-oStrictHostKeyChecking=no', '-oServerAliveInterval={}'.format(server_alive_interval), '-oServerAliveCountMax={}'.format(server_alive_count_max), '-oUserKnownHostsFile=/dev/null', '-oControlPath=none'])
    if 'ssh_gateway_key' in gateway:
        ssh_args.extend(['-oPasswordAuthentication=no', '-oChallengeResponseAuthentication=no', '-oPubkeyAuthentication=yes', '-oIdentitiesOnly=yes', '-oKbdInteractiveAuthentication=no', '-i {}'.format(gateway['ssh_gateway_key'])])
    command = 'nc -z -w5 -q0 {} {}'.format(host, port)
    pcmd = 'ssh {} {}@{} -p {} {}'.format(' '.join(ssh_args), gateway['ssh_gateway_user'], ssh_gateway, ssh_gateway_port, pipes.quote('date'))
    cmd = 'ssh {} {}@{} -p {} {}'.format(' '.join(ssh_args), gateway['ssh_gateway_user'], ssh_gateway, ssh_gateway_port, pipes.quote(command))
...
class Client:
    ...
    def __init__(self, server, username=None, password=None, port=445, encrypt=True, service_name=None):
    ...
    def remove_service(self, wait_timeout=10, sleep_wait=1):
        ...
        while True:
            try:
                ...
                continue
            break
        smb_tree = TreeConnect(self._client.session, '\\\\{}\\ADMIN$'.format(self._client.connection.server_name))
...
def wait_for_winexe(host, port, username, password, timeout=900):
    ...
    while True:
        try_count += 1
        try:
            ret_code = run_winexe_command('sc', 'query winexesvc', host, username, password, port)
            if ret_code == 0:
...
def wait_for_psexecsvc(host, port, username, password, timeout=900):
    ...
    while True:
        ...
        ret_code = 1
        try:
            (stdout, stderr, ret_code) = run_psexec_command('cmd.exe', '/c hostname', host, username, password, port=port)
        except Exception as exc:
        ...
        if time.time() - start > timeout:
            return False
        log.debug('Retrying psexec connection to host %s on port %s (try %s)', host, port, try_count)
        time.sleep(1)
...
def wait_for_winrm(host, port, username, password, timeout=900, use_ssl=True, verify=True):
    ...
    while True:
        trycount += 1
        try:
            winrm_kwargs = {'target': host, 'auth': (username, password), 'transport': transport}
            if not verify:
        ...
        if time.time() - start > timeout:
            ...
            return None
        log.debug('Retrying WinRM connection to host %s on port %s (try %s)', host, port, trycount)
        time.sleep(1)

def validate_windows_cred_winexe(host, username='Administrator', password=None, retries=10, retry_delay=1):
    """
    Check if the windows credentials are valid
    """
    cmd = 'winexe -U \'{}%{}\' //{} "hostname"'.format(username, password, host)
    logging_cmd = 'winexe -U \'{}%XXX-REDACTED-XXX\' //{} "hostname"'.format(username, host)
    for i in range(retries):
        ret_code = win_cmd(cmd, logging_command=logging_cmd)
    return ret_code == 0

def validate_windows_cred(host, username='Administrator', password=None, retries=10, retry_delay=1):
    """
    Check if the windows credentials are valid
    """
    for i in range(retries):
        ret_code = 1
        try:
            (stdout, stderr, ret_code) = run_psexec_command('cmd.exe', '/c hostname', host, username, password, port=445)
        except Exception as exc:
...
def wait_for_passwd(host, port=22, ssh_timeout=15, username='root', password=None, key_filename=None, maxtries=15, trysleep=1, display_ssh_output=True, gateway=None, known_hosts_file='/dev/null', hard_timeout=None):
    """
    ...
    while trycount < maxtries:
        connectfail = False
        try:
            kwargs = {'hostname': host, 'port': port, 'username': username, 'password_retries': maxtries, 'timeout': ssh_timeout, 'display_ssh_output': display_ssh_output, 'known_hosts_file': known_hosts_file, 'ssh_timeout': ssh_timeout, 'hard_timeout': hard_timeout}
            kwargs.update(__ssh_gateway_config_dict(gateway))
            if key_filename:
                if not os.path.isfile(key_filename):
                    raise SaltCloudConfigError("The defined key_filename '{}' does not exist".format(key_filename))
            ...
            trycount += 1
            log.debug('Attempting to authenticate as %s (try %s of %s)', username, trycount, maxtries)
...
def deploy_windows(host, port=445, timeout=900, username='Administrator', password=None, name=None, sock_dir=None, conf_file=None, start_action=None, parallel=False, minion_pub=None, minion_pem=None, minion_conf=None, keep_tmp=False, script_args=None, script_env=None, port_timeout=15, preseed_minion_keys=None, win_installer=None, master=None, tmp_dir='C:\\salttmp', opts=None, master_sign_pub_file=None, use_winrm=False, winrm_port=5986, winrm_use_ssl=True, winrm_verify_ssl=True, **kwargs):
    """
    ...
    if use_winrm and (not HAS_WINRM):
        log.error('WinRM requested but module winrm could not be imported. Ensure you are using version %s or higher.', WINRM_MIN_VER)
        return False
    ...
    if HAS_WINRM and use_winrm:
        winrm_session = wait_for_winrm(host=host, port=winrm_port, username=username, password=password, timeout=port_timeout * 60, use_ssl=winrm_use_ssl, verify=winrm_verify_ssl)
        if winrm_session is not None:
            service_available = True
    else:
        service_available = wait_for_psexecsvc(host=host, port=port, username=username, password=password, timeout=port_timeout * 60)
    if port_available and service_available:
        ...
        if master_sign_pub_file:
            ...
            try:
                salt.utils.smb.put_file(master_sign_pub_file, 'salt\\conf\\pki\\minion\\master_sign.pub', 'C$', conn=smb_conn)
        ...
        installer = comps[-1]
        salt.utils.smb.put_file(win_installer, 'salttemp\\{}'.format(installer), 'C$', conn=smb_conn)
        if use_winrm:
            winrm_cmd(winrm_session, 'c:\\salttemp\\{}'.format(installer), ['/S', '/master={}'.format(master), '/minion-name={}'.format(name)])
        else:
            cmd = 'c:\\salttemp\\{}'.format(installer)
            args = '/S /master={} /minion-name={}'.format(master, name)
            (stdout, stderr, ret_code) = run_psexec_command(cmd, args, host, username, password)
            if ret_code != 0:
                raise Exception('Fail installer {}'.format(ret_code))
        if minion_conf:
            if not isinstance(minion_conf, dict):
                raise DeprecationWarning('`salt.utils.cloud.deploy_windows` now only accepts dictionaries for its `minion_conf` parameter. Loading YAML...')
            minion_grains = minion_conf.pop('grains', {})
            if minion_grains:
                salt.utils.smb.put_str(salt_config_to_yaml(minion_grains, line_break='\r\n'), 'salt\\conf\\grains', conn=smb_conn)
            windows_minion_conf = {'ipc_mode': 'tcp', 'root_dir': 'c:\\salt', 'pki_dir': '/conf/pki/minion', 'multiprocessing': False}
            minion_conf = dict(minion_conf, **windows_minion_conf)
            salt.utils.smb.put_str(salt_config_to_yaml(minion_conf, line_break='\r\n'), 'salt\\conf\\minion', conn=smb_conn)
        if not keep_tmp:
            if use_winrm:
                winrm_cmd(winrm_session, 'rmdir', ['/Q', '/S', 'C:\\salttemp\\'])
            else:
                salt.utils.smb.delete_file('salttemp\\{}'.format(installer), 'C$', conn=smb_conn)
        ...
        if use_winrm:
            ...
            (stdout, stderr, ret_code) = run_psexec_command('cmd.exe', '/c net stop salt-minion', host, username, password)
            if ret_code != 0:
                return False
            log.debug('Run psexec: sc start salt-minion')
            (stdout, stderr, ret_code) = run_psexec_command('cmd.exe', '/c net start salt-minion', host, username, password)
            if ret_code != 0:
                return False
        fire_event('event', '{} has been deployed at {}'.format(name, host), 'salt/cloud/{}/deploy_windows'.format(name), args={'name': name}, sock_dir=opts.get('sock_dir', os.path.join(__opts__['sock_dir'], 'master')), transport=opts.get('transport', 'zeromq'))
        return True
    return False
...
def deploy_script(host, port=22, timeout=900, username='root', password=None, key_filename=None, script=None, name=None, sock_dir=None, provider=None, conf_file=None, start_action=None, make_master=False, master_pub=None, master_pem=None, master_conf=None, minion_pub=None, minion_pem=None, minion_conf=None, keep_tmp=False, script_args=None, script_env=None, ssh_timeout=15, maxtries=15, make_syndic=False, make_minion=True, display_ssh_output=True, preseed_minion_keys=None, parallel=False, sudo_password=None, sudo=False, tty=None, vm_=None, opts=None, tmp_dir='/tmp/.saltcloud', file_map=None, master_sign_pub_file=None, cloud_grains=None, force_minion_config=False, **kwargs):
    ...
    deploy_command = salt.config.get_cloud_config_value('deploy_command', vm_, opts, default=os.path.join(tmp_dir, 'deploy.sh'))
    if key_filename is not None and (not os.path.isfile(key_filename)):
        raise SaltCloudConfigError("The defined key_filename '{}' does not exist".format(key_filename))
    gateway = None
    ...
    if wait_for_port(host=host, port=port, gateway=gateway):
        ...
        if wait_for_passwd(host, port=port, username=username, password=password, key_filename=key_filename, ssh_timeout=ssh_timeout, display_ssh_output=display_ssh_output, gateway=gateway, known_hosts_file=known_hosts_file, maxtries=maxtries, hard_timeout=hard_timeout):
            log.debug('Logging into %s:%s as %s', host, port, username)
            ssh_kwargs = {'hostname': host, 'port': port, 'username': username, 'timeout': ssh_timeout, 'ssh_timeout': ssh_timeout, 'display_ssh_output': display_ssh_output, 'sudo_password': sudo_password, 'sftp': opts.get('use_sftp', False)}
            ...
            if root_cmd("test -e '{}'".format(tmp_dir), tty, sudo, allow_failure=True, **ssh_kwargs):
                ret = root_cmd('sh -c "( mkdir -p -m 700 \'{}\' )"'.format(tmp_dir), tty, sudo, **ssh_kwargs)
                if ret:
                    raise SaltCloudSystemExit("Can't create temporary directory in {} !".format(tmp_dir))
            if sudo:
                comps = tmp_dir.lstrip('/').rstrip('/').split('/')
                if comps:
                    if len(comps) > 1 or comps[0] != 'tmp':
                        ret = root_cmd('chown {} "{}"'.format(username, tmp_dir), tty, sudo, **ssh_kwargs)
                        if ret:
                            raise SaltCloudSystemExit('Cant set {} ownership on {}'.format(username, tmp_dir))
            ...
            for map_item in file_map:
                ...
                if not os.path.exists(map_item):
                    log.error('The local file "%s" does not exist, and will not be copied to "%s" on the target system', local_file, remote_file)
                ...
                if remote_dir not in remote_dirs:
                    root_cmd("mkdir -p '{}'".format(remote_dir), tty, sudo, **ssh_kwargs)
                    if ssh_kwargs['username'] != 'root':
                        root_cmd("chown {} '{}'".format(ssh_kwargs['username'], remote_dir), tty, sudo, **ssh_kwargs)
            ...
            if minion_pem:
                ...
                ret = root_cmd("chmod 600 '{}/minion.pem'".format(tmp_dir), tty, sudo, **ssh_kwargs)
                if ret:
                    raise SaltCloudSystemExit("Can't set perms on {}/minion.pem".format(tmp_dir))
            if minion_pub:
                ssh_file(opts, '{}/minion.pub'.format(tmp_dir), minion_pub, ssh_kwargs)
            if master_sign_pub_file:
                ssh_file(opts, '{}/master_sign.pub'.format(tmp_dir), kwargs=ssh_kwargs, local_file=master_sign_pub_file)
            if minion_conf:
                if not isinstance(minion_conf, dict):
                ...
                minion_grains = minion_conf.pop('grains', {})
                if minion_grains:
                    ssh_file(opts, '{}/grains'.format(tmp_dir), salt_config_to_yaml(minion_grains), ssh_kwargs)
                if cloud_grains and opts.get('enable_cloud_grains', True):
                    minion_conf['grains'] = {'salt-cloud': cloud_grains}
                ssh_file(opts, '{}/minion'.format(tmp_dir), salt_config_to_yaml(minion_conf), ssh_kwargs)
            if master_pem:
                ssh_file(opts, '{}/master.pem'.format(tmp_dir), master_pem, ssh_kwargs)
                ret = root_cmd("chmod 600 '{}/master.pem'".format(tmp_dir), tty, sudo, **ssh_kwargs)
                if ret:
                    raise SaltCloudSystemExit('Cant set perms on {}/master.pem'.format(tmp_dir))
            if master_pub:
            ...
            if master_conf:
                ...
                ssh_file(opts, '{}/master'.format(tmp_dir), salt_config_to_yaml(master_conf), ssh_kwargs)
            preseed_minion_keys_tempdir = '{}/preseed-minion-keys'.format(tmp_dir)
            if preseed_minion_keys is not None:
                ret = root_cmd("mkdir '{}'".format(preseed_minion_keys_tempdir), tty, sudo, **ssh_kwargs)
                if ret:
                    raise SaltCloudSystemExit('Cant create {}'.format(preseed_minion_keys_tempdir))
                ret = root_cmd("chmod 700 '{}'".format(preseed_minion_keys_tempdir), tty, sudo, **ssh_kwargs)
                if ret:
                    raise SaltCloudSystemExit("Can't set perms on {}".format(preseed_minion_keys_tempdir))
                if ssh_kwargs['username'] != 'root':
                    root_cmd("chown {} '{}'".format(ssh_kwargs['username'], preseed_minion_keys_tempdir), tty, sudo, **ssh_kwargs)
                ...
                if ssh_kwargs['username'] != 'root':
                    root_cmd("chown -R root '{}'".format(preseed_minion_keys_tempdir), tty, sudo, **ssh_kwargs)
                    if ret:
                        raise SaltCloudSystemExit("Can't set ownership for {}".format(preseed_minion_keys_tempdir))
            preflight_cmds = kwargs.get('preflight_cmds', [])
            for command in preflight_cmds:
                cmd_ret = root_cmd(command, tty, sudo, **ssh_kwargs)
                if cmd_ret:
                    raise SaltCloudSystemExit("Pre-flight command failed: '{}'".format(command))
            if script:
                ssh_file(opts, '{}/deploy.sh'.format(tmp_dir), script, ssh_kwargs)
                ret = root_cmd('sh -c "( chmod +x \'{}/deploy.sh\' )";exit $?'.format(tmp_dir), tty, sudo, **ssh_kwargs)
                if ret:
                    raise SaltCloudSystemExit("Can't set perms on {}/deploy.sh".format(tmp_dir))
            ...
            if start_action and (not parallel):
                ...
                process = multiprocessing.Process(target=check_auth, kwargs=dict(name=name, sock_dir=sock_dir, timeout=newtimeout, queue=queue), name='DeployScriptCheckAuth({})'.format(name))
            ...
            if script:
                ...
                if script_env:
                    if not isinstance(script_env, dict):
                        raise SaltCloudSystemExit("The 'script_env' configuration setting NEEDS to be a dictionary not a {}".format(type(script_env)))
                    environ_script_contents = ['#!/bin/sh']
                    for (key, value) in script_env.items():
                        environ_script_contents.append("setenv {0} '{1}' >/dev/null 2>&1 || export {0}='{1}'".format(key, value))
                    environ_script_contents.append(deploy_command)
                    ssh_file(opts, '{}/environ-deploy-wrapper.sh'.format(tmp_dir), '\n'.join(environ_script_contents), ssh_kwargs)
                    root_cmd("chmod +x '{}/environ-deploy-wrapper.sh'".format(tmp_dir), tty, sudo, **ssh_kwargs)
                    deploy_command = "'{}/environ-deploy-wrapper.sh'".format(tmp_dir)
                if root_cmd(deploy_command, tty, sudo, **ssh_kwargs) != 0:
                    raise SaltCloudSystemExit("Executing the command '{}' failed".format(deploy_command))
                log.debug("Executed command '%s'", deploy_command)
                if not keep_tmp:
                    root_cmd("rm -f '{}/deploy.sh'".format(tmp_dir), tty, sudo, **ssh_kwargs)
                    log.debug('Removed %s/deploy.sh', tmp_dir)
                    if script_env:
                        root_cmd("rm -f '{}/environ-deploy-wrapper.sh'".format(tmp_dir), tty, sudo, **ssh_kwargs)
                        log.debug('Removed %s/environ-deploy-wrapper.sh', tmp_dir)
            if keep_tmp:
                log.debug('Not removing deployment files from %s/', tmp_dir)
            else:
                if minion_pub:
                    root_cmd("rm -f '{}/minion.pub'".format(tmp_dir), tty, sudo, **ssh_kwargs)
                    log.debug('Removed %s/minion.pub', tmp_dir)
                if minion_pem:
                    root_cmd("rm -f '{}/minion.pem'".format(tmp_dir), tty, sudo, **ssh_kwargs)
                    log.debug('Removed %s/minion.pem', tmp_dir)
                if minion_conf:
                    root_cmd("rm -f '{}/grains'".format(tmp_dir), tty, sudo, **ssh_kwargs)
                    log.debug('Removed %s/grains', tmp_dir)
                    root_cmd("rm -f '{}/minion'".format(tmp_dir), tty, sudo, **ssh_kwargs)
                    log.debug('Removed %s/minion', tmp_dir)
                if master_sign_pub_file:
                    root_cmd('rm -f {}/master_sign.pub'.format(tmp_dir), tty, sudo, **ssh_kwargs)
                    log.debug('Removed %s/master_sign.pub', tmp_dir)
                if master_pub:
                    root_cmd("rm -f '{}/master.pub'".format(tmp_dir), tty, sudo, **ssh_kwargs)
                    log.debug('Removed %s/master.pub', tmp_dir)
                if master_pem:
                    root_cmd("rm -f '{}/master.pem'".format(tmp_dir), tty, sudo, **ssh_kwargs)
                    log.debug('Removed %s/master.pem', tmp_dir)
                if master_conf:
                    root_cmd("rm -f '{}/master'".format(tmp_dir), tty, sudo, **ssh_kwargs)
                    log.debug('Removed %s/master', tmp_dir)
                if preseed_minion_keys is not None:
                    root_cmd("rm -rf '{}'".format(preseed_minion_keys_tempdir), tty, sudo, **ssh_kwargs)
            ...
            if start_action and (not parallel):
                ...
                if queuereturn and start_action:
                    ...
                    root_cmd('salt-call {}'.format(start_action), tty, sudo, **ssh_kwargs)
                    log.info('Finished executing %s on the salt-minion', start_action)
            fire_event('event', '{} has been deployed at {}'.format(name, host), 'salt/cloud/{}/deploy_script'.format(name), args={'name': name, 'host': host}, sock_dir=opts.get('sock_dir', os.path.join(__opts__['sock_dir'], 'master')), transport=opts.get('transport', 'zeromq'))
            if file_map_fail or file_map_success:
                return {'File Upload Success': file_map_success, 'File Upload Failure': file_map_fail}
            return True
    return False
    ...
    if wait_for_port(host=host, port=port, gateway=gateway):
        ...
        if wait_for_passwd(host, port=port, username=username, password=password, key_filename=key_filename, ssh_timeout=ssh_timeout, display_ssh_output=display_ssh_output, gateway=gateway, known_hosts_file=known_hosts_file):
            ...
            if key_filename:
                ...
                ssh_kwargs['password'] = password
            if root_cmd('test -e \\"{}\\"'.format(tmp_dir), tty, sudo, allow_failure=True, **ssh_kwargs) and inline_script:
                log.debug('Found inline script to execute.')
                for cmd_line in inline_script:
                    log.info('Executing inline command: %s', cmd_line)
                    ret = root_cmd('sh -c "( {} )"'.format(cmd_line), tty, sudo, allow_failure=True, **ssh_kwargs)
                    if ret:
...
def _exec_ssh_cmd(cmd, error_msg=None, allow_failure=False, **kwargs):
    ...
    try:
        ...
        proc = salt.utils.vt.Terminal(cmd, shell=True, log_stdout=True, log_stderr=True, stream_stdout=kwargs.get('display_ssh_output', True), stream_stderr=kwargs.get('display_ssh_output', True))
        sent_password = 0
        while proc.has_unread_data:
            (stdout, stderr) = proc.recv()
            if stdout and SSH_PASSWORD_PROMP_RE.search(stdout):
                if 'key_filename' in kwargs and kwargs['key_filename'] and SSH_PASSWORD_PROMP_SUDO_RE.search(stdout):
                    proc.sendline(kwargs['sudo_password'])
                elif kwargs.get('password', None) and sent_password < password_retries:
                    sent_password += 1
        ...
        if proc.exitstatus != 0 and allow_failure is False:
            raise SaltCloudSystemExit("Command '{}' failed. Exit code: {}".format(cmd, proc.exitstatus))
...
def scp_file(dest_path, contents=None, kwargs=None, local_file=None):
    ...
    try:
        ...
        ssh_args = ['-oStrictHostKeyChecking=no', '-oServerAliveInterval={}'.format(kwargs.get('server_alive_interval', SERVER_ALIVE_INTERVAL)), '-oServerAliveCountMax={}'.format(kwargs.get('server_alive_count_max', SERVER_ALIVE_COUNT_MAX)), '-oUserKnownHostsFile=/dev/null', '-oControlPath=none']
        if local_file is not None:
            file_to_upload = local_file
            if os.path.isdir(local_file):
                ssh_args.append('-r')
        if 'key_filename' in kwargs:
            ssh_args.extend(['-oPasswordAuthentication=no', '-oChallengeResponseAuthentication=no', '-oPubkeyAuthentication=yes', '-oIdentitiesOnly=yes', '-oKbdInteractiveAuthentication=no', '-i {}'.format(kwargs['key_filename'])])
        if 'port' in kwargs:
        ...
        if file_to_upload is None:
            log.warning('No source file to upload. Please make sure that either file contents or the path to a local file are provided.')
        cmd = 'scp {0} {1} {2[username]}@{4}:{3} || echo "put {1} {3}" | sftp {0} {2[username]}@{4} || rsync -avz -e "ssh {0}" {1} {2[username]}@{2[hostname]}:{3}'.format(' '.join(ssh_args), file_to_upload, kwargs, dest_path, ipaddr)
        log.debug("SCP command: '%s'", cmd)
        retcode = _exec_ssh_cmd(cmd, error_msg="Failed to upload file '{0}': {1}\n{2}", password_retries=3, **kwargs)
    finally:
        if contents is not None:
            try:
...
def sftp_file(dest_path, contents=None, kwargs=None, local_file=None):
    ...
    try:
        ...
        ssh_args = ['-oStrictHostKeyChecking=no', '-oServerAliveInterval={}'.format(kwargs.get('server_alive_interval', SERVER_ALIVE_INTERVAL)), '-oServerAliveCountMax={}'.format(kwargs.get('server_alive_count_max', SERVER_ALIVE_COUNT_MAX)), '-oUserKnownHostsFile=/dev/null', '-oControlPath=none']
        if 'key_filename' in kwargs:
            ssh_args.extend(['-oPasswordAuthentication=no', '-oChallengeResponseAuthentication=no', '-oPubkeyAuthentication=yes', '-oIdentitiesOnly=yes', '-oKbdInteractiveAuthentication=no', '-oIdentityFile={}'.format(kwargs['key_filename'])])
        if 'port' in kwargs:
            ssh_args.append('-oPort={}'.format(kwargs['port']))
        ssh_args.append(__ssh_gateway_arguments(kwargs))
        try:
        ...
        if file_to_upload is None:
            log.warning('No source file to upload. Please make sure that either file contents or the path to a local file are provided.')
        cmd = 'echo "put {0} {1} {2}" | sftp {3} {4[username]}@{5}'.format(' '.join(put_args), file_to_upload, dest_path, ' '.join(ssh_args), kwargs, ipaddr)
...
def root_cmd(command, tty, sudo, allow_failure=False, **kwargs):
    ...
    ssh_args.extend(['-oStrictHostKeyChecking={}'.format(host_key_checking), '-oUserKnownHostsFile={}'.format(known_hosts_file), '-oControlPath=none'])
    if 'key_filename' in kwargs:
        ssh_args.extend(['-oPasswordAuthentication=no', '-oChallengeResponseAuthentication=no', '-oPubkeyAuthentication=yes', '-oIdentitiesOnly=yes', '-oKbdInteractiveAuthentication=no', '-i {}'.format(kwargs['key_filename'])])
...
def check_name(name, safe_chars):
    ...
    if regexp.search(name):
        raise SaltCloudException('{} contains characters not supported by this cloud provider. Valid characters are: {}'.format(name, safe_chars))
...
def remove_sshkey(host, known_hosts=None):
    ...
    if known_hosts is None:
        if 'HOME' in os.environ:
            ...
            try:
                log.info('Trace')
                known_hosts = '{}/.ssh/known_hosts'.format(pwd.getpwuid(os.getuid()).pwd_dir)
            except Exception:
                log.info('Trace')
                pass
...
def wait_for_ip(update_callback, update_args=None, update_kwargs=None, timeout=5 * 60, interval=5, interval_multiplier=1, max_failures=10):
    """
    ...
    duration = timeout
    while True:
        log.debug('Waiting for VM IP. Giving up in 00:%02d:%02d.', int(timeout // 60), int(timeout % 60))
        data = update_callback(*update_args, **update_kwargs)
        if data is False:
            log.debug("'update_callback' has returned 'False', which is considered a failure. Remaining Failures: %s.", max_failures)
            max_failures -= 1
            if max_failures <= 0:
                raise SaltCloudExecutionFailure('Too many failures occurred while waiting for the IP address.')
        elif data is not None:
            return data
        if timeout < 0:
            raise SaltCloudExecutionTimeout('Unable to get IP for 00:{:02d}:{:02d}.'.format(int(duration // 60), int(duration % 60)))
        time.sleep(interval)
        timeout -= interval
...
def cachedir_index_add(minion_id, profile, driver, provider, base=None):
    ...
    index.update({minion_id: {'id': minion_id, 'profile': profile, 'driver': driver, 'provider': prov_comps[0]}})
...
def request_minion_cachedir(minion_id, opts=None, fingerprint='', pubkey=None, provider=None, base=None):
    """
    ...
    init_cachedir(base)
    data = {'minion_id': minion_id, 'fingerprint': fingerprint, 'provider': provider}
...
def change_minion_cachedir(minion_id, cachedir, data=None, base=None):
    """
    ...
    with salt.utils.files.fopen(path, 'r') as fh_:
        cache_data = salt.utils.data.decode(salt.utils.msgpack.load(fh_, encoding=MSGPACK_ENCODING))
        ...
        for prov in os.listdir(prov_dir):
            ...
            for fname in os.listdir(min_dir):
                ...
                with salt.utils.files.fopen(fpath, 'rb') as fh_:
                    minions[driver][prov][minion_id] = salt.utils.data.decode(salt.utils.msgpack.load(fh_, encoding=MSGPACK_ENCODING))
    return minions
...
def update_bootstrap(config, url=None):
    ...
    if url.startswith('http') or '://' in url:
        ...
        if req.status_code != 200:
            return {'error': 'Failed to download the latest stable version of the bootstrap-salt.sh script from {}. HTTP error: {}'.format(url, req.status_code)}
    ...
    for node in cached_nodes:
        if node not in node_list:
            ...
            if 'diff_cache_events' in opts and opts['diff_cache_events']:
                fire_event('event', 'cached node missing from provider', 'salt/cloud/{}/cache_node_missing'.format(node), args={'missing node': node}, sock_dir=opts.get('sock_dir', os.path.join(__opts__['sock_dir'], 'master')), transport=opts.get('transport', 'zeromq'))
...
def diff_node_cache(prov_dir, node, new_data, opts):
    ...
    if not os.path.exists(path):
        ...
        fire_event('event', 'new node found', 'salt/cloud/{}/cache_node_new'.format(node), args={'new_data': event_data}, sock_dir=opts.get('sock_dir', os.path.join(__opts__['sock_dir'], 'master')), transport=opts.get('transport', 'zeromq'))
        return
    with salt.utils.files.fopen(path, 'r') as fh_:
        try:
            log.info('Trace')
    ...
    if diff != 0:
        fire_event('event', 'node data differs', 'salt/cloud/{}/cache_node_diff'.format(node), args={'new_data': _strip_cache_events(new_data, opts), 'cache_data': _strip_cache_events(cache_data, opts)}, sock_dir=opts.get('sock_dir', os.path.join(__opts__['sock_dir'], 'master')), transport=opts.get('transport', 'zeromq'))
...
def run_func_until_ret_arg(fun, kwargs, fun_call=None, argument_being_watched=None, required_argument_response=None):
    ...
    while status != required_argument_response:
        ...
        log.debug('Function: %s, Watched arg: %s, Response: %s', str(fun).split(' ')[1], argument_being_watched, status)
        time.sleep(5)
    return True
...
def get_salt_interface(vm_, opts):
    ...
    or 'private_ips'.
    """
    salt_host = salt.config.get_cloud_config_value('salt_interface', vm_, opts, default=False, search_global=False)
    if salt_host is False:
        salt_host = salt.config.get_cloud_config_value('ssh_interface', vm_, opts, default='public_ips', search_global=False)
    return salt_host
...
def check_key_path_and_mode(provider, key_path):
    ...
    """
    if not os.path.exists(key_path):
        log.error("The key file '%s' used in the '%s' provider configuration does not exist.\n", key_path, provider)
        return False
    key_mode = stat.S_IMODE(os.stat(key_path).st_mode)
    if key_mode not in (256, 384):
        log.error("The key file '%s' used in the '%s' provider configuration needs to be set to mode 0400 or 0600.\n", key_path, provider)
        return False
    return True
...
def userdata_template(opts, vm_, userdata):
    ...
    if userdata is None:
        return userdata
    userdata_template = salt.config.get_cloud_config_value('userdata_template', vm_, opts, search_global=False, default=None)
    if userdata_template is False:
        return userdata
    renderer = opts.get('userdata_template') if userdata_template is None else userdata_template
    if renderer is None:
        return userdata
    else:
        ...
        templated = salt.template.compile_template(':string:', rend, renderer, blacklist, whitelist, input_data=userdata)
        if not isinstance(templated, str):
            try:
                templated = ''.join(templated.readlines())
            except AttributeError:
                log.warning('Templated userdata resulted in non-string result (%s), converting to string', templated)
