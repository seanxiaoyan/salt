...
class NestDisplay:
    ...
    def ustring(self, indent, color, msg, prefix='', suffix='', endc=None):
        ...
        try:
            ...
            try:
                return fmt.format(indent, color, prefix, salt.utils.stringutils.to_unicode(msg), endc, suffix)
            except UnicodeDecodeError:
    ...
    def display(self, ret, indent, prefix, out):
        ...
        if ret is None or ret is True or ret is False:
            ...
            out.append(self.ustring(indent, self.LIGHT_YELLOW, repr(ret), prefix=prefix))
        elif isinstance(ret, str):
            first_line = True
            for line in ret.splitlines():
                line_prefix = ' ' * len(prefix) if not first_line else prefix
                if isinstance(line, bytes):
                    out.append(self.ustring(indent, self.YELLOW, 'Not string data', prefix=line_prefix))
                    break
