...
from salt.exceptions import CommandExecutionError, CommandNotFoundError, SaltClientError, SaltDaemonNotRunning, SaltException, SaltInvocationError, SaltMasterUnresolvableError, SaltReqTimeoutError, SaltSystemExit
...
def resolve_dns(opts, fallback=True):
    ...
    if check_dns is True:
        try:
            ...
            ret['master_ip'] = salt.utils.network.dns_check(opts['master'], int(opts['master_port']), True, opts['ipv6'])
            ...
            if opts['retry_dns']:
                while True:
                    if retry_dns_count is not None:
                        ...
                        retry_dns_count -= 1
                    log.error("Master hostname: '%s' not found or not responsive. Retrying in %s seconds", opts['master'], opts['retry_dns'])
            ...
            if opts.get('__role') == 'syndic':
                err = "Master address: '{}' could not be resolved. Invalid or unresolveable address. Set 'syndic_master' value in minion config.".format(master)
            else:
                err = "Master address: '{}' could not be resolved. Invalid or unresolveable address. Set 'master' value in minion config.".format(master)
            log.error(err)
    ...
    if opts['source_interface_name']:
        ...
        ret['source_ip'] = salt.utils.network.dns_check(opts['source_address'], int(opts['source_ret_port']), True, opts['ipv6'])
        log.debug('Using %s as source IP address', ret['source_ip'])
...
def master_event(type, master=None):
    ...
    """
    event_map = {'connected': '__master_connected', 'disconnected': '__master_disconnected', 'failback': '__master_failback', 'alive': '__master_alive'}
...
class MinionBase:
    ...
    def gen_modules(self, initial_load=False, context=None):
        ...
        if context is None:
            context = {}
        if initial_load:
            self.opts['pillar'] = salt.pillar.get_pillar(self.opts, self.opts['grains'], self.opts['id'], self.opts['saltenv'], pillarenv=self.opts.get('pillarenv')).compile_pillar()
        ...
        self.states = salt.loader.states(self.opts, functions=self.functions, utils=self.utils, serializers=self.serializers, context=context)
        self.rend = salt.loader.render(self.opts, functions=self.functions, context=context)
        self.matchers = salt.loader.matchers(self.opts)
        self.functions['sys.reload_modules'] = self.gen_modules
        self.executors = salt.loader.executors(self.opts, functions=self.functions, proxy=self.proxy, context=context)
        ...
        if opts['master_type'] != 'str' and opts['__role'] != 'syndic':
            if opts['master_type'] == 'func':
                ...
                if isinstance(opts['master'], list):
                    log.info('Got list of available master addresses: %s', opts['master'])
                    ...
                    if opts['master_shuffle']:
                        log.warning("Use of 'master_shuffle' detected. 'master_shuffle' is deprecated in favor of 'random_master'. Please update your minion config file.")
                    ...
                    if failback:
                        ...
                        log.info('Moving possibly failed master %s to the end of the list of masters', opts['master'])
                    ...
                    log.error("master_type set to 'failover' but 'master' is not of type list but of type %s", type(opts['master']))
                    sys.exit(salt.defaults.exitcodes.EX_GENERIC)
                if opts['retry_dns'] and opts['master_type'] == 'failover':
                    log.critical("'master_type' set to 'failover' but 'retry_dns' is not 0. Setting 'retry_dns' to 0 to failover to the next master on DNS errors.")
                    opts['retry_dns'] = 0
            else:
                log.error("Invalid keyword '%s' for variable 'master_type'", opts['master_type'])
                sys.exit(salt.defaults.exitcodes.EX_GENERIC)
        ...
        if isinstance(opts['master'], list):
            ...
            for master in opts['local_masters']:
                ...
                if opts['master_type'] == 'failover':
                    try:
                        opts['master_uri_list'].append(resolve_dns(opts, False)['master_uri'])
                    except SaltClientError:
                        continue
                else:
            ...
            while True:
                ...
                if tries > 0:
                    ...
                    log.debug('Connecting to master. Attempt %s (infinite attempts)', attempts)
                for master in opts['local_masters']:
                    opts['master'] = master
                    opts.update(prep_ip_port(opts))
                    if opts['master_type'] == 'failover':
                    ...
                    self.opts = opts
                    pub_channel = salt.channel.client.AsyncPubChannel.factory(opts, **factory_kwargs)
                    try:
                        ...
                        if exc.strerror.startswith('Could not access'):
                            log.info('Failed to initiate connection with Master %s: check ownership/permissions. Error message: %s', opts['master'], exc)
                        else:
                            log.info('Master %s could not be reached, trying next master (if any)', opts['master'])
                        pub_channel.close()
                        pub_channel = None
                        continue
                if not conn:
                    if attempts == tries:
                        ...
                        log.error("No master could be reached or all masters denied the minion's connection attempt.")
                        if pub_channel:
                            pub_channel.close()
                        raise last_exc
                else:
        ...
        if self.opts['master'] == DEFAULT_MINION_OPTS['master'] and self.opts['discovery'] is not False:
            ...
            if masters:
                ...
                if policy not in ['any', 'all']:
                    log.error('SSDP configuration matcher failure: unknown value "%s". Should be "any" or "all"', policy)
                else:
    ...
    def _return_retry_timer(self):
        ...
        if self.opts.get('return_retry_timer_max'):
            try:
                random_retry = random.randint(self.opts['return_retry_timer'], self.opts['return_retry_timer_max'])
                retry_msg = msg % random_retry
                log.debug('%s (randomized)', msg % random_retry)
                return random_retry
            except ValueError:
                log.error('Invalid value (return_retry_timer: %s or return_retry_timer_max: %s). Both must be positive integers.', self.opts['return_retry_timer'], self.opts['return_retry_timer_max'])
...
class SMinion(MinionBase):
    ...
    def __init__(self, opts, context=None):
        ...
        super().__init__(opts)
        if self.opts.get('file_client', 'remote') == 'remote' or self.opts.get('use_master_when_local', False):
...
class MasterMinion:
    ...
    def __init__(self, opts, returners=True, states=True, rend=True, matcher=True, whitelist=None, ignore_config_errors=True):
        self.opts = salt.config.mminion_config(opts['conf_file'], opts, ignore_config_errors=ignore_config_errors)
...
class MinionManager(MinionBase):
    ...
    def _bind(self):
        self.event_publisher = salt.utils.event.AsyncEventPublisher(self.opts, io_loop=self.io_loop)
    ...
    def _create_minion_object(self, opts, timeout, safe, io_loop=None, loaded_base_name=None, jid_queue=None, load_grains=True):
        """
        Helper function to return the correct type of object
        """
        return Minion(opts, timeout, safe, io_loop=io_loop, loaded_base_name=loaded_base_name, jid_queue=jid_queue, load_grains=load_grains)
    ...
    def _spawn_minions(self, timeout=60):
        ...
        for master in masters:
            ...
            minion = self._create_minion_object(s_opts, s_opts['auth_timeout'], False, io_loop=self.io_loop, loaded_base_name='salt.loader.{}'.format(s_opts['master']), jid_queue=self.jid_queue)
    ...
    def _connect_minion(self, minion):
        ...
        while True:
            try:
                ...
                failed = True
                log.error('Error while bringing up minion for multi-master. Is master at %s responding?', minion.opts['master'])
                last = time.time()
                if auth_wait < self.max_auth_wait:
                    auth_wait += self.auth_wait
                yield salt.ext.tornado.gen.sleep(auth_wait)
            except SaltMasterUnresolvableError:
                err = "Master address: '{}' could not be resolved. Invalid or unresolveable address. Set 'master' value in minion config.".format(minion.opts['master'])
                log.error(err)
                break
            except Exception as e:
                failed = True
                log.critical('Unexpected error while connecting to %s', minion.opts['master'], exc_info=True)
...
class Minion(MinionBase):
    ...
    def __init__(self, opts, timeout=60, safe=True, loaded_base_name=None, io_loop=None, jid_queue=None, load_grains=True):
        ...
        if zmq:
            if ZMQ_VERSION_INFO < (3, 2):
                log.warning('You have a version of ZMQ less than ZMQ 3.2! There are known connection keep-alive issues with ZMQ < 3.2 which may result in loss of contact with minions. Please upgrade your ZMQ!')
        if not salt.utils.platform.is_proxy():
            if load_grains:
                self.opts['grains'] = salt.loader.grains(opts)
        else:
            if self.opts.get('beacons_before_connect', False):
                log.warning("'beacons_before_connect' is not supported for proxy minions. Setting to False")
                self.opts['beacons_before_connect'] = False
            if self.opts.get('scheduler_before_connect', False):
                log.warning("'scheduler_before_connect' is not supported for proxy minions. Setting to False")
        ...
        if self.opts['random_startup_delay']:
            ...
            log.info('Minion sleeping for %s seconds due to configured startup_delay between 0 and %s seconds', sleep_time, self.opts['random_startup_delay'])
    ...
    def connect_master(self, failed=False):
        ...
        self.req_channel = salt.transport.client.AsyncReqChannel.factory(self.opts, io_loop=self.io_loop)
        if hasattr(self.req_channel, 'connect'):
    ...
    def _post_master_init(self, master):
        ...
        if not hasattr(self, 'schedule'):
            self.schedule = salt.utils.schedule.Schedule(self.opts, self.functions, self.returners, cleanup=[master_event(type='alive')])
        if self.opts['mine_enabled'] and 'mine.update' in self.functions:
            self.schedule.add_job({'__mine_interval': {'function': 'mine.update', 'minutes': self.opts['mine_interval'], 'jid_include': True, 'maxrunning': 2, 'run_on_start': True, 'return_job': self.opts.get('mine_return_job', False)}}, persist=True)
            log.info('Added mine.update to scheduler')
        else:
            self.schedule.delete_job('__mine_interval', persist=True)
        if self.opts['transport'] != 'tcp' and self.opts['master_alive_interval'] > 0 and self.connected:
            self.schedule.add_job({master_event(type='alive', master=self.opts['master']): {'function': 'status.master', 'seconds': self.opts['master_alive_interval'], 'jid_include': True, 'maxrunning': 1, 'return_job': False, 'kwargs': {'master': self.opts['master'], 'connected': True}}}, persist=True)
            if self.opts['master_failback'] and 'master_list' in self.opts and (self.opts['master'] != self.opts['master_list'][0]):
                self.schedule.add_job({master_event(type='failback'): {'function': 'status.ping_master', 'seconds': self.opts['master_failback_interval'], 'jid_include': True, 'maxrunning': 1, 'return_job': False, 'kwargs': {'master': self.opts['master_list'][0]}}}, persist=True)
            else:
    ...
    def _load_modules(self, force_refresh=False, notify=False, grains=None, opts=None, context=None):
        """
        ...
        if opts.get('modules_max_memory', -1) > 0 and HAS_PSUTIL and HAS_RESOURCE:
            log.debug('modules_max_memory set, enforcing a maximum of %s', opts['modules_max_memory'])
        ...
        if context is None:
            context = {}
        if grains is None:
            opts['grains'] = salt.loader.grains(opts, force_refresh, proxy=proxy, context=context)
        self.utils = salt.loader.utils(opts, proxy=proxy, context=context)
        if opts.get('multimaster', False):
            s_opts = copy.deepcopy(opts)
            functions = salt.loader.minion_mods(s_opts, utils=self.utils, proxy=proxy, loaded_base_name=self.loaded_base_name, notify=notify, context=context)
        else:
            functions = salt.loader.minion_mods(opts, utils=self.utils, notify=notify, proxy=proxy, context=context)
    ...
    def _send_req_sync(self, load, timeout):
        if self.opts['minion_sign_messages']:
            ...
            load['sig'] = sig
        with salt.utils.event.get_event('minion', opts=self.opts, listen=False) as event:
            return event.fire_event(load, '__master_req_channel_payload', timeout=timeout)
    ...
    def _send_req_async(self, load, timeout):
        if self.opts['minion_sign_messages']:
            ...
            load['sig'] = sig
        with salt.utils.event.get_event('minion', opts=self.opts, listen=False) as event:
            ret = (yield event.fire_event_async(load, '__master_req_channel_payload', timeout=timeout))
    ...
    def _fire_master(self, data=None, tag=None, events=None, pretag=None, timeout=60, sync=True, timeout_handler=None, include_startup_grains=False):
        ...
        """
        load = {'id': self.opts['id'], 'cmd': '_minion_event', 'pretag': pretag, 'tok': self.tok}
        if events:
            load['events'] = events
        elif data and tag:
            load['data'] = data
        ...
        if include_startup_grains:
            grains_to_add = {k: v for (k, v) in self.opts.get('grains', {}).items() if k in self.opts['start_event_grains']}
    ...
    def _handle_decoded_payload(self, data):
        ...
        if self.opts.get('grains_refresh_pre_exec'):
            if hasattr(self, 'proxy'):
                ...
                proxy = None
            self.opts['grains'] = salt.loader.grains(self.opts, force_refresh=True, proxy=proxy)
        ...
        if multiprocessing_enabled:
            ...
            with default_signals(signal.SIGINT, signal.SIGTERM):
                process = SignalHandlingProcess(target=self._target, name=name, args=(instance, self.opts, data, self.connected))
                process.register_after_fork_method(salt.utils.crypt.reinit_crypto)
        else:
            process = threading.Thread(target=self._target, args=(instance, self.opts, data, self.connected), name=name)
        if multiprocessing_enabled:
            with default_signals(signal.SIGINT, signal.SIGTERM):
                process.start()
        else:
    ...
    def _target(cls, minion_instance, opts, data, connected):
        if not minion_instance:
            ...
            if not hasattr(minion_instance, 'functions'):
                (functions, returners, function_errors, executors) = minion_instance._load_modules(grains=opts['grains'])
        ...
        with salt.ext.tornado.stack_context.StackContext(functools.partial(RequestContext, {'data': data, 'opts': opts})):
    ...
    def _thread_return(cls, minion_instance, opts, data):
        ...
        allow_missing_funcs = any([minion_instance.executors['{}.allow_missing_func'.format(executor)](function_name) for executor in executors if '{}.allow_missing_func'.format(executor) in minion_instance.executors])
        if function_name in minion_instance.functions or allow_missing_funcs is True:
            try:
                return_data = minion_instance._execute_job_function(function_name, function_args, executors, opts, data)
                ...
                msg = 'Passed invalid arguments to {}: {}\n{}'.format(function_name, exc, minion_instance.functions[function_name].__doc__ or '')
    ...
    def _return_pub(self, ret, ret_cmd='_return', timeout=60, sync=True):
        ...
        if ret_cmd == '_syndic_return':
            load = {'cmd': ret_cmd, 'id': self.opts['uid'], 'jid': jid, 'fun': fun, 'arg': ret.get('arg'), 'tgt': ret.get('tgt'), 'tgt_type': ret.get('tgt_type'), 'load': ret.get('__load__')}
        ...
        def timeout_handler(*_):
            log.warning('The minion failed to return the job information for job %s. This is often due to the master being shut down or overloaded. If the master is running, consider increasing the worker_threads value.', jid)
            return True
        if sync:
            try:
    ...
    def _return_pub_multi(self, rets, ret_cmd='_return', timeout=60, sync=True):
        ...
        for ret in rets:
            ...
            if ret_cmd == '_syndic_return':
                if not load:
                    load.update({'id': self.opts['id'], 'jid': jid, 'fun': fun, 'arg': ret.get('arg'), 'tgt': ret.get('tgt'), 'tgt_type': ret.get('tgt_type'), 'load': ret.get('__load__'), 'return': {}})
                if '__master_id__' in ret:
                    load['master_id'] = ret['__master_id__']
                for (key, value) in ret.items():
                    if key.startswith('__'):
    ...
    def _fire_master_minion_start(self):
        ...
        if self.opts['enable_legacy_startup_events']:
            self._fire_master('Minion {} started at {}'.format(self.opts['id'], time.asctime()), 'minion_start', include_startup_grains=include_grains)
        self._fire_master('Minion {} started at {}'.format(self.opts['id'], time.asctime()), tagify([self.opts['id'], 'start'], 'minion'), include_startup_grains=include_grains)
    ...
    def pillar_schedule_refresh(self, current, new):
        ...
        for item in current:
            ...
            if item in new:
                diff = salt.utils.dictdiffer.deep_diff(schedule_item, new[item], ignore=ignore)
                if diff.get('new'):
    ...
    def pillar_refresh(self, force_refresh=False, clean_cache=False):
        ...
        if self.connected:
            ...
            async_pillar = salt.pillar.get_async_pillar(self.opts, self.opts['grains'], self.opts['id'], self.opts['saltenv'], pillarenv=self.opts.get('pillarenv'), clean_cache=clean_cache)
            try:
                ...
                new_pillar['schedule'] = self.pillar_schedule_refresh(current_schedule, new_schedule)
        ...
        with salt.utils.event.get_event('minion', opts=self.opts, listen=False) as evt:
            evt.fire_event({'complete': True}, tag=salt.defaults.events.MINION_PILLAR_REFRESH_COMPLETE)
    ...
    def manage_schedule(self, tag, data):
        ...
        funcs = {'delete': ('delete_job', (name, persist)), 'add': ('add_job', (schedule, persist)), 'modify': ('modify_job', (name, schedule, persist)), 'enable': ('enable_schedule', (persist,)), 'disable': ('disable_schedule', (persist,)), 'enable_job': ('enable_job', (name, persist)), 'run_job': ('run_job', (name,)), 'disable_job': ('disable_job', (name, persist)), 'postpone_job': ('postpone_job', (name, data)), 'skip_job': ('skip_job', (name, data)), 'reload': ('reload', (schedule,)), 'list': ('list', (where,)), 'save_schedule': ('save_schedule', ()), 'get_next_fire_time': ('get_next_fire_time', (name,)), 'job_status': ('job_status', (name, fire_event))}
        try:
    ...
    def manage_beacons(self, tag, data):
        ...
        funcs = {'add': ('add_beacon', {'name': name, 'beacon_data': beacon_data}), 'modify': ('modify_beacon', {'name': name, 'beacon_data': beacon_data}), 'delete': ('delete_beacon', {'name': name}), 'enable': ('enable_beacons', {}), 'disable': ('disable_beacons', {}), 'enable_beacon': ('enable_beacon', {'name': name}), 'disable_beacon': ('disable_beacon', {'name': name}), 'list': ('list_beacons', {'include_opts': include_opts, 'include_pillar': include_pillar}), 'list_available': ('list_available_beacons', {}), 'validate_beacon': ('validate_beacon', {'name': name, 'beacon_data': beacon_data}), 'reset': ('reset', {})}
        try:
            (alias, params) = funcs.get(func)
    ...
    def _pre_tune(self):
        ...
        if self._running is None:
            ...
            log.error('This %s is already running. Not running %s.tune_in()', self.__class__.__name__, self.__class__.__name__)
            return
        try:
            log.info("%s is starting as user '%s'", self.__class__.__name__, salt.utils.user.get_user())
        except Exception as err:
            log.log(salt.utils.platform.is_windows() and logging.DEBUG or logging.ERROR, 'Failed to get the user who is starting %s', self.__class__.__name__, exc_info=err)
    ...
    def _mine_send(self, tag, data):
        ...
        with salt.channel.client.ReqChannel.factory(self.opts) as channel:
            data['tok'] = self.tok
            try:
                ret = channel.send(data, timeout=self._return_retry_timer(), tries=self.opts['return_retry_tries'])
                return ret
            except SaltReqTimeoutError:
    ...
    def handle_event(self, package):
        ...
        if tag.startswith('module_refresh'):
            _minion.module_refresh(force_refresh=data.get('force_refresh', False), notify=data.get('notify', False))
        elif tag.startswith('__master_req_channel_payload'):
            yield self.req_channel.send(data, timeout=self._return_retry_timer(), tries=self.opts['return_retry_tries'])
        elif tag.startswith('pillar_refresh'):
            yield _minion.pillar_refresh(force_refresh=data.get('force_refresh', False), clean_cache=data.get('clean_cache', False))
            ...
            if data.get('force_refresh', False) or _minion.grains_cache != _minion.opts['grains']:
            ...
            if tag.startswith(master_event(type='failback')):
                if data['master'] != self.opts['master_list'][0]:
                    raise SaltException("Bad master '{}' when mine failback is '{}'".format(data['master'], self.opts['master']))
                elif data['master'] == self.opts['master'][0]:
                    raise SaltException("Already connected to '{}'".format(data['master']))
            if self.connected:
                self.connected = False
                log.info('Connection to master %s lost', self.opts['master'])
                if self.opts['master_type'] != 'failover':
                    if self.opts['transport'] != 'tcp':
                        schedule = {'function': 'status.master', 'seconds': self.opts['master_alive_interval'], 'jid_include': True, 'maxrunning': 1, 'return_job': False, 'kwargs': {'master': self.opts['master'], 'connected': False}}
                        self.schedule.modify_job(name=master_event(type='alive', master=self.opts['master']), schedule=schedule)
                else:
                    if self.opts['transport'] != 'tcp':
                    ...
                    try:
                        (master, self.pub_channel) = (yield self.eval_master(opts=self.opts, failed=True, failback=tag.startswith(master_event(type='failback'))))
                    except SaltClientError:
                        pass
                    if self.connected:
                        self.opts['master'] = master
                        log.info('Re-initialising subsystems for new master %s', self.opts['master'])
                        self.req_channel = salt.transport.client.AsyncReqChannel.factory(self.opts, io_loop=self.io_loop)
                        ...
                        if self.opts['transport'] != 'tcp':
                            schedule = {'function': 'status.master', 'seconds': self.opts['master_alive_interval'], 'jid_include': True, 'maxrunning': 1, 'return_job': False, 'kwargs': {'master': self.opts['master'], 'connected': True}}
                            self.schedule.modify_job(name=master_event(type='alive', master=self.opts['master']), schedule=schedule)
                            if self.opts['master_failback'] and 'master_list' in self.opts:
                                if self.opts['master'] != self.opts['master_list'][0]:
                                    schedule = {'function': 'status.ping_master', 'seconds': self.opts['master_failback_interval'], 'jid_include': True, 'maxrunning': 1, 'return_job': False, 'kwargs': {'master': self.opts['master_list'][0]}}
                                    self.schedule.modify_job(name=master_event(type='failback'), schedule=schedule)
                                else:
            ...
            key = tuple(data['key'])
            log.debug('Updating auth data for %s: %s -> %s', key, salt.crypt.AsyncAuth.creds_map.get(key), data['creds'])
    ...
    def setup_scheduler(self, before_connect=False):
        ...
        if 'schedule' not in self.periodic_callbacks:
            if 'schedule' not in self.opts:
                self.opts['schedule'] = {}
            if not hasattr(self, 'schedule'):
                self.schedule = salt.utils.schedule.Schedule(self.opts, self.functions, self.returners, utils=self.utils, cleanup=[master_event(type='alive')])
            try:
                ...
                log.error('Exception occurred in attempt to initialize grain refresh routine during minion tune-in: %s', exc)
    ...
    def add_periodic_callback(self, name, method, interval=1):
        ...
        if name in self.periodic_callbacks:
            return False
        self.periodic_callbacks[name] = salt.ext.tornado.ioloop.PeriodicCallback(method, interval * 1000)
                    ...
                    def ping_timeout_handler(*_):
                        if self.opts.get('auth_safemode', False):
                            ...
                            try:
                                ...
                                log.warning('ping_interval reached without response from the master, but service.restart could not be run to restart the minion daemon. ping_interval requires that the minion is running under an init system.')
    ...
    def _target_load(self, load):
        ...
        if 'tgt_type' in load:
            ...
            if load['tgt_type'] in ('grain', 'grain_pcre', 'pillar'):
                ...
                return False
        elif not self.matchers['glob_match.match'](load['tgt']):
            return False
        return True
...
class Syndic(Minion):
    ...
    def fire_master_syndic_start(self):
        if self.opts['enable_legacy_startup_events']:
            self._fire_master('Syndic {} started at {}'.format(self.opts['id'], time.asctime()), 'syndic_start', sync=False)
        self._fire_master('Syndic {} started at {}'.format(self.opts['id'], time.asctime()), tagify([self.opts['id'], 'start'], 'syndic'), sync=False)
...
class SyndicManager(MinionBase):
    ...
    def _mark_master_dead(self, master):
        ...
        if self._syndics[master].done():
            ...
            log.info('Attempting to mark %s as dead, although it is already marked dead', master)
        ...
        for (master, syndic_future) in self.iter_master_options(master_id):
            ...
            if future is not None:
                if not future.done():
                    ...
                    continue
            future = getattr(syndic_future.result(), func)(values, '_syndic_return', timeout=self._return_retry_timer(), sync=False)
    ...
    def tune_in(self):
        ...
        future = self.local.event.set_event_handler(self._process_event)
        self.io_loop.add_future(future, self.reconnect_event_bus)
        self.forward_events = salt.ext.tornado.ioloop.PeriodicCallback(self._forward_events, self.opts['syndic_event_forward_timeout'] * 1000)
    ...
    def _process_event(self, raw):
        ...
        log.trace('Got event %s', mtag)
        tag_parts = mtag.split('/')
        if len(tag_parts) >= 4 and tag_parts[1] == 'job' and salt.utils.jid.is_jid(tag_parts[2]) and (tag_parts[3] == 'ret') and ('return' in data):
            if 'jid' not in data:
                return
    ...
    def _forward_events(self):
        ...
        if self.raw_events:
            ...
            self.raw_events = []
            self._call_syndic('_fire_master', kwargs={'events': events, 'pretag': tagify(self.opts['id'], base='syndic'), 'timeout': self._return_retry_timer(), 'sync': False})
        if self.delayed:
...
class ProxyMinionManager(MinionManager):
    ...
    def _create_minion_object(self, opts, timeout, safe, io_loop=None, loaded_base_name=None, jid_queue=None, load_grains=True):
        ...
        Helper function to return the correct type of object
        """
        return ProxyMinion(opts, timeout, safe, io_loop=io_loop, loaded_base_name=loaded_base_name, jid_queue=jid_queue, load_grains=True)
...
def _metaproxy_call(opts, fn_name):
    ...
    try:
        ...
        log.debug('No metaproxy key found in opts for id %s. Defaulting to standard proxy minion', opts['id'])
...
class SProxyMinion(SMinion):
    ...
    def gen_modules(self, initial_load=False, context=None):
        ...
        self.opts['pillar'] = salt.pillar.get_pillar(self.opts, self.opts['grains'], self.opts['id'], saltenv=self.opts['saltenv'], pillarenv=self.opts.get('pillarenv')).compile_pillar()
        if 'proxy' not in self.opts['pillar'] and 'proxy' not in self.opts:
            errmsg = 'No "proxy" configuration key found in pillar or opts dictionaries for id {id}. Check your pillar/options configuration and contents. Salt-proxy aborted.'.format(id=self.opts['id'])
            log.error(errmsg)
            self._running = False
        ...
        self.functions = salt.loader.minion_mods(self.opts, utils=self.utils, notify=False, proxy=self.proxy, context=context)
        self.returners = salt.loader.returners(self.opts, functions=self.functions, proxy=self.proxy, context=context)
        self.matchers = salt.loader.matchers(self.opts)
        self.functions['sys.reload_modules'] = self.gen_modules
        self.executors = salt.loader.executors(self.opts, functions=self.functions, proxy=self.proxy, context=context)
        ...
        if '{}.init'.format(fq_proxyname) not in self.proxy or '{}.shutdown'.format(fq_proxyname) not in self.proxy:
            errmsg = 'Proxymodule {} is missing an init() or a shutdown() or both. '.format(fq_proxyname) + 'Check your proxymodule.  Salt-proxy aborted.'
            log.error(errmsg)
            self._running = False
            raise SaltSystemExit(code=salt.defaults.exitcodes.EX_GENERIC, msg=errmsg)
        self.module_executors = self.proxy.get('{}.module_executors'.format(fq_proxyname), lambda : [])()
