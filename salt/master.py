...
class SMaster:
    ...
    def __getstate__(self):
        ...
        state.update({'key': self.key, 'master_key': self.master_key, 'secrets': SMaster.secrets})
        return state
...
class FileserverUpdate(salt.utils.process.SignalHandlingProcess):
            ...
            if backend in update_intervals:
                for (id_, interval) in update_intervals[backend].items():
                    if not interval:
                        ...
                        log.debug('An update_interval of 0 is not supported, falling back to %s', interval)
                ...
                try:
                    ...
                    interval = DEFAULT_INTERVAL
                    log.warning('%s key missing from configuration. Falling back to default interval of %d seconds', interval_key, interval)
                self.buckets.setdefault(interval, OrderedDict())[backend, update_func] = None
    ...
    def _do_update(backends):
        ...
        for (backend, update_args) in backends.items():
            ...
            try:
                if update_args:
                    log.debug('Updating %s fileserver cache for the following targets: %s', backend_name, update_args)
                    args = (update_args,)
                else:
                ...
                log.exception('Uncaught exception while updating %s fileserver cache', backend_name)
    ...
    def update(cls, interval, backends, timeout=300):
        ...
        while time.time() - start < timeout:
            log.debug('Performing fileserver updates for items with an update interval of %d', interval)
            cls._do_update(backends)
            log.debug('Completed fileserver updates for items with an update interval of %d, waiting %d seconds', interval, interval)
            with condition:
    ...
    def run(self):
        ...
        for interval in self.buckets:
            self.update_threads[interval] = threading.Thread(target=self.update, args=(interval, self.buckets[interval]))
...
class Master(SMaster):
    ...
    def __set_max_open_files(self):
        ...
        if mof_h == resource.RLIM_INFINITY:
            mof_h = mof_s
        log.info('Current values for max open files soft/hard setting: %s/%s', mof_s, mof_h)
        mof_c = self.opts['max_open_files']
        if mof_c > mof_h:
            log.info("The value for the 'max_open_files' setting, %s, is higher than the highest value the user running salt is allowed to set (%s). Defaulting to %s.", mof_c, mof_h, mof_h)
            mof_c = mof_h
        if mof_s < mof_c:
            ...
            try:
                (mof_s, mof_h) = resource.getrlimit(resource.RLIMIT_NOFILE)
                log.info('New values for max open files soft/hard values: %s/%s', mof_s, mof_h)
            except ValueError:
                log.critical('Failed to raise max open files setting to %s. If this value is too low, the salt-master will most likely fail to run properly.', mof_c)
    ...
    def _pre_flight(self):
        ...
        if self.opts.get('fileserver_verify_config', True):
            ...
            if not fileserver.servers:
                errors.append('Failed to load fileserver backends, the configured backends are: {}'.format(', '.join(self.opts['fileserver_backend'])))
            else:
                try:
                    log.info('Trace')
                    fileserver.init()
        ...
        if self.opts.get('git_pillar_verify_config', True):
            try:
                log.info('Trace')
                git_pillars = [x for x in self.opts.get('ext_pillar', []) if 'git' in x and (not isinstance(x['git'], str))]
            except TypeError:
                log.info('Trace')
                git_pillars = []
                critical_errors.append('Invalid ext_pillar configuration. It is likely that the external pillar type was not specified for one or more external pillars.')
            if git_pillars:
                try:
                    log.info('Trace')
                    ...
                    for repo in git_pillars:
                        ...
                        try:
                            log.info('Trace')
                            git_pillar = salt.utils.gitfs.GitPillar(new_opts, repo['git'], per_remote_overrides=salt.pillar.git_pillar.PER_REMOTE_OVERRIDES, per_remote_only=salt.pillar.git_pillar.PER_REMOTE_ONLY, global_only=salt.pillar.git_pillar.GLOBAL_ONLY)
                        except salt.exceptions.FileserverConfigError as exc:
                            log.info('Trace')
    ...
    def start(self):
        ...
        with salt.utils.process.default_signals(signal.SIGINT, signal.SIGTERM):
            SMaster.secrets['aes'] = {'secret': multiprocessing.Array(ctypes.c_char, salt.utils.stringutils.to_bytes(salt.crypt.Crypticle.generate_key_string())), 'serial': multiprocessing.Value(ctypes.c_longlong, lock=False), 'reload': salt.crypt.Crypticle.generate_key_string}
            ...
            self.process_manager.add_process(salt.utils.event.EventPublisher, args=(self.opts,), name='EventPublisher')
            ...
            self.process_manager.add_process(Maintenance, args=(self.opts,), kwargs={'master_secrets': SMaster.secrets}, name='Maintenance')
            if self.opts.get('event_return'):
                log.info('Creating master event return process')
                self.process_manager.add_process(salt.utils.event.EventReturn, args=(self.opts,), name='EventReturn')
            ...
            if self.opts['con_cache']:
                ...
                self.process_manager.add_process(salt.utils.master.ConnectedCache, args=(self.opts,), name='ConnectedCache')
            ...
            self.process_manager.add_process(ReqServer, args=(self.opts, self.key, self.master_key), kwargs=kwargs, name='ReqServer')
            self.process_manager.add_process(FileserverUpdate, args=(self.opts,), name='FileServerUpdate')
            if self.opts['discovery']:
                if salt.utils.ssdp.SSDPDiscoveryServer.is_available():
                    self.process_manager.add_process(salt.utils.ssdp.SSDPDiscoveryServer(port=self.opts['discovery']['port'], listen_ip=self.opts['interface'], answer={'mapping': self.opts['discovery'].get('mapping', {})}).run, name='SSDPDiscoveryServer')
                else:
    ...
    def __bind(self):
        ...
        if os.path.isfile(dfn):
            try:
                ...
                log.info('Trace')
                pass
        self.process_manager = salt.utils.process.ProcessManager(name='ReqServer_ProcessManager', wait_for_kill=1)
        req_channels = []
        ...
        with salt.utils.process.default_signals(signal.SIGINT, signal.SIGTERM):
            for ind in range(int(self.opts['worker_threads'])):
                ...
                self.process_manager.add_process(MWorker, args=(self.opts, self.master_key, self.key, req_channels), name=name)
        self.process_manager.run()
...
class MWorker(salt.utils.process.SignalHandlingProcess):
    ...
    def _handle_aes(self, data):
        ...
        with StackContext(functools.partial(RequestContext, {'data': data, 'opts': self.opts})):
            ret = run_func(data)
        if self.opts['master_stats']:
            self._post_stats(start, cmd)
        return ret
...
class AESFuncs(TransportMethods):
    ...
    """
    expose_methods = ('verify_minion', '_master_tops', '_master_opts', '_mine_get', '_mine', '_mine_delete', '_mine_flush', '_file_recv', '_pillar', '_minion_event', '_handle_minion_event', '_return', '_syndic_return', 'minion_runner', 'pub_ret', 'minion_pub', 'minion_publish', 'revoke_auth', '_serve_file', '_file_find', '_file_hash', '_file_hash_and_stat', '_file_list', '_file_list_emptydirs', '_dir_list', '_symlink_list', '_file_envs', '_ext_nodes')
    ...
    def __verify_minion(self, id_, token):
        ...
        try:
            ...
            log.warning('Salt minion claiming to be %s attempted to communicate with master, but key could not be read and verification was denied.', id_)
            return False
        ...
        log.error('Salt minion claiming to be %s has attempted to communicate with the master and could not be verified', id_)
        return False
    ...
    def __verify_minion_publish(self, clear_load):
        ...
        if ',' in clear_load['fun']:
            ...
            clear_load['arg'] = arg_
        return self.ckminions.auth_check(perms, clear_load['fun'], clear_load['arg'], clear_load['tgt'], clear_load.get('tgt_type', 'glob'), publish_validate=True)
    ...
    def __verify_load(self, load, verify_keys):
        ...
        if any((key not in load for key in verify_keys)):
            return False
        if 'tok' not in load:
            log.error("Received incomplete call from %s for '%s', missing '%s'", load['id'], inspect_stack()['co_name'], 'tok')
            return False
    ...
    def _file_recv(self, load):
        ...
        if len(load['data']) + load.get('loc', 0) > file_recv_max_size:
            log.error('file_recv_max_size limit of %d MB exceeded! %s will be truncated. To successfully push this file, adjust file_recv_max_size to an integer (in MB) large enough to accommodate it.', file_recv_max_size, load['path'])
            return False
        if 'tok' not in load:
            log.error("Received incomplete call from %s for '%s', missing '%s'", load['id'], inspect_stack()['co_name'], 'tok')
            return False
        ...
        if not os.path.normpath(cpath).startswith(self.opts['cachedir']):
            log.warning('Attempt to write received file outside of master cache directory! Requested path: %s. Access denied.', cpath)
            return False
    ...
    def _pillar(self, load):
        ...
        pillar = salt.pillar.get_pillar(self.opts, load['grains'], load['id'], load.get('saltenv', load.get('env')), ext=load.get('ext'), pillar_override=load.get('pillar_override', {}), pillarenv=load.get('pillarenv'), extra_minion_data=load.get('extra_minion_data'), clean_cache=load.get('clean_cache'))
        data = pillar.compile_pillar()
        self.fs_.update_opts()
        if self.opts.get('minion_data_cache', False):
            self.masterapi.cache.store('minions/{}'.format(load['id']), 'data', {'grains': load['grains'], 'pillar': data})
    ...
    def _handle_minion_event(self, load):
        ...
        if load.get('tag', '') == '_salt_error':
            log.error('Received minion error from [%s]: %s', id_, load['data']['message'])
        for event in load.get('events', []):
            ...
            if 'minions' in event_data:
                ...
                try:
                    ...
                    log.error('Could not add minion(s) %s for job %s: %s', minions, jid, exc)
    ...
    def _return(self, load):
        ...
        if 'sig' in load:
            ...
            sig = load.pop('sig')
            this_minion_pubkey = os.path.join(self.opts['pki_dir'], 'minions/{}'.format(load['id']))
            ...
            if not salt.crypt.verify_signature(this_minion_pubkey, serialized_load, sig):
                ...
                if self.opts['drop_messages_signature_fail']:
                    log.critical('drop_messages_signature_fail is enabled, dropping message from %s', load['id'])
                    return False
                else:
    ...
    def revoke_auth(self, load):
        ...
        if not self.opts.get('allow_minion_key_revoke', False):
            log.warning('Minion %s requested key revoke, but allow_minion_key_revoke is set to False', load['id'])
            return load
        if load is False:
            return load
        else:
    ...
    def run_func(self, func, load):
        ...
        if hasattr(self, func):
            try:
                ...
                log.trace('Master function call %s took %s seconds', func, time.time() - start)
            except Exception:
                ret = ''
                log.error('Error in function %s:\n', func, exc_info=True)
        else:
            log.error('Received function %s which is unavailable on the master, returning False', func)
...
class ClearFuncs(TransportMethods):
    ...
    """
    expose_methods = ('ping', 'publish', 'get_token', 'mk_token', 'wheel', 'runner')
    ...
    def runner(self, clear_load):
        ...
        if auth_type != 'user':
            runner_check = self.ckminions.runner_check(auth_check.get('auth_list', []), clear_load['fun'], clear_load.get('kwarg', {}))
            if not runner_check:
        ...
        try:
            ...
            return runner_client.asynchronous(fun, clear_load.get('kwarg', {}), username, local=True)
        except Exception as exc:
    ...
    def wheel(self, clear_load):
        ...
        username = auth_check.get('username')
        if auth_type != 'user':
            wheel_check = self.ckminions.wheel_check(auth_check.get('auth_list', []), clear_load['fun'], clear_load.get('kwarg', {}))
            if not wheel_check:
                return {'error': {'name': err_name, 'message': 'Authentication failure of type "{}" occurred for user {}.'.format(auth_type, username)}}
        ...
        try:
            ...
            data = {'fun': 'wheel.{}'.format(fun), 'jid': jid, 'tag': tag, 'user': username}
    ...
    def publish(self, clear_load):
        ...
        if publisher_acl.user_is_blacklisted(clear_load['user']) or publisher_acl.cmd_is_blacklisted(clear_load['fun']):
            log.error('%s does not have permissions to run %s. Please contact your local administrator if you believe this is in error.\n', clear_load['user'], clear_load['fun'])
            return {'error': {'name': 'AuthorizationError', 'message': 'Authorization error occurred.'}}
        delimiter = clear_load.get('kwargs', {}).get('delimiter', DEFAULT_TARGET_DELIM)
        _res = self.ckminions.check_minions(clear_load['tgt'], clear_load.get('tgt_type', 'glob'), delimiter)
        ...
        if auth_type != 'user' or (auth_type == 'user' and auth_list):
            authorized = self.ckminions.auth_check(auth_list, clear_load['fun'], clear_load['arg'], clear_load['tgt'], clear_load.get('tgt_type', 'glob'), minions=minions, whitelist=['saltutil.find_job'])
            if not authorized:
        ...
        if not self.opts.get('order_masters'):
            if not minions:
                return {'enc': 'clear', 'load': {'jid': None, 'minions': minions, 'error': 'Master could not resolve minions for target {}'.format(clear_load['tgt'])}}
        ...
        return {'enc': 'clear', 'load': {'jid': clear_load['jid'], 'minions': minions, 'missing': missing}}
    ...
    def _prep_jid(self, clear_load, extra):
        ...
        try:
            ...
            msg = "Failed to allocate a jid. The requested returner '{}' could not be loaded.".format(fstr.split('.')[0])
            log.error(msg)
            return {'error': msg}
        return jid
    ...
    def _prep_pub(self, minions, jid, clear_load, extra, missing):
        ...
        new_job_load = {'jid': clear_load['jid'], 'tgt_type': clear_load['tgt_type'], 'tgt': clear_load['tgt'], 'user': clear_load['user'], 'fun': clear_load['fun'], 'arg': clear_load['arg'], 'minions': minions, 'missing': missing}
        ...
        if self.opts['ext_job_cache']:
            ...
            try:
                ...
                if 'minions' not in arg_spec.args:
                    log.critical("The specified returner used for the external job cache '%s' does not have a 'minions' kwarg in the returner's save_load function.", self.opts['ext_job_cache'])
        ...
        try:
            ...
            log.critical('The specified returner threw a stack trace:\n', exc_info=True)
        payload = {'enc': 'aes'}
        load = {'fun': clear_load['fun'], 'arg': clear_load['arg'], 'tgt': clear_load['tgt'], 'jid': clear_load['jid'], 'ret': clear_load['ret']}
        ...
        if 'user' in clear_load:
            log.info('User %s Published command %s with jid %s', clear_load['user'], clear_load['fun'], clear_load['jid'])
            load['user'] = clear_load['user']
        else:
            log.info('Published command %s with jid %s', clear_load['fun'], clear_load['jid'])
