...
def present(name, params, **kwargs):
    ...
    if existing_obj:
        ...
        if diff_params:
            ...
            if dry_run:
                ...
                if action_update:
                    ...
                    ret['changes'] = {name: {'old': 'Zabbix Action "{}" differed in following parameters: {}'.format(name, diff_params), 'new': 'Zabbix Action "{}" fixed.'.format(name)}}
        else:
            ret['result'] = True
        ...
        ret['changes'] = {name: {'old': 'Zabbix Action "{}" does not exist.'.format(name), 'new': 'Zabbix Action "{}" would be created according definition.'.format(name)}}
    else:
        ...
        if action_create:
            ...
            ret['changes'] = {name: {'old': 'Zabbix Action "{}" did not exist.'.format(name), 'new': 'Zabbix Action "{}" created according definition.'.format(name)}}
    return ret
...
def absent(name, **kwargs):
    ...
    if not object_id:
        ...
        ret['changes'] = {name: {'old': 'Zabbix Action "{}" exists.'.format(name), 'new': 'Zabbix Action "{}" would be deleted.'.format(name)}}
    else:
        action_delete = __salt__['zabbix.run_query']('action.delete', [object_id], **kwargs)
        if action_delete:
            ret['result'] = True
            ret['comment'] = 'Zabbix Action "{}" deleted.'.format(name)
            ret['changes'] = {name: {'old': 'Zabbix Action "{}" existed.'.format(name), 'new': 'Zabbix Action "{}" deleted.'.format(name)}}
    return ret