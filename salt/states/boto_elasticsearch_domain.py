...
def present(name, DomainName, ElasticsearchClusterConfig=None, EBSOptions=None, AccessPolicies=None, SnapshotOptions=None, AdvancedOptions=None, Tags=None, region=None, key=None, keyid=None, profile=None, ElasticsearchVersion='1.5'):
    ...
    if _status.get('ElasticsearchVersion') != str(ElasticsearchVersion):
        ret['result'] = False
        ret['comment'] = 'Failed to update domain: version cannot be modified from {} to {}.'.format(_status.get('ElasticsearchVersion'), str(ElasticsearchVersion))
        return ret
...
def absent(name, DomainName, region=None, key=None, keyid=None, profile=None):
    ...
    """
    ret = {'name': DomainName, 'result': True, 'comment': '', 'changes': {}}
    r = __salt__['boto_elasticsearch_domain.exists'](DomainName, region=region, key=key, keyid=keyid, profile=profile)
    if 'error' in r:
        ret['result'] = False
    ...
    if __opts__['test']:
        ...
        ret['result'] = None
        return ret
    r = __salt__['boto_elasticsearch_domain.delete'](DomainName, region=region, key=key, keyid=keyid, profile=profile)
    if not r['deleted']:
        ret['result'] = False
        ret['comment'] = 'Failed to delete domain: {}.'.format(r['error']['message'])
        return ret
