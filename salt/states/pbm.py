...
def default_vsan_policy_configured(name, policy):
    ...
    try:
        ...
        if policy.get('subprofiles'):
            if len(policy['subprofiles']) > 1:
                raise ArgumentValueError('Multiple sub_profiles ({0}) are not supported in the input policy')
            subprofile = policy['subprofiles'][0]
            current_subprofile = current_policy['subprofiles'][0]
            capabilities_differ = list_diff(current_subprofile['capabilities'], subprofile.get('capabilities', []), key='id')
        ...
        if policy_differ.diffs or capabilities_differ.diffs or subprofile_differ.diffs:
            if 'name' in policy_differ.new_values or 'description' in policy_differ.new_values:
                raise ArgumentValueError("'name' and 'description' of the default VSAN policy cannot be updated")
            ...
            if __opts__['test']:
                ...
                if subprofile_differ.diffs or capabilities_differ.diffs:
                    ...
                    if subprofile_differ.diffs:
                        str_changes.extend(['  {}'.format(change) for change in subprofile_differ.changes_str.split('\n')])
                    if capabilities_differ.diffs:
                        str_changes.append('  capabilities:')
                        str_changes.extend(['  {}'.format(change) for change in capabilities_differ.changes_str2.split('\n')])
                comments.append("State {} will update the default VSAN policy on vCenter '{}':\n{}".format(name, vcenter, '\n'.join(str_changes)))
            else:
                __salt__['vsphere.update_storage_policy'](policy=current_policy['name'], policy_dict=policy_copy, service_instance=si)
                comments.append("Updated the default VSAN policy in vCenter '{}'".format(vcenter))
            log.info(comments[-1])
            new_values = policy_differ.new_values
            new_values['subprofiles'] = [subprofile_differ.new_values]
            new_values['subprofiles'][0]['capabilities'] = capabilities_differ.new_values
            ...
            old_values['subprofiles'][0]['capabilities'] = capabilities_differ.old_values
            ...
            changes.update({'default_vsan_policy': {'new': new_values, 'old': old_values}})
            log.trace(changes)
    ...
    if not changes_required:
        log.info('Trace')
        ret.update({'comment': "Default VSAN policy in vCenter '{}' is correctly configured. Nothing to be done.".format(vcenter), 'result': True})
    else:
        log.info('Trace')
...
def storage_policies_configured(name, policies):
    ...
    try:
        ...
        si = __salt__['vsphere.get_service_instance_via_proxy']()
        current_policies = __salt__['vsphere.list_storage_policies'](policy_names=[policy['name'] for policy in policies], service_instance=si)
        log.trace('current_policies = %s', current_policies)
        for policy in policies:
            policy_copy = copy.deepcopy(policy)
            filtered_policies = [p for p in current_policies if p['name'] == policy['name']]
            current_policy = filtered_policies[0] if filtered_policies else None
            if not current_policy:
                changes_required = True
                if __opts__['test']:
                    comments.append("State {} will create the storage policy '{}' on vCenter '{}'".format(name, policy['name'], vcenter))
                else:
                    __salt__['vsphere.create_storage_policy'](policy['name'], policy, service_instance=si)
                    comments.append("Created storage policy '{}' on vCenter '{}'".format(policy['name'], vcenter))
            ...
            if policy_differ.diffs or capabilities_differ.diffs or subprofile_differ.diffs:
                ...
                if __opts__['test']:
                    ...
                    __salt__['vsphere.update_storage_policy'](policy=current_policy['name'], policy_dict=policy_copy, service_instance=si)
                    comments.append("Updated the storage policy '{}' in vCenter '{}'".format(policy['name'], vcenter))
                log.info(comments[-1])
                new_values = policy_differ.new_values
                new_values['subprofiles'] = [subprofile_differ.new_values]
                new_values['subprofiles'][0]['capabilities'] = capabilities_differ.new_values
                ...
                comments.append("Storage policy '{}' is up to date. Nothing to be done.".format(policy['name']))
        __salt__['vsphere.disconnect'](si)
    except CommandExecutionError as exc:
        log.info('Trace')
        log.error('Error: %s', exc)
    ...
    if not changes_required:
        log.info('Trace')
        ret.update({'comment': "All storage policy in vCenter '{}' is correctly configured. Nothing to be done.".format(vcenter), 'result': True})
    else:
        log.info('Trace')
        ret.update({'comment': '\n'.join(comments), 'changes': {'storage_policies': changes}, 'result': None if __opts__['test'] else True})
    return ret
...
def default_storage_policy_assigned(name, policy, datastore):
    ...
    try:
        ...
        if existing_policy['name'] == policy:
            comment = "Storage policy '{}' is already assigned to datastore '{}'. Nothing to be done.".format(policy, datastore)
        else:
            changes_required = True
            changes = {'default_storage_policy': {'old': existing_policy['name'], 'new': policy}}
            if __opts__['test']:
                comment = "State {} will assign storage policy '{}' to datastore '{}'.".format(name, policy, datastore)
            else:
                __salt__['vsphere.assign_default_storage_policy_to_datastore'](policy=policy, datastore=datastore, service_instance=si)
                comment = "Storage policy '{} was assigned to datastore '{}'.".format(policy, name)
        log.info(comment)
    except CommandExecutionError as exc:
        log.info('Trace')
        log.error('Error: %s', exc)
