...
RESULT_CODES = ['Peer {0} added successfully.', 'Probe on localhost not needed', 'Host {0} is already in the peer group', 'Host {0} is already part of another cluster', 'Volume on {0} conflicts with existing volumes', 'UUID of {0} is the same as local uuid', '{0} responded with "unknown peer". This could happen if {0} doesn\'t have localhost defined', "Failed to add peer. Information on {0}'s logs", 'Cluster quorum is not met. Changing peers is not allowed.', 'Failed to update list of missed snapshots from {0}', 'Conflict comparing list of snapshots from {0}', 'Peer is already being detached from cluster.']
...
def peered(name):
    ...
    if not __salt__['glusterfs.peer'](name):
        ret['comment'] = 'Failed to peer with {}, please check logs for errors'.format(name)
        return ret
    ...
    if newpeers and any((name in v['hostnames'] for v in newpeers.values())):
        ...
        ret['comment'] = 'Host {} was successfully peered but did not appear in the list of peers'.format(name)
    return ret
...
def volume_present(name, bricks, stripe=False, replica=False, device_vg=False, transport='tcp', start=False, force=False, arbiter=False):
    ...
    if name not in volumes:
        if __opts__['test']:
            ...
            ret['result'] = None
            return ret
        vol_created = __salt__['glusterfs.create_volume'](name, bricks, stripe, replica, device_vg, transport, start, force, arbiter)
        if not vol_created:
...
def op_version(name, version):
    ...
    try:
        ...
        return ret
    if current == version:
        ret['comment'] = 'Glusterfs cluster.op-version for {} already set to {}'.format(name, version)
        ret['result'] = True
        return ret
    elif __opts__['test']:
        ret['comment'] = 'An attempt would be made to set the cluster.op-version for {} to {}.'.format(name, version)
        ret['result'] = None
        return ret
...
def max_op_version(name):
    ...
    if current == max_version:
        ret['comment'] = 'The cluster.op-version is already set to the cluster.max-op-version of {}'.format(current)
        ret['result'] = True
        return ret
    elif __opts__['test']:
        ret['comment'] = 'An attempt would be made to set the cluster.op-version to {}.'.format(max_version)
        ret['result'] = None
        return ret
