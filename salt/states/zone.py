...
def booted(name, single=False):
    ...
    if name in zones:
        ...
        ret['comment'] = []
        ret['comment'].append('The zone {} is not in the installed or booted state.'.format(name))
        for zone in zones:
    ...
    if name not in zones:
        if __opts__['test']:
            ...
            if not res_import['status']:
                ret['result'] = False
                ret['comment'] = 'Unable to import zone configuration for {}!'.format(name)
            else:
                ret['result'] = True
                ret['changes'][name] = 'imported'
                ret['comment'] = 'Zone {} was imported from {}.'.format(name, path)
            ...
            ret['result'] = False
            ret['comment'] = 'The file {} does not exists, unable to import!'.format(path)
    else:
        ret['result'] = True
...
def present(name, brand, zonepath, properties=None, resources=None):
    ...
    if __opts__['test']:
        ret['result'] = None
        ret['comment'].append('Cannot determine of changes would happen to the zone {}.'.format(name))
    if name not in zones:
        if __opts__['test']:
