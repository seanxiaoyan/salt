...
def _check_cron(user, cmd, minute=None, hour=None, daymonth=None, month=None, dayweek=None, comment=None, commented=None, identifier=None, special=None):
    """
    Return the changes
    """
    ...
    if special is None:
        ...
        for cron in lst['special']:
            if _cron_matched(cron, cmd, identifier):
                if any([_needs_change(x, y) for (x, y) in ((cron['spec'], special), (cron['identifier'], identifier), (cron['cmd'], cmd), (cron['comment'], comment), (cron['commented'], commented))]):
                    return 'update'
                return 'present'
    return 'absent'
...
def present(name, user='root', minute='*', hour='*', daymonth='*', month='*', dayweek='*', comment=None, commented=False, identifier=False, special=None):
    """
    ...
    if __opts__['test']:
        status = _check_cron(user, cmd=name, minute=minute, hour=hour, daymonth=daymonth, month=month, dayweek=dayweek, comment=comment, commented=commented, identifier=identifier, special=special)
        ret['result'] = None
        if status == 'absent':
        ...
        return ret
    if special is None:
        data = __salt__['cron.set_job'](user=user, minute=minute, hour=hour, daymonth=daymonth, month=month, dayweek=dayweek, cmd=name, comment=comment, commented=commented, identifier=identifier)
    else:
        data = __salt__['cron.set_special'](user=user, special=special, cmd=name, comment=comment, commented=commented, identifier=identifier)
    if data == 'present':
        ret['comment'] = 'Cron {} already present'.format(name)
        return ret
    if data == 'new':
...
def absent(name, user='root', identifier=False, special=None, **kwargs):
    ...
    if data == 'removed':
        ...
        ret['changes'] = {user: name}
        return ret
    ret['comment'] = 'Cron {} for user {} failed to commit with error {}'.format(name, user, data)
    ret['result'] = False
    return ret

def file(name, source_hash='', source_hash_name=None, user='root', template=None, context=None, replace=True, defaults=None, backup='', **kwargs):
    log.info('Trace')
    ...
    try:
        ...
        log.info('Trace')
        ret = {'changes': {}, 'comment': 'Could not identify group for user {}'.format(user), 'name': name, 'result': False}
        return ret
    ...
    if not replace and os.stat(cron_path).st_size > 0:
        ...
        return ret
    if __opts__['test']:
        fcm = __salt__['file.check_managed'](name=cron_path, source=source, source_hash=source_hash, source_hash_name=source_hash_name, user=user, group=group, mode=mode, attrs=[], template=template, context=context, defaults=defaults, saltenv=__env__, **kwargs)
    ...
    try:
        log.info('Trace')
        (sfn, source_sum, comment) = __salt__['file.get_managed'](name=cron_path, template=template, source=source, source_hash=source_hash, source_hash_name=source_hash_name, user=user, group=group, mode=mode, attrs=[], saltenv=__env__, context=context, defaults=defaults, skip_verify=False, **kwargs)
    ...
    try:
        log.info('Trace')
        ret = __salt__['file.manage_file'](name=cron_path, sfn=sfn, ret=ret, source=source, source_sum=source_sum, user=user, group=group, mode=mode, attrs=[], saltenv=__env__, backup=backup)
    except Exception as exc:
        log.info('Trace')
        ret['result'] = False
        ret['changes'] = {}
    ...
    if 'diff' in ret['changes']:
        ...
        if cron_ret['retcode'] == 0:
            ...
            ret['comment'] = 'Unable to update user {} crontab {}. Error: {}'.format(user, cron_path, cron_ret['stderr'])
            ret['result'] = False
            ret['changes'] = {}
    elif ret['result']:
...
def env_absent(name, user='root'):
    ...
    if data == 'removed':
        ...
        ret['changes'] = {user: name}
        return ret
    ret['comment'] = 'Cron env {} for user {} failed to commit with error {}'.format(name, user, data)
    ret['result'] = False
    return ret