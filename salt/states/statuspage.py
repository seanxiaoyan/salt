...
def _clear_dict(endpoint_props):
    ...
    """
    return {prop_name: prop_val for (prop_name, prop_val) in endpoint_props.items() if prop_val is not None}
...
def _ignore_keys(endpoint_props):
    ...
    """
    return {prop_name: prop_val for (prop_name, prop_val) in endpoint_props.items() if prop_name not in _DO_NOT_COMPARE_FIELDS}
...
def _clear_ignore(endpoint_props):
    ...
    """
    return {prop_name: prop_val for (prop_name, prop_val) in endpoint_props.items() if prop_name not in _DO_NOT_COMPARE_FIELDS and prop_val is not None}
...
def _update_on_fields(prev_ele, new_ele):
    ...
    """
    fields_update = {prop_name: prop_val for (prop_name, prop_val) in new_ele.items() if new_ele.get(prop_name) != prev_ele.get(prop_name) or prop_name in _MATCH_KEYS}
...
def _compute_diff(expected_endpoints, configured_endpoints):
    ...
    return {'add': new_endpoints, 'update': update_endpoints, 'remove': remove_endpoints}
...
def create(name, endpoint='incidents', api_url=None, page_id=None, api_key=None, api_version=None, **kwargs):
    ...
    if __opts__['test']:
        ...
        return ret
    sp_create = __salt__['statuspage.create'](endpoint=endpoint, api_url=api_url, page_id=page_id, api_key=api_key, api_version=api_version, **kwargs)
    if not sp_create.get('result'):
        ret['comment'] = 'Unable to create {endpoint}: {msg}'.format(endpoint=endpoint_sg, msg=sp_create.get('comment'))
    else:
...
def update(name, endpoint='incidents', id=None, api_url=None, page_id=None, api_key=None, api_version=None, **kwargs):
    ...
    if __opts__['test']:
        ...
        for (karg, warg) in kwargs.items():
            ...
            ret['changes'][endpoint][karg] = warg
        return ret
    sp_update = __salt__['statuspage.update'](endpoint=endpoint, id=id, api_url=api_url, page_id=page_id, api_key=api_key, api_version=api_version, **kwargs)
    if not sp_update.get('result'):
        ret['comment'] = 'Unable to update {endpoint} #{id}: {msg}'.format(endpoint=endpoint_sg, id=id, msg=sp_update.get('comment'))
    else:
        ret['comment'] = '{endpoint} #{id} updated!'.format(endpoint=endpoint_sg, id=id)
        ret['result'] = True
        ret['changes'] = sp_update.get('out')

def delete(name, endpoint='incidents', id=None, api_url=None, page_id=None, api_key=None, api_version=None):
    """
    ...
    if __opts__['test']:
        ...
        ret['result'] = None
    sp_delete = __salt__['statuspage.delete'](endpoint=endpoint, id=id, api_url=api_url, page_id=page_id, api_key=api_key, api_version=api_version)
    if not sp_delete.get('result'):
        ret['comment'] = 'Unable to delete {endpoint} #{id}: {msg}'.format(endpoint=endpoint_sg, id=id, msg=sp_delete.get('comment'))
    else:
        ret['comment'] = '{endpoint} #{id} deleted!'.format(endpoint=endpoint_sg, id=id)
        ret['result'] = True

def managed(name, config, api_url=None, page_id=None, api_key=None, api_version=None, pace=_PACE, allow_empty=False):
    """
    ...
    if not config and (not allow_empty):
        ret.update({'result': False, 'comment': 'Cannot remove everything. To allow this, please set the option `allow_empty` as True.'})
        return ret
    is_empty = True
    for (endpoint_name, endpoint_expected_config) in config.items():
        if endpoint_expected_config:
            is_empty = False
        endpoint_existing_config_ret = __salt__['statuspage.retrieve'](endpoint=endpoint_name, api_url=api_url, page_id=page_id, api_key=api_key, api_version=api_version)
    ...
    if __opts__.get('test'):
        ret.update({'comment': 'Testing mode. Would apply the following changes:', 'result': None})
        return ret
    for (endpoint_name, endpoint_diff) in complete_diff.items():
        endpoint_sg = endpoint_name[:-1]
        for new_endpoint in endpoint_diff.get('add'):
            log.debug('Defining new %s %s', endpoint_sg, new_endpoint)
            adding = __salt__['statuspage.create'](endpoint=endpoint_name, api_url=api_url, page_id=page_id, api_key=api_key, api_version=api_version, **new_endpoint)
        ...
        for update_endpoint in endpoint_diff.get('update'):
            ...
            updating = __salt__['statuspage.update'](endpoint=endpoint_name, id=endpoint_id, api_url=api_url, page_id=page_id, api_key=api_key, api_version=api_version, **update_endpoint)
        ...
        for remove_endpoint in endpoint_diff.get('remove'):
            ...
            removing = __salt__['statuspage.delete'](endpoint=endpoint_name, id=endpoint_id, api_url=api_url, page_id=page_id, api_key=api_key, api_version=api_version)
