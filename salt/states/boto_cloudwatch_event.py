...
def present(name, Name=None, ScheduleExpression=None, EventPattern=None, Description=None, RoleArn=None, State=None, Targets=None, region=None, key=None, keyid=None, profile=None):
    ...
    if not r.get('exists'):
        ...
        if 'error' in _describe:
            ret['result'] = False
            ret['comment'] = 'Failed to create event rule: {}.'.format(_describe['error']['message'])
            ret['changes'] = {}
            return ret
        ...
        return ret
    ret['comment'] = os.linesep.join([ret['comment'], 'CloudWatch event rule {} is present.'.format(Name)])
    ret['changes'] = {}
...
def absent(name, Name=None, region=None, key=None, keyid=None, profile=None):
    ...
    ret = {'name': Name, 'result': True, 'comment': '', 'changes': {}}
    Name = Name if Name else name
    r = __salt__['boto_cloudwatch_event.exists'](Name, region=region, key=key, keyid=keyid, profile=profile)
    if 'error' in r:
        ret['result'] = False
    ...
    if bool(ids):
        r = __salt__['boto_cloudwatch_event.remove_targets'](Rule=Name, Ids=ids, region=region, key=key, keyid=keyid, profile=profile)
        if 'error' in r:
            ret['result'] = False
            ret['comment'] = 'Failed to delete event rule: {}.'.format(r['error']['message'])
            return ret
        if r.get('failures'):
            ...
            return ret
    r = __salt__['boto_cloudwatch_event.delete'](Name, region=region, key=key, keyid=keyid, profile=profile)
    if not r['deleted']:
        ret['result'] = False
        ret['comment'] = 'Failed to delete event rule: {}.'.format(r['error']['message'])
        return ret
