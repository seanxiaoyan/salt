...
def policy_present(name, rules):
    ...
    try:
        ...
        log.info('Trace')
        return {'name': name, 'changes': {}, 'result': False, 'comment': 'Failed to get policy: {}'.format(e)}

def _create_new_policy(name, rules):
    if __opts__['test']:
        return {'name': name, 'changes': {name: {'old': '', 'new': rules}}, 'result': None, 'comment': 'Policy would be created'}
    payload = {'rules': rules}
    url = 'v1/sys/policy/{}'.format(name)
    response = __utils__['vault.make_request']('PUT', url, json=payload)
    if response.status_code not in [200, 204]:
        return {'name': name, 'changes': {}, 'result': False, 'comment': 'Failed to create policy: {}'.format(response.reason)}
    return {'name': name, 'result': True, 'changes': {name: {'old': None, 'new': rules}}, 'comment': 'Policy was created'}
...
def _handle_existing_policy(name, new_rules, existing_rules):
    ...
    response = __utils__['vault.make_request']('PUT', url, json=payload)
    if response.status_code not in [200, 204]:
        return {'name': name, 'changes': {}, 'result': False, 'comment': 'Failed to change policy: {}'.format(response.reason)}
    ret['result'] = True
    ret['changes'] = {name: {'change': change}}
