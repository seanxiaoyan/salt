...
def present(name, api_name, swagger_file, stage_name, api_key_required, lambda_integration_role, lambda_region=None, stage_variables=None, region=None, key=None, keyid=None, profile=None, lambda_funcname_format='{stage}_{api}_{resource}_{method}', authorization_type='NONE', error_response_template=None, response_template=None):
    ...
    try:
        ...
        swagger = _Swagger(api_name, stage_name, lambda_funcname_format, swagger_file, error_response_template, response_template, common_args)
        stage_vars = _get_stage_variables(stage_variables)
        ret = swagger.verify_api(ret)
        if ret.get('publish'):
            if __opts__['test']:
                ret['comment'] = '[stage: {}] will be reassociated to an already available deployment that matched the given [api_name: {}] and [swagger_file: {}].\nStage variables will be set to {}.'.format(stage_name, api_name, swagger_file, stage_vars)
                ret['result'] = None
                return ret
            return swagger.publish_api(ret, stage_vars)
        if ret.get('current'):
            if __opts__['test']:
                ret['comment'] = '[stage: {}] is already at desired state with an associated deployment matching the given [api_name: {}] and [swagger_file: {}].\nStage variables will be set to {}.'.format(stage_name, api_name, swagger_file, stage_vars)
                ret['result'] = None
            return swagger.overwrite_stage_variables(ret, stage_vars)
        if __opts__['test']:
            ret['comment'] = 'There is no deployment matching the given [api_name: {}] and [swagger_file: {}].  A new deployment will be created and the [stage_name: {}] will then be associated to the newly created deployment.\nStage variables will be set to {}.'.format(api_name, swagger_file, stage_name, stage_vars)
            ret['result'] = None
            return ret
        ...
        if ret.get('abort'):
            return ret
        ret = swagger.deploy_resources(ret, api_key_required=api_key_required, lambda_integration_role=lambda_integration_role, lambda_region=lambda_region, authorization_type=authorization_type)
        if ret.get('abort'):
            return ret
...
def absent(name, api_name, stage_name, nuke_api=False, region=None, key=None, keyid=None, profile=None):
    ...
    try:
        ...
        if __opts__['test']:
            if nuke_api:
                ret['comment'] = '[stage: {}] will be deleted, if there are no other active stages, the [api: {} will also be deleted.'.format(stage_name, api_name)
            else:
...
class _Swagger:
    ...
    methods.
    """
    SWAGGER_OBJ_V2_FIELDS = ('swagger', 'info', 'host', 'basePath', 'schemes', 'consumes', 'produces', 'paths', 'definitions', 'parameters', 'responses', 'securityDefinitions', 'security', 'tags', 'externalDocs')
    ...
    class SwaggerParameter:
        ...
        def location(self):
            ...
            if _location in _Swagger.SwaggerParameter.LOCATIONS:
                return _location
            raise ValueError('Unsupported parameter location: {} in Parameter Object'.format(_location))
        ...
        def name(self):
            ...
            if _name:
                ...
                return None
            raise ValueError('Parameter must have a name: {}'.format(_dict_to_json_pretty(self._paramdict)))
            ...
            if self.location == 'body':
                ...
                if _schema:
                    if '$ref' in _schema:
                        ...
                        return schema_name
                    raise ValueError('Body parameter must have a JSON reference to the schema definition due to Amazon API restrictions: {}'.format(self.name))
    ...
    class SwaggerMethodResponse:
        ...
        def schema(self):
            ...
            if _schema:
                ...
                raise ValueError('Method response must have a JSON reference to the schema definition: {}'.format(_schema))
            return None
    ...
    def __init__(self, api_name, stage_name, lambda_funcname_format, swagger_file_path, error_response_template, response_template, common_aws_args):
        ...
        if swagger_file_path is not None:
            if os.path.exists(swagger_file_path) and os.path.isfile(swagger_file_path):
                ...
                self._md5_filehash = _gen_md5_filehash(self._swagger_file, error_response_template, response_template)
            ...
            for (opname, opobj) in ops.items():
                ...
                for (rescode, resobj) in opobj.get('responses').items():
                    ...
                    if 'schema' not in resobj:
                        raise ValueError('missing schema field in path {}, op {}, response {}'.format(path, opname, rescode))
                    schemaobj = resobj.get('schema')
                    if '$ref' not in schemaobj:
                        raise ValueError('missing $ref field under schema in path {}, op {}, response {}'.format(path, opname, rescode))
                    schemaobjref = schemaobj.get('$ref', '/')
                    modelname = schemaobjref.split('/')[-1]
                    if modelname not in mods:
                        raise ValueError('model schema {} reference not found under /definitions'.format(schemaobjref))
                    ...
                    if 'properties' not in model:
                        raise ValueError('model schema {} must have properties fields'.format(modelname))
                    modelprops = model.get('properties')
                    if 'errorMessage' not in modelprops:
                        raise ValueError('model schema {} must have errorMessage as a property to match AWS convention. If pattern is not set, .+ will be used'.format(modelname))
    ...
    def _validate_lambda_funcname_format(self):
        ...
        try:
            ...
            log.info('Trace')
            raise ValueError('Invalid lambda_funcname_format {}.  Please review documentation for known substitutable keys'.format(self._lambda_funcname_format))
    ...
    def _validate_swagger_file(self):
        ...
        if self._swagger_version not in _Swagger.SWAGGER_VERSIONS_SUPPORTED:
            raise ValueError('Unsupported Swagger version: {},Supported versions are {}'.format(self._swagger_version, _Swagger.SWAGGER_VERSIONS_SUPPORTED))
    ...
    def _resolve_api_id(self):
        ...
        if apis:
            if len(apis) == 1:
                ...
                raise ValueError('Multiple APIs matching given name {} and description {}'.format(self.rest_api_name, self.info_json))
        ...
        if deploymentId:
            ...
            if not result.get('deleted'):
                ...
                if not result.get('deleted'):
                    ret['abort'] = True
                    ret['result'] = False
                    ret['comment'] = 'delete_stage delete_api_deployment, {}'.format(result.get('error'))
            else:
    ...
    def verify_api(self, ret):
        ...
        if self.restApiId:
            ...
            if deployed_label_json == self.deployment_label_json:
                ret['comment'] = 'Already at desired state, the stage {} is already at the desired deployment label:\n{}'.format(self._stage_name, deployed_label_json)
                ret['current'] = True
                return ret
            else:
    ...
    def deploy_api(self, ret):
        ...
        if not response.get('created'):
            ...
            if 'error' in response:
                ret['comment'] = 'Failed to create rest api: {}.'.format(response['error']['message'])
            return ret
    ...
    def delete_api(self, ret):
        ...
        if exists_response.get('exists'):
            if __opts__['test']:
                ret['comment'] = 'Rest API named {} is set to be deleted.'.format(self.rest_api_name)
                ret['result'] = None
                ret['abort'] = True
                return ret
            ...
            if not delete_api_response.get('deleted'):
                ...
                if 'error' in delete_api_response:
                    ret['comment'] = 'Failed to delete rest api: {}.'.format(delete_api_response['error']['message'])
                return ret
            ret = _log_changes(ret, 'delete_api', delete_api_response)
        else:
            ret['comment'] = 'api already absent for swagger file: {}, desc: {}'.format(self.rest_api_name, self.info_json)
        return ret
    ...
    def _aws_model_ref_from_swagger_ref(self, r):
        ...
        return 'https://apigateway.amazonaws.com/restapis/{}/models/{}'.format(self.restApiId, model_name)
    ...
    def _get_model_without_dependencies(self, models_dict):
        ...
        if next_model is None:
            raise ValueError('incomplete model definitions, models in dependency list not defined: {}'.format(models_dict))
    ...
    def deploy_models(self, ret):
        ...
        for (model, schema) in self.models():
            ...
            _schema.update({'$schema': _Swagger.JSON_SCHEMA_DRAFT_4, 'title': '{} Schema'.format(model)})
            ...
            if model_exists_response.get('exists'):
                ...
                if not update_model_schema_response.get('updated'):
                    ...
                    if 'error' in update_model_schema_response:
                        ret['comment'] = 'Failed to update existing model {} with schema {}, error: {}'.format(model, _dict_to_json_pretty(schema), update_model_schema_response['error']['message'])
                    return ret
                ...
                if not create_model_response.get('created'):
                    ...
                    if 'error' in create_model_response:
                        ret['comment'] = 'Failed to create model {}, schema {}, error: {}'.format(model, _dict_to_json_pretty(schema), create_model_response['error']['message'])
                    return ret
    ...
    def _lambda_uri(self, lambda_name, lambda_region):
        ...
        if lambda_region != apigw_region:
            if not lambda_desc.get('function'):
                lambda_desc = __salt__['boto_lambda.describe_function'](lambda_name, **self._common_aws_args)
        if not lambda_desc.get('function'):
            raise ValueError('Could not find lambda function {} in regions [{}, {}].'.format(lambda_name, lambda_region, apigw_region))
        lambda_arn = lambda_desc.get('function').get('FunctionArn')
        lambda_uri = 'arn:aws:apigateway:{}:lambda:path/2015-03-31/functions/{}/invocations'.format(apigw_region, lambda_arn)
        return lambda_uri
    ...
    def _get_response_template(self, method_name, http_status):
        if method_name == 'options' or not self._is_http_error_rescode(http_status):
            response_templates = {'application/json': self._response_template} if self._response_template else self.RESPONSE_OPTION_TEMPLATE
        else:
            response_templates = {'application/json': self._error_response_template} if self._error_response_template else self.RESPONSE_TEMPLATE
    ...
    def _parse_method_response(self, method_name, method_response, httpStatus):
        ...
        for header in method_response.headers:
            ...
            method_integration_response_params[response_header] = "'{}'".format(header_data.get('default')) if 'default' in header_data else "'*'"
    ...
    try:
        ...
        if not existing['plans']:
            if __opts__['test']:
                ...
                ret['result'] = None
                return ret
            result = __salt__['boto_apigateway.create_usage_plan'](name=plan_name, description=description, throttle=throttle, quota=quota, **common_args)
            if 'error' in result:
                ret['result'] = False
                ret['comment'] = 'Failed to create a usage plan {}, {}'.format(plan_name, result['error'])
                return ret
            ...
            if __opts__['test']:
                ...
                ret['result'] = None
                return ret
            result = __salt__['boto_apigateway.update_usage_plan'](plan['id'], throttle=throttle, quota=quota, **common_args)
            if 'error' in result:
                ret['result'] = False
                ret['comment'] = 'Failed to update a usage plan {}, {}'.format(plan_name, result['error'])
                return ret
...
def usage_plan_association_present(name, plan_name, api_stages, region=None, key=None, keyid=None, profile=None):
    ...
    try:
        ...
        if 'error' in result:
            ret['comment'] = 'Failed to associate a usage plan {} to the apis {}, {}'.format(plan_name, stages_to_add, result['error'])
            ret['result'] = False
            return ret
...
def usage_plan_association_absent(name, plan_name, api_stages, region=None, key=None, keyid=None, profile=None):
    ...
    try:
        ...
        if 'error' in result:
            ret['comment'] = 'Failed to disassociate a usage plan {} from the apis {}, {}'.format(plan_name, stages_to_remove, result['error'])
            ret['result'] = False
            return ret
