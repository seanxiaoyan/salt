...
def __virtual__():
    ...
    if 'vmadm.create' in __salt__ and 'imgadm.list' in __salt__:
        ...
        return (False, '{} state module can only be loaded on SmartOS compute nodes'.format(__virtualname__))
    ...
    try:
        with salt.utils.atomicfile.atomic_open('/usbkey/config', 'w') as config_file:
            ...
            for prop in salt.utils.odict.OrderedDict(sorted(config.items())):
                ...
                config_file.write(salt.utils.stringutils.to_str('{}={}\n'.format(prop, config[prop])))
            ...
            if prop not in instances:
                ...
                for instance in config[prop]:
                    ...
                    if 'vrrp_vrid' in instance_config:
                        instance_config['mac'] = '00:00:5e:00:01:{}'.format(hex(int(instance_config['vrrp_vrid'])).split('x')[-1].zfill(2))
        ...
        if imgtype == 'docker':
            ...
            for var in imgtags.get('docker:config', {}):
                ...
                if var == 'docker:env':
                    try:
                        log.info('Trace')
                        val_config = json.loads(vmconfig['internal_metadata'].get(var, ''))
...
def config_present(name, value):
    ...
    if not __opts__['test'] and ret['changes']:
        ...
        if not ret['result']:
            ret['comment'] = 'Could not add property {} with value "{}" to config'.format(name, value)
    return ret
...
def image_present(name):
    ...
    if _is_docker_uuid(name) and __salt__['imgadm.docker_to_uuid'](name):
        ret['result'] = True
        ret['comment'] = 'image {} ({}) is present'.format(name, __salt__['imgadm.docker_to_uuid'](name))
...
def image_absent(name):
    ...
    if not uuid or uuid not in __salt__['imgadm.list']():
    ...
    elif uuid in __salt__['vmadm.list'](order='image_uuid'):
        ret['result'] = False
        ...
        if image_count:
            ret['comment'] = 'image {} and {} children deleted'.format(name, image_count)
        else:
        ...
        if image['manifest']['name'] == 'docker-layer':
            while image:
                ...
                if 'origin' in image['manifest']:
                    ...
                    image = None
        elif image_uuid in __salt__['imgadm.delete'](image_uuid):
            ret['changes'][image_uuid] = None
        else:
            ret['result'] = False
            ret['comment'] = 'failed to delete images'
...
def vm_present(name, vmconfig, config=None):
    ...
    config = {'kvm_reboot': True, 'auto_import': False, 'auto_lx_vars': True, 'reprovision': False, 'enforce_tags': True, 'enforce_routes': True, 'enforce_internal_metadata': True, 'enforce_customer_metadata': True}
    config.update(state_config)
    log.debug('smartos.vm_present::%s::config - %s', name, config)
    vmconfig_type = {'collection': ['tags', 'customer_metadata', 'internal_metadata', 'routes'], 'instance': {'nics': 'mac', 'disks': 'path', 'filesystems': 'target', 'pci_devices': 'path'}, 'create_only': ['filesystems']}
    vmconfig_docker_keep = ['docker:id', 'docker:restartcount']
    vmconfig_docker_array = ['docker:env', 'docker:cmd', 'docker:entrypoint']
            ...
            if config['auto_import']:
                if not __opts__['test']:
                    ...
                    if vmconfig['image_uuid'] not in res:
                        ret['result'] = False
                        ret['comment'] = 'failed to import image {}'.format(vmconfig['image_uuid'])
            else:
                ret['result'] = False
            ...
            if config['auto_import']:
                if not __opts__['test']:
                    ...
                    if disk['image_uuid'] not in res:
                        ret['result'] = False
                        ret['comment'] = 'failed to import image {}'.format(disk['image_uuid'])
            else:
                ret['result'] = False
    ...
    if 'internal_metadata' in vmconfig:
        ...
        for var in vmconfig_docker_array:
            ...
            if isinstance(vmconfig['internal_metadata'][var], list):
                vmconfig['internal_metadata'][var] = json.dumps(vmconfig['internal_metadata'][var])
    ...
    if vmconfig['hostname'] in __salt__['vmadm.list'](order='hostname'):
        ret['result'] = True
        vmconfig = {'state': vmconfig, 'current': __salt__['vmadm.get'](vmconfig['hostname'], key='hostname'), 'changed': {}, 'reprovision_uuid': None}
        ...
        for prop in vmconfig['state']:
            if prop in vmconfig_type['instance'] or prop in vmconfig_type['collection'] or prop in vmconfig_type['create_only']:
                continue
            if prop in vmconfig['current']:
                if isinstance(vmconfig['current'][prop], list) or isinstance(vmconfig['current'][prop], dict):
                    if vmconfig['current'][prop] == vmconfig['state'][prop]:
                        continue
                elif '{}'.format(vmconfig['current'][prop]) == '{}'.format(vmconfig['state'][prop]):
                    continue
        ...
        for collection in vmconfig_type['collection']:
            ...
            if collection in vmconfig['state'] and vmconfig['state'][collection] is not None:
                for prop in vmconfig['state'][collection]:
                    if prop in vmconfig['current'][collection] and vmconfig['current'][collection][prop] == vmconfig['state'][collection][prop]:
                        continue
                    ...
                    vmconfig['changed']['set_{}'.format(collection)][prop] = vmconfig['state'][collection][prop]
                        ...
                        if state_cfg[vmconfig_type['instance'][instance]] == current_cfg[vmconfig_type['instance'][instance]]:
                            ...
                            for prop in state_cfg:
                                ...
                                if prop not in current_cfg:
                                    update_cfg[prop] = state_cfg[prop]
                            if update_cfg:
                                if 'update_{}'.format(instance) not in vmconfig['changed']:
                                    vmconfig['changed']['update_{}'.format(instance)] = []
                                update_cfg[vmconfig_type['instance'][instance]] = state_cfg[vmconfig_type['instance'][instance]]
                                vmconfig['changed']['update_{}'.format(instance)].append(update_cfg)
                    if add_instance:
            ...
            if instance in vmconfig['current'] and vmconfig['current'][instance] is not None:
                for current_cfg in vmconfig['current'][instance]:
                    ...
                    if remove_instance:
                        ...
                        vmconfig['changed']['remove_{}'.format(instance)].append(current_cfg[vmconfig_type['instance'][instance]])
        kvm_needs_start = False
        if not __opts__['test'] and vmconfig['changed']:
            if vmconfig['current']['brand'] == 'kvm' and config['kvm_reboot']:
                if 'add_disks' in vmconfig['changed'] or 'update_disks' in vmconfig['changed'] or 'remove_disks' in vmconfig['changed']:
        ...
        if ret['result']:
            ...
            if vmconfig['state']['hostname'] in ret['changes'] and ret['changes'][vmconfig['state']['hostname']]:
                ...
                ret['changes'] = {}
                ret['comment'] = 'vm {} is up to date'.format(vmconfig['state']['hostname'])
            if 'image_uuid' in vmconfig['current'] and vmconfig['reprovision_uuid'] != vmconfig['current']['image_uuid']:
                if config['reprovision']:
                    rret = __salt__['vmadm.reprovision'](vm=vmconfig['state']['hostname'], key='hostname', image=vmconfig['reprovision_uuid'])
                    if not isinstance(rret, bool) and 'Error' in rret:
                        ret['result'] = False
                        ret['comment'] = 'vm {} updated, reprovision failed'.format(vmconfig['state']['hostname'])
                    else:
                        ret['comment'] = 'vm {} updated and reprovisioned'.format(vmconfig['state']['hostname'])
                    ...
                    log.warning('smartos.vm_present::%s::reprovision - image_uuid in state does not match current, reprovision not allowed', name)
        else:
            ret['comment'] = 'vm {} failed to be updated'.format(vmconfig['state']['hostname'])
            if not isinstance(rret, bool) and 'Error' in rret:
                ret['comment'] = '{}'.format(rret['Error'])
    else:
        ret['result'] = True
