...
def present(name, description, vpc_id=None, vpc_name=None, rules=None, rules_egress=None, delete_ingress_rules=True, delete_egress_rules=True, region=None, key=None, keyid=None, profile=None, tags=None):
    """
    ...
    _ret = _security_group_present(name, description, vpc_id=vpc_id, vpc_name=vpc_name, region=region, key=key, keyid=keyid, profile=profile)
    ...
    if rules is not None:
        _ret = _rules_present(name, rules, delete_ingress_rules, vpc_id=vpc_id, vpc_name=vpc_name, region=region, key=key, keyid=keyid, profile=profile)
        ret['changes'] = dictupdate.update(ret['changes'], _ret['changes'])
        ret['comment'] = ' '.join([ret['comment'], _ret['comment']])
        if not _ret['result']:
            ret['result'] = _ret['result']
    if rules_egress is not None:
        _ret = _rules_egress_present(name, rules_egress, delete_egress_rules, vpc_id=vpc_id, vpc_name=vpc_name, region=region, key=key, keyid=keyid, profile=profile)
        ret['changes'] = dictupdate.update(ret['changes'], _ret['changes'])
        ret['comment'] = ' '.join([ret['comment'], _ret['comment']])
        if not _ret['result']:
            ret['result'] = _ret['result']
    _ret = _tags_present(name=name, tags=tags, vpc_id=vpc_id, vpc_name=vpc_name, region=region, key=key, keyid=keyid, profile=profile)
    ret['changes'] = dictupdate.update(ret['changes'], _ret['changes'])
    ret['comment'] = ' '.join([ret['comment'], _ret['comment']])
    if not _ret['result']:
        ret['result'] = _ret['result']
...
def _security_group_present(name, description, vpc_id=None, vpc_name=None, region=None, key=None, keyid=None, profile=None):
    """
    ...
    if not exists:
        if __opts__['test']:
            ...
            ret['result'] = None
            return ret
        created = __salt__['boto_secgroup.create'](name=name, description=description, vpc_id=vpc_id, vpc_name=vpc_name, region=region, key=key, keyid=keyid, profile=profile)
        if created:
...
def _check_rule(rule, _rule):
    ...
    if _rule.get('to_port') is None:
        _rule['to_port'] = -1
    if rule['ip_protocol'] == _rule['ip_protocol'] and str(rule['from_port']) == str(_rule['from_port']) and (str(rule['to_port']) == str(_rule['to_port'])):
...
def _get_rule_changes(rules, _rules):
    ...
    for rule in rules:
        ...
        if ip_protocol not in supported_protocols and (not '{}'.format(ip_protocol).isdigit() or int(ip_protocol) > 255):
            raise SaltInvocationError('Invalid ip_protocol {} specified in security group rule.'.format(ip_protocol))
        if ip_protocol == 'all':
            rule['ip_protocol'] = '-1'
...
def _rules_present(name, rules, delete_ingress_rules=True, vpc_id=None, vpc_name=None, region=None, key=None, keyid=None, profile=None):
    """
        ...
        for rule in rules:
            ...
            if _source_group_name:
                ...
                if _source_group_name_vpc:
                    ...
                    _group_vpc_id = None
                _group_id = __salt__['boto_secgroup.get_group_id'](name=_source_group_name, vpc_id=_group_vpc_id, vpc_name=_group_vpc_name, region=region, key=key, keyid=keyid, profile=profile)
                if not _group_id:
                    raise SaltInvocationError('source_group_name {} does not map to a valid source group id.'.format(_source_group_name))
    ...
    if to_create or to_delete:
        ...
        if to_delete:
            ...
            for rule in to_delete:
                _deleted = __salt__['boto_secgroup.revoke'](name, vpc_id=vpc_id, vpc_name=vpc_name, region=region, key=key, keyid=keyid, profile=profile, **rule)
                if not _deleted:
                    deleted = False
            if deleted:
        ...
        if to_create:
            ...
            for rule in to_create:
                _created = __salt__['boto_secgroup.authorize'](name, vpc_id=vpc_id, vpc_name=vpc_name, region=region, key=key, keyid=keyid, profile=profile, **rule)
                if not _created:
                    created = False
            if created:
                ret['comment'] = ' '.join([ret['comment'], 'Created rules on {} security group.'.format(name)])
            else:
                ret['comment'] = ' '.join([ret['comment'], 'Failed to create rules on {} security group.'.format(name)])
                ret['result'] = False
...
def _rules_egress_present(name, rules_egress, delete_egress_rules=True, vpc_id=None, vpc_name=None, region=None, key=None, keyid=None, profile=None):
    """
    given a group name or group name and vpc_id (or vpc name):
    1. get lists of desired rule changes (using _get_rule_changes)
    ...
    if to_create or to_delete:
        ...
        if to_delete:
            deleted = True
            for rule in to_delete:
                _deleted = __salt__['boto_secgroup.revoke'](name, vpc_id=vpc_id, vpc_name=vpc_name, region=region, key=key, keyid=keyid, profile=profile, egress=True, **rule)
                if not _deleted:
                    deleted = False
            if deleted:
                ret['comment'] = ' '.join([ret['comment'], 'Removed egress rule on {} security group.'.format(name)])
            else:
                ret['comment'] = ' '.join([ret['comment'], 'Failed to remove egress rule on {} security group.'.format(name)])
                ret['result'] = False
        if to_create:
            created = True
            for rule in to_create:
                _created = __salt__['boto_secgroup.authorize'](name, vpc_id=vpc_id, vpc_name=vpc_name, region=region, key=key, keyid=keyid, profile=profile, egress=True, **rule)
                if not _created:
                    created = False
            if created:
                ret['comment'] = ' '.join([ret['comment'], 'Created egress rules on {} security group.'.format(name)])
            else:
                ret['comment'] = ' '.join([ret['comment'], 'Failed to create egress rules on {} security group.'.format(name)])
                ret['result'] = False
...
def absent(name, vpc_id=None, vpc_name=None, region=None, key=None, keyid=None, profile=None):
    ...
    if sg:
        if __opts__['test']:
            ...
            ret['result'] = None
            return ret
        deleted = __salt__['boto_secgroup.delete'](name=name, group_id=None, region=region, key=key, keyid=keyid, profile=profile, vpc_id=vpc_id, vpc_name=vpc_name)
        if deleted:
    ...
    if tags:
        ...
        if tags_to_remove:
            if __opts__['test']:
                ...
                if not temp_ret:
                    ret['result'] = False
                    ret['comment'] = ' '.join([ret['comment'], 'Error attempting to delete tags {}.'.format(tags_to_remove)])
                    return ret
