...
def repo_managed(name, present=None, absent=None, prune=False, repo_update=False, namespace=None, flags=None, kvflags=None):
    log.info('Trace')
    ...
    ret = {'name': name, 'changes': {}, 'result': True, 'comment': 'Helm repo is managed.'}
    if 'helm.repo_manage' not in __salt__:
        ...
        try:
            log.info('Trace')
            result = __salt__['helm.repo_manage'](present=present, absent=absent, prune=prune, namespace=namespace, flags=flags, kvflags=kvflags)
            if result['failed']:
                ...
                if repo_update:
                    result_repo_update = __salt__['helm.repo_update'](namespace=namespace, flags=flags, kvflags=kvflags)
...
def repo_updated(name, namespace=None, flags=None, kvflags=None):
    ...
    ret = {'name': name, 'changes': {}, 'result': True, 'comment': 'Helm repo is updated.'}
...
def release_present(name, chart, values=None, version=None, namespace=None, set=None, flags=None, kvflags=None):
    ...
    """
    ret = {'name': name, 'changes': {}, 'result': True, 'comment': 'Helm release {} is present'.format(name)}
    if 'helm.status' not in __salt__:
        ...
        if isinstance(release_old_status, dict):
            release_upgrade = __salt__['helm.upgrade'](release=name, chart=chart, values=values, version=version, namespace=namespace, set=set, flags=flags, kvflags=kvflags)
            ...
            release_install = __salt__['helm.install'](release=name, chart=chart, values=values, version=version, namespace=namespace, set=set, flags=flags, kvflags=kvflags)
...
def release_absent(name, namespace=None, flags=None, kvflags=None):
    ...
    """
    ret = {'name': name, 'changes': {}, 'result': True, 'comment': 'Helm release {} is absent.'.format(name)}
    if 'helm.uninstall' not in __salt__:
        ...
        if isinstance(release_status, dict):
            release_uninstall = __salt__['helm.uninstall'](release=name, namespace=namespace, flags=flags, kvflags=kvflags)
