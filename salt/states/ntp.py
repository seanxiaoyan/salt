...
def managed(name, servers=None):
    ...
    """
    ret = {'name': name, 'changes': {}, 'result': True, 'comment': 'NTP servers already configured as specified'}
    ...
    if __opts__['test']:
        ret['result'] = None
        ret['comment'] = 'NTP servers will be updated to: {}'.format(', '.join(sorted(desired_servers)))
        return ret
