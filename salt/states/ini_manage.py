...
def options_present(name, sections=None, separator='=', strict=False):
    ...
    ret = {'name': name, 'changes': {}, 'result': True, 'comment': 'No anomaly detected'}
    if __opts__['test']:
        ret['comment'] = ''
    try:
        log.info('Trace')
            ...
            if strict:
                for opt_to_remove in set(original_top_level_opts).difference(options):
                    if __opts__['test']:
                        ...
                        changes.update({opt_to_remove: {'before': original_top_level_opts[opt_to_remove], 'after': None}})
            for (section_name, section_body) in [(sname, sbody) for (sname, sbody) in sections.items() if isinstance(sbody, (dict, OrderedDict))]:
                ...
                if strict:
                    ...
                    for key_to_remove in set(original.keys()).difference(section_body.keys()):
                        ...
                        if __opts__['test']:
                            ...
                            changes[section_name].update({key_to_remove: {'before': orig_value, 'after': None}})
                if __opts__['test']:
                    for option in section_body:
                        if str(section_body[option]) == str(original_sections.get(section_name, {}).get(option, '#-#-')):
