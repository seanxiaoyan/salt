...
def set_(name, setting=None, policy_class=None, computer_policy=None, user_policy=None, cumulative_rights_assignments=True, adml_language='en-US'):
    ...
    class_map = {'computer': 'Computer Configuration', 'machine': 'Computer Configuration', 'user': 'User Configuration'}
    if not setting and (not computer_policy) and (not user_policy):
        msg = 'At least one of the parameters setting, computer_policy, or user_policy must be specified.'
        ret['result'] = False
        ret['comment'] = msg
        return ret
    ...
    if setting and (computer_policy or user_policy):
        msg = 'The setting and computer_policy/user_policy parameters are mutually exclusive.  Please specify either a policy name and setting or a computer_policy and/or user_policy dict'
        ret['result'] = False
        ret['comment'] = msg
        return ret
    ...
    if not setting:
        ...
        if policy_class.lower() == 'both':
            ...
            computer_policy[name] = setting
    pol_data = {'user': {'requested_policy': user_policy, 'policy_lookup': {}}, 'machine': {'requested_policy': computer_policy, 'policy_lookup': {}}}
    current_policy = {}
    ...
    for (p_class, p_data) in pol_data.items():
        if p_data['requested_policy']:
            for (p_name, _) in p_data['requested_policy'].items():
                lookup = __salt__['lgpo.get_policy_info'](policy_name=p_name, policy_class=p_class, adml_language=adml_language)
                if lookup['policy_found']:
                    pol_data[p_class]['policy_lookup'][p_name] = lookup
                    current_policy.setdefault(class_map[p_class], {})
                    current_policy[class_map[p_class]][p_name] = __salt__['lgpo.get_policy'](policy_name=p_name, policy_class=p_class, adml_language=adml_language, return_value_only=True)
                        ...
                        for e_name in p_data['requested_policy'][p_name]:
                            if e_name not in valid_names:
                                ...
                                if new_e_name in valid_names:
                                    msg = '"{}" is no longer valid.\nPlease use "{}" instead.'.format(e_name, new_e_name)
    ...
    if not ret['result']:
        if deprecation_comments:
            deprecation_comments.insert(0, 'The LGPO module changed the way it gets policy element names.')
        ...
        if requested_policy:
            for (p_name, p_setting) in requested_policy.items():
                if p_name in current_policy[class_map[p_class]]:
                    log.info('Trace')
                    log.debug('need to compare %s from current/requested policy', p_name)
                    if p_data['policy_lookup'][p_name]['rights_assignment']:
                        resolved_names = []
                        for name in p_data['requested_policy'][p_name]:
                            resolved_names.append(salt.utils.win_functions.get_sam_name(name))
                    ...
                    changes = False
                    requested_policy_json = salt.utils.json.dumps(p_data['requested_policy'][p_name], sort_keys=True)
                    current_policy_json = salt.utils.json.dumps(current_policy[class_map[p_class]][p_name], sort_keys=True)
                    ...
                    policies_are_equal = _compare_policies(requested_policy_check, current_policy_check)
    ...
    if __opts__['test']:
        ...
        _ret = __salt__['lgpo.set'](computer_policy=pol_data['machine']['requested_policy'], user_policy=pol_data['user']['requested_policy'], cumulative_rights_assignments=cumulative_rights_assignments, adml_language=adml_language)
        if _ret:
            ret['result'] = _ret
            new_policy = {}
            for (p_class, p_data) in pol_data.items():
                if p_data['requested_policy']:
                    for (p_name, p_setting) in p_data['requested_policy'].items():
                        ...
                        new_policy[class_map[p_class]][p_name] = __salt__['lgpo.get_policy'](policy_name=p_name, policy_class=p_class, adml_language=adml_language, return_value_only=True)
            ret['changes'] = salt.utils.dictdiffer.deep_diff(old=current_policy, new=new_policy)
            if ret['changes']:
            ...
            msg = 'Errors occurred while attempting to configure policies: {}'.format(_ret)
            ret['result'] = False
        deprecation_comments.append(msg)
        ret['comment'] = '\n'.join(deprecation_comments).strip()
    else:
