...
def vm_configured(name, vm_name, cpu, memory, image, version, interfaces, disks, scsi_devices, serial_ports, datacenter, datastore, placement, cd_dvd_drives=None, sata_controllers=None, advanced_configs=None, template=None, tools=True, power_on=False, deploy=False):
    log.info('Trace')
    ...
    try:
        ...
        if vm_file:
            if __opts__['test']:
                result.update({'comment': 'The virtual machine {} will be registered.'.format(vm_name)})
            ...
            if __opts__['test']:
                result.update({'comment': 'The virtual machine {} will be created.'.format(vm_name)})
                __salt__['vsphere.disconnect'](service_instance)
                return result
            if template:
                result = vm_cloned(name)
            else:
                result = vm_created(name, vm_name, cpu, memory, image, version, interfaces, disks, scsi_devices, serial_ports, datacenter, datastore, placement, cd_dvd_drives=cd_dvd_drives, advanced_configs=advanced_configs, power_on=power_on)
            return result
    result = vm_updated(name, vm_name, cpu, memory, image, version, interfaces, disks, scsi_devices, serial_ports, datacenter, datastore, cd_dvd_drives=cd_dvd_drives, sata_controllers=sata_controllers, advanced_configs=advanced_configs, power_on=power_on)
    __salt__['vsphere.disconnect'](service_instance)
    log.trace(result)
    return result
...
def vm_updated(name, vm_name, cpu, memory, image, version, interfaces, disks, scsi_devices, serial_ports, datacenter, datastore, cd_dvd_drives=None, sata_controllers=None, advanced_configs=None, power_on=False):
    log.info('Trace')
    ...
    if not diffs:
        result.update({'result': True, 'changes': {}, 'comment': 'Virtual machine {} is already up to date'.format(vm_name)})
        return result
    if __opts__['test']:
        comment = "State vm_updated will update virtual machine '{}' in datacenter '{}':\n{}".format(vm_name, datacenter, '\n'.join([':\n'.join([key, difference.changes_str]) for (key, difference) in diffs.items()]))
    ...
    __salt__['vsphere.disconnect'](service_instance)
    result = {'name': name, 'result': True, 'changes': changes, 'comment': 'Virtual machine {} was updated successfully'.format(vm_name)}
    return result
...
def vm_created(name, vm_name, cpu, memory, image, version, interfaces, disks, scsi_devices, serial_ports, datacenter, datastore, placement, ide_controllers=None, sata_controllers=None, cd_dvd_drives=None, advanced_configs=None, power_on=False):
    ...
    __salt__['vsphere.disconnect'](service_instance)
    result = {'name': name, 'result': True, 'changes': changes, 'comment': 'Virtual machine {} created successfully'.format(vm_name)}
    return result
...
def vm_registered(vm_name, datacenter, placement, vm_file, power_on=False):
    ...
    if power_on:
        ...
        try:
            ...
            return result
    __salt__['vsphere.disconnect'](service_instance)
    result.update({'result': True, 'changes': {'name': vm_name, 'power_on': power_on}, 'comment': 'Virtual machine {} registered successfully'.format(vm_name)})
    return result