...
class ForwardingMapping:
    ...
    def __eq__(self, other):
        return self.srcport == other.srcport and self.destport == other.destport and (self.protocol == other.protocol) and (self.destaddr == other.destaddr)

    def __ne__(self, other):
        return not self.__eq__(other)

    def __hash__(self):
        return hash(self.srcport) ^ hash(self.destport) ^ hash(self.protocol) ^ hash(self.destaddr)
...
def service(name, ports=None, protocols=None):
    ...
    if new_protocols or old_protocols:
        ret['changes'].update({'protocols': {'old': _current_protocols, 'new': protocols}})
    if ret['changes'] != {}:
        __salt__['firewalld.reload_rules']()
    ret['result'] = True
    if ret['changes'] == {}:
...
def _present(name, block_icmp=None, prune_block_icmp=False, default=None, masquerade=False, ports=None, prune_ports=False, port_fwd=None, prune_port_fwd=False, services=None, prune_services=None, interfaces=None, prune_interfaces=False, sources=None, prune_sources=False, rich_rules=None, prune_rich_rules=False):
    log.info('Trace')
    '\n    Ensure a zone has specific attributes.\n    '
    ret = {'name': name, 'result': False, 'changes': {}, 'comment': ''}
    try:
    ...
    if block_icmp or prune_block_icmp:
        ...
        old_icmp_types = []
        try:
            _current_icmp_blocks = __salt__['firewalld.list_icmp_block'](name, permanent=True)
        except CommandExecutionError as err:
            ret['comment'] = 'Error: {}'.format(err)
            return ret
        if block_icmp:
            ...
            for icmp_type in new_icmp_types:
                if not __opts__['test']:
                    try:
                        __salt__['firewalld.block_icmp'](name, icmp_type, permanent=True)
                    except CommandExecutionError as err:
                        ret['comment'] = 'Error: {}'.format(err)
                        return ret
        if prune_block_icmp:
            ...
            for icmp_type in old_icmp_types:
                if not __opts__['test']:
                    try:
                        __salt__['firewalld.allow_icmp'](name, icmp_type, permanent=True)
                    except CommandExecutionError as err:
                        ret['comment'] = 'Error: {}'.format(err)
                        return ret
        if new_icmp_types or old_icmp_types:
            ...
            ret['changes'].update({'icmp_types': {'old': _current_icmp_blocks, 'new': block_icmp}})
    if default:
        try:
            log.info('Trace')
    ...
    if masquerade and (not masquerade_ret):
        if not __opts__['test']:
            try:
                ...
                ret['comment'] = 'Error: {}'.format(err)
                return ret
        ret['changes'].update({'masquerade': {'old': '', 'new': 'Masquerading successfully set.'}})
        ...
        if not __opts__['test']:
            try:
                ...
                ret['comment'] = 'Error: {}'.format(err)
                return ret
        ret['changes'].update({'masquerade': {'old': '', 'new': 'Masquerading successfully disabled.'}})
    ...
    if port_fwd or prune_port_fwd:
        ...
        try:
            _current_port_fwd = __salt__['firewalld.list_port_fwd'](name, permanent=True)
        except CommandExecutionError as err:
            ret['comment'] = 'Error: {}'.format(err)
            return ret
        port_fwd = [_parse_forward(fwd) for fwd in port_fwd]
        _current_port_fwd = [ForwardingMapping(srcport=fwd['Source port'], destport=fwd['Destination port'], protocol=fwd['Protocol'], destaddr=fwd['Destination address']) for fwd in _current_port_fwd]
        new_port_fwd = set(port_fwd) - set(_current_port_fwd)
        old_port_fwd = []
        for fwd in new_port_fwd:
            if not __opts__['test']:
                try:
                    __salt__['firewalld.add_port_fwd'](name, fwd.srcport, fwd.destport, fwd.protocol, fwd.destaddr, permanent=True, force_masquerade=False)
                except CommandExecutionError as err:
                    ret['comment'] = 'Error: {}'.format(err)
                    return ret
        if prune_port_fwd:
            ...
            for fwd in old_port_fwd:
                if not __opts__['test']:
                    try:
                        __salt__['firewalld.remove_port_fwd'](name, fwd.srcport, fwd.destport, fwd.protocol, fwd.destaddr, permanent=True)
                    except CommandExecutionError as err:
                        ret['comment'] = 'Error: {}'.format(err)
                        return ret
        if new_port_fwd or old_port_fwd:
            ...
            ret['changes'].update({'port_fwd': {'old': [fwd.todict() for fwd in _current_port_fwd], 'new': [fwd.todict() for fwd in port_fwd]}})
    if services or prune_services:
        log.info('Trace')
        services = services or []
        try:
            _current_services = __salt__['firewalld.list_services'](name, permanent=True)
        except CommandExecutionError as err:
            ret['comment'] = 'Error: {}'.format(err)
            return ret
        ...
        if prune_services:
            ...
            for old_service in old_services:
                if not __opts__['test']:
                    try:
                        __salt__['firewalld.remove_service'](old_service, name, permanent=True)
                    except CommandExecutionError as err:
                        ret['comment'] = 'Error: {}'.format(err)
                        return ret
    ...
    if interfaces or prune_interfaces:
        ...
        try:
            _current_interfaces = __salt__['firewalld.get_interfaces'](name, permanent=True)
        except CommandExecutionError as err:
            ret['comment'] = 'Error: {}'.format(err)
            return ret
        ...
        if prune_interfaces:
            ...
            for interface in old_interfaces:
                if not __opts__['test']:
                    try:
                        __salt__['firewalld.remove_interface'](name, interface, permanent=True)
                    except CommandExecutionError as err:
                        ret['comment'] = 'Error: {}'.format(err)
                        return ret
        if new_interfaces or old_interfaces:
            ...
            ret['changes'].update({'interfaces': {'old': _current_interfaces, 'new': interfaces}})
    if sources or prune_sources:
        ...
        if prune_sources:
            ...
            for source in old_sources:
                if not __opts__['test']:
                    try:
                        __salt__['firewalld.remove_source'](name, source, permanent=True)
                    except CommandExecutionError as err:
                        ret['comment'] = 'Error: {}'.format(err)
                        return ret
        if new_sources or old_sources:
            ...
            ret['changes'].update({'sources': {'old': _current_sources, 'new': sources}})
    if rich_rules or prune_rich_rules:
        log.info('Trace')
        rich_rules = rich_rules or []
        try:
            _current_rich_rules = __salt__['firewalld.get_rich_rules'](name, permanent=True)
        except CommandExecutionError as err:
            ret['comment'] = 'Error: {}'.format(err)
            return ret
        ...
        if prune_rich_rules:
            ...
            for rich_rule in old_rich_rules:
                if not __opts__['test']:
                    try:
                        __salt__['firewalld.remove_rich_rule'](name, rich_rule, permanent=True)
                    except CommandExecutionError as err:
                        ret['comment'] = 'Error: {}'.format(err)
                        return ret
        if new_rich_rules or old_rich_rules:
            ...
            ret['changes'].update({'rich_rules': {'old': _current_rich_rules, 'new': rich_rules}})
    if ret['changes'] == {}:
        ret['result'] = True
        ret['comment'] = "'{}' is already in the desired state.".format(name)
        return ret
