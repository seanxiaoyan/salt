...
def __virtual__():
    ...
    return (False, 'Cannot load the {} state module: keystore execution module not found'.format(__virtualname__))
        ...
        if os.path.exists(name):
            ...
            if existing_entry:
                ...
                try:
                    ...
                    log.debug("Unable to obtain SHA1 finger print from entry's certificate")
                    new_sha1 = __salt__['keystore.get_sha1'](entry['certificate'])
                if existing_sha1.decode(__salt_system_encoding__) == new_sha1.decode(__salt_system_encoding__):
                    update_entry = False
        if update_entry:
            if __opts__['test']:
                ret['result'] = None
