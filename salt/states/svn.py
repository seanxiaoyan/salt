...
def latest(name, target=None, rev=None, user=None, username=None, password=None, force=False, externals=True, trust=False, trust_failures=None):
    log.info('Trace')
    ...
    if __opts__['test']:
        ...
        if not os.path.exists(target):
            return _neutral_test(ret, "{} doesn't exist and is set to be checked out at revision {}.".format(target, new_rev))
        try:
            log.info('Trace')
    ...
    if svn_cmd == 'svn.update':
        ...
        current_rev = current_info[0]['Revision']
        new_rev = __salt__['svn.info'](cwd=target, targets=None, user=user, username=username, password=password, fmt='dict')[0]['Revision']
        ...
        ret['changes']['revision'] = __salt__['svn.info'](cwd=target, targets=None, user=user, username=username, password=password, fmt='dict')[0]['Revision']
    ret['comment'] = out
    return ret

def export(name, target=None, rev=None, user=None, username=None, password=None, force=False, overwrite=False, externals=True, trust=False, trust_failures=None):
    """
    ...
    if __opts__['test']:
        if not os.path.exists(target):
            return _neutral_test(ret, "{} doesn't exist and is set to be checked out.".format(target))
        svn_cmd = 'svn.list'
        rev = 'HEAD'
        out = __salt__[svn_cmd](cwd, target, user, username, password, *opts)
        return _neutral_test(ret, out)
...
def dirty(name, target, user=None, username=None, password=None, ignore_unversioned=False):
    """
