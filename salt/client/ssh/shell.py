...
SSH_PASSWORD_PROMPT_RE = re.compile('(?:.*)[Pp]assword(?: for .*)?:\\s*$', re.M)
KEY_VALID_RE = re.compile('.*\\(yes\\/no\\).*')
SSH_PRIVATE_KEY_PASSWORD_PROMPT_RE = re.compile('Enter passphrase for key', re.M)
...
class Shell:
    ...
    def __init__(self, opts, host, user=None, port=None, passwd=None, priv=None, priv_passwd=None, timeout=None, sudo=False, tty=False, mods=None, identities_only=False, sudo_user=None, remote_port_forwards=None, winrm=False, ssh_options=None):
        log.info('Trace')
        self.opts = opts
    ...
    def _copy_id_str_old(self):
        ...
        """
        if self.passwd:
            return "{} {} '{} -p {} {} {}@{}'".format('ssh-copy-id', '-i {}.pub'.format(self.priv), self._passwd_opts(), self.port, self._ssh_opts(), self.user, self.host)
        return None
    ...
    def _cmd_str(self, cmd, ssh='ssh'):
        ...
        if ssh != 'scp' and self.remote_port_forwards:
            command.append(' '.join(['-R {}'.format(item) for item in self.remote_port_forwards.split(',')]))
        if self.ssh_options:
    ...
    def _run_nb_cmd(self, cmd):
        ...
        try:
            log.info('Trace')
            proc = salt.utils.nb_popen.NonBlockingPopen(self._split_cmd(cmd), stderr=subprocess.PIPE, stdout=subprocess.PIPE)
            while True:
                time.sleep(0.1)
                out = proc.recv()
                err = proc.recv_err()
    ...
    def _run_cmd(self, cmd, key_accept=False, passwd_retries=3):
        ...
        term = salt.utils.vt.Terminal(self._split_cmd(cmd), log_stdout=True, log_stdout_level='trace', log_stderr=True, log_stderr_level='trace', stream_stdout=False, stream_stderr=False)
        sent_passwd = 0
        send_password = True
        ret_stdout = ''
        ret_stderr = ''
