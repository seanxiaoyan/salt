...
class SSH(MultiprocessingStateMixin):
    ...
    def __init__(self, opts):
        ...
        if os.path.exists(pull_sock) and zmq:
            self.event = salt.utils.event.get_event('master', opts['sock_dir'], opts=opts, listen=False)
        else:
            self.event = None
        self.opts = opts
        ...
        if not salt.utils.path.which('ssh'):
            raise salt.exceptions.SaltSystemExit(code=-1, msg='No ssh binary found in path -- ssh must be installed for salt-ssh to run. Exiting.')
        self.opts['_ssh_version'] = ssh_version()
        self.tgt_type = self.opts['selected_target_option'] if self.opts['selected_target_option'] else 'glob'
        self._expand_target()
        ...
        if '__master_opts__' in self.opts:
            if self.opts['__master_opts__'].get('ssh_use_home_key') and os.path.isfile(os.path.expanduser('~/.ssh/id_rsa')):
                priv = os.path.expanduser('~/.ssh/id_rsa')
            else:
                priv = self.opts['__master_opts__'].get('ssh_priv', os.path.join(self.opts['__master_opts__']['pki_dir'], 'ssh', 'salt-ssh.rsa'))
        else:
    ...
    def _get_roster(self):
        ...
        if roster_file not in self.__parsed_rosters:
            roster_data = compile_template(roster_file, salt.loader.render(self.opts, {}), self.opts['renderer'], self.opts['renderer_blacklist'], self.opts['renderer_whitelist'])
    ...
    def _update_targets(self):
        ...
        if salt.utils.network.is_reachable_host(hostname):
            self.opts['tgt'] = hostname
            self.targets[hostname] = {'passwd': self.opts.get('ssh_passwd', ''), 'host': hostname, 'user': user}
    ...
    def get_pubkey(self):
        ...
        """
        if '__master_opts__' in self.opts and self.opts['__master_opts__'].get('ssh_use_home_key') and os.path.isfile(os.path.expanduser('~/.ssh/id_rsa')):
            priv = os.path.expanduser('~/.ssh/id_rsa')
        else:
            priv = self.opts.get('ssh_priv', os.path.join(self.opts['pki_dir'], 'ssh', 'salt-ssh.rsa'))
        pub = '{}.pub'.format(priv)
    ...
    def key_deploy(self, host, ret):
        ...
        if ret[host].get('stderr', '').count('Permission denied'):
            target = self.targets[host]
            print('Permission denied for host {}, do you want to deploy the salt-ssh key? (password required):'.format(host))
            deploy = input('[Y/n] ')
            if deploy.startswith(('n', 'N')):
                return ret
            target['passwd'] = getpass.getpass('Password for {}@{}: '.format(target['user'], host))
    ...
    def _key_deploy_run(self, host, target, re_run=True):
        ...
        """
        argv = ['ssh.set_auth_key', target.get('user', 'root'), self.get_pubkey()]
        ...
        if re_run:
            target.pop('passwd')
            single = Single(self.opts, self.opts['argv'], host, mods=self.mods, fsclient=self.fsclient, thin=self.thin, **target)
    ...
    def handle_routine(self, que, opts, host, target, mine=False):
        ...
        try:
            ...
            if len(data) < 2 and 'local' in data:
                ...
                ret['ret'] = {'stdout': stdout, 'stderr': stderr, 'retcode': retcode}
        except Exception:
            log.info('Trace')
            ret['ret'] = {'stdout': stdout, 'stderr': stderr, 'retcode': retcode}
        que.put(ret)
        ...
        while True:
            ...
            if len(running) < self.opts.get('ssh_max_procs', 25) and (not init):
                ...
                if self.targets[host].get('winrm') and (not HAS_WINSHELL):
                    ...
                    log.debug(log_msg)
                    no_ret = {'fun_args': [], 'jid': None, 'return': log_msg, 'retcode': 1, 'fun': '', 'id': host}
                    yield {host: no_ret}
                    continue
                ...
                if not running[host]['thread'].is_alive():
                    if host not in returned:
                        try:
                            ...
                            pass
                        if host not in returned:
                            error = "Target '{}' did not return any data, probably due to an error.".format(host)
    ...
    def run_iter(self, mine=False, jid=None):
        ...
        if self.opts['master_job_cache'] == 'local_cache':
            self.returners['{}.save_load'.format(self.opts['master_job_cache'])](jid, job_load, minions=self.targets.keys())
        else:
            self.returners['{}.save_load'.format(self.opts['master_job_cache'])](jid, job_load)
        for ret in self.handle_ssh(mine=mine):
            ...
            if self.event:
                ...
                if 'fun' not in data:
                    data['fun'] = fun
                data['jid'] = jid
                self.event.fire_event(data, salt.utils.event.tagify([jid, 'ret', host], 'job'))
            yield ret
    ...
    def cache_job(self, jid, id_, ret, fun):
        ...
        """
        self.returners['{}.returner'.format(self.opts['master_job_cache'])]({'jid': jid, 'id': id_, 'return': ret, 'fun': fun})
    ...
    def run(self, jid=None):
        ...
        try:
            ...
            log.error('Could not save load with returner %s: %s', self.opts['master_job_cache'], exc, exc_info=True)
...
class Single:
    ...
    def __init__(self, opts, argv, id_, host, user=None, port=None, passwd=None, priv=None, priv_passwd=None, timeout=30, sudo=False, tty=False, mods=None, fsclient=None, thin=None, mine=False, minion_opts=None, identities_only=False, sudo_user=None, remote_port_forwards=None, winrm=False, ssh_options=None, **kwargs):
        self.mine = mine
        ...
        if kwargs.get('thin_dir'):
            ...
            self.thin_dir = thin_dir.replace('%%FQDNUUID%%', uuid.uuid3(uuid.NAMESPACE_DNS, salt.utils.network.get_fqhostname()).hex[:6])
        ...
        args = {'host': host, 'user': user, 'port': port, 'passwd': passwd, 'priv': priv, 'priv_passwd': priv_passwd, 'timeout': timeout, 'sudo': sudo, 'tty': tty, 'mods': self.mods, 'identities_only': identities_only, 'sudo_user': sudo_user, 'remote_port_forwards': remote_port_forwards, 'winrm': winrm, 'ssh_options': ssh_options}
        self.minion_opts = {'grains_cache': True, 'log_file': 'salt-call.log'}
        self.minion_opts.update(opts.get('ssh_minion_opts', {}))
        if minion_opts is not None:
            self.minion_opts.update(minion_opts)
        self.minion_opts.update({'root_dir': os.path.join(self.thin_dir, 'running_data'), 'id': self.id, 'sock_dir': '/', 'fileserver_list_cache_time': 3})
    ...
    def __arg_comps(self):
        ...
        parsed = salt.utils.args.parse_input(self.argv[1:], condition=False, no_parse=self.opts.get('no_parse', []))
        args = parsed[0]
        kws = parsed[1]
        return (fun, args, kws)
    ...
    def check_thin_dir(self):
        ...
        """
        (stdout, stderr, retcode) = self.shell.exec_cmd('test -d {}'.format(self.thin_dir))
        if retcode != 0:
            return False
        return True
    ...
    def deploy(self):
        ...
        Deploy salt-thin
        """
        self.shell.send(self.thin, os.path.join(self.thin_dir, 'salt-thin.tgz'))
        self.deploy_ext()
        return True
    ...
    def deploy_ext(self):
        ...
        if self.mods.get('file'):
            self.shell.send(self.mods['file'], os.path.join(self.thin_dir, 'salt-ext_mods.tgz'))
        return True
    ...
    def run(self, deploy_attempted=False):
        ...
        if self.ssh_pre_flight:
            if not self.opts.get('ssh_run_pre_flight', False) and self.check_thin_dir():
                log.info('%s thin dir already exists. Not running ssh_pre_flight script', self.thin_dir)
            elif not os.path.exists(self.ssh_pre_flight):
                log.error('The ssh_pre_flight script %s does not exist', self.ssh_pre_flight)
            else:
                (stdout, stderr, retcode) = self.run_ssh_pre_flight()
                if retcode != 0:
                    log.error('Error running ssh_pre_flight script %s', self.ssh_pre_file)
                    return (stdout, stderr, retcode)
                log.info('Successfully ran the ssh_pre_flight script: %s', self.ssh_pre_file)
        if self.opts.get('raw_shell', False):
    ...
    def run_wfunc(self):
        ...
        if refresh:
            ...
            pillar = salt.pillar.Pillar(popts, opts_pkg['grains'], opts_pkg['id'], opts_pkg.get('saltenv', 'base'))
            pillar_data = pillar.compile_pillar()
            data = {'opts': opts_pkg, 'grains': opts_pkg['grains'], 'pillar': pillar_data}
            if data_cache:
        ...
        try:
            ...
            result = 'An Exception occurred while executing {}: {}'.format(self.fun, exc)
            log.error(result, exc_info_on_loglevel=logging.DEBUG)
            retcode = 1
    ...
    def _cmd_str(self):
        ...
        if self.target.get('sudo'):
            sudo = "sudo -p '{}'".format(salt.client.ssh.shell.SUDO_PROMPT) if self.target.get('passwd') else 'sudo'
        else:
            sudo = ''
        ...
        if LOG_LEVELS['debug'] >= LOG_LEVELS[self.opts.get('log_level', 'info')]:
            debug = '1'
        arg_str = '\nOPTIONS.config = """\n{config}\n"""\nOPTIONS.delimiter = \'{delimeter}\'\nOPTIONS.saltdir = \'{saltdir}\'\nOPTIONS.checksum = \'{checksum}\'\nOPTIONS.hashfunc = \'{hashfunc}\'\nOPTIONS.version = \'{version}\'\nOPTIONS.ext_mods = \'{ext_mods}\'\nOPTIONS.wipe = {wipe}\nOPTIONS.tty = {tty}\nOPTIONS.cmd_umask = {cmd_umask}\nOPTIONS.code_checksum = {code_checksum}\nARGS = {arguments}\n'.format(config=self.minion_config, delimeter=RSTR, saltdir=self.thin_dir, checksum=thin_sum, hashfunc='sha1', version=salt.version.__version__, ext_mods=self.mods.get('version', ''), wipe=self.wipe, tty=self.tty, cmd_umask=self.cmd_umask, code_checksum=thin_code_digest, arguments=self.argv)
        py_code = SSH_PY_SHIM.replace('#%%OPTS', arg_str)
        py_code_enc = base64.encodebytes(py_code.encode('utf-8')).decode('utf-8')
        if not self.winrm:
            cmd = SSH_SH_SHIM.format(DEBUG=debug, SUDO=sudo, SUDO_USER=sudo_user or '', SSH_PY_CODE=py_code_enc, HOST_PY_MAJOR=sys.version_info[0], SET_PATH=self.set_path)
        else:
    ...
    def shim_cmd(self, cmd_str, extension='py'):
        ...
        with tempfile.NamedTemporaryFile(mode='w+b', prefix='shim_', delete=False) as shim_tmp_file:
            shim_tmp_file.write(salt.utils.stringutils.to_bytes(cmd_str))
        target_shim_file = '.{}.{}'.format(binascii.hexlify(os.urandom(6)).decode('ascii'), extension)
        if self.winrm:
    ...
    def cmd_block(self, is_retry=False):
        ...
        if error:
            if error == 'Python environment not found on Windows system':
                ...
                if not re.search(RSTR_RE, stdout) or not re.search(RSTR_RE, stderr):
                    return ('ERROR: Failure deploying thin, undefined state: {}'.format(stdout), stderr, retcode)
                while re.search(RSTR_RE, stdout):
                    stdout = re.split(RSTR_RE, stdout, 1)[1].strip()
                while re.search(RSTR_RE, stderr):
                    stderr = re.split(RSTR_RE, stderr, 1)[1].strip()
        ...
        if re.search(RSTR_RE, stderr):
            ...
            if 'deploy' == shim_command and retcode == salt.defaults.exitcodes.EX_THIN_DEPLOY:
                ...
                (stdout, stderr, retcode) = self.shim_cmd(cmd_str)
                if not re.search(RSTR_RE, stdout) or not re.search(RSTR_RE, stderr):
                    return ('ERROR: Failure deploying ext_mods: {}'.format(stdout), stderr, retcode)
                while re.search(RSTR_RE, stdout):
                    stdout = re.split(RSTR_RE, stdout, 1)[1].strip()
                while re.search(RSTR_RE, stderr):
                    stderr = re.split(RSTR_RE, stderr, 1)[1].strip()
...
def mod_data(fsclient):
    ...
    ext_tar_path = os.path.join(fsclient.opts['cachedir'], 'ext_mods.{}.tgz'.format(ver))
...
def ssh_version():
    ...
    """
    ret = subprocess.Popen(['ssh', '-V'], stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
    try:
        log.info('Trace')
