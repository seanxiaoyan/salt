...
from salt.exceptions import AuthenticationError, AuthorizationError, EauthAuthenticationError, PublishError, SaltClientError, SaltInvocationError, SaltReqTimeoutError
...
def get_local_client(c_path=os.path.join(syspaths.CONFIG_DIR, 'master'), mopts=None, skip_perm_errors=False, io_loop=None, auto_reconnect=False, listen=False):
    """
    ...
    return LocalClient(mopts=opts, skip_perm_errors=skip_perm_errors, io_loop=io_loop, auto_reconnect=auto_reconnect, listen=listen)
...
class LocalClient:
    ...
    def __init__(self, c_path=os.path.join(syspaths.CONFIG_DIR, 'master'), mopts=None, skip_perm_errors=False, io_loop=None, keep_loop=False, auto_reconnect=False, listen=False):
        """
        :param str c_path: Path of config file to use for opts.
        ...
        if mopts:
            ...
            if os.path.isdir(c_path):
                log.warning("%s expects a file path not a directory path(%s) to its 'c_path' keyword argument", self.__class__.__name__, c_path)
        ...
        self.listen = listen
        self.event = salt.utils.event.get_event('master', self.opts['sock_dir'], opts=self.opts, listen=self.listen, io_loop=io_loop, keep_loop=keep_loop)
    ...
    def gather_job_info(self, jid, tgt, tgt_type, listen=True, **kwargs):
        ...
        pub_data = self.run_job(tgt, 'saltutil.find_job', arg=[jid], tgt_type=tgt_type, timeout=timeout, listen=listen, **kwargs)
        if 'jid' in pub_data:
            log.info('Trace')
    ...
    def _check_pub_data(self, pub_data, listen=True):
        ...
        """
        if pub_data == '':
            raise EauthAuthenticationError('Failed to authenticate! This is most likely because this user is not permitted to execute commands, but there is a small possibility that a disk error occurred (check disk/inode usage).')
        ...
        if not self.opts.get('order_masters'):
            if not pub_data['minions']:
                print('No minions matched the target. No command was sent, no jid was assigned.')
                return {}
        if not listen:
            return pub_data
    ...
    def run_job(self, tgt, fun, arg=(), tgt_type='glob', ret='', timeout=None, jid='', kwarg=None, listen=False, **kwargs):
        log.info('Trace')
    ...
    def cmd_async(self, tgt, fun, arg=(), tgt_type='glob', ret='', jid='', kwarg=None, **kwargs):
        ...
        """
        pub_data = self.run_job(tgt, fun, arg, tgt_type, ret, jid=jid, kwarg=kwarg, listen=False, **kwargs)
        try:
            log.info('Trace')
            return pub_data['jid']
        except KeyError:
    ...
    def cmd_subset(self, tgt, fun, arg=(), tgt_type='glob', ret='', kwarg=None, subset=3, cli=False, progress=False, full_return=False, **kwargs):
        """
        ...
        if cli:
            func = self.cmd_cli
        return func(f_tgt, fun, arg, tgt_type='list', ret=ret, kwarg=kwarg, progress=progress, full_return=full_return, **kwargs)

    def cmd_batch(self, tgt, fun, arg=(), tgt_type='glob', ret='', kwarg=None, batch='10%', **kwargs):
        """
        ...
        arg = salt.utils.args.condition_input(arg, kwarg)
        opts = {'tgt': tgt, 'fun': fun, 'arg': arg, 'tgt_type': tgt_type, 'ret': ret, 'batch': batch, 'failhard': kwargs.get('failhard', self.opts.get('failhard', False)), 'raw': kwargs.get('raw', False)}
    ...
    def cmd_cli(self, tgt, fun, arg=(), timeout=None, tgt_type='glob', ret='', verbose=False, kwarg=None, progress=False, **kwargs):
        """
        ...
        was_listening = self.event.cpub
        try:
            self.pub_data = self.run_job(tgt, fun, arg, tgt_type, ret, timeout, kwarg=kwarg, listen=True, **kwargs)
            if not self.pub_data:
                yield self.pub_data
            else:
                try:
                    log.info('Trace')
                    for fn_ret in self.get_cli_event_returns(self.pub_data['jid'], self.pub_data['minions'], self._get_timeout(timeout), tgt, tgt_type, verbose, progress, **kwargs):
                        if not fn_ret:
                            continue
                        yield fn_ret
        ...
        try:
            ...
            if not pub_data:
                ...
                if kwargs.get('yield_pub_data'):
                    yield pub_data
                for fn_ret in self.get_iter_returns(pub_data['jid'], pub_data['minions'], timeout=self._get_timeout(timeout), tgt=tgt, tgt_type=tgt_type, **kwargs):
                    if not fn_ret:
                        continue
                    yield fn_ret
    ...
    def cmd_full_return(self, tgt, fun, arg=(), timeout=None, tgt_type='glob', ret='', verbose=False, kwarg=None, **kwargs):
        ...
        try:
            ...
            if not pub_data:
                return pub_data
            return self.get_cli_static_event_returns(pub_data['jid'], pub_data['minions'], timeout, tgt, tgt_type, verbose)
        finally:
            if not was_listening:
                self.event.close_pub()
    ...
    def get_iter_returns(self, jid, minions, timeout=None, tgt='*', tgt_type='glob', expect_minions=False, block=True, **kwargs):
        ...
        if self.opts['order_masters']:
            ret_iter = self.get_returns_no_block('(salt/job|syndic/.*)/{}'.format(jid), 'regex')
        else:
        ...
        while True:
            ...
            if time.time() > timeout_at and minions_running:
                ...
                if 'jid' not in jinfo:
                    ...
                    jinfo_iter = self.get_returns_no_block('salt/job/{}'.format(jinfo['jid']))
            ...
            for raw in jinfo_iter:
                ...
                try:
                    ...
                    if missing_key == 'retcode':
                        ...
                        log.debug("Passing on saltutil error. Key '%s' missing from client return. This may be an error in the client.", missing_key)
    ...
    def get_returns(self, jid, minions, timeout=None):
        ...
        log.debug('get_returns for jid %s sent to %s will timeout at %s', jid, minions, datetime.fromtimestamp(timeout_at).time())
        found = set()
        ret = {}
        try:
            log.info('Trace')
            if self.returners['{}.get_load'.format(self.opts['master_job_cache'])](jid) == {}:
                log.warning('jid does not exist')
                return ret
        except Exception as exc:
            log.info('Trace')
            raise SaltClientError('Master job cache returner [{}] failed to verify jid. Exception details: {}'.format(self.opts['master_job_cache'], exc))
        while True:
            ...
            if int(time.time()) > timeout_at:
                log.info('jid %s minions %s did not return in time', jid, minions - found)
                break
            time.sleep(0.01)
        return ret
    ...
    def get_full_returns(self, jid, minions, timeout=None):
        ...
        try:
            log.info('Trace')
            data = self.returners['{}.get_jid'.format(self.opts['master_job_cache'])](jid)
        except Exception as exc:
            log.info('Trace')
            raise SaltClientError('Returner {} could not fetch jid data. Exception details: {}'.format(self.opts['master_job_cache'], exc))
        for minion in data:
            m_data = {}
    ...
    def get_cache_returns(self, jid):
        ...
        try:
            ...
            log.info('Trace')
            raise SaltClientError('Could not examine master job cache. Error occurred in {} returner. Exception details: {}'.format(self.opts['master_job_cache'], exc))
        for minion in data:
            m_data = {}
            if 'return' in data[minion]:
                m_data['ret'] = data[minion].get('return')
    ...
    def get_cli_static_event_returns(self, jid, minions, timeout=None, tgt='*', tgt_type='glob', verbose=False, show_timeout=False, show_jid=False):
        log.info('Trace')
        '\n        Get the returns for the command line interface via the event system\n        '
        ...
        try:
            ...
            log.info('Trace')
            raise SaltClientError('Load could not be retrieved from returner {}. Exception details: {}'.format(self.opts['master_job_cache'], exc))
        while True:
            time_left = timeout_at - int(time.time())
            wait = max(1, time_left)
            ...
            if int(time.time()) > timeout_at:
                if verbose or show_timeout:
                    if self.opts.get('minion_data_cache', False) or tgt_type in ('glob', 'pcre', 'list'):
    ...
    def get_cli_event_returns(self, jid, minions, timeout=None, tgt='*', tgt_type='glob', verbose=False, progress=False, show_timeout=False, show_jid=False, **kwargs):
        ...
        return_count = 0
        for ret in self.get_iter_returns(jid, minions, timeout=timeout, tgt=tgt, tgt_type=tgt_type, expect_minions=kwargs.pop('expect_minions', False) or verbose or show_timeout, **kwargs):
            ...
            for (id_, min_ret) in ret.items():
                if min_ret.get('failed') is True:
                    ...
                    if self.opts['minion_data_cache'] and salt.cache.factory(self.opts).contains('minions/{}'.format(id_), 'data') and connected_minions and (id_ not in connected_minions):
                        yield {id_: {'out': 'no_return', 'ret': 'Minion did not return. [Not connected]', 'retcode': salt.defaults.exitcodes.EX_GENERIC}}
                    elif not os.path.exists(os.path.join(self.opts['syndic_dir'], id_)):
                        yield {id_: {'out': 'no_return', 'ret': 'Minion did not return. [No response]\nThe minions may not have all finished running and any remaining minions will return upon completion. To look up the return data for this job later, run the following command:\n\nsalt-run jobs.lookup_jid {}'.format(jid), 'retcode': salt.defaults.exitcodes.EX_GENERIC}}
                else:
                    yield {id_: min_ret}
        self._clean_up_subscriptions(jid)
    ...
    def _resolve_nodegroup(self, ng):
        ...
        if ng not in self.opts['nodegroups']:
            ...
            raise SaltInvocationError('Node group {} unavailable in {}'.format(ng, conf_file))
    ...
    def _prep_pub(self, tgt, fun, arg, tgt_type, ret, jid, timeout, **kwargs):
        ...
        payload_kwargs = {'cmd': 'publish', 'tgt': tgt, 'fun': fun, 'arg': arg, 'key': self.key, 'tgt_type': tgt_type, 'ret': ret, 'jid': jid}
        if kwargs:
    ...
    def pub(self, tgt, fun, arg=(), tgt_type='glob', ret='', jid='', timeout=5, listen=False, **kwargs):
        log.info('Trace')
        ...
        if self.opts.get('ipc_mode', '') != 'tcp' and (not os.path.exists(os.path.join(self.opts['sock_dir'], 'publish_pull.ipc'))):
            log.error('Unable to connect to the salt master publisher at %s', self.opts['sock_dir'])
            raise SaltClientError
        payload_kwargs = self._prep_pub(tgt, fun, arg, tgt_type, ret, jid, timeout, **kwargs)
        master_uri = 'tcp://{}:{}'.format(salt.utils.zeromq.ip_bracket(self.opts['interface']), str(self.opts['ret_port']))
        with salt.channel.client.ReqChannel.factory(self.opts, crypt='clear', master_uri=master_uri) as channel:
            try:
                log.info('Trace')
                ...
                log.error(err)
                raise SaltReqTimeoutError('Salt request timed out. The master is not responding. You may need to run your command with `--async` in order to bypass the congested event bus. With `--async`, the CLI tool will print the job id (jid) and exit immediately without listening for responses. You can then use `salt-run jobs.lookup_jid` to look up the results of the job in the job cache later.')
            if not payload:
    ...
    def pub_async(self, tgt, fun, arg=(), tgt_type='glob', ret='', jid='', timeout=5, io_loop=None, listen=True, **kwargs):
        ...
        if self.opts.get('ipc_mode', '') != 'tcp' and (not os.path.exists(os.path.join(self.opts['sock_dir'], 'publish_pull.ipc'))):
            ...
            raise SaltClientError
        payload_kwargs = self._prep_pub(tgt, fun, arg, tgt_type, ret, jid, timeout, **kwargs)
        master_uri = 'tcp://' + salt.utils.zeromq.ip_bracket(self.opts['interface']) + ':' + str(self.opts['ret_port'])
        with salt.channel.client.AsyncReqChannel.factory(self.opts, io_loop=io_loop, crypt='clear', master_uri=master_uri) as channel:
            try:
                log.info('Trace')
                if listen and (not self.event.connect_pub(timeout=timeout)):
                    raise SaltReqTimeoutError()
            ...
            if not payload:
                raise salt.ext.tornado.gen.Return(payload)
        raise salt.ext.tornado.gen.Return({'jid': payload['load']['jid'], 'minions': payload['load']['minions']})
...
class FunctionWrapper(dict):
    ...
    def __load_functions(self):
        ...
        """
        return set(self.local.cmd(self.minion, 'sys.list_functions').get(self.minion, []))
...
class ProxyCaller:
    ...
    def cmd(self, fun, *args, **kwargs):
        ...
        data.update(kwargs)
        executors = getattr(self.sminion, 'module_executors', []) or self.opts.get('module_executors', ['direct_call'])
