        ...
        if self.info:
            ...
            if isinstance(self.info, dict):
                ...
                if info_without_changes:
                    self.strerror_without_changes = exc_str_prefix + _nested_output(info_without_changes)
                else:
...
class FileLockError(SaltException):
    ...
    def __init__(self, message, time_start=None, *args, **kwargs):
        ...
        if time_start is None:
            log.warning('time_start should be provided when raising a FileLockError. Defaulting to current time as a fallback, but this may result in an inaccurate timeout.')
...
class SaltRenderError(SaltException):
    ...
    def __init__(self, message, line_num=None, buf='', marker='    <======================', trace=None):
        import salt.utils.stringutils
        self.error = message
        try:
        ...
        if self.line_num and self.buffer:
            ...
            self.context = salt.utils.stringutils.get_context(self.buffer, self.line_num, marker=marker)
            exc_str += '; line {}\n\n{}'.format(self.line_num, salt.utils.stringutils.to_unicode(self.context))
        super().__init__(exc_str)
