...
def _initial_state():
    return {'services': {'apache': 'running', 'ntp': 'running', 'samba': 'stopped'}, 'packages': {'coreutils': '1.0', 'apache': '2.4', 'tinc': '1.4', 'redbull': '999.99'}}
...
def grains():
    ...
    with _loaded_state(__opts__) as state:
        if 'grains_cache' not in state:
            state['grains_cache'] = {'dummy_grain_1': 'one', 'dummy_grain_2': 'two', 'dummy_grain_3': 'three'}
