...
def init(opts):
    ...
    DETAILS['headers'] = {'Content-Type': 'application/x-www-form-urlencoded', 'Content-Length': 62, 'USER-Agent': 'lwp-request/2.06'}
...
def set_config_modify(dn=None, inconfig=None, hierarchical=False):
    ...
    if hierarchical is True:
        h = 'true'
    payload = '<configConfMo cookie="{}" inHierarchical="{}" dn="{}"><inConfig>{}</inConfig></configConfMo>'.format(cookie, h, dn, inconfig)
...
def logon():
    ...
    content = {}
    payload = "<aaaLogin inName='{}' inPassword='{}'></aaaLogin>".format(DETAILS['username'], DETAILS['password'])
    r = __utils__['http.query'](DETAILS['url'], data=payload, method='POST', decode_type='plain', decode=True, verify_ssl=DETAILS['verify_ssl'], raise_error=False, status=True, headers=DETAILS['headers'])
...
def logout(cookie=None):
    ...
    __utils__['http.query'](DETAILS['url'], data=payload, method='POST', decode_type='plain', decode=True, verify_ssl=DETAILS['verify_ssl'], raise_error=True, headers=DETAILS['headers'])
    return
