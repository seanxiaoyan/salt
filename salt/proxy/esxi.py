    ...
    if 'vcenter' in proxy_conf:
        ...
        if mechanism == 'userpass':
            ...
            if 'passwords' not in proxy_conf and len(proxy_conf['passwords']) > 0:
                log.critical("Mechanism is set to 'userpass' , but no 'passwords' key found in pillar for this proxy.")
                return False
            for key in ('username', 'passwords'):
                DETAILS[key] = proxy_conf[key]
        elif mechanism == 'sspi':
            if 'domain' not in proxy_conf:
                log.critical("Mechanism is set to 'sspi' , but no 'domain' key found in pillar for this proxy.")
                return False
            if 'principal' not in proxy_conf:
                log.critical("Mechanism is set to 'sspi' , but no 'principal' key found in pillar for this proxy.")
                return False
...
def ping():
    ...
    if DETAILS.get('esxi_host'):
        return True
    elif DETAILS['mechanism'] == 'userpass':
        find_credentials(DETAILS['host'])
        try:
            log.info('Trace')
            __salt__['vsphere.system_info'](host=DETAILS['host'], username=DETAILS['username'], password=DETAILS['password'])
        except SaltSystemExit as err:
            log.warning(err)
            return False
    return True
...
def _grains(host, protocol=None, port=None):
    ...
    ret = __salt__['vsphere.system_info'](host=host, username=username, password=password, protocol=protocol, port=port)
