...
try:
    ...
    from jnpr.junos.exception import ConnectAuthError, ConnectClosedError, ConnectError, ConnectRefusedError, ConnectTimeoutError, ProbeError, RpcError, RpcTimeoutError
...
def init(opts):
    ...
    optional_args = ['user', 'username', 'password', 'passwd', 'port', 'gather_facts', 'mode', 'baud', 'attempts', 'auto_probe', 'ssh_private_key_file', 'ssh_config', 'normalize', 'huge_tree']
...
def alive(opts):
    ...
    if not local_connected:
        __salt__['event.fire_master']({}, 'junos/proxy/{}/stop'.format(opts['proxy']['host']))
...
def ping():
    ...
    if hasattr(dev._conn, '_session'):
        if dev._conn._session._transport is not None and dev._conn._session._transport.is_active() or (dev._conn._session._transport is None and isinstance(dev._conn._session, IOProc)):
