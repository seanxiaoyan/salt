...
def _default_ca_certs():
    if certifi is None:
        raise Exception("The 'certifi' package is required to use https in simple_httpclient")
...
class SimpleAsyncHTTPClient(AsyncHTTPClient):
    ...
    def fetch_impl(self, request, callback):
        ...
        if not len(self.active) < self.max_clients:
            timeout_handle = self.io_loop.add_timeout(self.io_loop.time() + min(request.connect_timeout, request.request_timeout), functools.partial(self._on_timeout, key, 'in request queue'))
        else:
            timeout_handle = None
    ...
    def _on_timeout(self, key, info=None):
        ...
        timeout_response = HTTPResponse(request, 599, error=HTTPError(599, error_message), request_time=self.io_loop.time() - request.start_time)
...
class _HTTPConnection(httputil.HTTPMessageDelegate):
    ...
    def __init__(self, io_loop, client, request, release_callback, final_callback, max_buffer_size, tcp_client, max_header_size, max_body_size):
        ...
        with stack_context.ExceptionStackContext(self._handle_exception):
            ...
            self.tcp_client.connect(host, port, af=af, ssl_options=ssl_options, max_buffer_size=self.max_buffer_size, callback=self._on_connect)
    ...
    def _on_connect(self, stream):
        ...
        if self.request.request_timeout:
            self._timeout = self.io_loop.add_timeout(self.start_time + self.request.request_timeout, stack_context.wrap(functools.partial(self._on_timeout, 'during request')))
        if self.request.method not in self._SUPPORTED_METHODS and (not self.request.allow_nonstandard_methods):
            raise KeyError('unknown method %s' % self.request.method)
        for key in ('network_interface', 'proxy_host', 'proxy_port', 'proxy_username', 'proxy_password', 'proxy_auth_mode'):
    ...
    def _create_connection(self, stream):
        ...
        connection = HTTP1Connection(stream, True, HTTP1ConnectionParameters(no_keep_alive=True, max_header_size=self.max_header_size, max_body_size=self.max_body_size, decompress=self.request.decompress_response), self._sockaddr)
        return connection
    ...
    def _read_response(self):
        self.io_loop.add_future(self.connection.read_response(self), lambda f: f.result())
    ...
    def _handle_exception(self, typ, value, tb):
        ...
        if self.final_callback:
            ...
            self._run_callback(HTTPResponse(self.request, 599, error=value, request_time=self.io_loop.time() - self.start_time))
    ...
    def _should_follow_redirect(self):
        return self.request.follow_redirects and self.request.max_redirects > 0 and (self.code in (301, 302, 303, 307, 308))
    ...
    def finish(self):
        ...
        if self.request.streaming_callback:
            ...
            buffer = BytesIO(data)
        response = HTTPResponse(original_request, self.code, reason=getattr(self, 'reason', None), headers=self.headers, request_time=self.io_loop.time() - self.start_time, buffer=buffer, effective_url=self.request.url)
