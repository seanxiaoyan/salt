...
class CurlAsyncHTTPClient(AsyncHTTPClient):
        ...
        with stack_context.NullContext():
            while True:
                ...
                while self._free_list and self._requests:
                    ...
                    curl.info = {'headers': httputil.HTTPHeaders(), 'buffer': BytesIO(), 'request': request, 'callback': callback, 'curl_start_time': time.time()}
                    try:
                        log.info('Trace')
    ...
    def _finish(self, curl, curl_error=None, curl_message=None):
        ...
        try:
            log.info('Trace')
            info['callback'](HTTPResponse(request=info['request'], code=code, headers=info['headers'], buffer=buffer, effective_url=effective_url, error=error, reason=info['headers'].get('X-Http-Reason', None), request_time=time.time() - info['curl_start_time'], time_info=time_info))
        except Exception:
            log.info('Trace')
