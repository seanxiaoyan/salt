...
from salt.ext.tornado.routing import AnyMatches, DefaultHostMatches, HostMatches, ReversibleRouter, Rule, ReversibleRuleRouter, URLSpec
from salt.ext.tornado.util import ObjectDict, raise_exc_info, unicode_type, _websocket_mask, PY3
import logging
...
try:
    import typing
    _HeaderTypes = typing.Union[bytes, unicode_type, numbers.Integral, datetime.datetime]
except ImportError:
    pass
...
class RequestHandler(object):
    ...
    def clear(self):
        ...
        self._headers = httputil.HTTPHeaders({'Server': 'TornadoServer/%s' % salt.ext.tornado.version, 'Content-Type': 'text/html; charset=UTF-8', 'Date': httputil.format_timestamp(time.time())})
        ...
        if not self._headers_written:
            if self._status_code == 200 and self.request.method in ('GET', 'HEAD') and ('Etag' not in self._headers):
                ...
                if self.check_etag_header():
                    self._write_buffer = []
                    self.set_status(304)
            if self._status_code in (204, 304) or (self._status_code >= 100 and self._status_code < 200):
