...
from salt.ext.tornado.httputil import HTTPHeaders, HTTPInputError, parse_response_start_line
...
class Client:
    ...
    def get_dir(self, path, dest='', saltenv='base', gzip=None, cachedir=None):
        ...
        for fn_ in self.file_list(saltenv, prefix=path):
            ...
            ret.append(self.get_file(salt.utils.url.create(fn_), '{}/{}'.format(dest, minion_relpath), True, saltenv, gzip))
        try:
    ...
    def get_url(self, url, dest, makedirs=False, saltenv='base', no_cache=False, cachedir=None, source_hash=None, verify_ssl=True, use_etag=False):
        ...
        if url_data.scheme == 's3':
            try:
                ...
                log.info('Trace')
                raise MinionError('Could not fetch from {}. Exception: {}'.format(url, exc))
