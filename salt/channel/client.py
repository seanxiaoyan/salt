...
class ReqChannel:
    ...
    def factory(opts, **kwargs):
        return SyncWrapper(AsyncReqChannel.factory, (opts,), kwargs, loop_kwarg='io_loop')
...
class PushChannel:
    ...
    @staticmethod
    def factory(opts, **kwargs):
        return SyncWrapper(AsyncPushChannel.factory, (opts,), kwargs, loop_kwarg='io_loop')
...
class PullChannel:
    ...
    @staticmethod
    def factory(opts, **kwargs):
        return SyncWrapper(AsyncPullChannel.factory, (opts,), kwargs, loop_kwarg='io_loop')
...
class AsyncReqChannel:
    ...
    """
    async_methods = ['crypted_transfer_decode_dictentry', '_crypted_transfer', '_uncrypted_transfer', 'send', 'connect']
    close_methods = ['close']
    ...
    def _package_load(self, load):
        log.info('Trace')
        return {'enc': self.crypt, 'load': load, 'version': 2}

    @salt.ext.tornado.gen.coroutine
    def crypted_transfer_decode_dictentry(self, load, dictkey=None, timeout=60):
        nonce = uuid.uuid4().hex
        load['nonce'] = nonce
        if not self.auth.authenticated:
            yield self.auth.authenticate()
        ret = (yield self.transport.send(self._package_load(self.auth.crypticle.dumps(load)), timeout=timeout))
        ...
        if not self.verify_signature(signed_msg['data'], signed_msg['sig']):
            raise salt.crypt.AuthenticationError('Pillar payload signature failed to validate.')
    ...
    def _crypted_transfer(self, load, timeout=60, raw=False):
        ...
        def _do_transfer():
            log.info('Trace')
            data = (yield self.transport.send(self._package_load(self.auth.crypticle.dumps(load)), timeout=timeout))
            if data:
                data = self.auth.crypticle.loads(data, raw, nonce=nonce)
            if not raw or self.ttype == 'tcp':
...
class AsyncPubChannel:
    ...
    """
    async_methods = ['connect', '_decode_messages']
    close_methods = ['close']
    ...
    def connect(self):
        ...
        try:
            ...
            yield self.transport.connect(publish_port, self.connect_callback, self.disconnect_callback)
            ...
            if '-|RETRY|-' not in str(exc):
                raise salt.exceptions.SaltClientError('Unable to sign_in to master: {}'.format(exc))
    ...
    def connect_callback(self, result):
        ...
        try:
            ...
            if self._reconnected:
                ...
                load = {'id': self.opts['id'], 'cmd': '_minion_event', 'pretag': None, 'tok': self.token, 'data': data, 'tag': tag}
            ...
            log.error('Caught exception in PubChannel connect callback %r', exc, exc_info=True)
    ...
    def _verify_master_signature(self, payload):
        ...
        if self.opts.get('sign_pub_messages'):
            ...
            if not salt.crypt.verify_signature(self.master_pubkey_path, payload['load'], payload.get('sig')):
