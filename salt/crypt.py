...
import salt.version
from salt.exceptions import AuthenticationError, InvalidKeyError, MasterExit, SaltClientError, SaltReqTimeoutError
...
def gen_keys(keydir, keyname, keysize, user=None, passphrase=None):
    ...
    if not os.access(keydir, os.W_OK):
        raise OSError('Write access denied to "{}" for user "{}".'.format(os.path.abspath(keydir), getpass.getuser()))
    with salt.utils.files.set_umask(191):
        if HAS_M2:
            if not passphrase:
                gen.save_pem(priv, cipher=None)
            else:
                gen.save_pem(priv, cipher='des_ede3_cbc', callback=lambda x: salt.utils.stringutils.to_bytes(passphrase))
        else:
...
class MasterKeys(dict):
    ...
    def __init__(self, opts):
        ...
        if opts['master_sign_pubkey']:
            if opts['master_use_pubkey_signature']:
                self.sig_path = os.path.join(self.opts['pki_dir'], opts['master_pubkey_signature'])
                if os.path.isfile(self.sig_path):
                    with salt.utils.files.fopen(self.sig_path) as fp_:
                        self.pub_signature = fp_.read()
                    log.info("Read %s's signature from %s", os.path.basename(self.pub_path), self.opts['master_pubkey_signature'])
                else:
                    log.error('Signing the master.pub key with a signature is enabled but no signature file found at the defined location %s', self.sig_path)
                    log.error("The signature-file may be either named differently or has to be created with 'salt-key --gen-signature'")
                    sys.exit(1)
            else:
                key_pass = salt.utils.sdb.sdb_get(self.opts['signing_key_pass'], self.opts)
                self.pub_sign_path = os.path.join(self.opts['pki_dir'], opts['master_sign_key_name'] + '.pub')
                self.rsa_sign_path = os.path.join(self.opts['pki_dir'], opts['master_sign_key_name'] + '.pem')
    ...
    def __get_keys(self, name='master', passphrase=None):
        ...
        if not os.path.exists(path):
            ...
            gen_keys(self.opts['pki_dir'], name, self.opts['keysize'], self.opts.get('user'), passphrase)
        if HAS_M2:
            log.info('Trace')
...
class AsyncAuth:
    ...
    def __key(cls, opts, io_loop=None):
        return (opts['pki_dir'], opts['id'], opts['master_uri'])
    ...
    def authenticated(self):
        return hasattr(self, '_authenticate_future') and self._authenticate_future.done() and (self._authenticate_future.exception() is None)
    ...
    def _authenticate(self):
        ...
        creds = None
        with salt.channel.client.AsyncReqChannel.factory(self.opts, crypt='clear', io_loop=self.io_loop) as channel:
            error = None
            while True:
                try:
                ...
                if creds == 'retry':
                    ...
                    if self.opts.get('caller'):
                        if self.opts.get('local_masters', None):
                            error = SaltClientError('Minion failed to authenticate with the master, has the minion key been accepted?')
                            break
                        else:
                            print('Minion failed to authenticate with the master, has the minion key been accepted?')
                            sys.exit(2)
    ...
    def sign_in(self, timeout=60, safe=True, tries=1, channel=None):
        ...
        if not channel:
            close_channel = True
            channel = salt.channel.client.AsyncReqChannel.factory(self.opts, crypt='clear', io_loop=self.io_loop)
    ...
    def handle_signin_response(self, sign_in_payload, payload):
        ...
        if 'ret' in payload:
            if not payload['ret']:
                if self.opts['rejected_retry']:
                    log.error("The Salt Master has rejected this minion's public key.\nTo repair this issue, delete the public key for this minion on the Salt Master.\nThe Salt Minion will attempt to re-authenicate.")
                    return 'retry'
                else:
                    log.critical("The Salt Master has rejected this minion's public key!\nTo repair this issue, delete the public key for this minion on the Salt Master and restart this minion.\nOr restart the Salt Master in open mode to clean out the keys. The Salt Minion will now exit.")
                    time.sleep(random.randint(10, 20))
                    sys.exit(salt.defaults.exitcodes.EX_NOPERM)
            elif payload['ret'] == 'full':
                return 'full'
            else:
                log.error('The Salt Master has cached the public key for this node, this salt minion will wait for %s seconds before attempting to re-authenticate', self.opts['acceptance_wait_time'])
                return 'retry'
        ...
        elif self.opts.get('master_finger', False):
    ...
    def get_keys(self):
        ...
        if not os.path.exists(self.rsa_path):
            ...
            gen_keys(self.opts['pki_dir'], 'minion', self.opts['keysize'], self.opts.get('user'))
    ...
    def verify_signing_master(self, payload):
        try:
            ...
            if self.verify_pubkey_sig(payload['pub_key'], payload['pub_sig']):
                log.info('Received signed and verified master pubkey from master %s', self.opts['master'])
                m_pub_fn = os.path.join(self.opts['pki_dir'], self.mpub)
                ...
                log.error('Received signed public-key from master %s but signature verification failed!', self.opts['master'])
                return False
    ...
    def check_auth_deps(self, payload):
        ...
        if 'pub_sig' in payload and self.opts['verify_master_pubkey_sign']:
            ...
            log.error('The masters sent its public-key signature, but signature verification is not enabled on the minion. Either enable signature verification on the minion or disable signing the public key on the master!')
            return False
        elif 'pub_sig' not in payload and self.opts['verify_master_pubkey_sign']:
            log.error('The master did not send its public-key signature, but signature verification is enabled on the minion. Either disable signature verification on the minion or enable signing the public on the master!')
            return False
    ...
    def _finger_fail(self, finger, master_key):
        log.critical("The specified fingerprint in the master configuration file:\n%s\nDoes not match the authenticating master's key:\n%s\nVerify that the configured fingerprint matches the fingerprint of the correct master and that this minion is not subject to a man-in-the-middle attack.", finger, salt.utils.crypt.pem_finger(master_key, sum_type=self.opts['hash_type']))
        sys.exit(42)
...
class SAuth(AsyncAuth):
    ...
    def authenticate(self, _=None):
        ...
        if not acceptance_wait_time_max:
            acceptance_wait_time_max = acceptance_wait_time
        with salt.channel.client.ReqChannel.factory(self.opts, crypt='clear') as channel:
            while True:
        ...
        if isinstance(payload, dict):
            if 'serial' in payload:
                ...
                if serial <= self.serial:
                    log.critical('A message with an invalid serial was received.\nthis serial: %d\nlast serial: %d\nThe minion will not honor this request.', serial, self.serial)
                    return {}
                self.serial = serial
        return payload