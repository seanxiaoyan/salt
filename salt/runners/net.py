...
def _get_net_runner_opts():
    ...
    return {'target': runner_opts.get('target', _DEFAULT_TARGET), 'expr_form': runner_opts.get('expr_form', _DEFAULT_EXPR_FORM), 'ignore_interfaces': runner_opts.get('ignore_interfaces', _DEFAULT_IGNORE_INTF), 'display': runner_opts.get('display', _DEFAULT_DISPLAY), 'outputter': runner_opts.get('outputter', _DEFAULT_OUTPUTTER)}
...
def _display_runner(rows, labels, title, display=_DEFAULT_DISPLAY):
    ...
    if display:
        ...
        if net_runner_opts.get('outputter') == 'table':
            ret = salt.output.out_format({'rows': rows, 'labels': labels}, 'table', __opts__, title=title, rows_key='rows', labels_key='labels')
        else:
            ret = salt.output.out_format(rows, net_runner_opts.get('outputter'), __opts__)
        print(ret)
    else:
        return rows
...
def _find_interfaces_ip(mac):
    ...
    for (device, device_interfaces) in all_interfaces.items():
        ...
        for (interface, interface_details) in device_interfaces.get('out', {}).items():
            ...
            interface_ips = ['{}/{}'.format(ip_addr, addr_details.get('prefix_length', '32')) for (ip_addr, addr_details) in ip_addresses.items()]
            return (device, interface, interface_ips)
    return ('', '', [])

def _find_interfaces_mac(ip):
    """
...
def interfaces(device=None, interface=None, title=None, pattern=None, ipnet=None, best=True, display=_DEFAULT_DISPLAY):
    ...
    labels = {'device': 'Device', 'interface': 'Interface', 'interface_description': 'Interface Description', 'is_up': 'UP', 'is_enabled': 'Enabled', 'speed': 'Speed [Mbps]', 'mac': 'MAC Address', 'ips': 'IP Addresses'}
    rows = []
    net_runner_opts = _get_net_runner_opts()
    if pattern:
        title = 'Pattern "{}" found in the description of the following interfaces'.format(pattern)
    if not title:
        title = 'Details'
        if interface:
    ...
    for (device, net_interfaces_out) in all_interfaces.items():
        ...
        for (interface_name, interface_details) in selected_device_interfaces.items():
            ...
            if not all_ipaddrs.get(device, {}).get('result', False):
                continue
            ips = []
            device_entry = {'device': device, 'interface': interface_name, 'interface_description': interface_description, 'is_up': interface_details.get('is_up', '') or '', 'is_enabled': interface_details.get('is_enabled', '') or '', 'speed': interface_details.get('speed', '') or '', 'mac': napalm_helpers.convert(napalm_helpers.mac, interface_details.get('mac_address', '') or ''), 'ips': []}
            intf_entry_found = False
            for (intrf, interface_ips) in all_ipaddrs.get(device, {}).get('out', {}).items():
                if intrf.split('.')[0] == interface_name:
                    ip_addresses = interface_ips.get('ipv4', {})
                    ip_addresses.update(interface_ips.get('ipv6', {}))
                    ips = ['{}/{}'.format(ip_addr, addr_details.get('prefix_length', '32')) for (ip_addr, addr_details) in ip_addresses.items()]
                    interf_entry = {}
                    ...
                    if ipnet:
                        inet_ips = [str(ip) for ip in ips if _ipnet_belongs(ip)]
                        if inet_ips:
                            if best:
...
def findarp(device=None, interface=None, mac=None, ip=None, display=_DEFAULT_DISPLAY):
    """
    ...
    .. code-block:: text
        ...
        ________________________________________________________________________________
    """
    labels = {'device': 'Device', 'interface': 'Interface', 'mac': 'MAC', 'ip': 'IP', 'age': 'Age'}
    rows = []
    ...
    for (device, device_arp) in all_arp.items():
        ...
        for arp_entry in arp_table:
            if mac and arp_entry.get('mac', '').lower() == mac.lower() or (interface and interface in arp_entry.get('interface', '')) or (ip and napalm_helpers.convert(napalm_helpers.ip, arp_entry.get('ip', '')) == napalm_helpers.convert(napalm_helpers.ip, ip)):
                rows.append({'device': device, 'interface': arp_entry.get('interface'), 'mac': napalm_helpers.convert(napalm_helpers.mac, arp_entry.get('mac')), 'ip': napalm_helpers.convert(napalm_helpers.ip, arp_entry.get('ip')), 'age': arp_entry.get('age')})
    return _display_runner(rows, labels, title, display=display)
...
def findmac(device=None, mac=None, interface=None, vlan=None, display=_DEFAULT_DISPLAY):
    ...
    .. code-block:: text
        ...
        _____________________________________________________________________________________________
    """
    labels = {'device': 'Device', 'interface': 'Interface', 'mac': 'MAC', 'vlan': 'VLAN', 'static': 'Static', 'active': 'Active', 'moves': 'Moves', 'last_move': 'Last Move'}
    rows = []
    ...
    for (device, device_mac) in all_mac.items():
        ...
        for mac_entry in mac_table:
            if mac and napalm_helpers.convert(napalm_helpers.mac, mac_entry.get('mac', '')) == napalm_helpers.convert(napalm_helpers.mac, mac) or (interface and interface in mac_entry.get('interface', '')) or (vlan and str(mac_entry.get('vlan', '')) == str(vlan)):
                rows.append({'device': device, 'interface': mac_entry.get('interface'), 'mac': napalm_helpers.convert(napalm_helpers.mac, mac_entry.get('mac')), 'vlan': mac_entry.get('vlan'), 'static': mac_entry.get('static'), 'active': mac_entry.get('active'), 'moves': mac_entry.get('moves'), 'last_move': mac_entry.get('last_move')})
    return _display_runner(rows, labels, title, display=display)
...
def lldp(device=None, interface=None, title=None, pattern=None, chassis=None, display=_DEFAULT_DISPLAY):
    ...
    labels = {'device': 'Device', 'interface': 'Interface', 'parent_interface': 'Parent Interface', 'remote_chassis_id': 'Remote Chassis ID', 'remote_port_id': 'Remote Port ID', 'remote_port_desc': 'Remote Port Description', 'remote_system_name': 'Remote System Name', 'remote_system_desc': 'Remote System Description'}
    rows = []
    if pattern:
        title = 'Pattern "{}" found in one of the following LLDP details'.format(pattern)
    if not title:
    ...
    for (device, device_lldp) in all_lldp.items():
        ...
        for (intrf, interface_lldp) in lldp_interfaces.items():
            ...
            for lldp_row in interface_lldp:
                ...
                if pattern:
                    ptl = pattern.lower()
                    if not (ptl in rsn.lower() or ptl in rsd.lower() or ptl in rpd.lower() or (ptl in rci.lower())):
                        continue
                if chassis:
                    if napalm_helpers.convert(napalm_helpers.mac, rci) != napalm_helpers.convert(napalm_helpers.mac, chassis):
                        continue
                rows.append({'device': device, 'interface': intrf, 'parent_interface': lldp_row.get('parent_interface', '') or '', 'remote_chassis_id': napalm_helpers.convert(napalm_helpers.mac, rci), 'remote_port_id': rpi, 'remote_port_descr': rpd, 'remote_system_name': rsn, 'remote_system_descr': rsd})
    return _display_runner(rows, labels, title, display=display)
...
def find(addr, best=True, display=_DEFAULT_DISPLAY):
    ...
    ip = ''
    ipnet = None
    results = {'int_net': [], 'int_descr': [], 'int_name': [], 'int_ip': [], 'int_mac': [], 'int_device': [], 'lldp_descr': [], 'lldp_int': [], 'lldp_device': [], 'lldp_mac': [], 'lldp_device_int': [], 'mac_device': [], 'mac_int': [], 'arp_device': [], 'arp_int': [], 'arp_mac': [], 'arp_ip': []}
    ...
    if not mac:
        try:
            log.info('Trace')
            ip = napalm_helpers.convert(napalm_helpers.ip, addr)
        except ValueError:
            log.info('Trace')
            pass
    ...
    if mac:
        ...
        if device and interface:
            title = 'Interface {interface} on {device} has the physical address ({mac})'.format(interface=interface, device=device, mac=mac)
        ...
        if device and interface:
            title = 'IP Address {ip} is set for interface {interface}, on {device}'.format(interface=interface, device=device, ip=ip)
...
def multi_find(*patterns, **kwargs):
    ...
    for pattern in set(patterns):
        search_result = find(pattern, best=kwargs.get('best', True), display=kwargs.get('display', _DEFAULT_DISPLAY))
