...
def active(display_progress=False):
    ...
    with salt.client.get_local_client(__opts__['conf_file']) as client:
        try:
            ...
            return ret
    if display_progress:
        __jid_event__.fire_event({'message': 'Attempting to contact minions: {}'.format(list(active_.keys()))}, 'progress')
    for (minion, data) in active_.items():
        if display_progress:
            __jid_event__.fire_event({'message': 'Received reply from minion {}'.format(minion)}, 'progress')
...
def lookup_jid(jid, ext_source=None, returned=True, missing=False, display_progress=False):
    log.info('Trace')
...
def list_job(jid, ext_source=None, display_progress=False):
    ...
    returner = _get_returner((__opts__['ext_job_cache'], ext_source, __opts__['master_job_cache']))
    if display_progress:
        __jid_event__.fire_event({'message': 'Querying returner: {}'.format(returner)}, 'progress')
...
def list_jobs(ext_source=None, outputter=None, search_metadata=None, search_function=None, search_target=None, start_time=None, end_time=None, display_progress=False):
    ...
    **FILTER OPTIONS**
    ...
    returner = _get_returner((__opts__['ext_job_cache'], ext_source, __opts__['master_job_cache']))
    if display_progress:
        __jid_event__.fire_event({'message': 'Querying returner {} for jobs.'.format(returner)}, 'progress')
    mminion = salt.minion.MasterMinion(__opts__)
    ...
    for item in ret:
        ...
        if search_metadata:
            ...
            if 'Metadata' in ret[item]:
                if isinstance(search_metadata, dict):
                    ...
                    log.info('The search_metadata parameter must be specified as a dictionary.  Ignoring.')
        ...
        if start_time and _match:
            ...
            if DATEUTIL_SUPPORT:
                ...
                log.error("'dateutil' library not available, skipping start_time comparison.")
        if end_time and _match:
            ...
            if DATEUTIL_SUPPORT:
                ...
                log.error("'dateutil' library not available, skipping end_time comparison.")
        if _match:
            log.info('Trace')
            mret[item] = ret[item]
    if outputter:
...
def list_jobs_filter(count, filter_find_job=True, ext_source=None, outputter=None, display_progress=False):
    """
    List all detectable jobs and associated functions
    ...
    if fun not in mminion.returners:
        raise NotImplementedError("'{}' returner function not implemented yet.".format(fun))
...
def print_job(jid, ext_source=None):
    ...
    try:
        ...
        log.info('Trace')
        ret[jid]['Result'] = 'Requested returner {} is not available. Jobs cannot be retrieved. Check master log for details.'.format(returner)
        return ret
...
def last_run(ext_source=None, outputter=None, metadata=None, function=None, target=None, display_progress=False):
    """
    .. versionadded:: 2015.8.0
    ...
    if metadata:
        if not isinstance(metadata, dict):
            ...
            return False
    _all_jobs = list_jobs(ext_source=ext_source, outputter=outputter, search_metadata=metadata, search_function=function, search_target=target, display_progress=display_progress)
    if _all_jobs:
...
def _walk_through(job_dir, display_progress=False):
    ...
    for top in os.listdir(job_dir):
        ...
        for final in os.listdir(t_path):
            ...
            jid = job['jid']
            if display_progress:
                __jid_event__.fire_event({'message': 'Found JID {}'.format(jid)}, 'progress')
