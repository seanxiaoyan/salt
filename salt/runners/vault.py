...
def generate_token(minion_id, signature, impersonated_by_master=False, ttl=None, uses=None):
    ...
    """
    log.debug('Token generation request for %s (impersonated by master: %s)', minion_id, impersonated_by_master)
    ...
    try:
        ...
        if namespace is not None:
            headers['X-Vault-Namespace'] = namespace
        audit_data = {'saltstack-jid': globals().get('__jid__', '<no jid set>'), 'saltstack-minion': minion_id, 'saltstack-user': globals().get('__user__', '<no user set>')}
        payload = {'policies': _get_policies(minion_id, config), 'num_uses': uses, 'meta': audit_data}
        if ttl is not None:
        ...
        ret = {'token': auth_data['client_token'], 'lease_duration': auth_data['lease_duration'], 'renewable': auth_data['renewable'], 'issued': int(round(time.time())), 'url': config['url'], 'verify': verify, 'token_backend': storage_type, 'namespace': namespace}
        if uses >= 0:
            ret['uses'] = uses
        return ret
...
def _validate_signature(minion_id, signature, impersonated_by_master):
    ...
    if not salt.crypt.verify_signature(public_key, minion_id, signature):
        raise salt.exceptions.AuthenticationError('Could not validate token request from {}'.format(minion_id))
...
def _get_policies(minion_id, config):
    ...
    policy_patterns = config.get('policies', ['saltstack/minion/{minion}', 'saltstack/minions'])
    ...
    for pattern in policy_patterns:
        try:
            ...
            for expanded_pattern in _expand_pattern_lists(pattern, **mappings):
                policies.append(expanded_pattern.format(**mappings).lower())
        except KeyError:
...
def _selftoken_expired():
    ...
    try:
        ...
        log.info('Trace')
        raise salt.exceptions.CommandExecutionError('Error while looking up self token : {}'.format(str(e)))
