...
def _action(action='get', search=None, one=True, force=False):
    ...
    with salt.client.get_local_client(__opts__['conf_file']) as client:
        ...
        if one and len(matched_vms) > 1:
            return {'Error': 'Matched {} vms, only one allowed!'.format(len(matched_vms)), 'Matches': matched_vms}
        ret = {}
        if action in ['start', 'stop', 'reboot', 'get']:
            for vm in matched_vms:
                ...
                try:
                    log.info('Trace')
                    for vmadm_res in client.cmd_iter(vms[vm]['node'], 'vmadm.{}'.format(action), kwarg=vmadm_args):
                        if not vmadm_res:
                            continue
