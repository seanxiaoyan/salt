...
def diff(*args, **kwargs):
    ...
    for k in bulk_ret:
        ...
        for (idx, _) in enumerate(to_result):
            to_result[idx] += '\n'
        outs = ''
        outs += ''.join(difflib.unified_diff(from_result, to_result, fromfile=bulk_ret[0]['pool'][0], tofile=k['pool'][0], n=0))
        print(outs)
        print()
    return bulk_ret
...
def _get_pool_results(*args, **kwargs):
    ...
    if tgt_type not in ['compound', 'pcre']:
        tgt_type = 'compound'
    kwargs_passthru = {key: value for (key, value) in kwargs.items() if not key.startswith('_')}
