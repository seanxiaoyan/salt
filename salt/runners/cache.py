...
def grains(tgt, tgt_type='glob', **kwargs):
    ...
    """
    pillar_util = salt.utils.master.MasterPillarUtil(tgt, tgt_type, use_cached_grains=True, grains_fallback=False, opts=__opts__)
...
def pillar(tgt=None, tgt_type='glob', **kwargs):
    ...
    """
    pillar_util = salt.utils.master.MasterPillarUtil(tgt, tgt_type, use_cached_grains=True, grains_fallback=False, use_cached_pillar=True, pillar_fallback=False, opts=__opts__)
...
def mine(tgt=None, tgt_type='glob', **kwargs):
    ...
    .. code-block:: bash
        ...
        salt-run cache.mine
    """
    pillar_util = salt.utils.master.MasterPillarUtil(tgt, tgt_type, use_cached_grains=False, grains_fallback=False, use_cached_pillar=False, pillar_fallback=False, opts=__opts__)
    cached_mine = pillar_util.get_cached_mine_data()
    return cached_mine

def _clear_cache(tgt=None, tgt_type='glob', clear_pillar_flag=False, clear_grains_flag=False, clear_mine_flag=False, clear_mine_func_flag=None):
    """
    ...
    if tgt is None:
        return False
    pillar_util = salt.utils.master.MasterPillarUtil(tgt, tgt_type, use_cached_grains=True, grains_fallback=False, use_cached_pillar=True, pillar_fallback=False, opts=__opts__)
    return pillar_util.clear_cached_minion_data(clear_pillar=clear_pillar_flag, clear_grains=clear_grains_flag, clear_mine=clear_mine_flag, clear_mine_func=clear_mine_func_flag)
...
def clear_all(tgt=None, tgt_type='glob'):
    ...
    """
    return _clear_cache(tgt, tgt_type, clear_pillar_flag=True, clear_grains_flag=True, clear_mine_flag=True)
...
def clear_git_lock(role, remote=None, **kwargs):
    ...
    type_ = salt.utils.args.split_input(kwargs.pop('type', ['update', 'checkout', 'mountpoint']))
    if kwargs:
        salt.utils.args.invalid_kwargs(kwargs)
    if role == 'gitfs':
        git_objects = [salt.utils.gitfs.GitFS(__opts__, __opts__['gitfs_remotes'], per_remote_overrides=salt.fileserver.gitfs.PER_REMOTE_OVERRIDES, per_remote_only=salt.fileserver.gitfs.PER_REMOTE_ONLY)]
        ...
        for ext_pillar in __opts__['ext_pillar']:
            ...
            if key == 'git':
                if not isinstance(ext_pillar['git'], list):
                    continue
                obj = salt.utils.gitfs.GitPillar(__opts__, ext_pillar['git'], per_remote_overrides=salt.pillar.git_pillar.PER_REMOTE_OVERRIDES, per_remote_only=salt.pillar.git_pillar.PER_REMOTE_ONLY, global_only=salt.pillar.git_pillar.GLOBAL_ONLY)
                git_objects.append(obj)
    elif role == 'winrepo':
        winrepo_dir = __opts__['winrepo_dir']
        winrepo_remotes = __opts__['winrepo_remotes']
        git_objects = []
        for (remotes, base_dir) in ((winrepo_remotes, winrepo_dir), (__opts__['winrepo_remotes_ng'], __opts__['winrepo_dir_ng'])):
            obj = salt.utils.gitfs.WinRepo(__opts__, remotes, per_remote_overrides=salt.runners.winrepo.PER_REMOTE_OVERRIDES, per_remote_only=salt.runners.winrepo.PER_REMOTE_ONLY, global_only=salt.runners.winrepo.GLOBAL_ONLY, cache_root=base_dir)
            git_objects.append(obj)
    else:
    ...
    for obj in git_objects:
        for lock_type in type_:
            (cleared, errors) = _clear_lock(obj.clear_lock, role, remote=remote, lock_type=lock_type)
            if cleared:
...
def cloud(tgt, provider=None):
    ...
    if not isinstance(tgt, str):
        return {}
    opts = salt.config.cloud_config(os.path.join(os.path.dirname(__opts__['conf_file']), 'cloud'))
